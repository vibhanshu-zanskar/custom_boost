...
void saver_tests_1(int index,
    ...
    std::ostream& err)
{
    zr_boost_1_81::io::ios_flags_saver     ifls(output);
    zr_boost_1_81::io::ios_precision_saver iprs(output);
    zr_boost_1_81::io::ios_width_saver     iws(output);
    zr_boost_1_81::io::ios_tie_saver       its(input);
    zr_boost_1_81::io::ios_rdbuf_saver     irs(output);
    zr_boost_1_81::io::ios_fill_saver      ifis(output);
    zr_boost_1_81::io::ios_locale_saver    ils(output);
    zr_boost_1_81::io::ios_iword_saver     iis(output, index);
    zr_boost_1_81::io::ios_pword_saver     ipws(output, index);
    ...
    try {
        zr_boost_1_81::io::ios_exception_saver ies(output);
        zr_boost_1_81::io::ios_iostate_saver ias(output);
...
void saver_tests_2(int index,
    ...
    std::ostream& err)
{
    zr_boost_1_81::io::ios_tie_saver   its(input, &err);
    zr_boost_1_81::io::ios_rdbuf_saver irs(output, err.rdbuf());
    zr_boost_1_81::io::ios_iword_saver iis(output, index, 69L);
    zr_boost_1_81::io::ios_pword_saver ipws(output, index, &err);
    zr_boost_1_81::io::ios_flags_saver ifls(output,
        (output.flags() & ~std::ios_base::adjustfield) |
        std::ios_base::showpos |
        std::ios_base::boolalpha |
        (std::ios_base::internal & std::ios_base::adjustfield));
    zr_boost_1_81::io::ios_precision_saver iprs(output, 9);
    zr_boost_1_81::io::ios_fill_saver ifis(output, '@');
    output << "Hello world";
    zr_boost_1_81::io::ios_width_saver iws(output, 12);
    output << -16 + 34.5678901234;
    std::locale loc(std::locale::classic(), new backward_bool_names);
    zr_boost_1_81::io::ios_locale_saver ils(output, loc);
    output << true;
    BOOST_TEST(&err == output.pword(index));
    BOOST_TEST(69L == output.iword(index));
    try {
        zr_boost_1_81::io::ios_exception_saver ies(output, std::ios_base::eofbit);
        zr_boost_1_81::io::ios_iostate_saver ias(output,
