...
struct Person {
    ...
    struct hana_accessors_impl {
        static BOOST_HANA_CONSTEXPR_LAMBDA auto apply() {
            return zr_boost_1_81::hana::make_tuple(
                zr_boost_1_81::hana::make_pair(BOOST_HANA_STRING("name"),
                [](auto&& p) -> decltype(auto) {
                    return zr_boost_1_81::hana::id(std::forward<decltype(p)>(p).name);
                }),
                zr_boost_1_81::hana::make_pair(BOOST_HANA_STRING("age"),
                [](auto&& p) -> decltype(auto) {
                    return zr_boost_1_81::hana::id(std::forward<decltype(p)>(p).age);
                })
            );
        }
    };
