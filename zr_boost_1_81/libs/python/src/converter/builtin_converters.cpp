...
// when long long is absent but __int64 is present.
  ...
  struct wstring_rvalue_from_python
  {
      ...
      static std::wstring extract(PyObject* intermediate)
      {
          ...
          if (buf == NULL) {
              zr_boost_1_81::python::throw_error_already_set();
          }
...
{
#if PY_VERSION_HEX >= 0x03000000
    return x ? PyUnicode_FromString(x) : zr_boost_1_81::python::detail::none();
#else
    return x ? PyString_FromString(x) : zr_boost_1_81::python::detail::none();
#endif
}
  
BOOST_PYTHON_DECL PyObject* do_return_to_python(PyObject* x)
{
    return x ? x : zr_boost_1_81::python::detail::none();
}
  
BOOST_PYTHON_DECL PyObject* do_arg_to_python(PyObject* x)
{
    if (x == 0)
        return zr_boost_1_81::python::detail::none();
