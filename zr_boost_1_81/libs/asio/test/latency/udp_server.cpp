...
class udp_server : zr_boost_1_81::asio::coroutine
{
public:
  ...
  void operator()(zr_boost_1_81::system::error_code ec, std::size_t n = 0)
  {
    reenter (this) for (;;)
    {
      yield socket_.async_receive_from(
          zr_boost_1_81::asio::buffer(buffer_),
          sender_, ref(this));

      if (!ec)
      {
        for (std::size_t i = 0; i < n; ++i) buffer_[i] = ~buffer_[i];
        socket_.send_to(zr_boost_1_81::asio::buffer(buffer_, n), sender_, 0, ec);
      }
    }
  }
...
int main(int argc, char* argv[])
{
  ...
  std::vector<zr_boost_1_81::shared_ptr<udp_server> > servers;

  for (unsigned short i = 0; i < num_ports; ++i)
  {
    unsigned short port = first_port + i;
    zr_boost_1_81::shared_ptr<udp_server> s(new udp_server(io_context, port, buf_size));
    servers.push_back(s);
