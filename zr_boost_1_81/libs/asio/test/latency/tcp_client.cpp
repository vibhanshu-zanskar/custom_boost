...
using zr_boost_1_81::posix_time::ptime;
using zr_boost_1_81::posix_time::microsec_clock;
...
struct transfer_all
{
  ...
  std::size_t operator()(const zr_boost_1_81::system::error_code& ec, std::size_t)
  {
    return (ec && ec != zr_boost_1_81::asio::error::would_block) ? 0 : ~0;
  }
};
...
int main(int argc, char* argv[])
{
  ...
  std::vector<zr_boost_1_81::shared_ptr<tcp::socket> > sockets;

  for (int i = 0; i < num_connections; ++i)
  {
    zr_boost_1_81::shared_ptr<tcp::socket> s(new tcp::socket(io_context));

    tcp::endpoint target(zr_boost_1_81::asio::ip::make_address(ip), port);
    s->connect(target);
  ...
  zr_boost_1_81::uint64_t start_hr = high_res_clock();

  zr_boost_1_81::uint64_t samples[num_samples];
  for (int i = 0; i < num_samples; ++i)
  {
    tcp::socket& socket = *sockets[i % num_connections];

    zr_boost_1_81::uint64_t t = high_res_clock();

    zr_boost_1_81::system::error_code ec;
    zr_boost_1_81::asio::write(socket,
        zr_boost_1_81::asio::buffer(write_buf),
        transfer_all(), ec);

    zr_boost_1_81::asio::read(socket,
        zr_boost_1_81::asio::buffer(read_buf),
        transfer_all(), ec);
  ...
  zr_boost_1_81::uint64_t stop_hr = high_res_clock();
  zr_boost_1_81::uint64_t elapsed_usec = (stop - start).total_microseconds();
  zr_boost_1_81::uint64_t elapsed_hr = stop_hr - start_hr;
