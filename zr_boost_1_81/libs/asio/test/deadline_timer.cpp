...
void decrement_to_zero(zr_boost_1_81::asio::deadline_timer* t, int* count)
{
  if (*count > 0)
  {
    --(*count);
    ...
    t->async_wait(zr_boost_1_81::bind(decrement_to_zero, t, count));
...
void cancel_timer(zr_boost_1_81::asio::deadline_timer* t)
{
  std::size_t num_cancelled = t->cancel();
  BOOST_ASIO_CHECK(num_cancelled == 1);
}

void cancel_one_timer(zr_boost_1_81::asio::deadline_timer* t)
{
...
void deadline_timer_test()
{
  ...
  zr_boost_1_81::asio::deadline_timer t1(ioc, seconds(1));
  t1.wait();
  ...
  zr_boost_1_81::asio::deadline_timer t2(ioc, seconds(1) + microseconds(500000));
  t2.wait();
  ...
  zr_boost_1_81::asio::deadline_timer t3(ioc, seconds(5));
  t3.async_wait(zr_boost_1_81::bind(increment, &count));
  ...
  zr_boost_1_81::asio::deadline_timer t4(ioc, seconds(1));
  t4.async_wait(zr_boost_1_81::bind(decrement_to_zero, &t4, &count));
  ...
  zr_boost_1_81::asio::deadline_timer t5(ioc, seconds(10));
  t5.async_wait(zr_boost_1_81::bind(increment_if_not_cancelled, &count,
        zr_boost_1_81::asio::placeholders::error));
  zr_boost_1_81::asio::deadline_timer t6(ioc, seconds(1));
  t6.async_wait(zr_boost_1_81::bind(cancel_timer, &t5));
  ...
  // counter.
  zr_boost_1_81::asio::deadline_timer t7(ioc, seconds(3));
  t7.async_wait(zr_boost_1_81::bind(increment_if_not_cancelled, &count,
        zr_boost_1_81::asio::placeholders::error));
  t7.async_wait(zr_boost_1_81::bind(increment_if_not_cancelled, &count,
        zr_boost_1_81::asio::placeholders::error));
  zr_boost_1_81::asio::deadline_timer t8(ioc, seconds(1));
  t8.async_wait(zr_boost_1_81::bind(cancel_one_timer, &t7));
...
void timer_handler(const zr_boost_1_81::system::error_code&)
{
}
...
void deadline_timer_cancel_test()
{
  ...
  struct timer
  {
    zr_boost_1_81::asio::deadline_timer t;
    timer() : t(io_context) { t.expires_at(zr_boost_1_81::posix_time::pos_infin); }
  } timers[50];
...
void deadline_timer_custom_allocation_test()
{
  ...
  for (int i = 0; i < 50; ++i)
  {
    timers[i].t.expires_at(zr_boost_1_81::posix_time::pos_infin);
    timers[i].t.async_wait(custom_allocation_timer_handler(&allocation_count));
  }

  for (int i = 50; i < 100; ++i)
  {
    timers[i].t.expires_at(zr_boost_1_81::posix_time::neg_infin);
    timers[i].t.async_wait(custom_allocation_timer_handler(&allocation_count));
  }
...
void deadline_timer_thread_test()
{
  ...
  zr_boost_1_81::asio::deadline_timer t1(ioc);
  zr_boost_1_81::asio::deadline_timer t2(ioc);
  int count = 0;

  zr_boost_1_81::asio::detail::thread th(zr_boost_1_81::bind(io_context_run, &ioc));

  t2.expires_from_now(zr_boost_1_81::posix_time::seconds(2));
  t2.wait();

  t1.expires_from_now(zr_boost_1_81::posix_time::seconds(2));
  t1.async_wait(zr_boost_1_81::bind(increment, &count));

  t2.expires_from_now(zr_boost_1_81::posix_time::seconds(4));
  t2.wait();
...
void deadline_timer_async_result_test()
{
  ...
  t1.expires_from_now(zr_boost_1_81::posix_time::seconds(1));
...
zr_boost_1_81::asio::deadline_timer make_timer(zr_boost_1_81::asio::io_context& ioc, int* count)
{
  zr_boost_1_81::asio::deadline_timer t(ioc);
  t.expires_from_now(zr_boost_1_81::posix_time::seconds(1));
  t.async_wait(zr_boost_1_81::bind(increment, count));
  return t;
}
...
void deadline_timer_move_test()
{
  ...
  zr_boost_1_81::asio::deadline_timer t1 = make_timer(io_context1, &count);
  zr_boost_1_81::asio::deadline_timer t2 = make_timer(io_context2, &count);
  zr_boost_1_81::asio::deadline_timer t3 = std::move(t1);
