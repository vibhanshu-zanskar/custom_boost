...
auto coro_simple_cancel_impl(zr_boost_1_81::asio::io_context& ) noexcept
  -> zr_boost_1_81::asio::experimental::coro<void() noexcept, zr_boost_1_81::system::error_code>
{
    BOOST_ASIO_CHECK(
        !(co_await this_coro::cancellation_state).cancelled());

    zr_boost_1_81::asio::steady_timer timer{
...
void coro_simple_cancel()
{
  ...
  zr_boost_1_81::asio::cancellation_signal sig;

  auto k = coro_simple_cancel_impl(ctx);

  zr_boost_1_81::system::error_code res_ec;
  k.async_resume(
      zr_boost_1_81::asio::bind_cancellation_slot(sig.slot(),
        [&](zr_boost_1_81::system::error_code ec) {res_ec = ec;}));
  zr_boost_1_81::asio::post(ctx, [&]{sig.emit(zr_boost_1_81::asio::cancellation_type::all);});

  BOOST_ASIO_CHECK(!res_ec);

  ctx.run();

  BOOST_ASIO_CHECK(res_ec == zr_boost_1_81::asio::error::operation_aborted);
}

auto coro_throw_cancel_impl(zr_boost_1_81::asio::io_context& )
  -> zr_boost_1_81::asio::experimental::coro<void() , void>
{
    zr_boost_1_81::asio::steady_timer timer{
        co_await this_coro::executor,
        std::chrono::seconds(1)};
...
void coro_throw_cancel()
{
  ...
  catch (zr_boost_1_81::system::system_error& se)
  {
    BOOST_ASIO_CHECK(se.code() == zr_boost_1_81::asio::error::operation_aborted);
  }
}

auto coro_simple_cancel_nested_k(zr_boost_1_81::asio::io_context&, int& cnt) noexcept
  -> zr_boost_1_81::asio::experimental::coro<
      void() noexcept,
      zr_boost_1_81::system::error_code>
{
  zr_boost_1_81::asio::steady_timer timer{
          co_await this_coro::executor,
...
auto coro_simple_cancel_nested_kouter(
    zr_boost_1_81::asio::io_context& ctx, int& cnt) noexcept
  -> zr_boost_1_81::asio::experimental::coro<
      zr_boost_1_81::system::error_code() noexcept,
      zr_boost_1_81::system::error_code>
{
