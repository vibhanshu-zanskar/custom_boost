...
zr_boost_1_81::asio::experimental::coro<int> throwing_generator(
...
zr_boost_1_81::asio::awaitable<void> throwing_generator_test()
{
  int val = 0;
  bool destr = false;
  {
    auto gi = throwing_generator(
        co_await zr_boost_1_81::asio::this_coro::executor,
        val, destr);
    bool caught = false;
    try
    {
...
void run_throwing_generator_test()
{
  ...
  zr_boost_1_81::asio::co_spawn(ctx, throwing_generator_test(), zr_boost_1_81::asio::detached);
  ctx.run();
}

zr_boost_1_81::asio::experimental::coro<int(int)> throwing_stacked(
    zr_boost_1_81::asio::any_io_executor exec, int &val,
    bool &destroyed_inner, bool &destroyed)
{
  BOOST_ASIO_CHECK((co_await zr_boost_1_81::asio::this_coro::throw_if_cancelled()));
...
zr_boost_1_81::asio::awaitable<void> throwing_generator_stacked_test()
{
  int val = 0;
  bool destr = false, destr_inner = false;
  {
    ...
    try
    {
      for (int i = 0; i < 10; i++)
      {
        BOOST_ASIO_CHECK(val == i);
        const auto next =
          co_await gi.async_resume(42, zr_boost_1_81::asio::use_awaitable);
        BOOST_ASIO_CHECK(next);
        BOOST_ASIO_CHECK(val == *next);
        BOOST_ASIO_CHECK(val == i + 1);
      }
...
{
  zr_boost_1_81::asio::io_context ctx;
  zr_boost_1_81::asio::co_spawn(ctx,
