...
zr_boost_1_81::asio::experimental::coro<int> generator_impl(
    zr_boost_1_81::asio::any_io_executor, int& last, bool& destroyed)
{
...
zr_boost_1_81::asio::awaitable<void> generator_test()
{
  int val = 0;
  bool destr = false;
  {
    auto gi = generator_impl(
        co_await zr_boost_1_81::asio::this_coro::executor, val, destr);

    for (int i = 0; i < 10; i++)
    {
      BOOST_ASIO_CHECK(val == i);
      const auto next = co_await gi.async_resume(zr_boost_1_81::asio::use_awaitable);
...
void run_generator_test()
{
  ...
  zr_boost_1_81::asio::co_spawn(ctx, generator_test, zr_boost_1_81::asio::detached);
  ctx.run();
}

zr_boost_1_81::asio::experimental::coro<void, int> task_test(zr_boost_1_81::asio::io_context&)
{
  co_return 42;
}

zr_boost_1_81::asio::experimental::coro<void, int> task_throw(zr_boost_1_81::asio::io_context&)
{
...
zr_boost_1_81::asio::experimental::coro<char> completion_generator_test_impl(
    zr_boost_1_81::asio::any_io_executor, int limit)
{
  for (int i = 0; i < limit; i++)
    co_yield i;
}

zr_boost_1_81::asio::awaitable<void> completion_generator_test()
{
  std::vector<int> res;
  auto g = completion_generator_test_impl(
      co_await zr_boost_1_81::asio::this_coro::executor, 10);

  BOOST_ASIO_CHECK(g.is_open());
  while (auto val = co_await g.async_resume(zr_boost_1_81::asio::use_awaitable))
    res.push_back(*val);
...
{
  zr_boost_1_81::asio::io_context ctx;
  zr_boost_1_81::asio::co_spawn(ctx, completion_generator_test, zr_boost_1_81::asio::detached);
  ctx.run();
}

zr_boost_1_81::asio::experimental::coro<int(int)>
symmetrical_test_impl(zr_boost_1_81::asio::any_io_executor)
{
  int i = 0;
  while (true)
    i = (co_yield i) + i;
...
zr_boost_1_81::asio::awaitable<void> symmetrical_test()
{
  auto g = symmetrical_test_impl(co_await zr_boost_1_81::asio::this_coro::executor);

  BOOST_ASIO_CHECK(g.is_open());

  BOOST_ASIO_CHECK(0  == (co_await g.async_resume(0,
          zr_boost_1_81::asio::use_awaitable)).value_or(-1));
...
{
  zr_boost_1_81::asio::io_context ctx;
  zr_boost_1_81::asio::co_spawn(ctx, symmetrical_test, zr_boost_1_81::asio::detached);
  ctx.run();
}
