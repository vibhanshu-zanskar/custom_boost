...
{
public:
  explicit impl_0_completion_args(zr_boost_1_81::asio::io_context& ioc)
    : ioc_(ioc),
      state_(starting)
  {
  }
  ...
  void operator()(Self& self)
  {
    ...
    case starting:
      state_ = posting;
      zr_boost_1_81::asio::post(ioc_, BOOST_ASIO_MOVE_CAST(Self)(self));
      break;
    case posting:
      self.complete();
      break;
...
private:
  zr_boost_1_81::asio::io_context& ioc_;
  enum { starting, posting } state_;
};

template <typename CompletionToken>
BOOST_ASIO_INITFN_RESULT_TYPE(CompletionToken, void())
async_0_completion_args(zr_boost_1_81::asio::io_context& ioc,
    BOOST_ASIO_MOVE_ARG(CompletionToken) token)
{
  return zr_boost_1_81::asio::async_compose<CompletionToken, void()>(
...
{
public:
  explicit impl_1_completion_arg(zr_boost_1_81::asio::io_context& ioc)
    : ioc_(ioc),
      state_(starting)
  {
  }
...
async_1_completion_arg(zr_boost_1_81::asio::io_context& ioc,
    BOOST_ASIO_MOVE_ARG(CompletionToken) token)
{
  return zr_boost_1_81::asio::async_compose<CompletionToken, void(int)>(
...
typedef zr_boost_1_81::asio::enable_terminal_cancellation default_filter;
...
public:
  explicit impl_cancellable(CancellationFilter cancellation_filter,
      zr_boost_1_81::asio::system_timer& timer)
  ...
  template <typename Self>
  void operator()(Self& self,
      const zr_boost_1_81::system::error_code& ec = zr_boost_1_81::system::error_code())
  {
    switch (state_)
    {
    case starting:
      if (!zr_boost_1_81::asio::is_same<CancellationFilter, default_filter>::value)
        self.reset_cancellation_state(cancellation_filter_);
      state_ = waiting;
      timer_.expires_after(zr_boost_1_81::asio::chrono::milliseconds(100));
...
private:
  ...
  zr_boost_1_81::asio::system_timer& timer_;
...
async_cancellable(CancellationFilter cancellation_filter,
    ...
    BOOST_ASIO_MOVE_ARG(CompletionToken) token)
{
  return zr_boost_1_81::asio::async_compose<CompletionToken, void(bool)>(
...
void compose_default_cancellation_test()
{
  ...
  zr_boost_1_81::asio::io_context ioc;
  zr_boost_1_81::asio::system_timer timer(ioc);
  zr_boost_1_81::asio::cancellation_signal signal;
  int count = 0;
  bool result = false;
  ...
  async_cancellable(default_filter(), timer,
      zr_boost_1_81::asio::bind_cancellation_slot(signal.slot(),
        bindns::bind(&compose_partial_cancellation_handler,
        &count, &result, _1)));

  // Total cancellation unsupported. Operation completes successfully.
  signal.emit(zr_boost_1_81::asio::cancellation_type::total);
  ...
  signal.emit(zr_boost_1_81::asio::cancellation_type::partial);
  ...
  signal.emit(zr_boost_1_81::asio::cancellation_type::terminal);
...
void compose_partial_cancellation_test()
{
  ...
  async_cancellable(zr_boost_1_81::asio::enable_partial_cancellation(), timer,
      bindns::bind(&compose_partial_cancellation_handler,
        &count, &result, _1));
...
void compose_total_cancellation_test()
{
  ...
  async_cancellable(zr_boost_1_81::asio::enable_total_cancellation(), timer,
      bindns::bind(&compose_partial_cancellation_handler,
        &count, &result, _1));
