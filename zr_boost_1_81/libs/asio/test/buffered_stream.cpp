...
typedef zr_boost_1_81::asio::buffered_stream<
    zr_boost_1_81::asio::ip::tcp::socket> stream_type;

void write_some_handler(const zr_boost_1_81::system::error_code&, std::size_t)
{
}

void flush_handler(const zr_boost_1_81::system::error_code&, std::size_t)
{
}

void fill_handler(const zr_boost_1_81::system::error_code&, std::size_t)
{
}

void read_some_handler(const zr_boost_1_81::system::error_code&, std::size_t)
{
}
...
void test_compile()
{
  ...
  try
  {
    ...
    array<zr_boost_1_81::asio::mutable_buffer, 2> mutable_buffers = {{
        zr_boost_1_81::asio::buffer(mutable_char_buffer, 10),
        zr_boost_1_81::asio::buffer(mutable_char_buffer + 10, 10) }};
    array<zr_boost_1_81::asio::const_buffer, 2> const_buffers = {{
        zr_boost_1_81::asio::buffer(const_char_buffer, 10),
        zr_boost_1_81::asio::buffer(const_char_buffer + 10, 10) }};
...
void test_sync_operations()
{
  ...
  zr_boost_1_81::asio::ip::tcp::acceptor acceptor(io_context,
      zr_boost_1_81::asio::ip::tcp::endpoint(zr_boost_1_81::asio::ip::tcp::v4(), 0));
  zr_boost_1_81::asio::ip::tcp::endpoint server_endpoint = acceptor.local_endpoint();
  server_endpoint.address(zr_boost_1_81::asio::ip::address_v4::loopback());
  ...
  const zr_boost_1_81::asio::const_buffer write_buf = zr_boost_1_81::asio::buffer(write_data);

  std::size_t bytes_written = 0;
  while (bytes_written < sizeof(write_data))
  {
    bytes_written += client_socket.write_some(
        zr_boost_1_81::asio::buffer(write_buf + bytes_written));
    client_socket.flush();
  }

  char read_data[sizeof(write_data)];
  const zr_boost_1_81::asio::mutable_buffer read_buf = zr_boost_1_81::asio::buffer(read_data);

  std::size_t bytes_read = 0;
  while (bytes_read < sizeof(read_data))
  {
    bytes_read += server_socket.read_some(
        zr_boost_1_81::asio::buffer(read_buf + bytes_read));
  }
  ...
  bytes_read = client_socket.read_some(
      zr_boost_1_81::asio::buffer(read_buf), error);

  BOOST_ASIO_CHECK(bytes_read == 0);
  BOOST_ASIO_CHECK(error == zr_boost_1_81::asio::error::eof);

  client_socket.close(error);
}

void handle_accept(const zr_boost_1_81::system::error_code& e)
{
  BOOST_ASIO_CHECK(!e);
}

void handle_write(const zr_boost_1_81::system::error_code& e,
    std::size_t bytes_transferred,
    std::size_t* total_bytes_written)
{
  BOOST_ASIO_CHECK(!e);
  if (e)
    throw zr_boost_1_81::system::system_error(e); // Terminate test.
  *total_bytes_written += bytes_transferred;
}

void handle_flush(const zr_boost_1_81::system::error_code& e)
{
  BOOST_ASIO_CHECK(!e);
}

void handle_read(const zr_boost_1_81::system::error_code& e,
    std::size_t bytes_transferred,
...
void handle_read_eof(const zr_boost_1_81::system::error_code& e,
    std::size_t bytes_transferred)
{
  BOOST_ASIO_CHECK(e == zr_boost_1_81::asio::error::eof);
...
void test_async_operations()
{
  ...
  server_socket.close();
  client_socket.async_read_some(zr_boost_1_81::asio::buffer(read_buf), handle_read_eof);
}
