...
{
  zr_boost_1_81::asio::detail::test_name();
  zr_boost_1_81::asio::detail::test_errors();
...
inline int end_test_suite(const char* name)
{
  ...
  long errors = zr_boost_1_81::asio::detail::test_errors();
  if (errors == 0)
...
{
  test_name() = name;
  long errors_before = zr_boost_1_81::asio::detail::test_errors();
  Test();
...
#define BOOST_ASIO_TEST_SUITE(name, tests) \
  int main() \
  { \
    zr_boost_1_81::asio::detail::begin_test_suite(name); \
    tests \
    return zr_boost_1_81::asio::detail::end_test_suite(name); \
  }

#define BOOST_ASIO_TEST_CASE(test) \
  zr_boost_1_81::asio::detail::run_test<&test>(#test);

#define BOOST_ASIO_TEST_CASE2(test1, test2) \
  zr_boost_1_81::asio::detail::run_test<&test1, test2>(#test1 "," #test2);

#define BOOST_ASIO_TEST_CASE3(test1, test2, test3) \
  zr_boost_1_81::asio::detail::run_test<&test1, test2, test3>( \
    #test1 "," #test2 "," #test3);

#define BOOST_ASIO_TEST_CASE4(test1, test2, test3, test4) \
  zr_boost_1_81::asio::detail::run_test<&test1, test2, test3, test4>( \
    #test1 "," #test2 "," #test3 "," #test4);

#define BOOST_ASIO_TEST_CASE5(test1, test2, test3, test4, test5) \
  zr_boost_1_81::asio::detail::run_test<&test1, test2, test3, test4, test5>( \
    #test1 "," #test2 "," #test3 "," #test4 "," #test5);

#define BOOST_ASIO_COMPILE_TEST_CASE(test) \
  zr_boost_1_81::asio::detail::compile_test<&test>(#test);
