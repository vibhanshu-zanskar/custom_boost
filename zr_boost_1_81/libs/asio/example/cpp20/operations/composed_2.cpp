...
template <
  zr_boost_1_81::asio::completion_token_for<void(zr_boost_1_81::system::error_code, std::size_t)>
    CompletionToken>
auto async_write_message(tcp::socket& socket,
  ...
  // However, when the completion token is zr_boost_1_81::asio::yield_context (used for
  // stackful coroutines) the return type would be std::size_t, and when the
  // completion token is zr_boost_1_81::asio::use_future it would be std::future<std::size_t>.
  // When the completion token is zr_boost_1_81::asio::deferred, the return type differs for
  ...
  // the return type of zr_boost_1_81::asio::async_initiate.
{
  ...
  //
  //   void(zr_boost_1_81::system::error_code error, std::size_t)
  //
  ...
  // straight through.)
  auto initiation = [](
      zr_boost_1_81::asio::completion_handler_for<void(zr_boost_1_81::system::error_code, std::size_t)>
      ...
      bool allow_partial_write)
  {
    ...
    else
    {
      ...
      return zr_boost_1_81::asio::async_write(socket,
          zr_boost_1_81::asio::buffer(message, std::strlen(message)),
          std::forward<decltype(completion_handler)>(completion_handler));
    }
  };
  ...
  return zr_boost_1_81::asio::async_initiate<
    CompletionToken, void(zr_boost_1_81::system::error_code, std::size_t)>(
...
void test_callback()
{
  ...
  async_write_message(socket, "Testing callback\r\n", false,
      [](const zr_boost_1_81::system::error_code& error, std::size_t n)
      {
        if (!error)
        {
...
void test_deferred()
{
  ...
  zr_boost_1_81::asio::async_operation auto op = async_write_message(
      socket, "Testing deferred\r\n", false, zr_boost_1_81::asio::deferred);
...
void test_future()
{
  ...
  std::future<std::size_t> f = async_write_message(
      socket, "Testing future\r\n", false, zr_boost_1_81::asio::use_future);
