...
// NOTE: This example requires the new zr_boost_1_81::asio::async_initiate function. For
...
template <typename T,
  zr_boost_1_81::asio::completion_token_for<void(zr_boost_1_81::system::error_code)> CompletionToken>
auto async_write_messages(tcp::socket& socket,
  ...
  // void. In this example, when the completion token is zr_boost_1_81::asio::yield_context
  // (used for stackful coroutines) the return type would also be void, as
  // there is no non-error argument to the completion handler. When the
  // completion token is zr_boost_1_81::asio::use_future it would be std::future<void>. When
  // the completion token is zr_boost_1_81::asio::deferred, the return type differs for each
  // asynchronous operation.
  //
  // In C++20 we can omit the return type as it is automatically deduced from
  // the return type of zr_boost_1_81::asio::async_initiate.
{
  ...
  //
  //   void(zr_boost_1_81::system::error_code error)
  //
  ...
  // straight through.)
  auto initiation = [](
      zr_boost_1_81::asio::completion_handler_for<void(zr_boost_1_81::system::error_code)>
        auto&& completion_handler,
      tcp::socket& socket,
      std::unique_ptr<std::string> encoded_message,
      std::size_t repeat_count,
      std::unique_ptr<zr_boost_1_81::asio::steady_timer> delay_timer)
  {
    ...
    struct intermediate_completion_handler
    {
      ...
      std::unique_ptr<zr_boost_1_81::asio::steady_timer> delay_timer_;
      ...
      zr_boost_1_81::asio::executor_work_guard<tcp::socket::executor_type> io_work_;
      ...
      void operator()(const zr_boost_1_81::system::error_code& error, std::size_t = 0)
      {
        if (!error)
        {
          switch (state_)
          ...
          case waiting:
            ...
            zr_boost_1_81::asio::async_write(socket_,
                zr_boost_1_81::asio::buffer(*encoded_message_), std::move(*this));
            return; // Composed operation not yet complete.
          }
        }
      ...
      using executor_type = zr_boost_1_81::asio::associated_executor_t<
          typename std::decay<decltype(completion_handler)>::type,
          tcp::socket::executor_type>;

      executor_type get_executor() const noexcept
      {
        return zr_boost_1_81::asio::get_associated_executor(
      ...
      // handler does not have its own.
      using allocator_type = zr_boost_1_81::asio::associated_allocator_t<
          typename std::decay<decltype(completion_handler)>::type,
          std::allocator<void>>;

      allocator_type get_allocator() const noexcept
      {
        return zr_boost_1_81::asio::get_associated_allocator(
    ...
    // completion handler.
    auto encoded_message_buffer = zr_boost_1_81::asio::buffer(*encoded_message);
    zr_boost_1_81::asio::async_write(socket, encoded_message_buffer,
        intermediate_completion_handler{
          socket, std::move(encoded_message),
          repeat_count, std::move(delay_timer),
          intermediate_completion_handler::starting,
          zr_boost_1_81::asio::make_work_guard(socket.get_executor()),
  ...
  std::unique_ptr<zr_boost_1_81::asio::steady_timer> delay_timer(
      new zr_boost_1_81::asio::steady_timer(socket.get_executor()));

  // The zr_boost_1_81::asio::async_initiate function takes:
  //
  ...
  return zr_boost_1_81::asio::async_initiate<
    CompletionToken, void(zr_boost_1_81::system::error_code)>(
...
void test_deferred()
{
  ...
  zr_boost_1_81::asio::async_operation auto op = async_write_messages(
      socket, "Testing deferred\r\n", 5, zr_boost_1_81::asio::deferred);
...
void test_future()
{
  ...
  std::future<void> f = async_write_messages(
      socket, "Testing future\r\n", 5, zr_boost_1_81::asio::use_future);
