  ...
  // the return type of zr_boost_1_81::asio::async_initiate.
{
      ...
      const char* message)
  {
    ...
    //
    //   void(zr_boost_1_81::system::error_code error, std::size n)
    //
    ...
    auto executor = zr_boost_1_81::asio::get_associated_executor(
        completion_handler, socket.get_executor());

    // ... and then binding this executor to our adapted completion handler
    // using the zr_boost_1_81::asio::bind_executor function.
    std::size_t length = std::strlen(message);
    if (length == 0)
    {
      zr_boost_1_81::asio::post(
          zr_boost_1_81::asio::bind_executor(executor,
            std::bind(std::forward<decltype(completion_handler)>(
                completion_handler), zr_boost_1_81::asio::error::invalid_argument)));
    }
    else
    {
      zr_boost_1_81::asio::async_write(socket,
          zr_boost_1_81::asio::buffer(message, length),
          zr_boost_1_81::asio::bind_executor(executor,
            std::bind(std::forward<decltype(completion_handler)>(
...
void test_deferred()
{
  ...
  zr_boost_1_81::asio::async_operation auto op =
    async_write_message(socket, "", zr_boost_1_81::asio::deferred);
...
void test_future()
{
  ...
  std::future<void> f = async_write_message(
      socket, "", zr_boost_1_81::asio::use_future);
