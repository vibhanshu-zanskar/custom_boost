...
auto async_read_input(const std::string& prompt, CompletionToken&& token)
{
      ...
      const std::string& prompt)
  {
    ...
    auto work = zr_boost_1_81::asio::make_work_guard(handler);
    ...
    read_input(prompt,
        ...
        ](std::string result) mutable
        {
          ...
          auto alloc = zr_boost_1_81::asio::get_associated_allocator(
              handler, zr_boost_1_81::asio::recycling_allocator<void>());

          // Dispatch the completion handler through the handler's associated
          // executor, using the handler's associated allocator.
          zr_boost_1_81::asio::dispatch(work.get_executor(),
              zr_boost_1_81::asio::bind_allocator(alloc,
                [
                  handler = std::move(handler),
