...
auto async_write_messages(tcp::socket& socket,
  ...
  // the return type of zr_boost_1_81::asio::async_compose.
{
  ...
  // provided for us by the zr_boost_1_81::asio::async_compose function, and takes care
  ...
  return zr_boost_1_81::asio::async_compose<
    CompletionToken, void(zr_boost_1_81::system::error_code)>(
      [
        ...
        // The coroutine state.
        coro = zr_boost_1_81::asio::coroutine()
      ]
      (
        auto& self,
      ...
      ) mutable
      {
        reenter (coro)
        {
          while (repeat_count > 0)
          {
            ...
            yield zr_boost_1_81::asio::async_write(socket,
                zr_boost_1_81::asio::buffer(*encoded_message), std::move(self));
            if (error)
              break;
          }
