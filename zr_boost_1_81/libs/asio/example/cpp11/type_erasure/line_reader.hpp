...
private:
  virtual void async_read_line_impl(std::string prompt,
      zr_boost_1_81::asio::any_completion_handler<void(zr_boost_1_81::system::error_code, std::string)> handler) = 0;
...
public:
  ...
  auto async_read_line(std::string prompt, CompletionToken&& token)
    -> decltype(
        zr_boost_1_81::asio::async_initiate<CompletionToken, void(zr_boost_1_81::system::error_code, std::string)>(
          initiate_read_line(), token, this, prompt))
  {
    return zr_boost_1_81::asio::async_initiate<CompletionToken, void(zr_boost_1_81::system::error_code, std::string)>(
        initiate_read_line(), token, this, prompt);
  }
};
