...
{
public:
  session(zr_boost_1_81::asio::ssl::stream<tcp::socket> socket)
...
private:
  void do_handshake()
  {
    ...
    socket_.async_handshake(zr_boost_1_81::asio::ssl::stream_base::server, 
        [this, self](const zr_boost_1_81::system::error_code& error)
        {
          if (!error)
          {
            do_read();
  ...
  void do_read()
  {
    ...
    socket_.async_read_some(zr_boost_1_81::asio::buffer(data_),
        [this, self](const zr_boost_1_81::system::error_code& ec, std::size_t length)
        {
          if (!ec)
          {
            do_write(length);
  ...
  void do_write(std::size_t length)
  {
    ...
    zr_boost_1_81::asio::async_write(socket_, zr_boost_1_81::asio::buffer(data_, length),
        [this, self](const zr_boost_1_81::system::error_code& ec,
          std::size_t /*length*/)
        {
          if (!ec)
          {
  ...
  zr_boost_1_81::asio::ssl::stream<tcp::socket> socket_;
  char data_[1024];
};
...
private:
  ...
  void do_accept()
  {
    acceptor_.async_accept(
        [this](const zr_boost_1_81::system::error_code& error, tcp::socket socket)
        {
          if (!error)
          {
            std::make_shared<session>(
                zr_boost_1_81::asio::ssl::stream<tcp::socket>(
