...
// NOTE: This example requires the new zr_boost_1_81::asio::async_compose function. For
...
struct async_write_messages_implementation
{
  ...
  // The coroutine state.
  zr_boost_1_81::asio::coroutine coro_;
  ...
  // completion handler is provided for us by the zr_boost_1_81::asio::async_compose
  ...
  void operator()(Self& self,
      const zr_boost_1_81::system::error_code& error = zr_boost_1_81::system::error_code(),
      std::size_t = 0)
  {
    reenter (coro_)
    {
      while (repeat_count_ > 0)
      {
        ...
        yield zr_boost_1_81::asio::async_write(socket_,
            zr_boost_1_81::asio::buffer(*encoded_message_), std::move(self));
        if (error)
          break;
      }
...
auto async_write_messages(tcp::socket& socket,
  ...
  //
  // In C++11 we deduce the type from the call to zr_boost_1_81::asio::async_compose.
  -> decltype(
      zr_boost_1_81::asio::async_compose<
          ...
          token, socket))
{
  ...
  // The zr_boost_1_81::asio::async_compose function takes:
  //
  ...
  return zr_boost_1_81::asio::async_compose<
    CompletionToken, void(zr_boost_1_81::system::error_code)>(
      async_write_messages_implementation{socket,
        std::move(encoded_message), repeat_count,
        std::move(delay_timer), zr_boost_1_81::asio::coroutine()},
      token, socket);
}
