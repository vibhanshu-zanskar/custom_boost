...
private:
  ...
  void read_line()
  {
    ...
    zr_boost_1_81::asio::async_read_until(socket_,
        ...
        [this, self](const zr_boost_1_81::system::error_code& error, std::size_t n)
        {
  ...
  void check_deadline(steady_timer& deadline)
  {
    auto self(shared_from_this());
    deadline.async_wait(
        [this, self, &deadline](const zr_boost_1_81::system::error_code& /*error*/)
        {
          // Check if the session was stopped while the operation was pending.
          if (stopped())
            return;
...
private:
  ...
  {
    zr_boost_1_81::system::error_code ignored_error;
    socket_.send(zr_boost_1_81::asio::buffer(msg), 0, ignored_error);
  }
