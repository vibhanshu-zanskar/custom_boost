...
typedef zr_boost_1_81::asio::basic_stream_socket<tcp,
    zr_boost_1_81::asio::io_context::executor_type> tcp_socket;
...
struct close_after
{
  close_after(zr_boost_1_81::asio::chrono::steady_clock::duration t, tcp_socket& s)
    : timeout_(t), socket_(s)
  {
  }

  // The maximum time to wait for an asynchronous operation to complete.
  zr_boost_1_81::asio::chrono::steady_clock::duration timeout_;
...
template <typename T>
class async_result<close_after, void(zr_boost_1_81::system::error_code, T)>
{
public:
  ...
  public:
    ...
    void operator()(zr_boost_1_81::system::error_code ec, T t)
    {
      *ec_ = ec;
      *t_ = t;
    }
  ...
  return_type get()
  {
    zr_boost_1_81::asio::io_context& io_context = zr_boost_1_81::asio::query(
        socket_.get_executor(), zr_boost_1_81::asio::execution::context);
    ...
    return ec_ ? throw zr_boost_1_81::system::system_error(ec_) : t_;
  }
...
int main(int argc, char* argv[])
{
  try
  {
    ...
    zr_boost_1_81::asio::async_connect(socket, endpoints,
        close_after(zr_boost_1_81::asio::chrono::seconds(10), socket));
    ...
    zr_boost_1_81::asio::async_write(socket, zr_boost_1_81::asio::buffer(msg),
        close_after(zr_boost_1_81::asio::chrono::seconds(10), socket));

    for (std::string input_buffer;;)
    {
      // Run an asynchronous read operation with a timeout.
      std::size_t n = zr_boost_1_81::asio::async_read_until(socket,
          zr_boost_1_81::asio::dynamic_buffer(input_buffer), '\n',
          close_after(zr_boost_1_81::asio::chrono::seconds(10), socket));
