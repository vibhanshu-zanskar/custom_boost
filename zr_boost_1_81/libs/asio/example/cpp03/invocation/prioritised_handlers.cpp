...
class handler_priority_queue : public zr_boost_1_81::asio::execution_context
{
public:
  void add(int priority, zr_boost_1_81::function<void()> function)
  {
  ...
  zr_boost_1_81::asio::executor_binder<Handler, executor>
  wrap(int priority, Handler handler)
  {
    return zr_boost_1_81::asio::bind_executor(executor(*this, priority), handler);
  }
...
private:
  ...
  {
  public:
    queued_handler(int p, zr_boost_1_81::function<void()> f)
  ...
  private:
    int priority_;
    zr_boost_1_81::function<void()> function_;
  };
...
void high_priority_handler(const zr_boost_1_81::system::error_code& /*ec*/)
{
  std::cout << "High priority handler\n";
}

void middle_priority_handler(const zr_boost_1_81::system::error_code& /*ec*/)
{
...
int main()
{
  ...
  zr_boost_1_81::asio::post(io_context, pri_queue.wrap(0, low_priority_handler));

  // Start an asynchronous accept that will complete immediately.
  tcp::endpoint endpoint(zr_boost_1_81::asio::ip::address_v4::loopback(), 0);
  ...
  zr_boost_1_81::asio::steady_timer timer(io_context);
  timer.expires_at(zr_boost_1_81::asio::steady_timer::time_point::min());
