...
public:
  ...
  void do_read(zr_boost_1_81::system::error_code& ec)
  {
    if (std::size_t len = socket_.read_some(zr_boost_1_81::asio::buffer(data_), ec))
    {
      write_buffer_ = zr_boost_1_81::asio::buffer(data_, len);
      state_ = writing;
    }
  }
  ...
  void do_write(zr_boost_1_81::system::error_code& ec)
  {
    if (std::size_t len = socket_.write_some(
          zr_boost_1_81::asio::buffer(write_buffer_), ec))
    {
      write_buffer_ = write_buffer_ + len;
      state_ = zr_boost_1_81::asio::buffer_size(write_buffer_) > 0 ? writing : reading;
    }
  }
...
private:
  ...
  zr_boost_1_81::array<char, 128> data_;
  zr_boost_1_81::asio::const_buffer write_buffer_;
};
...
class connection
  : public zr_boost_1_81::enable_shared_from_this<connection>
{
public:
  typedef zr_boost_1_81::shared_ptr<connection> pointer;

  static pointer create(const zr_boost_1_81::asio::any_io_executor& ex)
  {
...
private:
  connection(const zr_boost_1_81::asio::any_io_executor& ex)
    : socket_(ex),
  ...
  void start_operations()
  {
    ...
    if (session_impl_.want_read() && !read_in_progress_)
    {
      ...
      socket_.async_wait(tcp::socket::wait_read,
          zr_boost_1_81::bind(&connection::handle_read,
            shared_from_this(),
    ...
    if (session_impl_.want_write() && !write_in_progress_)
    {
      ...
      socket_.async_wait(tcp::socket::wait_write,
          zr_boost_1_81::bind(&connection::handle_write,
            shared_from_this(),
            zr_boost_1_81::asio::placeholders::error));
    }
  }

  void handle_read(zr_boost_1_81::system::error_code ec)
  {
    read_in_progress_ = false;
    ...
    if (!ec || ec == zr_boost_1_81::asio::error::would_block)
      start_operations();
  ...
  void handle_write(zr_boost_1_81::system::error_code ec)
  {
    write_in_progress_ = false;
...
private:
  void start_accept()
  {
    ...
    acceptor_.async_accept(new_connection->socket(),
        zr_boost_1_81::bind(&server::handle_accept, this, new_connection,
          zr_boost_1_81::asio::placeholders::error));
  }
