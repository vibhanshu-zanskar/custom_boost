...
zr_boost_1_81::tribool request_parser::consume(request& req, char c)
{
  reenter (this)
  {
    req.method.clear();
    ...
    while (is_char(c) && !is_ctl(c) && !is_tspecial(c) && c != ' ')
    {
      ...
      yield return zr_boost_1_81::indeterminate;
    }
    if (req.method.empty())
      return false;
    ...
    for (std::size_t i = 0; i < req.headers.size(); ++i)
    {
      if (headers_equal(req.headers[i].name, content_length_name_))
      {
        ...
        {
          content_length_ =
            zr_boost_1_81::lexical_cast<std::size_t>(req.headers[i].value);
        }
        catch (zr_boost_1_81::bad_lexical_cast&)
        {
          return false;
        }
      }
