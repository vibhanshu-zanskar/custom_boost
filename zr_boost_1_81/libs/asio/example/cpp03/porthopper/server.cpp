...
typedef zr_boost_1_81::shared_ptr<tcp::socket> tcp_socket_ptr;
typedef zr_boost_1_81::shared_ptr<zr_boost_1_81::asio::steady_timer> timer_ptr;
typedef zr_boost_1_81::shared_ptr<control_request> control_request_ptr;
...
public:
      ...
      next_frame_number_(1)
  {
    ...
    acceptor_.async_accept(*new_socket,
        zr_boost_1_81::bind(&server::handle_accept, this,
          zr_boost_1_81::asio::placeholders::error, new_socket));

    // Start the timer used to generate outgoing frames.
    timer_.expires_after(zr_boost_1_81::asio::chrono::milliseconds(100));
    timer_.async_wait(zr_boost_1_81::bind(&server::handle_timer, this));
  }

  // Handle a new control connection.
  void handle_accept(const zr_boost_1_81::system::error_code& ec, tcp_socket_ptr socket)
  {
    if (!ec)
    {
      // Start receiving control requests on the connection.
      control_request_ptr request(new control_request);
      zr_boost_1_81::asio::async_read(*socket, request->to_buffers(),
          zr_boost_1_81::bind(&server::handle_control_request, this,
            zr_boost_1_81::asio::placeholders::error, socket, request));
    }
  ...
  void handle_control_request(const zr_boost_1_81::system::error_code& ec,
      tcp_socket_ptr socket, control_request_ptr request)
  {
    if (!ec)
    {
      ...
      timer_ptr delay_timer(
          new zr_boost_1_81::asio::steady_timer(acceptor_.get_executor()));
      delay_timer->expires_after(zr_boost_1_81::asio::chrono::seconds(2));
      delay_timer->async_wait(
          zr_boost_1_81::bind(&server::handle_control_request_timer, this,
