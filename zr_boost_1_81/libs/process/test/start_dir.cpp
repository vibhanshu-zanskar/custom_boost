...
struct test_dir
{
    ...
    { BOOST_REQUIRE_NO_THROW(zr_boost_1_81::process::filesystem::create_directory(s)); }
    ~test_dir() { zr_boost_1_81::process::filesystem::remove(s_); }
};
...
BOOST_AUTO_TEST_CASE(start_in_dir)
{
    ...
    std::error_code ec;
    bp::child c(
        bp::exe=zr_boost_1_81::process::filesystem::absolute(master_test_suite().argv[1]).string(),
        bp::args +={"test", "--pwd"},
        bp::start_dir = dir.s_,
        bp::std_out>is,
        ec
    ...
    std::string s;
    std::getline(is, s);
    auto path_read = zr_boost_1_81::process::filesystem::absolute(zr_boost_1_81::process::filesystem::path(s)).string();
    auto path_set  = zr_boost_1_81::process::filesystem::absolute(dir.s_).string();
