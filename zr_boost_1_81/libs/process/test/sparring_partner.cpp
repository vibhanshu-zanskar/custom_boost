...
using namespace zr_boost_1_81::program_options;
...
int main(int argc, char *argv[])
{
    ...
    {
        std::cout << zr_boost_1_81::process::filesystem::current_path().string() << std::endl;
    }
    else if (vm.count("query"))
    {
        auto key = vm["query"].as<std::string>();
        auto env = zr_boost_1_81::this_process::environment();
        auto val = env[key];
        if (val.empty())
    ...
    {
        using namespace zr_boost_1_81::iostreams;
        std::vector<std::string> v = vm["posix-echo-one"].as<std::vector<std::string> >();
        int fd = zr_boost_1_81::lexical_cast<int>(v[0]);
    ...
    else if (vm.count("posix-echo-two"))
    {
        ...
        int fd1 = zr_boost_1_81::lexical_cast<int>(v[0]);
        file_descriptor_sink sink1(fd1, close_handle);
        stream<file_descriptor_sink> os1(sink1);
        os1 << v[1] << std::endl;
        int fd2 = zr_boost_1_81::lexical_cast<int>(v[2]);
    ...
    else if (vm.count("has-handle"))
    {
        ...
        const auto handle = reinterpret_cast<zr_boost_1_81::this_process::native_handle_type>(vm["has-handle"].as<std::uintptr_t>());
#else
        const auto handle = static_cast<zr_boost_1_81::this_process::native_handle_type>(vm["has-handle"].as<std::uintptr_t>());
#endif
        auto all_handles = zr_boost_1_81::this_process::get_handles();
