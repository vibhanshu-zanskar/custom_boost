...
namespace read_graphviz_detail
{
        ...
        const std::string& errmsg, const token& bad_token)
    {
        return bad_graphviz_syntax(errmsg + " (token is \""
            + zr_boost_1_81::lexical_cast< std::string >(bad_token) + "\")");
    }
    ...
    struct tokenizer
    {
        ...
        // Precomputed regexes
        zr_boost_1_81::regex stuff_to_skip;
        zr_boost_1_81::regex basic_id_token;
        zr_boost_1_81::regex punctuation_token;
        zr_boost_1_81::regex number_token;
        zr_boost_1_81::regex quoted_string_token;
        zr_boost_1_81::regex xml_tag_token;
        zr_boost_1_81::regex cdata;
        ...
        void skip()
        {
            zr_boost_1_81::match_results< std::string::const_iterator > results;
#ifndef NDEBUG
            bool found =
#endif
                zr_boost_1_81::regex_search(begin, end, results, stuff_to_skip);
#ifndef NDEBUG
            BOOST_ASSERT(found);
#endif
            zr_boost_1_81::sub_match< std::string::const_iterator > sm1
                = results.suffix();
        ...
        token get_token_raw()
        {
            ...
            bool found;
            zr_boost_1_81::match_results< std::string::const_iterator > results;
            found = zr_boost_1_81::regex_search(begin, end, results, basic_id_token);
            if (found)
            {
                std::string str = results[1].str();
                std::string str_lower = zr_boost_1_81::algorithm::to_lower_copy(str);
            ...
            found = zr_boost_1_81::regex_search(begin, end, results, punctuation_token);
            if (found)
            {
                std::string str = results[1].str();
                begin = results.suffix().first;
            ...
            found = zr_boost_1_81::regex_search(begin, end, results, number_token);
            if (found)
            {
                std::string str = results[1].str();
                begin = results.suffix().first;
            ...
            found
                = zr_boost_1_81::regex_search(begin, end, results, quoted_string_token);
            if (found)
            {
                std::string str = results[1].str();
                begin = results.suffix().first;
            ...
            if (*begin == '<')
            {
                ...
                do
                {
                    ...
                    found = zr_boost_1_81::regex_search(
                    ...
                    found = zr_boost_1_81::regex_search(begin, end, results, cdata);
                    if (found)
                    {
                        begin = results.suffix().first;
                        continue;
    ...
    struct parser
    {
        ...
        {
            zr_boost_1_81::throw_exception(parse_error(str, peek()));
        }
        ...
        subgraph_name parse_subgraph(const token& first_token)
        {
            ...
            {
                name = "___subgraph_"
                    + zr_boost_1_81::lexical_cast< std::string >(++sgcounter);
            }
    ...
    void translate_results_to_graph(
        const parser_result& r, ::zr_boost_1_81::detail::graph::mutate_graph* mg)
    {
        typedef zr_boost_1_81::detail::graph::edge_t edge;
...
namespace detail
{
    namespace graph
    {
        ...
        BOOST_GRAPH_DECL bool read_graphviz_new(
            const std::string& str, zr_boost_1_81::detail::graph::mutate_graph* mg)
        {
