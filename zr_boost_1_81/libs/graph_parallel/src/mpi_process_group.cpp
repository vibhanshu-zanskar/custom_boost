...
struct mpi_process_group::deallocate_block
{
  ...
  void operator()(int* block_num)
  {
    ...
    fprintf(stderr, "Processor %i deallocated block #%i\n", 
            zr_boost_1_81::mpi::communicator().rank(), *block_num);
#endif
...
mpi_process_group::impl::impl(std::size_t num_headers, std::size_t buffer_sz, 
  ...
  : comm(parent_comm, zr_boost_1_81::mpi::comm_duplicate),
    oob_reply_comm(parent_comm, zr_boost_1_81::mpi::comm_duplicate),
    allocated_tags(zr_boost_1_81::mpi::environment::max_tag())
{
  max_sent=0;
  max_received=0;
  int n = comm.size();
...
void mpi_process_group::impl::set_batch_size(std::size_t header_num, std::size_t buffer_sz)
{
  ...
  zr_boost_1_81::mpi::packed_oarchive oa(comm);
...
mpi_process_group::impl::~impl()
{
  ...
  delete blocks.front();
  if (!zr_boost_1_81::mpi::environment::finalized()) 
...
mpi_process_group::~mpi_process_group()  {
  /*
  std::string msg = zr_boost_1_81::lexical_cast<std::string>(process_id(*this)) + " " +
            zr_boost_1_81::lexical_cast<std::string>(impl_->max_received) + " " +
            zr_boost_1_81::lexical_cast<std::string>(impl_->max_sent) + "\n";
     std::cerr << msg << "\n";
     */
}
...
#endif
#endif
    zr_boost_1_81::mpi::packed_oarchive oa(impl_->comm,req.buffer);
    oa << outgoing;
...
void mpi_process_group::receive_batch(zr_boost_1_81::mpi::status& status) const
{
  ...
  zr_boost_1_81::mpi::packed_iarchive in(impl_->comm,size);
...
poll(bool wait, int block, bool synchronizing) const
{
  ...
  //wait = false;
  optional<zr_boost_1_81::mpi::status> status;
  bool finished=false;
  optional<std::pair<int, int> > result;
  do {
    poll_requests(block);
