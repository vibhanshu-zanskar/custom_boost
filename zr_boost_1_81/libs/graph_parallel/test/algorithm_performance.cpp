                                       ...
                                       typename property_traits<EdgeWeightMap>::value_type C)
{
  ...
  m = zr_boost_1_81::parallel::all_reduce(pg, m, std::plus<edges_size_type>());
...

  {
    ...
    n0 = zr_boost_1_81::parallel::all_reduce(pg, n0, std::plus<vertices_size_type>());
...
void test_undirected_algorithms(const Graph& g)
{
  ...
  start = get_time();
  num_components = zr_boost_1_81::graph::distributed::connected_components_ps(g, component);
  end = get_time();
  if (id == 0) 
...
int main(int argc, char* argv[])
{
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--vertices")
      n = zr_boost_1_81::lexical_cast<size_t>( argv[i+1] );

    if (arg == "--seed")
      seed = zr_boost_1_81::lexical_cast<uint64_t>( argv[i+1] );

    if (arg == "--edges")
      m = zr_boost_1_81::lexical_cast<size_t>( argv[i+1] );

    if (arg == "--max-weight")
      c = zr_boost_1_81::lexical_cast<int>( argv[i+1] );

    if (arg == "--batch-buffer-size") {
      batch_buffer_size = zr_boost_1_81::lexical_cast<size_t>( argv[i+1] );
      num_headers = batch_buffer_size / 16;
      num_headers = num_headers > 0 ? num_headers : 1;
    }

    if (arg == "--rewire-probability")
      p = zr_boost_1_81::lexical_cast<double>( argv[i+1] );

    if (arg == "--num-sources")
      num_sources = zr_boost_1_81::lexical_cast<size_t>( argv[i + 1] );
