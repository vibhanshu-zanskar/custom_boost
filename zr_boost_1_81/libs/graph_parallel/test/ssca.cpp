                                         ...
                                         typename graph_traits<Graph>::vertex_descriptor> > & global_S)
{
  ...
  int global_max = all_reduce(pg, max_, zr_boost_1_81::parallel::maximum<int>());
  if (max_ < global_max)
    S.clear();
                             ...
                             int subGraphEdgeLength)
{
  ...
  zr_boost_1_81::queue<vertex_descriptor> Q;
                        ...
                        std::vector<typename graph_traits<Graph>::vertex_descriptor>& max_bc_vec)
{
  ...
  using zr_boost_1_81::parallel::all_gather;
  ...
  centrality_type global_max = all_reduce(pg, max_, zr_boost_1_81::parallel::minimum<int>());
...
//                    get(edge_weight, g), 
  ...
  n0 = zr_boost_1_81::parallel::all_reduce(pg, local_n0, std::plus<vertices_size_type>());
...
int main(int argc, char* argv[])
{
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--vertices")
      n = zr_boost_1_81::lexical_cast<vertices_size_type>( argv[i+1] );

    if (arg == "--seed")
      seed = zr_boost_1_81::lexical_cast<uint64_t>( argv[i+1] );

    if (arg == "--full-bc")
      full_bc = (argv[i+1]== "true");

    if (arg == "--max-weight")
      maxEdgeWeight = zr_boost_1_81::lexical_cast<int>( argv[i+1] );

    if (arg == "--subgraph-edge-length")
      subGraphEdgeLength = zr_boost_1_81::lexical_cast<int>( argv[i+1] );
    
    if (arg == "--edges")
      m = zr_boost_1_81::lexical_cast<edges_size_type>( argv[i+1] );

    if (arg == "--k4alpha")
      K4Alpha = zr_boost_1_81::lexical_cast<int>( argv[i+1] );
    ...
    if (arg == "--scale") {
      vertices_size_type scale  = zr_boost_1_81::lexical_cast<vertices_size_type>( argv[i+1] );
