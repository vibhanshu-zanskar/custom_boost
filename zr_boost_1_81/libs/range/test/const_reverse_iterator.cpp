// Boost.Range library
//
//  Copyright Neil Groves 2014. Use, modification and
//  distribution is subject to the Boost Software License, Version
//  1.0. (See accompanying file LICENSE_1_0.txt or copy at
//  http://www.boost.org/LICENSE_1_0.txt)
//
// For more information, see http://www.boost.org/libs/range/
//

#define BOOST_ALLOW_DEPRECATED_HEADERS
#include <boost/range/const_reverse_iterator.hpp>
#include <boost/static_assert.hpp>
#include <boost/type_traits/is_same.hpp>

#include <boost/test/test_tools.hpp>
#include <boost/test/unit_test.hpp>

#include <vector>

namespace zr_boost_1_81_range_test
{
    namespace
    {

void test_const_reverse_iterator()
{
    typedef std::vector<int> cont;

    BOOST_STATIC_ASSERT((
        zr_boost_1_81::is_same<
            zr_boost_1_81::reverse_iterator<cont::const_iterator>,
            zr_boost_1_81::range_const_reverse_iterator<cont>::type
        >::value));

    BOOST_STATIC_ASSERT((
        zr_boost_1_81::is_same<
            zr_boost_1_81::reverse_iterator<cont::const_iterator>,
            zr_boost_1_81::range_const_reverse_iterator<const cont>::type
        >::value));

#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
    BOOST_STATIC_ASSERT((
        zr_boost_1_81::is_same<
            zr_boost_1_81::reverse_iterator<cont::const_iterator>,
            zr_boost_1_81::range_const_reverse_iterator<cont&&>::type
        >::value));
#endif
}

    } // anonymous namespace
} // namespace zr_boost_1_81_range_test

zr_boost_1_81::unit_test::test_suite* init_unit_test_suite( int argc, char* argv[] )
{
    zr_boost_1_81::unit_test::test_suite* test =
        BOOST_TEST_SUITE(
                "Boost.Range range_const_reverse_iterator meta-function");

    test->add(BOOST_TEST_CASE(&zr_boost_1_81_range_test::test_const_reverse_iterator));

    return test;
}
