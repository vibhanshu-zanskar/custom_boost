...
#define BOOST_LIB_NAME zr_boost_1_81_test_exec_monitor
...
namespace brdm = zr_boost_1_81::range_detail_microsoft;
...
{
    typedef typename zr_boost_1_81::range_iterator<SampleRange>::type iter_t;

    for (iter_t it = zr_boost_1_81::begin(sample), last = zr_boost_1_81::end(sample); it != last; ++it) {
        arr.Add(*it); // moves ownership
    }

    return zr_boost_1_81::distance(arr) == zr_boost_1_81::distance(sample);
}
...
{
    typedef typename zr_boost_1_81::range_iterator<SampleRange>::type iter_t;
    typedef typename zr_boost_1_81::range_value<SampleRange>::type val_t;

    for (iter_t it = zr_boost_1_81::begin(sample), last = zr_boost_1_81::end(sample); it != last; ++it) {
        lst.AddTail(*it); // moves ownership
    }

    return zr_boost_1_81::distance(lst) == zr_boost_1_81::distance(sample);
}
...
{
    typedef typename zr_boost_1_81::range_iterator<AtlMapT>::type iter_t;

    for (iter_t it = zr_boost_1_81::begin(map), last = zr_boost_1_81::end(map); it != last; ++it) {
...
{
    typedef typename zr_boost_1_81::range_iterator<AtlMapT>::type iter_t;
    typedef typename zr_boost_1_81::range_const_iterator<MapT>::type siter_t;

    bool result = true;

    result = result && (zr_boost_1_81::distance(map) == zr_boost_1_81::distance(sample));
    if (!result)
        return false;
    ...
    {
        for (siter_t it = zr_boost_1_81::begin(sample), last = zr_boost_1_81::end(sample); it != last; ++it) {
...
{
    typedef typename zr_boost_1_81::range_const_iterator<SampleMap>::type iter_t;
...
{
    typedef typename zr_boost_1_81::range_value<Range>::type val_t;
...
void test_CAutoPtrArray(Range& sample)
{
    ...
    BOOST_STATIC_ASSERT(( brdm::test_mutable_iter<rng_t, zr_boost_1_81::indirect_iterator< ATL::CAutoPtr<val_t> *> >::value ));
    BOOST_STATIC_ASSERT(( brdm::test_const_iter  <rng_t, zr_boost_1_81::indirect_iterator< ATL::CAutoPtr<val_t> const*> >::value ));
...
void test_CAutoPtrList(Range& sample)
{
    ...
    BOOST_STATIC_ASSERT(( brdm::test_mutable_iter< rng_t, zr_boost_1_81::indirect_iterator< brdm::list_iterator<rng_t,       ATL::CAutoPtr<val_t> > > >::value ));
    BOOST_STATIC_ASSERT(( brdm::test_const_iter  < rng_t, zr_boost_1_81::indirect_iterator< brdm::list_iterator<rng_t const, ATL::CAutoPtr<val_t> const> > >::value ));
...
void test_CHeapPtrList(const Range& sample)
{
    ...
    BOOST_STATIC_ASSERT(( brdm::test_mutable_iter< rng_t, zr_boost_1_81::indirect_iterator< brdm::list_iterator<rng_t,       ATL::CHeapPtr<val_t> > > >::value ));
    BOOST_STATIC_ASSERT(( brdm::test_const_iter  < rng_t, zr_boost_1_81::indirect_iterator< brdm::list_iterator<rng_t const, ATL::CHeapPtr<val_t> const> > >::value ));
...
void test_CSimpleStringT(const Range& sample)
{
    ...
    typedef typename zr_boost_1_81::mpl::if_< zr_boost_1_81::is_same<val_t, char>,
        ATL::CAtlStringA,
        ATL::CAtlStringW
    >::type derived_t;
...
void test_CStaticString(const Range& sample)
{
#if !defined(BOOST_RANGE_ATL_NO_TEST_UNDOCUMENTED_RANGE)
    {
        ...
        BOOST_CHECK( *(zr_boost_1_81::begin(rng)+4) == 'o' );
        BOOST_CHECK( *(zr_boost_1_81::end(rng)-3) == 'i' );
    }

    {
        ...
        BOOST_CHECK( *(zr_boost_1_81::begin(rng)+4) == L'o' );
        BOOST_CHECK( *(zr_boost_1_81::end(rng)-3) == L'i' );
    }
#endif
...
void test_CSimpleMap(const Range& sample)
{
    ...
    BOOST_CHECK( zr_boost_1_81::begin(rng)->get<0>() == 3.0 );
    BOOST_CHECK( (zr_boost_1_81::end(rng)-1)->get<1>() == 2.0 );
...
void test_CAtlMap(const MapT& sample)
{
    ...
    rng_t rng;
    zr_boost_1_81::function_requires< zr_boost_1_81::ForwardRangeConcept<rng_t> >();
...
void test_CRBTree(const MapT& sample)
{
    ...
    zr_boost_1_81::function_requires< zr_boost_1_81::BidirectionalRangeConcept<rng_t> >();
...
void test_atl()
{
...
#if !(_ATL_VER < 0x0700)
    {
        ...
        zr_boost_1_81::iterator_range< ptr_t * > workaround(ptrs, ptrs+8);
...

    {
        ...
        zr_boost_1_81::iterator_range< ptr_t * > workaround(ptrs, ptrs+5);
