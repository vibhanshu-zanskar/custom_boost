...
namespace zr_boost_1_81
{
    namespace
    {
        ...
        void test_generate_impl(Container& cont)
        {
            ...
            Container test(cont);
            zr_boost_1_81::generate(test, generator_fn());

            BOOST_CHECK_EQUAL_COLLECTIONS( reference.begin(), reference.end(),
                test.begin(), test.end() );
                
            Container test2(cont);
            zr_boost_1_81::generate(zr_boost_1_81::make_iterator_range(test2), generator_fn());
...
init_unit_test_suite(int argc, char* argv[])
{
    ...
    test->add( BOOST_TEST_CASE( &zr_boost_1_81::test_generate ) );
