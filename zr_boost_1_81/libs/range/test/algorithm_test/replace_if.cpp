    ...
    namespace
    {
        ...
        void test_replace_if_impl(Container& cont, UnaryPredicate pred)
        {
            ...
            std::replace_if(reference.begin(), reference.end(),
                zr_boost_1_81::bind(pred, _1, what), with_what);

            std::vector<int> target(cont.begin(), cont.end());
            zr_boost_1_81::replace_if(target, zr_boost_1_81::bind(pred, _1, what), with_what);

            BOOST_CHECK_EQUAL_COLLECTIONS( reference.begin(), reference.end(),
                                           target.begin(), target.end() );
                
            std::vector<int> target2(cont.begin(), cont.end());
            zr_boost_1_81::replace_if(zr_boost_1_81::make_iterator_range(target2),
                              zr_boost_1_81::bind(pred, _1, what), with_what);
...
init_unit_test_suite(int argc, char* argv[])
{
    ...
    test->add( BOOST_TEST_CASE( &zr_boost_1_81::test_replace_if ) );
