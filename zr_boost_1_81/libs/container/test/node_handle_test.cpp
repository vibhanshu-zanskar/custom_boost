   ...
   : public new_allocator<Node>
{
   ...
   trace_allocator(BOOST_RV_REF(trace_allocator) other)
      : base_t(zr_boost_1_81::move(BOOST_MOVE_BASE(base_t, other))), m_state(MoveConstructed), m_value(other.m_value)
   {
      ++count;
   }
   ...
   {
      zr_boost_1_81::adl_move_swap(m_value, other.m_value);
...
void test_move_constructor()
{
   ...
   node_t::reset_count();
   {
      ...
      BOOST_TEST(node_alloc_t::count == 1);
      {
         ...
         BOOST_TEST(node_alloc_t::count == 2);
         {
            node_handle_set_t nh2(zr_boost_1_81::move(nh));
            BOOST_TEST(nh.empty());
...
void test_related_constructor()
{
   ...
   node_t::reset_count();
   {
      ...
      BOOST_TEST(node_alloc_t::count == 1);
      {
         node_handle_set_t nh;
         {
            node_handle_map_t nh2(zr_boost_1_81::move(nh));
            BOOST_TEST(nh.empty());
            BOOST_TEST(nh2.empty());
            BOOST_TEST(node_alloc_t::count == 1);
         }
...
void test_move_assignment()
{
   //empty = full
   {
      ...
      nh_to = zr_boost_1_81::move(nh_from);
