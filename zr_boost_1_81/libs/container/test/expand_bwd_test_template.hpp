...
bool CheckEqualVector(const Vector1 &vector1, const Vector2 &vector2)
{
   if(vector1.size() != vector2.size())
      return false;
   return zr_boost_1_81::container::algo_equal(vector1.begin(), vector1.end(), vector2.begin());
}
   ...
   for(unsigned int pos = 0; pos < sizeof(Position)/sizeof(Position[0]); ++pos){
      ...
      for(unsigned int iteration = 0; iteration < Iterations; ++iteration)
      {
         zr_boost_1_81::movelib::unique_ptr<char[]> memptr =
            zr_boost_1_81::movelib::make_unique_definit<char[]>(MemorySize*sizeof(value_type));
