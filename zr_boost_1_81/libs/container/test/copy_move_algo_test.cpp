   ...
   for (unsigned backmove = 2u; backmove != 0u; ) {
      ...
      for (unsigned int pos = 0; pos < sizeof(Position) / sizeof(Position[0]); ++pos) {
         ...
         for (unsigned int iteration = 0; iteration < Iterations; ++iteration)
         {
            ...
            typedef zr_boost_1_81::container::new_allocator<value_type> allocator_type;
            typedef dtl::insert_range_proxy<allocator_type, typename std_vector_val_t::iterator> proxy_t;
            {
               ...
               zr_boost_1_81::movelib::unique_ptr<char[]> memptr =
                  zr_boost_1_81::movelib::make_unique_definit<char[]>(BufferSize * sizeof(value_type));
               ...
               BOOST_TEST(zr_boost_1_81::container::algo_equal(initial_data.begin(), initial_data.end(), final_memory));

               zr_boost_1_81::container::destroy_alloc_n(a, final_memory, InitialSize[iteration] + InsertSize[iteration]);
            }
            BOOST_TEST(life_count<value_type>::check(0));
         }
      }
