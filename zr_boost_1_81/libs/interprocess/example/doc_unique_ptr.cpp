...
int main ()
{
   ...
   //<-
   #if 1
   std::string mapped_file(zr_boost_1_81::interprocess::ipcdetail::get_temporary_path());
   ...
   } remover(MappedFile);
   {
      ...
      for(int i = 0; i < 100; ++i){
         ...
         unique_vector->push_back(zr_boost_1_81::move(p));
      ...
      for(int i = 99; !unique_vector->empty(); --i){
         unique_list->push_front(zr_boost_1_81::move(unique_vector->back()));
