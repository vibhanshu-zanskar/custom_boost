...
struct test_lock
{
   ...
   typedef zr_boost_1_81::interprocess::scoped_lock<mutex_type> lock_type;
   ...
   void operator()()
   {
      ...
      {
         lock_type lock(interprocess_mutex, zr_boost_1_81::interprocess::defer_lock);
...
{
   typedef M mutex_type;
   typedef zr_boost_1_81::interprocess::scoped_lock<mutex_type> try_to_lock_type;
   ...
   void operator()()
   {
      ...
      // Test the lock's constructors.
      {
         try_to_lock_type lock(interprocess_mutex, zr_boost_1_81::interprocess::try_to_lock);
         BOOST_INTERPROCESS_CHECK(lock ? true : false);
      }
      {
         try_to_lock_type lock(interprocess_mutex, zr_boost_1_81::interprocess::defer_lock);
         BOOST_INTERPROCESS_CHECK(!lock);
      }
...
{
   typedef M mutex_type;
   typedef zr_boost_1_81::interprocess::scoped_lock<mutex_type> timed_lock_type;
   ...
   void operator()()
   {
      ...
      {
         timed_lock_type lock(interprocess_mutex, zr_boost_1_81::interprocess::defer_lock);
         BOOST_INTERPROCESS_CHECK(!lock);
      }
...
void lock_and_sleep(void *arg, M &sm)
{
   ...
   zr_boost_1_81::interprocess::scoped_lock<M> l(sm);
   if(pdata->m_secs){
      zr_boost_1_81::interprocess::ipcdetail::thread_sleep(unsigned(1000*pdata->m_secs));
   }
   else{
      zr_boost_1_81::interprocess::ipcdetail::thread_sleep(unsigned(1000*2*BaseSeconds));
   }
...
{
   data<M> *pdata = static_cast<data<M>*>(arg);
   zr_boost_1_81::interprocess::scoped_lock<M> l(sm, zr_boost_1_81::interprocess::defer_lock);
   if (l.try_lock()){
      zr_boost_1_81::interprocess::ipcdetail::thread_sleep(unsigned(1000*2*BaseSeconds));
      ++shared_val;
      pdata->m_value = shared_val;
...
{
   data<M> *pdata = static_cast<data<M>*>(arg);
   zr_boost_1_81::interprocess::scoped_lock<M>
...
void test_mutex_lock()
{
   ...
   zr_boost_1_81::interprocess::ipcdetail::OS_thread_t tm1;
   zr_boost_1_81::interprocess::ipcdetail::thread_launch(tm1, thread_adapter<M>(&lock_and_sleep, &d1, mtx));
   ...
   zr_boost_1_81::interprocess::ipcdetail::OS_thread_t tm2;
   zr_boost_1_81::interprocess::ipcdetail::thread_launch(tm2, thread_adapter<M>(&lock_and_sleep, &d2, mtx));

   //Wait completion

   zr_boost_1_81::interprocess::ipcdetail::thread_join(tm1);
   zr_boost_1_81::interprocess::ipcdetail::thread_sleep(unsigned(1000*1*BaseSeconds));
   zr_boost_1_81::interprocess::ipcdetail::thread_join(tm2);
...
void test_mutex_lock_timeout()
{
   ...
   //Wait 1*BaseSeconds
   zr_boost_1_81::interprocess::ipcdetail::thread_sleep(unsigned(1000*wait_time_s));

   // Locker two launches, and attempts to hold the lock for wait_time_s * 2 seconds.
   zr_boost_1_81::interprocess::ipcdetail::OS_thread_t tm2;
   zr_boost_1_81::interprocess::ipcdetail::thread_launch(tm2, thread_adapter<M>(&lock_and_catch_errors, &d2, mtx));
   ...
   BOOST_INTERPROCESS_CHECK(d2.m_error == zr_boost_1_81::interprocess::timeout_when_locking_error);
}
...
void test_mutex_try_lock()
{
   ...
   // Locker one launches, holds the lock for 2*BaseSeconds seconds.
   zr_boost_1_81::interprocess::ipcdetail::OS_thread_t tm1;
   zr_boost_1_81::interprocess::ipcdetail::thread_launch(tm1, thread_adapter<M>(&try_lock_and_sleep, &d1, mtx));
   ...
   zr_boost_1_81::interprocess::ipcdetail::OS_thread_t tm2;
   zr_boost_1_81::interprocess::ipcdetail::thread_launch(tm2, thread_adapter<M>(&try_lock_and_sleep, &d2, mtx));
...
void test_mutex_timed_lock()
{
   for (int flag = 0; flag != (int)ETimedLockFlagsMax; ++flag)
   {
      ...
      // Locker one launches, holds the lock for 2*BaseSeconds seconds.
      zr_boost_1_81::interprocess::ipcdetail::OS_thread_t tm1;
      zr_boost_1_81::interprocess::ipcdetail::thread_launch(tm1, thread_adapter<M>(&timed_lock_and_sleep, &d1, mtx));
      ...
      zr_boost_1_81::interprocess::ipcdetail::OS_thread_t tm2;
      zr_boost_1_81::interprocess::ipcdetail::thread_launch(tm2, thread_adapter<M>(&timed_lock_and_sleep, &d2, mtx));
