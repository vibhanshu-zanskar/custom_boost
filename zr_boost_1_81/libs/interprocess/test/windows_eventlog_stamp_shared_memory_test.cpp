...
{
   shared_dir = zr_boost_1_81::interprocess::ipcdetail::get_temporary_path();
   shared_dir += "/boostipctesteventlog_";
   shared_dir += zr_boost_1_81::interprocess::test::get_process_id_name();
   if(!dir_created)
...
{
   shared_dir = zr_boost_1_81::interprocess::ipcdetail::get_temporary_wpath();
   shared_dir += L"/boostipctesteventlog_";
   shared_dir += zr_boost_1_81::interprocess::test::get_process_id_wname();
   if(!dir_created)
      ipcdetail::open_or_create_shared_directory(shared_dir.c_str());
   dir_created = true;
}

}}}   //namespace zr_boost_1_81::interprocess::ipcdetail
...
int main ()
{
   ...
   BOOST_TRY{
      ...
      //Create and get name, size and address
      {
         ...
         shared_memory move_ctor(zr_boost_1_81::move(shm1));
