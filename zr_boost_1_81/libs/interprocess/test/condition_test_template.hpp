...
{
    zr_boost_1_81::interprocess::scoped_lock<Mutex>
      lock(data->mutex);
...
void do_test_condition_notify_one()
{
   ...
   zr_boost_1_81::interprocess::ipcdetail::thread_launch(thread, bind_function(&condition_test_thread<Condition, Mutex>, &data));
   //Make sure thread is blocked
   zr_boost_1_81::interprocess::ipcdetail::thread_sleep(1000);
   {
      zr_boost_1_81::interprocess::scoped_lock<Mutex>
         lock(data.mutex);
      BOOST_INTERPROCESS_CHECK(lock ? true : false);
...
void do_test_condition_notify_all()
{
   ...
   zr_boost_1_81::interprocess::ipcdetail::OS_thread_t thgroup[std::size_t(NUMTHREADS)];
   condition_test_data<Condition, Mutex> data;

   for(int i = 0; i< NUMTHREADS; ++i){
      zr_boost_1_81::interprocess::ipcdetail::thread_launch(thgroup[i], bind_function(&condition_test_thread<Condition, Mutex>, &data));
   }
   ...
   for(int i = 0; i< NUMTHREADS; ++i){
      zr_boost_1_81::interprocess::ipcdetail::thread_join(thgroup[i]);
   }
...
void do_test_condition_waits_step( condition_test_data<Condition, Mutex> &data
                                 , zr_boost_1_81::interprocess::scoped_lock<Mutex> &lock
                                 , int awoken)
{
      zr_boost_1_81::interprocess::ipcdetail::thread_sleep(1000);
      data.notified++;
...
void do_test_condition_waits()
{
   condition_test_data<Condition, Mutex> data;
   zr_boost_1_81::interprocess::ipcdetail::OS_thread_t thread;
   zr_boost_1_81::interprocess::ipcdetail::thread_launch(thread, bind_function(&condition_test_waits<Condition, Mutex>, &data));
...
struct condition_func
{
   ...
   void operator()()
   {
      zr_boost_1_81::interprocess::scoped_lock<Mutex>lock(mutex_);
      while(count == 0){
         ++waiting_readers;
...
void do_test_condition_queue_notify_one(void)
{
   ...
   //Create threads that will decrease count
   {
      ...
      for(int i = 0; i< NumThreads; ++i){
         ...
         zr_boost_1_81::interprocess::ipcdetail::thread_launch(thgroup[i], func);
      }

      //Add 20 elements one by one in the queue simulation
      //The sender will block if it fills the queue
      for(int i = 0; i < NumThreads; ++i){
         zr_boost_1_81::interprocess::scoped_lock<Mutex> lock(mutex);
