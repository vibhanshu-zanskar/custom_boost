...
bool copyable_only(V1 *, V2 *, zr_boost_1_81::interprocess::ipcdetail::false_type)
{
   return true;
}
...
template<class V1, class V2>
bool copyable_only(V1 *shmvector, V2 *stdvector, zr_boost_1_81::interprocess::ipcdetail::true_type)
{
...

   {
      ...
      shmvector->insert(shmvector->begin()+std::ptrdiff_t(size/2u), 50u, zr_boost_1_81::move(move_me));
      if(!test::CheckEqualContainers(shmvector, stdvector)) return false;
   }
   {
      IntType move_me(2);
      shmvector->assign(shmvector->size()/2, zr_boost_1_81::move(move_me));
      stdvector->assign(stdvector->size()/2, 2);
      if(!test::CheckEqualContainers(shmvector, stdvector)) return false;
   }
   {
      IntType move_me(3);
      shmvector->assign(shmvector->size()*3-1, zr_boost_1_81::move(move_me));
...
int vector_test()
{
...

   {
      ...
      BOOST_TRY{
         ...
         for(int i = 0; i < max; ++i){
            IntType new_int(i);
            shmvector->insert(shmvector->end(), zr_boost_1_81::move(new_int));
...

         {
            ...
            for(int i = 0; i < 50; ++i){
               IntType new_int(-1);
               //BOOST_STATIC_ASSERT((::zr_boost_1_81::move_ipcdetail::is_copy_constructible<zr_boost_1_81::interprocess::test::movable_int>::value == false));
         ...
         shmvector->push_back(zr_boost_1_81::move(push_back_this));
         ...
         for(int i = 0; i < max; ++i){
            ...
            shmvector->insert(shmvector->begin(), zr_boost_1_81::move(insert_this));
