...
bool test_serialize_db()
{
   ...
   message_queue::remove(test::get_process_id_name());
   {
      ...
      BOOST_CATCH(zr_boost_1_81::interprocess::bad_alloc &){} BOOST_CATCH_END
...
static zr_boost_1_81::interprocess::message_queue *pmessage_queue;

void receiver()
{
   zr_boost_1_81::interprocess::message_queue::size_type recvd_size;
   unsigned int priority;
   int nummsg = NumMsg;
...
{
   zr_boost_1_81::interprocess::message_queue::remove(test::get_process_id_name());
   {
      zr_boost_1_81::movelib::unique_ptr<zr_boost_1_81::interprocess::message_queue>
         ptr(new zr_boost_1_81::interprocess::message_queue
               (create_only, test::get_process_id_name(), 10, 10));
      pmessage_queue = ptr.get();

      //Launch the receiver thread
      zr_boost_1_81::interprocess::ipcdetail::OS_thread_t thread;
      zr_boost_1_81::interprocess::ipcdetail::thread_launch(thread, &receiver);
      zr_boost_1_81::interprocess::ipcdetail::thread_yield();
      ...
      zr_boost_1_81::interprocess::ipcdetail::thread_join(thread);
   }
   zr_boost_1_81::interprocess::message_queue::remove(test::get_process_id_name());
   return true;
}
...
static zr_boost_1_81::interprocess::message_queue *global_queue = 0;
...
bool test_multi_sender_receiver()
{
   ...
   BOOST_TRY {
      zr_boost_1_81::interprocess::message_queue::remove(test::get_process_id_name());
      zr_boost_1_81::interprocess::message_queue mq
         (zr_boost_1_81::interprocess::open_or_create, test::get_process_id_name(), MULTI_QUEUE_SIZE, 1);
      global_queue = &mq;
      std::vector<zr_boost_1_81::interprocess::ipcdetail::OS_thread_t> threads(MULTI_THREAD_COUNT*2);

      //Launch senders receiver thread
      for (std::size_t i = 0; i < MULTI_THREAD_COUNT; i++) {
         zr_boost_1_81::interprocess::ipcdetail::thread_launch
            (threads[i], &multisend);
      }
      ...
      for (std::size_t i = 0; i < MULTI_THREAD_COUNT*2; i++) {
         zr_boost_1_81::interprocess::ipcdetail::thread_join(threads[i]);
