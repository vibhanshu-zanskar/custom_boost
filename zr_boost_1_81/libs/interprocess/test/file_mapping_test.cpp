...
{
   file_mapping f;
   return file_mapping(zr_boost_1_81::move(f));
}
...
int main ()
{
   BOOST_TRY{
      ...
      //See if the pattern is correct in the file
      {
         ...
         zr_boost_1_81::container::vector<unsigned char> memory(FileSize/2 +1);
      ...
      #endif   //BOOST_INTERPROCESS_WCHAR_NAMED_RESOURCES
      {
         ...
         file_mapping mapping(get_filename().c_str(), read_only);
         file_mapping move_ctor(zr_boost_1_81::move(mapping));
