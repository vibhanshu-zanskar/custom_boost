...
namespace zr_boost_1_81 { namespace interprocess { namespace test {

template<typename SM>
void plain_exclusive(void *arg, SM &sm)
{
   data<SM> *pdata = static_cast<data<SM>*>(arg);
   zr_boost_1_81::interprocess::scoped_lock<SM> l(sm);
   zr_boost_1_81::interprocess::ipcdetail::thread_sleep(unsigned(1000*3*BaseSeconds));
   shared_val += 10;
...
{
   data<SM> *pdata = static_cast<data<SM>*>(arg);
   zr_boost_1_81::interprocess::sharable_lock<SM> l(sm);
   if(pdata->m_secs){
      zr_boost_1_81::interprocess::ipcdetail::thread_sleep(unsigned(1000*pdata->m_secs*BaseSeconds));
   }
   pdata->m_value = shared_val;
}
...
{
   data<SM> *pdata = static_cast<data<SM>*>(arg);
   zr_boost_1_81::interprocess::scoped_lock<SM> l(sm, zr_boost_1_81::interprocess::defer_lock);
   if (l.try_lock()){
      zr_boost_1_81::interprocess::ipcdetail::thread_sleep(unsigned(1000*3*BaseSeconds));
      shared_val += 10;
      pdata->m_value = shared_val;
...
{
   data<SM> *pdata = static_cast<data<SM>*>(arg);
   zr_boost_1_81::interprocess::sharable_lock<SM> l(sm, zr_boost_1_81::interprocess::defer_lock);
   if (l.try_lock()){
      if(pdata->m_secs){
         zr_boost_1_81::interprocess::ipcdetail::thread_sleep(unsigned(1000*pdata->m_secs*BaseSeconds));
      }
...
{
   {
      ...
      zr_boost_1_81::interprocess::ipcdetail::OS_thread_t tw1;
      zr_boost_1_81::interprocess::ipcdetail::thread_launch(tw1, thread_adapter<SM>(plain_exclusive, &e1, mtx));

      // Writer two launches, tries to grab the lock, "clearly"
      //  after Writer one will already be holding it.
      zr_boost_1_81::interprocess::ipcdetail::thread_sleep(unsigned(1000*1*BaseSeconds));
      zr_boost_1_81::interprocess::ipcdetail::OS_thread_t tw2;
      zr_boost_1_81::interprocess::ipcdetail::thread_launch(tw2, thread_adapter<SM>(plain_exclusive, &e2, mtx));

      // Reader one launches, "clearly" after writer two, and "clearly"
      //   while writer 1 still holds the lock
      zr_boost_1_81::interprocess::ipcdetail::thread_sleep(unsigned(1000*1*BaseSeconds));
      zr_boost_1_81::interprocess::ipcdetail::OS_thread_t thr1;
      zr_boost_1_81::interprocess::ipcdetail::thread_launch(thr1, thread_adapter<SM>(plain_shared,&s1, mtx));
      zr_boost_1_81::interprocess::ipcdetail::OS_thread_t thr2;
      zr_boost_1_81::interprocess::ipcdetail::thread_launch(thr2, thread_adapter<SM>(plain_shared,&s2, mtx));

      zr_boost_1_81::interprocess::ipcdetail::thread_join(thr2);
      zr_boost_1_81::interprocess::ipcdetail::thread_join(thr1);
      zr_boost_1_81::interprocess::ipcdetail::thread_join(tw2);
      zr_boost_1_81::interprocess::ipcdetail::thread_join(tw1);
...
void test_try_sharable_mutex()
{
   ...
   // Writer one launches, holds the lock for 3*BaseSeconds seconds.
   zr_boost_1_81::interprocess::ipcdetail::OS_thread_t tw1;
   zr_boost_1_81::interprocess::ipcdetail::thread_launch(tw1, thread_adapter<SM>(try_exclusive,&e1,mtx));

   // Reader one launches, "clearly" after writer #1 holds the lock
   //   and before it releases the lock.
   zr_boost_1_81::interprocess::ipcdetail::thread_sleep(unsigned(1000*1*BaseSeconds));
   zr_boost_1_81::interprocess::ipcdetail::OS_thread_t thr1;
   zr_boost_1_81::interprocess::ipcdetail::thread_launch(thr1, thread_adapter<SM>(try_shared,&s1,mtx));

   // Writer two launches in the same timeframe.
   zr_boost_1_81::interprocess::ipcdetail::OS_thread_t tw2;
   zr_boost_1_81::interprocess::ipcdetail::thread_launch(tw2, thread_adapter<SM>(try_exclusive,&e2,mtx));
...
{
   data<SM> *pdata = static_cast<data<SM>*>(arg);
   zr_boost_1_81::interprocess::scoped_lock<SM>
      l (sm, zr_boost_1_81::interprocess::defer_lock);
...
{
   data<SM> *pdata = static_cast<data<SM>*>(arg);
   zr_boost_1_81::interprocess::sharable_lock<SM>
      l(sm, zr_boost_1_81::interprocess::defer_lock);
...
void test_timed_sharable_mutex()
{
   for (int flag = 0; flag != (int)ETimedLockFlagsMax; ++flag)
   {
      ...
      zr_boost_1_81::interprocess::ipcdetail::OS_thread_t tw1;
      zr_boost_1_81::interprocess::ipcdetail::thread_launch(tw1, thread_adapter<SM>(timed_exclusive,&e1,mtx));
      ...
      zr_boost_1_81::interprocess::ipcdetail::OS_thread_t tw2;
      zr_boost_1_81::interprocess::ipcdetail::thread_launch(tw2, thread_adapter<SM>(timed_exclusive,&e2,mtx));
      ...
      zr_boost_1_81::interprocess::ipcdetail::OS_thread_t thr1;
      zr_boost_1_81::interprocess::ipcdetail::thread_launch(thr1, thread_adapter<SM>(timed_shared,&s1,mtx));

      zr_boost_1_81::interprocess::ipcdetail::OS_thread_t thr2;
      zr_boost_1_81::interprocess::ipcdetail::thread_launch(thr2, thread_adapter<SM>(timed_shared,&s2,mtx));
