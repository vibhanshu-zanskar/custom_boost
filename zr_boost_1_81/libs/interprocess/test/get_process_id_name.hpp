...
inline void get_process_id_name(std::string &str)
{
   ...
   sstr << "process_" << zr_boost_1_81::interprocess::ipcdetail::get_current_process_id() << std::ends;
   str = sstr.str().c_str();
}
...
{
   std::stringstream sstr;
   sstr << "process_" << zr_boost_1_81::interprocess::ipcdetail::get_current_process_id() << "_" << ptr << std::ends;
   str = sstr.str().c_str();
}
...
inline void get_process_id_wname(std::wstring &str)
{
   ...
   sstr << L"process_" << zr_boost_1_81::interprocess::ipcdetail::get_current_process_id() << std::ends;
   str = sstr.str().c_str();
}
