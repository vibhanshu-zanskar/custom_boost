...
bool test_allocation_shrink(Allocator &a)
{
   ...
   for(std::size_t i = 0, max = buffers.size()
      ...
      if(a.template allocation_command<char>
         ( zr_boost_1_81::interprocess::shrink_in_place | zr_boost_1_81::interprocess::nothrow_allocation, i*2
...
bool test_allocation_expand(Allocator &a)
{
   ...
   for(std::size_t i = 0, max = buffers.size()
      ...
      while(a.template allocation_command<char>
         ( zr_boost_1_81::interprocess::expand_fwd | zr_boost_1_81::interprocess::nothrow_allocation, min_size
...
bool test_allocation_shrink_and_expand(Allocator &a)
{
   ...
   for(std::size_t i = 0; true; ++i){
      ...
      void *ptr = a.template allocation_command<char>
         ( zr_boost_1_81::interprocess::allocate_new | zr_boost_1_81::interprocess::nothrow_allocation, i, received_size = i*2, reuse);
      if(!ptr){
         ptr = a.template allocation_command<char>
            ( zr_boost_1_81::interprocess::allocate_new | zr_boost_1_81::interprocess::nothrow_allocation, 1, received_size = i*2, reuse);
         if(!ptr)
            break;
      }
      buffers.push_back(ptr);
   ...
   for(std::size_t i = 0, max = buffers.size()
      ...
      char *reuse = static_cast<char*>(buffers[i]);
      if(a.template allocation_command<char>
         ( zr_boost_1_81::interprocess::shrink_in_place | zr_boost_1_81::interprocess::nothrow_allocation, received_sizes[i]
         , received_size = i, reuse)){
   ...
   for(std::size_t i = 0, max = buffers.size()
      ...
      char *reuse = static_cast<char*>(buffers[i]);
      if(a.template allocation_command<char>
         ( zr_boost_1_81::interprocess::expand_fwd | zr_boost_1_81::interprocess::nothrow_allocation, request_size
   ...
   for(std::size_t sizeof_object = 1; sizeof_object < 20u; ++sizeof_object){
      ...
      for(std::size_t i = 0; true; ++i){
         ...
         void *ret = a.raw_allocation_command
            ( zr_boost_1_81::interprocess::expand_bwd | zr_boost_1_81::interprocess::nothrow_allocation, min_size
...
bool test_many_deallocation(Allocator &a)
{
...

   {
      for(std::size_t i = 0; true; ++i){
         ...
         if(chain.empty())
            break;
         buffers.push_back(zr_boost_1_81::move(chain));
      }
      for(std::size_t i = 0, max = buffers.size(); i != max; ++i){
