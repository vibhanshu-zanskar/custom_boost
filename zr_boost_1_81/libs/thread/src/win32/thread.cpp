...
#else
        __declspec(thread) zr_boost_1_81::detail::thread_data_base* current_thread_data_base;
#endif

        void create_current_thread_tls_key()
        {
            zr_boost_1_81tss_cleanup_implemented(); // if anyone uses TSS, we need the cleanup linked in
#if !BOOST_PLAT_WINDOWS_RUNTIME
...
// Windows CE doesn't define _beginthreadex
        ...
        {
            zr_boost_1_81::csbl::unique_ptr<ThreadProxyData> data(reinterpret_cast<ThreadProxyData*>(args));
        ...
        bool win32::scoped_winrt_thread::start(thread_func address, void *parameter, unsigned int *thrdId)
        {
            ...
            *thrdId = ++threadCount;
                ...
                ([address, parameter, completionHandle](ABI::Windows::Foundation::IAsyncAction *)
            {
                ...
                ::zr_boost_1_81::detail::thread_data_base* const thread_info(reinterpret_cast<::zr_boost_1_81::detail::thread_data_base*>(parameter));
            ...
            if(current_thread_data)
            {
                while(! current_thread_data->tss_data.empty() || current_thread_data->thread_exit_callbacks)
                {
                    while(current_thread_data->thread_exit_callbacks)
                    {
                        ...
                        {
                            (*current_node->func)();
                            zr_boost_1_81::detail::heap_delete(current_node->func);
                        }
                        zr_boost_1_81::detail::heap_delete(current_node);
                    }
...
#pragma warning(pop)
        ...
        bool interruptible_wait(detail::win32::handle handle_to_wait_for, detail::internal_platform_timepoint const &timeout)
        {
            ...
            {
                zr_boost_1_81::intmax_t const time_left_msec = (timeout - detail::internal_platform_clock::now()).getMs();
            ...
            zr_boost_1_81::intmax_t time_left_msec(INFINITE);
    ...
    BOOST_THREAD_DECL void __cdecl zr_boost_1_81on_process_enter()
    {}

    BOOST_THREAD_DECL void __cdecl zr_boost_1_81on_thread_enter()
    {}

    BOOST_THREAD_DECL void __cdecl zr_boost_1_81on_process_exit()
    {
        zr_boost_1_81::cleanup_tls_key();
    }

    BOOST_THREAD_DECL void __cdecl zr_boost_1_81on_thread_exit()
    {
        zr_boost_1_81::run_thread_exit_callbacks();
    }
