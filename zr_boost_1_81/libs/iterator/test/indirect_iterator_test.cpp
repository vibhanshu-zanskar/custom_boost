...
{
    typedef zr_boost_1_81::indirect_iterator<typename Container::iterator> iterator;

    typedef zr_boost_1_81::indirect_iterator<
...
void more_indirect_iterator_tests()
{
    ...
    zr_boost_1_81::random_access_iterator_test(db + 1, store.size() - 1, zr_boost_1_81::next(store.begin()));

    *db = 999;
    BOOST_TEST(store.front() == 999);

    // Borland C++ is getting very confused about the typedefs here
    typedef zr_boost_1_81::indirect_iterator<iterator_set::iterator> indirect_set_iterator;
    typedef zr_boost_1_81::indirect_iterator<
...
# endif
    ...
    *zr_boost_1_81::prior(se) = 888;
    BOOST_TEST(store.back() == 888);
    BOOST_TEST(std::equal(sb, se, store.begin()));

    zr_boost_1_81::bidirectional_iterator_test(zr_boost_1_81::next(sb), store[1], store[2]);
...
# if BOOST_WORKAROUND(BOOST_MSVC, == 1200)
  zr_boost_1_81::shared_ptr<dummyT> zz((dummyT*)0);  // Why? I don't know, but it suppresses a bad instantiation.
# endif

  typedef std::vector<zr_boost_1_81::shared_ptr<dummyT> > shared_t;
  shared_t shared;

  // Concept checks
  {
    typedef zr_boost_1_81::indirect_iterator<shared_t::iterator> iter_t;
    ...
    typedef zr_boost_1_81::indirect_iterator<
        shared_t::iterator
      , zr_boost_1_81::iterator_value<shared_t::iterator>::type const
    > c_iter_t;
...
# endif
  ...
  // Test indirect_iterator_generator
  {
      for (int jj = 0; jj < N; ++jj)
          shared.push_back(zr_boost_1_81::shared_ptr<dummyT>(new dummyT(jj)));

      dummyT* ptr[N];
      for (int k = 0; k < N; ++k)
          ptr[k] = array + k;

      typedef zr_boost_1_81::indirect_iterator<dummyT**> indirect_iterator;

      typedef zr_boost_1_81::indirect_iterator<dummyT**, dummyT const>
          const_indirect_iterator;

      indirect_iterator i(ptr);
      zr_boost_1_81::random_access_iterator_test(i, N, array);

      zr_boost_1_81::random_access_iterator_test(
          zr_boost_1_81::indirect_iterator<shared_t::iterator>(shared.begin())
          , N, array);

      zr_boost_1_81::random_access_iterator_test(zr_boost_1_81::make_indirect_iterator(ptr), N, array);
      ...
      zr_boost_1_81::random_access_iterator_test(zr_boost_1_81::make_indirect_iterator(const_ptr), N, array);
