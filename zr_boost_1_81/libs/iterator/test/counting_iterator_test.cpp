    ...
    std::random_access_iterator_tag)
{
    ...
    BOOST_TEST(!std::binary_search(start, zr_boost_1_81::prior(finish), *finish));
    ...
    zr_boost_1_81::random_access_iterator_test(start, v.size(), v.begin());
}
...
void category_test(CountingIterator start, Value v1, std::bidirectional_iterator_tag)
{
    ...
    // Note that this test requires a that the first argument is
    // dereferenceable /and/ a valid iterator prior to the first argument
    zr_boost_1_81::bidirectional_iterator_test(start, v1, v2);
}
...
void category_test(CountingIterator start, CountingIterator finish, Value v1, std::forward_iterator_tag)
{
    Value v2 = v1;
    ++v2;
    if (finish != start && finish != zr_boost_1_81::next(start))
        zr_boost_1_81::forward_readable_iterator_test(start, finish, v1, v2);
}
...
void test_aux(CountingIterator start, CountingIterator finish, Value v1)
{
    ...
    for (CountingIterator p = start
             ; p != finish && zr_boost_1_81::next(p) != finish
             ; ++p)
    {
        BOOST_TEST(zr_boost_1_81::next(*p) == *zr_boost_1_81::next(p));
    }
...
{
    test_aux(zr_boost_1_81::make_counting_iterator(start), zr_boost_1_81::make_counting_iterator(finish), start);
}
...
void test_integer3(Integer* = 0, Category* = 0, Difference* = 0) // default arg works around MSVC bug
{
    Integer start = 0;
    Integer finish = 120;
    typedef zr_boost_1_81::counting_iterator<Integer,Category,Difference> iterator;
...
int main()
{
    ...
    test_integer<zr_boost_1_81::long_long_type>();
    test_integer<zr_boost_1_81::ulong_long_type>();
#endif
#if defined(BOOST_HAS_INT128)
    test_integer<zr_boost_1_81::int128_type>();
    test_integer<zr_boost_1_81::uint128_type>();
#endif
...
# endif
    ...
    int array[2000];
    test(zr_boost_1_81::make_counting_iterator(array), zr_boost_1_81::make_counting_iterator(array+2000-1));
