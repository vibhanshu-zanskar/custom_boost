...
{
    T m_x;
    explicit wrapper(typename zr_boost_1_81::call_traits<T>::param_type x)
        : m_x(x)
    { }
    template <class U>
    wrapper(const wrapper<U>& other,
        typename zr_boost_1_81::enable_if< zr_boost_1_81::is_convertible<U,T> >::type* = 0)
        : m_x(other.m_x)
    { }
};
...
struct derived : base
{
    ...
    {
        state = zr_boost_1_81::polymorphic_cast<const derived *>(&b)->state;
    }

    virtual bool equal(const base &b) const
    {
        return state == zr_boost_1_81::polymorphic_cast<const derived *>(&b)->state;
    }
...
int main()
{
    {
        int state = 0;
        zr_boost_1_81::readable_iterator_test(counter_iterator<int const&>(&state), 0);
        state = 3;
        zr_boost_1_81::readable_iterator_test(counter_iterator<proxy>(&state), 3);
        zr_boost_1_81::writable_iterator_test(counter_iterator<proxy>(&state), 9, 7);
        BOOST_TEST(state == 8);
    }
    ...
    {
        derived d(1);
        zr_boost_1_81::readable_iterator_test(abstract_iterator<derived *, base>(&d), derived(1));
    }
