...
bool plus_n_test(RandomAccessIterator first, RandomAccessIterator last, ForwardIterator first2)
{
    ...
    for(int n = 0; i != last; ++n)
        i = first + n, j = zr_boost_1_81::next(first2, n);
    return std::distance(first, i) == std::distance(first2, j);
}
...
bool minus_n_test(RandomAccessIterator first, RandomAccessIterator last, BidirectionalIterator last2)
{
    ...
    for(int n = 0; i != first; ++n)
        i = last - n, j = zr_boost_1_81::prior(last2, n);
    return std::distance(i, last) == std::distance(j, last2);
}
...
{
    Iterator i = zr_boost_1_81::prior(last, size);
    return i == first;
}
...
int main(int, char*[])
{
    ...
    // Tests with integers
    BOOST_TEST(zr_boost_1_81::next(5) == 6);
    BOOST_TEST(zr_boost_1_81::next(5, 7) == 12);
    BOOST_TEST(zr_boost_1_81::prior(5) == 4);
    BOOST_TEST(zr_boost_1_81::prior(5, 7) == -2);
    BOOST_TEST(zr_boost_1_81::prior(5, 7u) == -2);
