...
void
request::preserve(zr_boost_1_81::shared_ptr<void> d) {
  if (!m_preserved) {
    m_preserved = d; 
  } else {
    zr_boost_1_81::shared_ptr<void> cdr = m_preserved;
    typedef std::pair<zr_boost_1_81::shared_ptr<void>, zr_boost_1_81::shared_ptr<void> > cons;
    zr_boost_1_81::shared_ptr<cons> p(new cons(d, cdr));
    m_preserved = p;
  }
}
...
request::dynamic_handler::wait()
{
  ...
  if (error_code == MPI_ERR_IN_STATUS) {
    ...
    if (stats[0].MPI_ERROR == MPI_SUCCESS 
      ...
      zr_boost_1_81::throw_exception(exception("MPI_Waitall", stats[1].MPI_ERROR));
    else
      zr_boost_1_81::throw_exception(exception("MPI_Waitall", stats[0].MPI_ERROR));
  } else if (error_code != MPI_SUCCESS) {
    // There was an error somewhere in the MPI_Waitall call; throw
    // an exception for it.
    zr_boost_1_81::throw_exception(exception("MPI_Waitall", error_code));
  } 
...
request::dynamic_handler::test()
{
  ...
  if (error_code == MPI_ERR_IN_STATUS) {
    ...
    if (stats[0].MPI_ERROR == MPI_SUCCESS 
        || stats[0].MPI_ERROR == MPI_ERR_PENDING)
      zr_boost_1_81::throw_exception(exception("MPI_Testall", stats[1].MPI_ERROR));
    else
      zr_boost_1_81::throw_exception(exception("MPI_Testall", stats[0].MPI_ERROR));
  } else if (error_code != MPI_SUCCESS) {
    // There was an error somewhere in the MPI_Testall call; throw
    // an exception for it.
    zr_boost_1_81::throw_exception(exception("MPI_Testall", error_code));
  }
