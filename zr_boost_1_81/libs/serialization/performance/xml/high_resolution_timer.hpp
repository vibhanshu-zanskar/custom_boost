    ...
    public:
        ...
        high_resolution_timer(double t) 
        {
            ...
            if (!QueryPerformanceFrequency(&frequency))
                zr_boost_1_81::throw_exception(std::runtime_error("Couldn't acquire frequency"));
        ...
        { 
            if (!QueryPerformanceCounter(&start_time))
                zr_boost_1_81::throw_exception(std::runtime_error("Couldn't initialize start_time"));
        } 
        double elapsed() const                  // return elapsed time in seconds
        { 
            LARGE_INTEGER now;
            if (!QueryPerformanceCounter(&now))
                zr_boost_1_81::throw_exception(std::runtime_error("Couldn't get current time"));
    ...
    public:
        ...
        { 
            ...
            if (-1 == clock_getres(CLOCK_REALTIME, &resolution))
                zr_boost_1_81::throw_exception(std::runtime_error("Couldn't get resolution"));
...
namespace xml {
    ...
    private:
        bool use_backup;
        timespec start_time;
        zr_boost_1_81::timer start_time_backup;
    }; 
...
//  simply fall back to zr_boost_1_81::timer
#include <boost/timer.hpp>
