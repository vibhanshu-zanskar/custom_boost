...
int main()
{
    zr_boost_1_81::optional<Foo> oldFoo = Foo(10);
    std::ostringstream outStream;
    zr_boost_1_81::archive::text_oarchive outArchive(outStream);
    outArchive & oldFoo;

    zr_boost_1_81::optional<Foo> newFoo;
    std::istringstream inStream(outStream.str());
    zr_boost_1_81::archive::text_iarchive inArchive(inStream);
    inArchive & newFoo;
...
int test_main(int, char *argv[])
{
	...
	std::wstring w2 = L"апр"; // some non-latin (for example russians) letters
    {
        ...
        {
            zr_boost_1_81::archive::xml_woarchive oa(ofs);
            oa << zr_boost_1_81::serialization::make_nvp("key1", s1);
            //oa << zr_boost_1_81::serialization::make_nvp("key2", w1);
            //oa << zr_boost_1_81::serialization::make_nvp("key3", w2); // here exception is thrown
        }
    }
    std::string new_s1;
    //std::wstring new_w1;
    //std::wstring new_w2;
    {
        ...
        {
            zr_boost_1_81::archive::xml_wiarchive ia(ifs);
            ia >> zr_boost_1_81::serialization::make_nvp("key1", new_s1);
            //ia >> zr_boost_1_81::serialization::make_nvp("key2", new_w1);
            //ia >> zr_boost_1_81::serialization::make_nvp("key3", new_w2); // here exception is thrown
        }
    }
...
int main() {
        zr_boost_1_81::archive::xml_oarchive oa( std::cerr );
        int bob = 3;
        oa << zr_boost_1_81::serialization::make_nvp( "bob", bob );
}
...
typedef zr_boost_1_81::shared_ptr<boxed_int> pi;

void go(std::istream &is) {
    std::vector<pi> vv;
    try
    {
        zr_boost_1_81::archive::binary_iarchive ia(is);
        ia & vv;
    }
...
namespace bai = zr_boost_1_81::archive::iterators;
int main()
{
    std::string input =
...
void f()
{
  ...
  zr_boost_1_81::archive::binary_iarchive ar(iss);
  std::vector<int> out;
  ar >> out;
}
...
public zr_boost_1_81::archive::xml_iarchive_impl<xml_iarchive_nan>
{
protected:
    friend class zr_boost_1_81::archive::detail::interface_iarchive<xml_iarchive_nan>;
    friend class zr_boost_1_81::archive::detail::common_iarchive<xml_iarchive_nan>;
    friend class zr_boost_1_81::archive::basic_xml_iarchive<xml_iarchive_nan>;
    friend class zr_boost_1_81::archive::load_access;

    using zr_boost_1_81::archive::xml_iarchive_impl<xml_iarchive_nan>::load;
    void load(double & t)
    {
        char c='0';
        if(!is.fail())
        {
            ...
            if(c!='n')
            {
                zr_boost_1_81::archive::xml_iarchive_impl<xml_iarchive_nan>::load(t);
            }
            else
            {
                char c1='0',c2='0';
                ...
                if(is.fail()||(!((c=='n')&&(c1=='a')&&(c2=='n'))))
                    zr_boost_1_81::serialization::throw_exception(
                                                          zr_boost_1_81::archive::archive_exception(
                                                                                            zr_boost_1_81::archive::archive_exception::input_stream_error));
                else
            ...
            zr_boost_1_81::serialization::throw_exception(
                                                  zr_boost_1_81::archive::archive_exception(zr_boost_1_81::archive::archive_exception::input_stream_error));
    }

public:
    xml_iarchive_nan(std::istream & is, unsigned int flags = 0) :
    zr_boost_1_81::archive::xml_iarchive_impl<xml_iarchive_nan>(is, flags)
    {}
    ~xml_iarchive_nan(){};
};
...
// zr_boost_1_81::variant not correctly serialized when reusing the memory location #203
#include <boost/archive/text_iarchive.hpp>
#include <boost/archive/text_oarchive.hpp>
...
int main() {
  zr_boost_1_81::variant<int> variant = 1;
  ...
  // read it two times
  zr_boost_1_81::variant<int> v1;
  zr_boost_1_81::variant<int> v2;
  zr_boost_1_81::archive::text_iarchive ia(ss);
  ia >> zr_boost_1_81::serialization::make_nvp("var1", v1);
  ia >> zr_boost_1_81::serialization::make_nvp("var2", v2);

  assert(zr_boost_1_81::get<int>(v1) == 1);
  assert(zr_boost_1_81::get<int>(v2) == 1);  // Fails: v2 is actually zero
}
