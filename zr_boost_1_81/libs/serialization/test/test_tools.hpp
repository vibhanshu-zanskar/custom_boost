...
namespace archive {
    const char * test_filename(const char * dir = NULL, char *fname = NULL){
        ...
        ibuffer[0] = '\0';
        if(NULL == dir){
            dir = zr_boost_1_81::archive::tmpdir();
        }
        STRCPY(ibuffer, dir);
        std::strcat(ibuffer, "/");
        if(NULL == fname){
            ...
            zr_boost_1_81::filesystem::path tmp_path =
                zr_boost_1_81::filesystem::unique_path("%%%%%");
...
namespace archive {
    char const * tmpnam(char * buffer) {
        ...
        if (name[0] == 0) {
            zr_boost_1_81::filesystem::path tempdir(tmpdir());
            zr_boost_1_81::filesystem::path tempfilename =
                zr_boost_1_81::filesystem::unique_path("serialization-%%%%");
            zr_boost_1_81::filesystem::path temp = tempdir / tempfilename;
...
#define BOOST_CHECK_MESSAGE( P, M )  \
    ((P)? (void)0 : ::zr_boost_1_81::detail::error_impl( (M) , __FILE__, __LINE__, BOOST_CURRENT_FUNCTION))
...
} } // zr_boost_1_81::detail

#define BOOST_WARN_MESSAGE( P, M )  \
    ((P)? (void)0 : ::zr_boost_1_81::detail::msg_impl( (M) , __FILE__, __LINE__, BOOST_CURRENT_FUNCTION))
...
main(int argc, char * argv[]){
    zr_boost_1_81::serialization::get_singleton_module().lock();
    ...
    zr_boost_1_81::serialization::get_singleton_module().unlock();

    int error_count = zr_boost_1_81::report_errors();
    if(error_count > 0)
        retval = error_count;
    return retval;
}
