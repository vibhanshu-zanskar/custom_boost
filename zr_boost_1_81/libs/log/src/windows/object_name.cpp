...
static zr_boost_1_81::atomic< HANDLE > g_user_private_namespace;

//! Closes the private namespace on process exit
void close_user_namespace()
{
    HANDLE h = g_user_private_namespace.load(zr_boost_1_81::memory_order_acquire);
    if (h)
    {
        ClosePrivateNamespace(h, 0);
        g_user_private_namespace.store((HANDLE)NULL, zr_boost_1_81::memory_order_release);
    }
}
...
bool init_user_namespace()
{
    ...
    if (BOOST_UNLIKELY(!h))
    {
        ...
        zr_boost_1_81::log::ipc::aux::auto_handle h_process_token;
        ...
        if (h)
        {
            ...
            if (g_user_private_namespace.compare_exchange_strong(expected, h, zr_boost_1_81::memory_order_acq_rel, zr_boost_1_81::memory_order_acquire))
            {
    ...
    case object_name::user:
        {
...
#endif // BOOST_USE_WINAPI_VERSION >= BOOST_WINAPI_VERSION_WIN6
            {
                ...
                if (BOOST_UNLIKELY(!GetUserNameExW(NameSamCompatible, buf, &len)))
                {
                    ...
                    BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to obtain the current user name", (err));
                }

                std::replace(buf, buf + len, L'\\', L'.');

                prefix = "Local\\boost.log.user." + zr_boost_1_81::log::aux::utf16_to_utf8(buf);
            }
        }
        break;
