...
private:
    ...
    struct header
    {
        ...
        zr_boost_1_81::ipc_atomic< uint32_t > m_initialized;
        ...
        zr_boost_1_81::log::ipc::aux::interprocess_mutex::shared_state m_mutex_state;
        //! Shared state of the condition variable used to block writers when the queue is full.
        zr_boost_1_81::log::ipc::aux::interprocess_condition_variable::shared_state m_nonfull_queue_state;
            ...
            m_get_pos(0u)
        {
            ...
            m_initialized.opaque_add(1u, zr_boost_1_81::memory_order_release);
        }
        ...
        static uint32_t get_abi_tag() BOOST_NOEXCEPT
        {
            ...
            hash.mix(zr_boost_1_81::log::aux::make_fourcc('w', 'n', 't', '5'));
...
private:
    ...
    zr_boost_1_81::log::ipc::aux::mapped_shared_memory m_shared_memory;
    ...
    zr_boost_1_81::log::ipc::aux::interprocess_event m_nonempty_queue;
    ...
    zr_boost_1_81::log::ipc::aux::auto_handle m_stop;
...
public:
        ...
        m_name(name)
    {
        ...
        const std::wstring wname = zr_boost_1_81::log::aux::utf8_to_utf16(name.c_str());
    ...
    operation_result send(void const* message_data, size_type message_size)
    {
        ...
        zr_boost_1_81::log::ipc::aux::interprocess_mutex::optional_unlock unlock(m_mutex);
    ...
    bool try_send(void const* message_data, size_type message_size)
    {
        ...
        zr_boost_1_81::log::ipc::aux::interprocess_mutex::auto_unlock unlock(m_mutex);
    ...
    void stop_local()
    {
        BOOST_VERIFY(zr_boost_1_81::winapi::SetEvent(m_stop.get()) != 0);
    }

    void reset_local()
    {
        BOOST_VERIFY(zr_boost_1_81::winapi::ResetEvent(m_stop.get()) != 0);
    }
...
private:
    ...
    void create_stop_event()
    {
        ...
        (
            NULL, // permissions
            NULL, // name
            zr_boost_1_81::winapi::CREATE_EVENT_MANUAL_RESET_,
        ...
        if (BOOST_UNLIKELY(h == NULL))
        {
            ...
            BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to create an stop event object", (err));
        }
    ...
    void adopt_queue(std::wstring const& name, std::size_t shmem_size, permissions const& perms)
    {
        ...
        {
            uint32_t initialized = hdr->m_initialized.load(zr_boost_1_81::memory_order_acquire);
            if (initialized)
            {
                goto done;
            }
