...
void init_light_rw_mutex_impl()
{
    ...
    if (hKernel32)
    {
        g_pInitializeLWRWMutex =
            (init_fun_t)zr_boost_1_81::winapi::get_proc_address(hKernel32, "InitializeSRWLock");
        if (g_pInitializeLWRWMutex)
        {
            g_pLockExclusiveLWRWMutex =
                (lock_exclusive_fun_t)zr_boost_1_81::winapi::get_proc_address(hKernel32, "AcquireSRWLockExclusive");
            if (g_pLockExclusiveLWRWMutex)
            {
                g_pUnlockExclusiveLWRWMutex =
                    (unlock_exclusive_fun_t)zr_boost_1_81::winapi::get_proc_address(hKernel32, "ReleaseSRWLockExclusive");
                if (g_pUnlockExclusiveLWRWMutex)
                {
                    g_pLockSharedLWRWMutex =
                        (lock_shared_fun_t)zr_boost_1_81::winapi::get_proc_address(hKernel32, "AcquireSRWLockShared");
                    if (g_pLockSharedLWRWMutex)
                    {
                        g_pUnlockSharedLWRWMutex =
                            (unlock_shared_fun_t)zr_boost_1_81::winapi::get_proc_address(hKernel32, "ReleaseSRWLockShared");
