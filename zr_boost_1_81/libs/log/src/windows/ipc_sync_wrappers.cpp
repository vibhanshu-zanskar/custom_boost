...
void interprocess_event::create(const wchar_t* name, bool manual_reset, permissions const& perms)
{
    ...
    zr_boost_1_81::winapi::HANDLE_ h = zr_boost_1_81::winapi::CreateEventExW
    (
        reinterpret_cast< zr_boost_1_81::winapi::SECURITY_ATTRIBUTES_* >(perms.get_native()),
        name,
        zr_boost_1_81::winapi::CREATE_EVENT_MANUAL_RESET_ * manual_reset,
        zr_boost_1_81::winapi::SYNCHRONIZE_ | zr_boost_1_81::winapi::EVENT_MODIFY_STATE_
    );
#else
    zr_boost_1_81::winapi::HANDLE_ h = zr_boost_1_81::winapi::CreateEventW
    (
        reinterpret_cast< zr_boost_1_81::winapi::SECURITY_ATTRIBUTES_* >(perms.get_native()),
        manual_reset,
        false,
    ...
    if (BOOST_UNLIKELY(h == NULL))
    {
        ...
        BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to create an interprocess event object", (err));
    }
...
{
    zr_boost_1_81::winapi::HANDLE_ h = zr_boost_1_81::winapi::OpenEventW(zr_boost_1_81::winapi::SYNCHRONIZE_ | zr_boost_1_81::winapi::EVENT_MODIFY_STATE_, false, name);
    if (BOOST_UNLIKELY(h == NULL))
    {
        const zr_boost_1_81::winapi::DWORD_ err = zr_boost_1_81::winapi::GetLastError();
        BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to open an interprocess event object", (err));
    }

    m_event.init(h);
}

zr_boost_1_81::atomic< interprocess_semaphore::is_semaphore_zero_count_t > interprocess_semaphore::is_semaphore_zero_count(&interprocess_semaphore::is_semaphore_zero_count_init);
...
{
#if BOOST_USE_WINAPI_VERSION >= BOOST_WINAPI_VERSION_WIN6
    zr_boost_1_81::winapi::HANDLE_ h = zr_boost_1_81::winapi::CreateSemaphoreExW
    (
        reinterpret_cast< zr_boost_1_81::winapi::SECURITY_ATTRIBUTES_* >(perms.get_native()),
        0, // initial count
        (std::numeric_limits< zr_boost_1_81::winapi::LONG_ >::max)(), // max count
        name,
        0u, // flags
        zr_boost_1_81::winapi::SYNCHRONIZE_ | zr_boost_1_81::winapi::SEMAPHORE_MODIFY_STATE_ | zr_boost_1_81::winapi::SEMAPHORE_QUERY_STATE_
    );
#else
    zr_boost_1_81::winapi::HANDLE_ h = zr_boost_1_81::winapi::CreateSemaphoreW
    (
        reinterpret_cast< zr_boost_1_81::winapi::SECURITY_ATTRIBUTES_* >(perms.get_native()),
        0, // initial count
        (std::numeric_limits< zr_boost_1_81::winapi::LONG_ >::max)(), // max count
    ...
    if (h == NULL)
    {
        ...
        {
            BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to create an interprocess semaphore object", (err));
        }
    }
...
{
    zr_boost_1_81::winapi::HANDLE_ h = zr_boost_1_81::winapi::OpenSemaphoreW(zr_boost_1_81::winapi::SYNCHRONIZE_ | zr_boost_1_81::winapi::SEMAPHORE_MODIFY_STATE_ | zr_boost_1_81::winapi::SEMAPHORE_QUERY_STATE_, false, name);
    if (BOOST_UNLIKELY(h == NULL))
    {
        const zr_boost_1_81::winapi::DWORD_ err = zr_boost_1_81::winapi::GetLastError();
        BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to open an interprocess semaphore object", (err));
    }

    m_sem.init(h);
}

bool interprocess_semaphore::is_semaphore_zero_count_init(zr_boost_1_81::winapi::HANDLE_ h)
{
    ...
    if (ntdll)
    {
        nt_query_semaphore_t ntqs = (nt_query_semaphore_t)zr_boost_1_81::winapi::get_proc_address(ntdll, "NtQuerySemaphore");
        if (ntqs)
        {
    ...
    is_semaphore_zero_count.store(impl, zr_boost_1_81::memory_order_release);

    return impl(h);
}

bool interprocess_semaphore::is_semaphore_zero_count_nt_query_semaphore(zr_boost_1_81::winapi::HANDLE_ h)
{
    ...
    if (BOOST_UNLIKELY(err != 0u))
    {
        ...
        zr_boost_1_81::log::aux::snprintf(buf, sizeof(buf), "0x%08x", static_cast< unsigned int >(err));
        BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, std::string("Failed to test an interprocess semaphore object for zero count, NT status: ") + buf, (ERROR_INVALID_HANDLE));
    }

    return info.current_count == 0u;
}

bool interprocess_semaphore::is_semaphore_zero_count_emulated(zr_boost_1_81::winapi::HANDLE_ h)
{
    const zr_boost_1_81::winapi::DWORD_ retval = zr_boost_1_81::winapi::WaitForSingleObject(h, 0u);
    if (retval == zr_boost_1_81::winapi::wait_timeout)
    {
        return true;
    }
    else if (BOOST_UNLIKELY(retval != zr_boost_1_81::winapi::wait_object_0))
    {
        const zr_boost_1_81::winapi::DWORD_ err = zr_boost_1_81::winapi::GetLastError();
        BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to test an interprocess semaphore object for zero count", (err));
    }

    // Restore the decremented counter
    BOOST_VERIFY(!!zr_boost_1_81::winapi::ReleaseSemaphore(h, 1, NULL));
...
{
    uint32_t old_state = m_shared_state->m_lock_state.load(zr_boost_1_81::memory_order_relaxed);
    ...
    {
        m_shared_state->m_lock_state.fetch_sub(1u, zr_boost_1_81::memory_order_acq_rel);
        throw;
    }
}

bool interprocess_mutex::lock_slow(zr_boost_1_81::winapi::HANDLE_ abort_handle)
{
    uint32_t old_state = m_shared_state->m_lock_state.load(zr_boost_1_81::memory_order_relaxed);
    mark_waiting_and_try_lock(old_state);
...
inline void interprocess_mutex::mark_waiting_and_try_lock(uint32_t& old_state)
{
    ...
    while (!m_shared_state->m_lock_state.compare_exchange_weak(old_state, new_state, zr_boost_1_81::memory_order_acq_rel, zr_boost_1_81::memory_order_relaxed));
}
...
bool interprocess_condition_variable::wait(interprocess_mutex::optional_unlock& lock, zr_boost_1_81::winapi::HANDLE_ abort_handle)
{
    ...
    zr_boost_1_81::winapi::HANDLE_ handles[2u] = { m_current_semaphore->m_semaphore.get_handle(), abort_handle };

    interprocess_mutex* const mutex = lock.disengage();
    mutex->unlock();

    zr_boost_1_81::winapi::DWORD_ retval = zr_boost_1_81::winapi::WaitForMultipleObjects(2u, handles, false, zr_boost_1_81::winapi::INFINITE_);

    if (BOOST_UNLIKELY(retval == zr_boost_1_81::winapi::WAIT_FAILED_))
    {
        const zr_boost_1_81::winapi::DWORD_ err = zr_boost_1_81::winapi::GetLastError();
        ...
        if (err == ERROR_INVALID_HANDLE)
            retval = zr_boost_1_81::winapi::WAIT_OBJECT_0_;
        else
            BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to block on an interprocess semaphore object", (err));
    }
    ...
    return retval == zr_boost_1_81::winapi::WAIT_OBJECT_0_;
}
...
interprocess_condition_variable::semaphore_info* interprocess_condition_variable::get_semaphore(uint32_t id)
{
    ...
    if (res.second)
    {
        ...
        zr_boost_1_81::log::aux::unique_ptr< semaphore_info > p(new semaphore_info(id));
...
interprocess_condition_variable::semaphore_info* interprocess_condition_variable::get_unused_semaphore()
{
    ...
    while (it != end)
    {
        ...
        else if (it->check_non_zero_timeout(now))
        {
            ...
            m_semaphore_info_list.erase_and_dispose(it++, zr_boost_1_81::checked_deleter< semaphore_info >());
        }
        else
        {
            ++it;
...
inline void interprocess_condition_variable::generate_semaphore_name(uint32_t id) BOOST_NOEXCEPT
{
    ...
    *p++ = zr_boost_1_81::log::aux::g_hex_char_table[0][id >> 28];
    *p++ = zr_boost_1_81::log::aux::g_hex_char_table[0][(id >> 24) & 0x0000000Fu];

    *p++ = zr_boost_1_81::log::aux::g_hex_char_table[0][(id >> 20) & 0x0000000Fu];
    *p++ = zr_boost_1_81::log::aux::g_hex_char_table[0][(id >> 16) & 0x0000000Fu];

    *p++ = zr_boost_1_81::log::aux::g_hex_char_table[0][(id >> 12) & 0x0000000Fu];
    *p++ = zr_boost_1_81::log::aux::g_hex_char_table[0][(id >> 8) & 0x0000000Fu];

    *p++ = zr_boost_1_81::log::aux::g_hex_char_table[0][(id >> 4) & 0x0000000Fu];
    *p = zr_boost_1_81::log::aux::g_hex_char_table[0][id & 0x0000000Fu];
}
