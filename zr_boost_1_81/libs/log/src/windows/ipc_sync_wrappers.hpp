...
public:
    ...
    zr_boost_1_81::winapi::HANDLE_ get_handle() const BOOST_NOEXCEPT { return m_event.get(); }

    void set()
    {
        if (BOOST_UNLIKELY(!zr_boost_1_81::winapi::SetEvent(m_event.get())))
        {
            const zr_boost_1_81::winapi::DWORD_ err = zr_boost_1_81::winapi::GetLastError();
            BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to set an interprocess event object", (err));
        }
    }

    void set_noexcept() BOOST_NOEXCEPT
    {
        BOOST_VERIFY(!!zr_boost_1_81::winapi::SetEvent(m_event.get()));
    }

    void reset()
    {
        if (BOOST_UNLIKELY(!zr_boost_1_81::winapi::ResetEvent(m_event.get())))
        {
            const zr_boost_1_81::winapi::DWORD_ err = zr_boost_1_81::winapi::GetLastError();
            BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to reset an interprocess event object", (err));
        }
    }

    void wait()
    {
        const zr_boost_1_81::winapi::DWORD_ retval = zr_boost_1_81::winapi::WaitForSingleObject(m_event.get(), zr_boost_1_81::winapi::infinite);
        if (BOOST_UNLIKELY(retval != zr_boost_1_81::winapi::wait_object_0))
        {
            const zr_boost_1_81::winapi::DWORD_ err = zr_boost_1_81::winapi::GetLastError();
            BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to block on an interprocess event object", (err));
        }
    }

    bool wait(zr_boost_1_81::winapi::HANDLE_ abort_handle)
    {
        zr_boost_1_81::winapi::HANDLE_ handles[2u] = { m_event.get(), abort_handle };
        const zr_boost_1_81::winapi::DWORD_ retval = zr_boost_1_81::winapi::WaitForMultipleObjects(2u, handles, false, zr_boost_1_81::winapi::infinite);
        if (retval == (zr_boost_1_81::winapi::wait_object_0 + 1u))
        {
...
{
private:
    typedef zr_boost_1_81::winapi::DWORD_ NTSTATUS_;
    struct semaphore_basic_information
    {
        zr_boost_1_81::winapi::ULONG_ current_count; // current semaphore count
        zr_boost_1_81::winapi::ULONG_ maximum_count; // max semaphore count
    };
    typedef NTSTATUS_ (__stdcall *nt_query_semaphore_t)(zr_boost_1_81::winapi::HANDLE_ h, unsigned int info_class, semaphore_basic_information* pinfo, zr_boost_1_81::winapi::ULONG_ info_size, zr_boost_1_81::winapi::ULONG_* ret_len);
    typedef bool (*is_semaphore_zero_count_t)(zr_boost_1_81::winapi::HANDLE_ h);

private:
    auto_handle m_sem;

    static zr_boost_1_81::atomic< is_semaphore_zero_count_t > is_semaphore_zero_count;
...
public:
    ...
    zr_boost_1_81::winapi::HANDLE_ get_handle() const BOOST_NOEXCEPT { return m_sem.get(); }

    void post(uint32_t count)
    {
        BOOST_ASSERT(count <= static_cast< uint32_t >((std::numeric_limits< zr_boost_1_81::winapi::LONG_ >::max)()));

        if (BOOST_UNLIKELY(!zr_boost_1_81::winapi::ReleaseSemaphore(m_sem.get(), static_cast< zr_boost_1_81::winapi::LONG_ >(count), NULL)))
        {
            const zr_boost_1_81::winapi::DWORD_ err = zr_boost_1_81::winapi::GetLastError();
            BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to post on an interprocess semaphore object", (err));
        }
    }

    bool is_zero_count() const
    {
        return is_semaphore_zero_count.load(zr_boost_1_81::memory_order_acquire)(m_sem.get());
    }

    void wait()
    {
        const zr_boost_1_81::winapi::DWORD_ retval = zr_boost_1_81::winapi::WaitForSingleObject(m_sem.get(), zr_boost_1_81::winapi::infinite);
        if (BOOST_UNLIKELY(retval != zr_boost_1_81::winapi::wait_object_0))
        {
            const zr_boost_1_81::winapi::DWORD_ err = zr_boost_1_81::winapi::GetLastError();
    ...
    bool wait(zr_boost_1_81::winapi::HANDLE_ abort_handle)
    {
        zr_boost_1_81::winapi::HANDLE_ handles[2u] = { m_sem.get(), abort_handle };
        const zr_boost_1_81::winapi::DWORD_ retval = zr_boost_1_81::winapi::WaitForMultipleObjects(2u, handles, false, zr_boost_1_81::winapi::infinite);
        if (retval == (zr_boost_1_81::winapi::wait_object_0 + 1u))
        {
            // Wait was interrupted
...
private:
    static bool is_semaphore_zero_count_init(zr_boost_1_81::winapi::HANDLE_ h);
    static bool is_semaphore_zero_count_nt_query_semaphore(zr_boost_1_81::winapi::HANDLE_ h);
    static bool is_semaphore_zero_count_emulated(zr_boost_1_81::winapi::HANDLE_ h);
};
...
public:
    ...
    struct shared_state
    {
        zr_boost_1_81::atomic< uint32_t > m_lock_state;
...
public:
    ...
    bool try_lock()
    {
        return !m_shared_state->m_lock_state.bit_test_and_set(lock_flag_bit, zr_boost_1_81::memory_order_acquire);
    }
    ...
    bool lock(zr_boost_1_81::winapi::HANDLE_ abort_handle)
    {
    ...
    {
        const uint32_t old_count = m_shared_state->m_lock_state.fetch_add(lock_flag_value, zr_boost_1_81::memory_order_release);
        if ((old_count & event_set_flag_value) == 0u && (old_count > lock_flag_value))
        {
            if (!m_shared_state->m_lock_state.bit_test_and_set(event_set_flag_bit, zr_boost_1_81::memory_order_relaxed))
            {
...
private:
    void lock_slow();
    bool lock_slow(zr_boost_1_81::winapi::HANDLE_ abort_handle);
...
struct tick_count_clock
{
    ...
    typedef zr_boost_1_81::winapi::ULONGLONG_ time_point;
#else
    typedef zr_boost_1_81::winapi::DWORD_ time_point;
#endif

    static time_point now() BOOST_NOEXCEPT
    {
#if BOOST_USE_WINAPI_VERSION >= BOOST_WINAPI_VERSION_WIN6
        return zr_boost_1_81::winapi::GetTickCount64();
#else
        return zr_boost_1_81::winapi::GetTickCount();
#endif
    }
};
...
{
private:
    typedef zr_boost_1_81::intrusive::list_base_hook<
        zr_boost_1_81::intrusive::tag< struct for_sem_order_by_usage >,
        zr_boost_1_81::intrusive::link_mode< zr_boost_1_81::intrusive::safe_link >
    > semaphore_info_list_hook_t;

    typedef zr_boost_1_81::intrusive::set_base_hook<
        zr_boost_1_81::intrusive::tag< struct for_sem_lookup_by_id >,
        zr_boost_1_81::intrusive::link_mode< zr_boost_1_81::intrusive::safe_link >,
        zr_boost_1_81::intrusive::optimize_size< true >
    ...
    typedef zr_boost_1_81::intrusive::list<
        semaphore_info,
        zr_boost_1_81::intrusive::base_hook< semaphore_info_list_hook_t >,
        zr_boost_1_81::intrusive::constant_time_size< false >
    > semaphore_info_list;

    typedef zr_boost_1_81::intrusive::set<
        semaphore_info,
        zr_boost_1_81::intrusive::base_hook< semaphore_info_set_hook_t >,
        zr_boost_1_81::intrusive::compare< semaphore_info::order_by_id >,
        zr_boost_1_81::intrusive::constant_time_size< false >
    > semaphore_info_set;
...
public:
    ...
    ~interprocess_condition_variable()
    {
        ...
        m_semaphore_info_list.clear_and_dispose(zr_boost_1_81::checked_deleter< semaphore_info >());
    }
    ...
    bool wait(interprocess_mutex::optional_unlock& lock, zr_boost_1_81::winapi::HANDLE_ abort_handle);
