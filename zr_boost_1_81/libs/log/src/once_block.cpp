...
zr_boost_1_81::winapi::SRWLOCK_ g_OnceBlockMutex = BOOST_WINAPI_SRWLOCK_INIT;
zr_boost_1_81::winapi::CONDITION_VARIABLE_ g_OnceBlockCond = BOOST_WINAPI_CONDITION_VARIABLE_INIT;

} // namespace

BOOST_LOG_API bool once_block_sentry::enter_once_block() const BOOST_NOEXCEPT
{
    zr_boost_1_81::winapi::AcquireSRWLockExclusive(&g_OnceBlockMutex);
    ...
    while (flag.status != once_block_flag::initialized)
    {
        if (flag.status == once_block_flag::uninitialized)
        {
            ...
            zr_boost_1_81::winapi::ReleaseSRWLockExclusive(&g_OnceBlockMutex);
        ...
        else
        {
            ...
            {
                BOOST_VERIFY(zr_boost_1_81::winapi::SleepConditionVariableSRW(
                    &g_OnceBlockCond, &g_OnceBlockMutex, zr_boost_1_81::winapi::INFINITE_, 0));
            }
        }
    }
...
BOOST_LOG_API void once_block_sentry::commit() BOOST_NOEXCEPT
{
    ...
    zr_boost_1_81::winapi::ReleaseSRWLockExclusive(&g_OnceBlockMutex);
    zr_boost_1_81::winapi::WakeAllConditionVariable(&g_OnceBlockCond);
}
...
BOOST_LOG_ANONYMOUS_NAMESPACE {
    ...
    public:
        ...
        typedef zr_boost_1_81::winapi::BOOL_ (BOOST_WINAPI_WINAPI_CC *SleepConditionVariableSRW_t)(winapi_condition_variable*, winapi_srwlock*, zr_boost_1_81::winapi::DWORD_, zr_boost_1_81::winapi::ULONG_);
                ...
                else
                {
                    while (flag.status == once_block_flag::being_initialized)
                    {
                        BOOST_VERIFY(m_pSleepConditionVariableSRW(
                            &m_Cond, &m_Mutex, zr_boost_1_81::winapi::INFINITE_, 0));
                    }
                }
            }
    ...
    once_block_impl_base* create_once_block_impl()
    {
        ...
        if (hKernel32)
        {
            once_block_impl_nt6::InitializeSRWLock_t pInitializeSRWLock =
                (once_block_impl_nt6::InitializeSRWLock_t)zr_boost_1_81::winapi::get_proc_address(hKernel32, "InitializeSRWLock");
            if (pInitializeSRWLock)
            {
                once_block_impl_nt6::AcquireSRWLockExclusive_t pAcquireSRWLockExclusive =
                    (once_block_impl_nt6::AcquireSRWLockExclusive_t)zr_boost_1_81::winapi::get_proc_address(hKernel32, "AcquireSRWLockExclusive");
                if (pAcquireSRWLockExclusive)
                {
                    once_block_impl_nt6::ReleaseSRWLockExclusive_t pReleaseSRWLockExclusive =
                        (once_block_impl_nt6::ReleaseSRWLockExclusive_t)zr_boost_1_81::winapi::get_proc_address(hKernel32, "ReleaseSRWLockExclusive");
                    if (pReleaseSRWLockExclusive)
                    {
                        once_block_impl_nt6::InitializeConditionVariable_t pInitializeConditionVariable =
                            (once_block_impl_nt6::InitializeConditionVariable_t)zr_boost_1_81::winapi::get_proc_address(hKernel32, "InitializeConditionVariable");
                        if (pInitializeConditionVariable)
                        {
                            once_block_impl_nt6::SleepConditionVariableSRW_t pSleepConditionVariableSRW =
                                (once_block_impl_nt6::SleepConditionVariableSRW_t)zr_boost_1_81::winapi::get_proc_address(hKernel32, "SleepConditionVariableSRW");
                            if (pSleepConditionVariableSRW)
                            {
                                once_block_impl_nt6::WakeAllConditionVariable_t pWakeAllConditionVariable =
                                    (once_block_impl_nt6::WakeAllConditionVariable_t)zr_boost_1_81::winapi::get_proc_address(hKernel32, "WakeAllConditionVariable");
