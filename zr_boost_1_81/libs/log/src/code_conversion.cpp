...
std::wstring utf8_to_utf16(const char* str)
{
    ...
    int len = zr_boost_1_81::winapi::MultiByteToWideChar(zr_boost_1_81::winapi::CP_UTF8_, zr_boost_1_81::winapi::MB_ERR_INVALID_CHARS_, str, static_cast< int >(utf8_len), NULL, 0);
    if (BOOST_LIKELY(len > 0))
    {
        std::wstring wstr;
        wstr.resize(len);

        len = zr_boost_1_81::winapi::MultiByteToWideChar(zr_boost_1_81::winapi::CP_UTF8_, zr_boost_1_81::winapi::MB_ERR_INVALID_CHARS_, str, static_cast< int >(utf8_len), &wstr[0], len);
        if (BOOST_LIKELY(len > 0))
        {
            return wstr;
        }
...
std::string utf16_to_utf8(const wchar_t* wstr)
{
    ...
    const zr_boost_1_81::winapi::DWORD_ flags =
#if BOOST_USE_WINAPI_VERSION >= BOOST_WINAPI_VERSION_WIN6
        zr_boost_1_81::winapi::WC_ERR_INVALID_CHARS_;
#else
        0u;
#endif
    int len = zr_boost_1_81::winapi::WideCharToMultiByte(zr_boost_1_81::winapi::CP_UTF8_, flags, wstr, static_cast< int >(utf16_len), NULL, 0, NULL, NULL);
    if (BOOST_LIKELY(len > 0))
    {
        std::string str;
        str.resize(len);

        len = zr_boost_1_81::winapi::WideCharToMultiByte(zr_boost_1_81::winapi::CP_UTF8_, flags, wstr, static_cast< int >(utf16_len), &str[0], len, NULL, NULL);
        if (BOOST_LIKELY(len > 0))
        {
            return str;
        }
