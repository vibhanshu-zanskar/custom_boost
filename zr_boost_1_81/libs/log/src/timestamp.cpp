...
BOOST_LOG_API get_tick_count_t get_tick_count = &zr_boost_1_81::winapi::GetTickCount64;
...
uint64_t BOOST_WINAPI_WINAPI_CC get_tick_count_init()
{
    ...
    if (hKernel32)
    {
        get_tick_count_t p = (get_tick_count_t)zr_boost_1_81::winapi::get_proc_address(hKernel32, "GetTickCount64");
        if (p)
        {
            // Use native API
...
BOOST_LOG_API int64_t duration::milliseconds() const
{
    ...
    BOOST_LOG_ONCE_BLOCK()
    {
        ...
        if (err != KERN_SUCCESS)
        {
            BOOST_LOG_THROW_DESCR_PARAMS(system_error, "Failed to initialize timebase info", (zr_boost_1_81::system::errc::not_supported));
        }
    }
