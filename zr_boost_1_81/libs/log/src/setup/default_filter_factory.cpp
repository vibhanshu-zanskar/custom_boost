...
public:
    template< typename StringT >
    severity_or_string_predicate(relation_type const& rel, StringT const& string_operand, zr_boost_1_81::log::trivial::severity_level severity_operand) :
    ...
    result_type operator() (zr_boost_1_81::log::trivial::severity_level val) const
    {
        return relation_type::operator() (val, m_severity_operand);
    }

private:
    const zr_boost_1_81::log::trivial::severity_level m_severity_operand;
};
...
filter default_filter_factory< CharT >::parse_argument(attribute_name const& name, string_type const& arg)
{
    ...
    else
    {
        ...
        {
            zr_boost_1_81::log::trivial::severity_level lvl;
            if (name == zr_boost_1_81::log::aux::default_attribute_names::severity() && zr_boost_1_81::log::trivial::from_string(arg.data(), arg.size(), lvl))
            {
                typedef severity_or_string_predicate< RelationT > predicate;
                typedef mpl::vector< zr_boost_1_81::log::trivial::severity_level, BOOST_PP_SEQ_ENUM(BOOST_LOG_STANDARD_STRING_TYPES()) > value_types;
