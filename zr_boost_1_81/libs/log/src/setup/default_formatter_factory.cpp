...
#if !defined(BOOST_LOG_NO_THREADS)
#define BOOST_LOG_AUX_THREAD_ID_TYPE() (zr_boost_1_81::log::aux::thread::id)
#else
#define BOOST_LOG_AUX_THREAD_ID_TYPE()
#endif

#define BOOST_LOG_AUX_LOG_ATTRIBUTE_VALUE_TYPES()\
    (zr_boost_1_81::log::trivial::severity_level)\
    (zr_boost_1_81::log::attributes::named_scope_list)\
    (zr_boost_1_81::log::aux::process::id)\
...
private:
    ...
    struct visitor
    {
        ...
        void operator() (zr_boost_1_81::posix_time::ptime const& value) const
        {
            if (!value.is_special())
            {
                std::tm t = zr_boost_1_81::posix_time::to_tm(value);
                char buf[32];
                std::size_t len = std::strftime(buf, sizeof(buf), "%Y-%m-%d %H:%M:%S", &t);
                std::size_t size = sizeof(buf) - len;
                int res = zr_boost_1_81::log::aux::snprintf(buf + len, size, ".%.6u", static_cast< unsigned int >(value.time_of_day().total_microseconds() % 1000000));
                if (res < 0)
                    buf[len] = '\0';
        ...
        void operator() (zr_boost_1_81::local_time::local_date_time const& value) const
        {
            if (!value.is_special())
            {
        ...
        void operator() (zr_boost_1_81::gregorian::date const& value) const
        {
            if (!value.is_special())
            {
                std::tm t = zr_boost_1_81::gregorian::to_tm(value);
                char buf[32];
        ...
        void operator() (zr_boost_1_81::posix_time::time_duration const& value) const
        {
            if (!value.is_special())
            {
                zr_boost_1_81::posix_time::time_duration val = value;
                if (val.is_negative())
                {
                    m_strm << '-';
                    val = -val;
                ...
                char buf[64];
                int len = zr_boost_1_81::log::aux::snprintf(buf, sizeof(buf), "%.2llu:%.2u:%.2u.%.6u", hours, minutes, seconds, useconds);
                if (len > 0)
                {
        ...
        void operator() (zr_boost_1_81::gregorian::date_duration const& value) const
        {
            if (!value.is_special())
            {
        ...
        void operator() (zr_boost_1_81::date_time::period< PointRepT, DurationRepT > const& value) const
        {
            m_strm << '[';
    ...
    private:
        ...
        void format_special_date_time(zr_boost_1_81::date_time::special_values value) const
        {
            switch (value)
            {
            case zr_boost_1_81::date_time::not_a_date_time:
                m_strm << "not-a-date-time";
                break;
            case zr_boost_1_81::date_time::pos_infin:
                m_strm << "+infinity";
                break;
            case zr_boost_1_81::date_time::neg_infin:
                m_strm << "-infinity";
                break;
            default:
                break;
...
public:
    ...
    result_type operator() (record_view const& rec, basic_formatting_ostream< CharT >& strm) const
    {
        ...
        zr_boost_1_81::log::visit< value_types >(m_attribute_name, rec, visitor(strm));
    }
