...
public:
    ...
    filter get_filter()
    {
        if (m_Subexpressions.empty())
            return filter();
        return zr_boost_1_81::move(m_Subexpressions.top());
    }
...
private:
    ...
    iterator_type parse_relation(iterator_type begin, iterator_type end)
    {
        ...
        if (p != end)
        {
            ...
            m_Operand = zr_boost_1_81::in_place(operand);
        }
    ...
    void on_negation()
    {
        if (!m_Subexpressions.empty())
        {
            m_Subexpressions.top() = !zr_boost_1_81phoenix::bind(m_Subexpressions.top(), zr_boost_1_81phoenix::placeholders::_1);
        }
        else
        {
    ...
    void on_and()
    {
        if (!m_Subexpressions.empty())
        {
            filter right = zr_boost_1_81::move(m_Subexpressions.top());
            m_Subexpressions.pop();
            if (!m_Subexpressions.empty())
            {
                filter const& left = m_Subexpressions.top();
                m_Subexpressions.top() = zr_boost_1_81phoenix::bind(left, zr_boost_1_81phoenix::placeholders::_1) && zr_boost_1_81phoenix::bind(right, zr_boost_1_81phoenix::placeholders::_1);
                return;
            }
        }
