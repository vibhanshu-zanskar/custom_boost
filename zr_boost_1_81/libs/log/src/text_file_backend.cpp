...
BOOST_LOG_ANONYMOUS_NAMESPACE {
    ...
    public:
        ...
        path_string_type operator()(path_string_type const& pattern, unsigned int counter) const
        {
            ...
            m_Facet.put(sbuf_it, m_Stream, m_Stream.fill(), zr_boost_1_81::log::attributes::local_time_traits::get_clock());
            if (m_Stream.good())
            {
                return m_Stream.str();
            }
    ...
    void parse_file_name_pattern(filesystem::path const& pattern, filesystem::path& storage_dir, filesystem::path& file_name_pattern, zr_boost_1_81::log::aux::light_function< path_string_type (unsigned int) >& file_name_generator)
    {
        ...
        if (placeholder_count > 0)
        {
            if (counter_found)
            {
                ...
                file_name_generator = zr_boost_1_81::bind(date_and_time_formatter(),
                    zr_boost_1_81::bind(file_counter_formatter(counter_pos, width), name_pattern, zr_boost_1_81::placeholders::_1), zr_boost_1_81::placeholders::_1);
            }
            else
            {
                // Only date/time placeholders in the pattern
                file_name_generator = zr_boost_1_81::bind(date_and_time_formatter(), name_pattern, zr_boost_1_81::placeholders::_1);
            }
        }
        else if (counter_found)
        {
            // Only counter placeholder in the pattern
            file_name_generator = zr_boost_1_81::bind(file_counter_formatter(counter_pos, width), name_pattern, zr_boost_1_81::placeholders::_1);
        }
        else
        {
    ...
    private:
        ...
        {
            base_type::get_instance() = zr_boost_1_81::make_shared< file_collector_repository >();
        }
    };
        ...
        filesystem::path const& target_dir, uintmax_t max_size, uintmax_t min_free_space, uintmax_t max_files)
    {
        ...
        file_collectors::iterator it = std::find_if(m_Collectors.begin(), m_Collectors.end(),
            zr_boost_1_81::bind(&file_collector::is_governed, zr_boost_1_81::placeholders::_1, zr_boost_1_81::cref(target_dir)));
        ...
        if (!p)
        {
            p = zr_boost_1_81::make_shared< file_collector >(
...
struct text_file_backend::implementation
{
    ...
    zr_boost_1_81::log::aux::light_function< path_string_type (unsigned int) > m_FileNameGenerator;
    ...
    zr_boost_1_81::log::aux::light_function< path_string_type (unsigned int) > m_TargetFileNameGenerator;
