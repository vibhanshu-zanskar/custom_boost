...
#include <boost/thread/thread.hpp> // zr_boost_1_81at_thread_exit
...
class severity_level_holder :
    public zr_boost_1_81::log::aux::lazy_singleton< severity_level_holder, zr_boost_1_81::log::aux::thread_specific< uintmax_t* > >
{
};
...
{
    zr_boost_1_81::log::aux::thread_specific< uintmax_t* >& tss = severity_level_holder::get();
    uintmax_t* p = tss.get();
    if (BOOST_UNLIKELY(!p))
    {
        ...
        tss.set(ptr.get());
        p = ptr.release();
        zr_boost_1_81::this_thread::zr_boost_1_81at_thread_exit(zr_boost_1_81::bind(checked_deleter< uintmax_t >(), p));
    }
    return *p;
}
