    ...
    public public_data
{
    ...
    typedef zr_boost_1_81::log::aux::stateless_allocator< char > stateless_allocator;
    //! Sink pointer type
...
private:
    ...
    private_data(BOOST_RV_REF(attribute_value_set) values, uint32_t capacity) BOOST_NOEXCEPT :
        public_data(zr_boost_1_81::move(values)),
...
public:
    ...
    static private_data* create(BOOST_RV_REF(attribute_value_set) values, uint32_t capacity)
    {
        ...
        (
            sizeof(private_data) +
            zr_boost_1_81::log::aux::alignment_gap_between< private_data, sink_ptr >::value +
            capacity * sizeof(sink_ptr)
        ));
        new (p) private_data(zr_boost_1_81::move(values), capacity);
        return p;
    }
...
#if !defined(BOOST_LOG_NO_THREADS)
    zr_boost_1_81::atomic< bool > m_enabled;
#else
    bool m_enabled;
#endif
    //! Global filter
...
public:
    //! Constructor
    implementation() :
        m_default_sink(zr_boost_1_81::make_shared< sinks::aux::default_sink >()),
        m_enabled(true)
    {
    }
    ...
    BOOST_FORCEINLINE record open_record(BOOST_FWD_REF(SourceAttributesT) source_attributes)
    {
...
#if !defined(BOOST_LOG_NO_THREADS)
        if (BOOST_LIKELY(m_enabled.load(zr_boost_1_81::memory_order_relaxed)))
#else
        ...
        try
        {
...
#endif
            {
                ...
                attribute_value_set attr_values(zr_boost_1_81::forward< SourceAttributesT >(source_attributes), tsd->m_thread_attributes, m_global_attributes);
    ...
    void apply_sink_filter(shared_ptr< sinks::sink > const& sink, record_view::private_data*& rec_impl, attribute_value_set*& attr_values, uint32_t remaining_capacity)
    {
        try
        {
            if (sink->will_consume(*attr_values))
            {
                ...
                if (!impl)
                {
                    rec_impl = impl = record_view::private_data::create(zr_boost_1_81::move(*attr_values), remaining_capacity);
...
{
#if !defined(BOOST_LOG_NO_THREADS)
    return m_impl->m_enabled.exchange(enabled, zr_boost_1_81::memory_order_relaxed);
#else
...
{
#if !defined(BOOST_LOG_NO_THREADS)
    return m_impl->m_enabled.load(zr_boost_1_81::memory_order_relaxed);
#else
...
{
    return m_impl->open_record(zr_boost_1_81::move(source_attributes));
}
