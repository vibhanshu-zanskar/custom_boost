...
private:
    ...
    struct block_header
    {
        ...
        {
            return static_cast< size_type >(zr_boost_1_81::alignment::align_up(sizeof(block_header), data_alignment));
        }
    ...
    struct header
    {
        ...
        zr_boost_1_81::ipc_atomic< uint32_t > m_ref_count;
        ...
        zr_boost_1_81::log::ipc::aux::interprocess_mutex m_mutex;
        //! Condition variable used to block readers when the queue is empty.
        zr_boost_1_81::log::ipc::aux::interprocess_condition_variable m_nonempty_queue;
        //! Condition variable used to block writers when the queue is full.
        zr_boost_1_81::log::ipc::aux::interprocess_condition_variable m_nonfull_queue;
            ...
            m_get_pos(0u)
        {
            ...
            m_ref_count.opaque_add(1u, zr_boost_1_81::memory_order_release);
        }
        ...
        static uint32_t get_abi_tag() BOOST_NOEXCEPT
        {
            ...
            zr_boost_1_81::log::aux::murmur3_32 hash(zr_boost_1_81::log::aux::make_fourcc('r', 'e', 'l', 'q'));

            // This FOURCC identifies the queue implementation
            hash.mix(zr_boost_1_81::log::aux::make_fourcc('p', 't', 'h', 'r'));
...
#undef BOOST_LOG_MIX_HEADER_MEMBER
        ...
        block_header* get_block(uint32_t index) const BOOST_NOEXCEPT
        {
            ...
            unsigned char* p = const_cast< unsigned char* >(reinterpret_cast< const unsigned char* >(this)) + zr_boost_1_81::alignment::align_up(sizeof(header), BOOST_LOG_CPU_CACHE_LINE_SIZE);
...
private:
    ...
    zr_boost_1_81::interprocess::shared_memory_object m_shared_memory;
    //! Shared memory mapping into the process address space
    zr_boost_1_81::interprocess::mapped_region m_region;
    ...
    zr_boost_1_81::atomic< bool > m_stop;
...
public:
        ...
        m_name(name)
    {
        ...
        zr_boost_1_81::interprocess::permissions ipc_perms(perms.get_native());
        while (true)
        {
            try
            {
                zr_boost_1_81::interprocess::shared_memory_object shared_memory(zr_boost_1_81::interprocess::create_only, name.c_str(), zr_boost_1_81::interprocess::read_write, ipc_perms);
                m_shared_memory.swap(shared_memory);
                break;
            }
            catch (zr_boost_1_81::interprocess::interprocess_exception& e)
            {
        ...
        m_name(name)
    {
        ...
        while (true)
        {
            while (true)
            {
                ...
                {
                    if (e.get_error_code() == zr_boost_1_81::interprocess::already_exists_error)
                        break;
            ...
            while (true)
            {
                try
                {
                    zr_boost_1_81::interprocess::shared_memory_object shared_memory(zr_boost_1_81::interprocess::open_only, name.c_str(), zr_boost_1_81::interprocess::read_write);
                    m_shared_memory.swap(shared_memory);
                    created = false;
                    goto done;
                }
                ...
                {
                    if (e.get_error_code() == zr_boost_1_81::interprocess::not_found_error)
                        break;
    ...
    operation_result send(void const* message_data, size_type message_size)
    {
        ...
        if (m_stop.load(zr_boost_1_81::memory_order_relaxed))
            return aborted;

        lock_queue();
        zr_boost_1_81::log::ipc::aux::interprocess_mutex::auto_unlock unlock(hdr->m_mutex);
    ...
    void stop_local()
    {
        ...
        m_stop.store(true, zr_boost_1_81::memory_order_relaxed);
    ...
    void reset_local()
    {
        m_stop.store(false, zr_boost_1_81::memory_order_relaxed);
    }
...
private:
    ...
    {
        return zr_boost_1_81::alignment::align_up(sizeof(header), BOOST_LOG_CPU_CACHE_LINE_SIZE) + static_cast< std::size_t >(capacity) * static_cast< std::size_t >(block_size);
    }
    ...
    void create_region(uint32_t capacity, size_type block_size)
    {
        ...
        zr_boost_1_81::interprocess::mapped_region(m_shared_memory, zr_boost_1_81::interprocess::read_write, 0u, shmem_size).swap(m_region);
    ...
    void adopt_region()
    {
        ...
        while (true)
        {
            zr_boost_1_81::interprocess::offset_t shm_size = 0;
        ...
        zr_boost_1_81::interprocess::mapped_region(m_shared_memory, zr_boost_1_81::interprocess::read_write, 0, shmem_size).swap(m_region);

        // Wait until the mapped region becomes initialized
        header* const hdr = get_header();
        for (i = 0u; i < region_init_wait_loops; ++i)
        {
            uint32_t ref_count = hdr->m_ref_count.load(zr_boost_1_81::memory_order_acquire);
            while (ref_count > 0u)
            {
                if (hdr->m_ref_count.compare_exchange_weak(ref_count, ref_count + 1u, zr_boost_1_81::memory_order_acq_rel, zr_boost_1_81::memory_order_acquire))
                    goto done;
            }

            if (i < region_init_wait_loops_pause)
                zr_boost_1_81::log::aux::pause();
        ...
        try
        {
            ...
            if (!zr_boost_1_81::log::aux::is_power_of_2(hdr->m_block_size))
    ...
    void close_region() BOOST_NOEXCEPT
    {
        ...
        if (hdr->m_ref_count.fetch_sub(1u, zr_boost_1_81::memory_order_acq_rel) == 1u)
        {
            zr_boost_1_81::interprocess::shared_memory_object::remove(m_shared_memory.get_name());

            hdr->~header();

            zr_boost_1_81::interprocess::mapped_region().swap(m_region);
            zr_boost_1_81::interprocess::shared_memory_object().swap(m_shared_memory);
    ...
    void lock_queue()
    {
        ...
        catch (zr_boost_1_81::log::ipc::aux::lock_owner_dead&)
        {
...
BOOST_LOG_API void reliable_message_queue::create(object_name const& name, uint32_t capacity, size_type block_size, overflow_policy oflow_policy, permissions const& perms)
{
    ...
    if (!zr_boost_1_81::log::aux::is_power_of_2(block_size))
        BOOST_THROW_EXCEPTION(std::invalid_argument("Interprocess message queue block size is not a power of 2"));
    try
    {
        m_impl = new implementation(open_mode::create_only, name, capacity, static_cast< size_type >(zr_boost_1_81::alignment::align_up(block_size, BOOST_LOG_CPU_CACHE_LINE_SIZE)), oflow_policy, perms);
    }
    catch (zr_boost_1_81::exception& e)
    {
        e << zr_boost_1_81::log::ipc::object_name_info(name);
        throw;
    }
    catch (zr_boost_1_81::interprocess::interprocess_exception& e)
    {
        BOOST_THROW_EXCEPTION(zr_boost_1_81::enable_error_info(system_error(zr_boost_1_81::system::error_code(e.get_native_error(), zr_boost_1_81::system::system_category()), e.what())) << zr_boost_1_81::log::ipc::object_name_info(name));
    }
}
...
BOOST_LOG_API void reliable_message_queue::open_or_create(object_name const& name, uint32_t capacity, size_type block_size, overflow_policy oflow_policy, permissions const& perms)
{
    ...
    try
    {
        m_impl = new implementation(open_mode::open_or_create, name, capacity, static_cast< size_type >(zr_boost_1_81::alignment::align_up(block_size, BOOST_LOG_CPU_CACHE_LINE_SIZE)), oflow_policy, perms);
    }
    catch (zr_boost_1_81::exception& e)
    {
        e << zr_boost_1_81::log::ipc::object_name_info(name);
...
BOOST_LOG_API void reliable_message_queue::clear()
{
    ...
    {
        e << zr_boost_1_81::log::ipc::object_name_info(m_impl->name());
        throw;
    }
}
...
{
    zr_boost_1_81::interprocess::shared_memory_object::remove(name.c_str());
}
