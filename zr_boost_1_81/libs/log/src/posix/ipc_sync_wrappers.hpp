...
struct pthread_mutex_attributes
{
    ...
    pthread_mutex_attributes()
    {
        ...
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to initialize pthread mutex attributes", (err));
    }
...
struct pthread_condition_variable_attributes
{
    ...
    pthread_condition_variable_attributes()
    {
        ...
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to initialize pthread condition variable attributes", (err));
    }
...
struct interprocess_mutex
{
    ...
    interprocess_mutex()
    {
        ...
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to set pthread mutex type", (err));
        err = pthread_mutexattr_setpshared(&attrs.attrs, PTHREAD_PROCESS_SHARED);
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to make pthread mutex process-shared", (err));
#if defined(BOOST_LOG_HAS_PTHREAD_MUTEX_ROBUST)
        err = pthread_mutexattr_setrobust(&attrs.attrs, PTHREAD_MUTEX_ROBUST);
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to make pthread mutex robust", (err));
#endif

        err = pthread_mutex_init(&this->mutex, &attrs.attrs);
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to initialize pthread mutex", (err));
    }
...
#endif
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to lock pthread mutex", (err));
    }
    ...
    void recover()
    {
        ...
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to recover pthread mutex from a crashed thread", (err));
    }
#endif
...
struct interprocess_condition_variable
{
    ...
    interprocess_condition_variable()
    {
        ...
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to make pthread condition variable process-shared", (err));

        err = pthread_cond_init(&this->cond, &attrs.attrs);
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to initialize pthread condition variable", (err));
    }
    ...
    void notify_one()
    {
        ...
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to notify one thread on a pthread condition variable", (err));
    }
    ...
    void notify_all()
    {
        ...
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to notify all threads on a pthread condition variable", (err));
    }
    ...
    void wait(interprocess_mutex& mutex)
    {
        ...
        if (BOOST_UNLIKELY(err != 0))
            BOOST_LOG_THROW_DESCR_PARAMS(zr_boost_1_81::log::system_error, "Failed to wait on a pthread condition variable", (err));
    }
...
typedef zr_boost_1_81::interprocess::interprocess_condition interprocess_condition_variable;
