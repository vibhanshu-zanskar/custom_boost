...
{
    while (m_state.exchange(0u, zr_boost_1_81::memory_order_acq_rel) == 0u)
    {
        m_state.wait(0u, zr_boost_1_81::memory_order_relaxed);
    }
}
...
{
    if (m_state.load(zr_boost_1_81::memory_order_relaxed) != 0u)
    {
        zr_boost_1_81::atomic_thread_fence(zr_boost_1_81::memory_order_release);
    }
    else if (m_state.exchange(1u, zr_boost_1_81::memory_order_release) == 0u)
    {
        m_state.notify_one();
    }
}
...
{
    zr_boost_1_81::atomic_thread_fence(zr_boost_1_81::memory_order_acq_rel);
    while (true)
    {
    ...
    m_state.clear(zr_boost_1_81::memory_order_relaxed);
}

//! Sets the object to a signalled state
BOOST_LOG_API void sem_based_event::set_signalled()
{
    if (!m_state.test_and_set(zr_boost_1_81::memory_order_release))
    {
...
BOOST_LOG_API void winapi_based_event::set_signalled()
{
    ...
    else if (m_state.exchange(1u, zr_boost_1_81::memory_order_release) == 0u)
    {
        ...
        else
        {
            ...
            {
                const DWORD err = GetLastError();
                m_state.store(0u, zr_boost_1_81::memory_order_relaxed);
                BOOST_LOG_THROW_DESCR_PARAMS(system_error, "Failed to wake the blocked thread", (err));
            }
        }
    }
...
{
    zr_boost_1_81::unique_lock< zr_boost_1_81::mutex > lock(m_mutex);
    while (!m_state)
    {
        m_cond.wait(lock);
    }
...
{
    zr_boost_1_81::lock_guard< zr_boost_1_81::mutex > lock(m_mutex);
    if (!m_state)
    {
        m_state = true;
        m_cond.notify_one();
