...
void test(unsigned int record_count, zr_boost_1_81::barrier& bar)
{
    BOOST_LOG_SCOPED_THREAD_TAG("ThreadID", zr_boost_1_81::this_thread::get_id());
...
//    typedef sinks::synchronous_sink< fake_backend > fake_sink;
    ...
    for (unsigned int i = 0; i < SINK_COUNT; ++i)
        logging::core::get()->add_sink(zr_boost_1_81::make_shared< fake_sink >());
...
//    logging::core::get()->set_filter(severity > error); // all records don't pass the filter
    ...
    zr_boost_1_81::barrier bar(THREAD_COUNT);
    zr_boost_1_81::thread_group threads;

    for (unsigned int i = 1; i < THREAD_COUNT; ++i)
        threads.create_thread(zr_boost_1_81::bind(&test, record_count, zr_boost_1_81::ref(bar)));

    zr_boost_1_81::posix_time::ptime start = zr_boost_1_81::date_time::microsec_clock< zr_boost_1_81::posix_time::ptime >::universal_time(), end;
    test(record_count, bar);
    if (THREAD_COUNT > 1)
        threads.join_all();
    end = zr_boost_1_81::date_time::microsec_clock< zr_boost_1_81::posix_time::ptime >::universal_time();
