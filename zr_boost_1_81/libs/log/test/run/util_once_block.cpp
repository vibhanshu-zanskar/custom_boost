...
zr_boost_1_81::mutex m;
typedef zr_boost_1_81::lock_guard< zr_boost_1_81::mutex > scoped_lock;
...
void once_block_flag_thread(zr_boost_1_81::barrier& barrier)
{
    int my_once_value = 0;
    barrier.wait();
...
{
    zr_boost_1_81::thread_group group;
    zr_boost_1_81::barrier barrier(static_cast< unsigned int >(THREAD_COUNT));

    try
    {
        for (unsigned int i = 0; i < THREAD_COUNT; ++i)
        {
            group.create_thread(zr_boost_1_81::bind(&once_block_flag_thread, zr_boost_1_81::ref(barrier)));
        }
        group.join_all();
    }
    catch (...)
...
void once_block_thread(zr_boost_1_81::barrier& barrier)
{
    int my_once_value = 0;
    barrier.wait();
    for (unsigned int i = 0; i < LOOP_COUNT; ++i)
...
BOOST_AUTO_TEST_CASE(once_block)
{
    ...
    try
    {
        for (unsigned int i = 0; i < THREAD_COUNT; ++i)
        {
            group.create_thread(zr_boost_1_81::bind(&once_block_thread, zr_boost_1_81::ref(barrier)));
        }
        group.join_all();
    }
    catch(...)
...
void once_block_with_exception_thread(zr_boost_1_81::barrier& barrier)
{
    barrier.wait();
    try
    {
...
BOOST_AUTO_TEST_CASE(once_block_retried_on_exception)
{
    ...
    try
    {
        for (unsigned int i = 0; i < THREAD_COUNT; ++i)
        {
            group.create_thread(zr_boost_1_81::bind(&once_block_with_exception_thread, zr_boost_1_81::ref(barrier)));
        }
        group.join_all();
    }
    catch(...)
