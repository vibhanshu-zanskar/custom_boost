...
typedef zr_boost_1_81::log::ipc::reliable_message_queue queue_t;
typedef queue_t::size_type size_type;

inline zr_boost_1_81::log::ipc::object_name generate_ipc_queue_name()
{
...
#if defined(BOOST_WINDOWS)
        << +zr_boost_1_81::winapi::GetCurrentProcessId();
#else
        << +getpid();
#endif
    return zr_boost_1_81::log::ipc::object_name(zr_boost_1_81::log::ipc::object_name::session, strm.str());
}

const zr_boost_1_81::log::ipc::object_name ipc_queue_name = generate_ipc_queue_name();
...
BOOST_AUTO_TEST_CASE(basic_functionality)
{
    ...
    try
    {
        queue_t queue(zr_boost_1_81::log::open_mode::open_only, ipc_queue_name);
    ...
    {
        queue_t queue(zr_boost_1_81::log::open_mode::create_only, ipc_queue_name, capacity, block_size);
    ...
    try
    {
        queue_t queue_a(zr_boost_1_81::log::open_mode::create_only, ipc_queue_name, capacity, block_size);
        queue_t queue_b(zr_boost_1_81::log::open_mode::create_only, ipc_queue_name, capacity, block_size);
    ...
    // Opening an existing queue
    {
        ...
        queue_t queue_b(zr_boost_1_81::log::open_mode::open_or_create, ipc_queue_name, capacity * 2u, block_size * 2u); // queue geometry differs from the existing queue
        ...
        queue_t queue_c(zr_boost_1_81::log::open_mode::open_only, ipc_queue_name);
    ...
    {
        queue_t queue_a(zr_boost_1_81::log::open_mode::create_only, ipc_queue_name, capacity, block_size);
        queue_t queue_b(zr_boost_1_81::move(queue_a));
        BOOST_CHECK(!queue_a.is_open());
        BOOST_CHECK(equal_strings(queue_b.name().c_str(), ipc_queue_name.c_str()));
        BOOST_CHECK(queue_b.is_open());
        BOOST_CHECK_EQUAL(queue_b.capacity(), capacity);
    ...
    // Move assignment operator.
    {
        queue_t queue_a(zr_boost_1_81::log::open_mode::create_only, ipc_queue_name, capacity, block_size);
        queue_t queue_b;
        queue_b = zr_boost_1_81::move(queue_a);
        BOOST_CHECK(!queue_a.is_open());
        BOOST_CHECK(equal_strings(queue_b.name().c_str(), ipc_queue_name.c_str()));
        BOOST_CHECK(queue_b.is_open());
        BOOST_CHECK_EQUAL(queue_b.capacity(), capacity);
...
BOOST_AUTO_TEST_CASE(message_passing)
{
    ...
    {
        queue_t queue_a(zr_boost_1_81::log::open_mode::create_only, ipc_queue_name, 1u, block_size);
        queue_t queue_b(zr_boost_1_81::log::open_mode::open_only, ipc_queue_name);
    ...
    {
        queue_t queue_a(zr_boost_1_81::log::open_mode::create_only, ipc_queue_name, 1u, block_size, queue_t::fail_on_overflow);
        BOOST_TEST_PASSPOINT();
        BOOST_CHECK(queue_a.send(message1, sizeof(message1) - 1u) == queue_t::succeeded);
        BOOST_TEST_PASSPOINT();
    ...
    {
        queue_t queue_a(zr_boost_1_81::log::open_mode::create_only, ipc_queue_name, 1u, block_size, queue_t::throw_on_overflow);
        BOOST_TEST_PASSPOINT();
        BOOST_CHECK(queue_a.send(message1, sizeof(message1) - 1u) == queue_t::succeeded);
        BOOST_TEST_PASSPOINT();
        try
        ...
        catch (zr_boost_1_81::log::capacity_limit_reached&)
        {
            BOOST_TEST_PASSPOINT();
        }
    }
    ...
    {
        queue_t queue_a(zr_boost_1_81::log::open_mode::create_only, ipc_queue_name, 5u, block_size);
        queue_t queue_b(zr_boost_1_81::log::open_mode::open_only, ipc_queue_name);
...
void multithreaded_message_passing_feeding_thread(const char* message, unsigned int& failure_count)
{
    ...
    queue_t queue(zr_boost_1_81::log::open_mode::open_or_create, ipc_queue_name, capacity, block_size);
    for (unsigned int i = 0; i < message_count; ++i)
    {
        failure_count += queue.send(message, len) != queue_t::succeeded;
    }

    zr_boost_1_81::atomic_thread_fence(zr_boost_1_81::memory_order_release);
}
...
BOOST_AUTO_TEST_CASE(multithreaded_message_passing)
{
    ...
    zr_boost_1_81::thread thread1(&multithreaded_message_passing_feeding_thread, "Thread 1", zr_boost_1_81::ref(failure_count1));
    zr_boost_1_81::thread thread2(&multithreaded_message_passing_feeding_thread, "Thread 2", zr_boost_1_81::ref(failure_count2));
    zr_boost_1_81::thread thread3(&multithreaded_message_passing_feeding_thread, "Thread 3", zr_boost_1_81::ref(failure_count3));
    ...
    zr_boost_1_81::atomic_thread_fence(zr_boost_1_81::memory_order_acquire);
...
{
    queue_t feeder_queue(zr_boost_1_81::log::open_mode::open_or_create, ipc_queue_name, 1u, block_size);
    queue_t::operation_result feeder_results[3];
    queue_t reader_queue(zr_boost_1_81::log::open_mode::open_only, ipc_queue_name);
    ...
    zr_boost_1_81::thread feeder_thread(&stop_reset_feeding_thread, zr_boost_1_81::ref(feeder_queue), feeder_results, 3);
    zr_boost_1_81::thread reader_thread(&stop_reset_reading_thread, zr_boost_1_81::ref(reader_queue), reader_results, 1);

    BOOST_TEST_PASSPOINT();

    reader_thread.join();
    BOOST_TEST_PASSPOINT();
    zr_boost_1_81::this_thread::sleep_for(zr_boost_1_81::chrono::milliseconds(500));
    ...
    zr_boost_1_81::thread(&stop_reset_feeding_thread, zr_boost_1_81::ref(feeder_queue), feeder_results, 1).swap(feeder_thread);
    zr_boost_1_81::thread(&stop_reset_reading_thread, zr_boost_1_81::ref(reader_queue), reader_results, 2).swap(reader_thread);
