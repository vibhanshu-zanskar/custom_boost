...
{
    zr_boost_1_81::log::attribute_set set, dummy;
    ...
    while (i < 100)
    {
        ...
        strm << "Attr" << i;
        zr_boost_1_81::log::attribute_name name = strm.str();

        std::pair< zr_boost_1_81::log::attribute_set::iterator, bool > res = set.insert(name, zr_boost_1_81::log::attributes::make_constant(5));
        ++i;

        BOOST_CHECK(res.second); // check that insertion succeeded
        // check that lookup works
        zr_boost_1_81::log::attribute_set::iterator find_result = set.find(name);
        BOOST_CHECK(find_result != set.end());
        BOOST_CHECK(find_result == res.first);
    }

    zr_boost_1_81::log::attribute_value_set vset(set, dummy, dummy);
    BOOST_CHECK_EQUAL(vset.size(), set.size());

    // Check that all inserted elements are findable
    unsigned int j = 0;
    for (zr_boost_1_81::log::attribute_value_set::const_iterator it = vset.begin(), end = vset.end(); it != end; ++it, ++j)
    {
        zr_boost_1_81::log::attribute_name key = it->first;
