...
BOOST_AUTO_TEST_CASE(filtering)
{
    ...
    zr_boost_1_81::shared_ptr< core > pCore = core::get();
    zr_boost_1_81::shared_ptr< test_sink > pSink(new test_sink());
    ...
    // No filtering at all
    {
        ...
        BOOST_REQUIRE(rec);
        pCore->push_record(zr_boost_1_81::move(rec));
    ...
    // Only one sink of the two accepts the record
    {
        ...
        zr_boost_1_81::shared_ptr< test_sink > pSink2(new test_sink());
...
#ifndef BOOST_LOG_NO_THREADS
    {
        zr_boost_1_81::thread th(&thread_attributes_test);
        th.join();
        BOOST_CHECK_EQUAL(pSink->m_RecordCounter, 1UL);
        BOOST_CHECK_EQUAL(pSink->m_Consumed[data::attr1()], 0UL);
        BOOST_CHECK_EQUAL(pSink->m_Consumed[data::attr2()], 1UL);
