...
//  with a zr_boost_1_81::system using directive increases use scenario coverage.
...
namespace
{
  ...
  int divide(int dividend, int divisor, zr_boost_1_81::system::error_code& ec = zr_boost_1_81::throws())
  {
    if (divisor == 0)                            // is there an error?
    {
      if (&ec == &zr_boost_1_81::throws())               // throw on error
        throw "oops!";                           // whatever exception you prefer
      ec = error_code(EDOM, generic_category()); // report error via error_code
      return 0;
    }
    
    if (&ec != &zr_boost_1_81::throws())                 // error reporting via error_code
      ec.clear();
  ...
  void test_throws_usage()
  {
    ...
    //error_code should_fail(zr_boost_1_81::throws());     // should fail at runtime
    //zr_boost_1_81::throws() = ec;                        // should fail at runtime
  }
}
