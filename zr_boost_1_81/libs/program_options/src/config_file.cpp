...
namespace zr_boost_1_81 { namespace program_options { namespace detail {
    ...
    common_config_file_iterator::add_option(const char* name)
    {
        ...
        if (*s.rbegin() == '*') {
            ...
            if (bad_prefixes)
                zr_boost_1_81::throw_exception(error("options '" + string(name) + "' and '" +
            ...
            if (!s.empty()) {
                ...
                if (*s.begin() == '[' && *s.rbegin() == ']') {
                    ...
                    if (!registered && !m_allow_unregistered)
                        zr_boost_1_81::throw_exception(unknown_option(name));
                    ...
                    zr_boost_1_81::throw_exception(invalid_config_file_syntax(s, invalid_syntax::unrecognized_line));
                }
            }
        }
        if (!found)
...
#if 0
using zr_boost_1_81::program_options::config_file;
