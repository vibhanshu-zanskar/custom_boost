...
// Extended CCYY-MM-DD
  ...
  //bad lexical cast?
  try {
    zr_boost_1_81::gregorian::date d(zr_boost_1_81::gregorian::from_string(s));
...
/* not currently passing due to a bug in zr_boost_1_81::offset_separator (reported 2005-Aug-02)
  ...
  */      
    ...
    std::wstring w1_string(zr_boost_1_81::lexical_cast<std::wstring>("2000-Oct-31 is Halloween 2k!"));
    ...
    try {
      ...
      std::cout << "Shouldn't be reached." << 
        zr_boost_1_81::gregorian::to_simple_string(bd) << std::endl;
    }
    catch(zr_boost_1_81::gregorian::bad_month&){
  ...
  try {
    ...
    zr_boost_1_81::gregorian::date bad_day(zr_boost_1_81::gregorian::from_string(s2)); //won't construct
    ...
    std::cout << "Oh oh, this shouldn't be reached: "
              << zr_boost_1_81::gregorian::to_iso_string(bad_day) << std::endl;

  }
  catch(zr_boost_1_81::gregorian::bad_day_of_month&) { //expected
  ...
  try {
    ...
    zr_boost_1_81::gregorian::date bad_month(zr_boost_1_81::date_time::parse_date<zr_boost_1_81::gregorian::date>(s2));
    check("check bad month", false); //fail the test
    //The line below won't execute, but make the compiler think
    //we are using bad day....
    std::cout << "Oh oh, this shouldn't be reached: "
              << zr_boost_1_81::gregorian::to_iso_string(bad_month) << std::endl;

  }
  catch(zr_boost_1_81::gregorian::bad_month&) { //expected
  ...
  try {
    ...
    date d1(zr_boost_1_81::gregorian::from_undelimited_string(ud));
    //  std::cout << to_string(d1) << std::endl;
    check("undelimited date string", d1 == date(2001,Oct,9));
    
  
    std::string ad("2001/10/09");
    date d2(zr_boost_1_81::date_time::parse_date<date>(ad));
