...
//     check("default construction -- 0 min", td.minutes() == 0); 
  ...
  if (time_duration::resolution() >= zr_boost_1_81::date_time::micro) {
  ...
  if (time_duration::resolution() >= zr_boost_1_81::date_time::tenth) {
  ...
  if (time_duration::resolution() >= zr_boost_1_81::date_time::milli) {
    millisec ms(9);
  ...
  {
    ptime start(zr_boost_1_81::gregorian::date(2000, 1, 1));
    ptime end(zr_boost_1_81::gregorian::date(2000, 5, 1));
  ...
  zr_boost_1_81::int64_t tms = static_cast<zr_boost_1_81::int64_t>(3600)*1000000*1001; //ms per sec
  check("total microseconds 1000 hours", hours(1001).total_microseconds() == tms);
  tms = static_cast<zr_boost_1_81::int64_t>(3600)*365*24*1000;
  check("total milliseconds - one year", (t2-t1).total_milliseconds() == tms);
  tms = 3600*365*24*static_cast<zr_boost_1_81::int64_t>(1000000000);
