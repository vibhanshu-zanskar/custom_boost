...
void check_filesize(const std::string& filename, std::ifstream::pos_type expectedSize)
{
    ...
    check_equal("check file size is " + zr_boost_1_81::lexical_cast<std::string>(expectedSize), in.tellg(), expectedSize);
}
...
std::string get_fname(int version)
{
    return "time_duration_serialization." +
        ...
        ".v" + zr_boost_1_81::lexical_cast<std::string>(version);
}
...
int main() {
    ...
    zr_boost_1_81::archive::binary_oarchive oa(ofs);
    oa << td;
    ofs.close();
    ...
    zr_boost_1_81::archive::binary_iarchive ia(ifs);
    time_duration tmp;
    ia >> tmp;
    ifs.close();
