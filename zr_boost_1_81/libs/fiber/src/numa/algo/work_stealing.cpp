...
std::vector< std::uint32_t > get_local_cpus( std::uint32_t node_id, std::vector< zr_boost_1_81::fibers::numa::node > const& topo) {
    for ( auto & node : topo) {
        if ( node_id == node.id) {
...
std::vector< std::uint32_t > get_remote_cpus( std::uint32_t node_id, std::vector< zr_boost_1_81::fibers::numa::node > const& topo) {
...
void
work_stealing::init_( std::vector< zr_boost_1_81::fibers::numa::node > const& topo,
...
work_stealing::work_stealing(
    ...
    std::vector< zr_boost_1_81::fibers::numa::node > const& topo,
    bool suspend) :
        cpu_id_{ cpu_id },
    ...
    zr_boost_1_81::fibers::numa::pin_thread( cpu_id_);
    // calculate thread count
    std::size_t thread_count = 0;
    for ( auto & node : topo) {
        thread_count += node.logical_cpus.size();
    }
    static zr_boost_1_81::fibers::detail::thread_barrier b{ thread_count };
...
work_stealing::pick_next() noexcept {
    ...
    if ( nullptr != victim) {
        zr_boost_1_81::context::detail::prefetch_range( victim, sizeof( context) );
