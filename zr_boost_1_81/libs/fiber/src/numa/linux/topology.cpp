...
namespace al = zr_boost_1_81::algorithm;
...
std::vector< node > topology() {
    ...
    for ( std::uint32_t cpu_id : cpus) {
        fs::path cpu_path{
            zr_boost_1_81::str(
                zr_boost_1_81::format("/sys/devices/system/cpu/cpu%d/") % cpu_id) };
    ...
    for ( auto entry : map) {
        ...
        fs::path distance_path{
            zr_boost_1_81::str(
                zr_boost_1_81::format("/sys/devices/system/node/node%d/distance") % entry.second.id) };
