...
private:
    zr_boost_1_81::context::fiber
    run_( zr_boost_1_81::context::fiber && c) {
...
public:
    dispatcher_context( zr_boost_1_81::context::preallocated const& palloc, default_stack && salloc) :
        context{ 0, type::dispatcher_context, launch::post } {
        c_ = zr_boost_1_81::context::fiber{ std::allocator_arg, palloc, salloc,
...
static intrusive_ptr< context > make_dispatcher_context() {
    ...
    return intrusive_ptr< context >{
        new ( storage) dispatcher_context{
                zr_boost_1_81::context::preallocated{ storage, size, sctx }, std::move( salloc) } };
}
...
context::resume() noexcept {
    ...
    std::move( c_).resume_with([prev](zr_boost_1_81::context::fiber && c){
                prev->c_ = std::move( c);
                return zr_boost_1_81::context::fiber{};
            });
}
...
context::resume( detail::spinlock_lock & lk) noexcept {
    ...
    std::swap( context_initializer::active_, prev);
    // pass pointer to the context that resumes `this`
    std::move( c_).resume_with([prev,&lk](zr_boost_1_81::context::fiber && c){
                prev->c_ = std::move( c);
                lk.unlock();
                return zr_boost_1_81::context::fiber{};
            });
...
context::resume( context * ready_ctx) noexcept {
    ...
    std::swap( context_initializer::active_, prev);
    // pass pointer to the context that resumes `this`
    std::move( c_).resume_with([prev,ready_ctx](zr_boost_1_81::context::fiber && c){
                prev->c_ = std::move( c);
...
context::suspend_with_cc() noexcept {
    ...
    std::swap( context_initializer::active_, prev);
    // pass pointer to the context that resumes `this`
    return std::move( c_).resume_with([prev](zr_boost_1_81::context::fiber && c){
                prev->c_ = std::move( c);
                return zr_boost_1_81::context::fiber{};
            });
}
