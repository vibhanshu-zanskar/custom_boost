...
template<typename T>
bool check_sortedness(const zr_boost_1_81::numeric::ublas::coordinate_vector<T>& vector) {
  bool result = true;
  typedef zr_boost_1_81::numeric::ublas::coordinate_vector<T> vector_type;
...
BOOST_UBLAS_TEST_DEF( test_coordinate_vector_inplace_merge_random )
{
  ...
  for (size_t repeats = 1; repeats < max_repeats; ++repeats ) {
    for (size_t size = 1; size < max_size; size += 5) {
      ...
      zr_boost_1_81::numeric::ublas::coordinate_vector<double> vector_coord(size_vec);
      zr_boost_1_81::numeric::ublas::vector<double> vector_dense(size_vec, 0);
