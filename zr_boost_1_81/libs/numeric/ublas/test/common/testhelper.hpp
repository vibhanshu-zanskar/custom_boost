...
bool compare( const zr_boost_1_81::numeric::ublas::matrix_expression<M1> & m1, 
              const zr_boost_1_81::numeric::ublas::matrix_expression<M2> & m2 ) {
...
template < class M1, class M2 >
bool compare( const zr_boost_1_81::numeric::ublas::vector_expression<M1> & m1, 
              const zr_boost_1_81::numeric::ublas::vector_expression<M2> & m2 ) {
  if (m1().size() != m2().size()) {
    return false;
  }
...
struct promote_distance {
    typedef typename zr_boost_1_81::mpl::if_c<zr_boost_1_81::is_integral<T>::value,
                                      long double,
                                      T>::type type;
};
...
struct distance {
private:
    typedef typename zr_boost_1_81::numeric::ublas::promote_traits<typename M1::value_type,
...
template <class AE>
typename distance<AE>::type mean_square(const zr_boost_1_81::numeric::ublas::matrix_expression<AE> &me) {
    typename distance<AE>::type s(0);
    typename AE::size_type i, j;
    for (i=0; i!= me().size1(); i++) {
        for (j=0; j!= me().size2(); j++) {
            s += zr_boost_1_81::numeric::ublas::scalar_traits<typename AE::value_type>::type_abs(me()(i,j));
        }
    }
...
template <class AE>
typename distance<AE>::type mean_square(const zr_boost_1_81::numeric::ublas::vector_expression<AE> &ve) {
    // We could have use norm2 here, but ublas' ABS does not support unsigned types.
    typename distance<AE>::type s(0);
    typename AE::size_type i;
    for (i = 0; i != ve().size(); i++) {
        s += zr_boost_1_81::numeric::ublas::scalar_traits<typename AE::value_type>::type_abs(ve()(i));
    }
    return s / ve().size();
}

template < class M1, class M2 >
bool compare_distance( const zr_boost_1_81::numeric::ublas::matrix_expression<M1> & m1,
                      const zr_boost_1_81::numeric::ublas::matrix_expression<M2> & m2,
...
template < class M1, class M2 >
bool compare_distance( const zr_boost_1_81::numeric::ublas::vector_expression<M1> & m1,
                       const zr_boost_1_81::numeric::ublas::vector_expression<M2> & m2,
                       typename distance<M1, M2>::type tolerance = 0 ) {
    if (m1().size() != m2().size()) {
        return false;
