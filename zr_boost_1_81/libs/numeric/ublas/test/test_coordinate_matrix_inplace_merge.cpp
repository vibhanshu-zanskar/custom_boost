...
template<typename T>
bool check_sortedness(const zr_boost_1_81::numeric::ublas::coordinate_matrix<T>& matrix) {
  bool result = true;
  typedef zr_boost_1_81::numeric::ublas::coordinate_matrix<T> matrix_type;
...
BOOST_UBLAS_TEST_DEF( test_coordinate_matrix_inplace_merge_random )
{
  ...
  for (size_t repeats = 1; repeats < max_repeats; ++repeats ) {
    for (size_t size = 1; size < max_size; size += 5) {
      ...
      zr_boost_1_81::numeric::ublas::coordinate_matrix<double> matrix_coord(size_x, size_y);
      zr_boost_1_81::numeric::ublas::matrix<double> matrix_dense(size_x, size_y, 0);
