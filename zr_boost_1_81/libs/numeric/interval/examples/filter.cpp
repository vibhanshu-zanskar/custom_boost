...
int det_sign_algo1(const matrix<double> &a) {
  ...
  typedef zr_boost_1_81::numeric::interval_lib::unprotect<I_dbl>::type I;
  matrix<I> u(dim);
  for(int i = 0; i < dim; i++) {
  ...
  for(int i = 0; i < dim; i++) {
    ...
    // U[i,?]
    {
      ...
      if (zr_boost_1_81::numeric::interval_lib::cerlt(pivot, 0.)) sig = -sig;
  ...
  // norm of PAUiLi-I computed with intervals
  {
    ...
    for(int i = 0; i < dim; i++) {
      ...
      for(int j = 0; j < dim; j++) {
        ...
        for(int k = j; k < dim; k++) {
          using zr_boost_1_81::numeric::interval_lib::mul;
