...
std::basic_ostream<CharType, CharTraits> &operator<<
  (std::basic_ostream<CharType, CharTraits> &stream,
   const zr_boost_1_81::numeric::interval<T, Policies> &value)
{
  if (empty(value)) {
    return stream << "[]";
  } else {
   ...
   const zr_boost_1_81::numeric::interval<T, Policies> &value)
{
  if (empty(value)) {
    ...
    zr_boost_1_81::io::ios_precision_saver state(stream, std::numeric_limits<T>::digits10);
    return stream << lower(value);
    ...
    zr_boost_1_81::io::ios_precision_saver state(stream, range);
   ...
   const zr_boost_1_81::numeric::interval<T, Policies> &value)
{
  if (empty(value)) {
    ...
    zr_boost_1_81::numeric::interval<T, Policies> r = widen(value, eps2);
...
std::basic_istream<CharType, CharTraits> &operator>>
  (std::basic_istream<CharType, CharTraits> &stream,
   zr_boost_1_81::numeric::interval<T, Policies> &value)
{
  T l, u;
  char c = 0;
  stream >> c;
  ...
  if (stream)
    ...
    value = zr_boost_1_81::numeric::interval<T, Policies>::empty();
  return stream;
}
