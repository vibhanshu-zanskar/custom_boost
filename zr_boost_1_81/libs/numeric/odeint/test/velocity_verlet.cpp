...
typedef zr_boost_1_81::array< double , 2 > array_type;
...
BOOST_FIXTURE_TEST_CASE( test_with_array_ref , velocity_verlet_fixture )
{
    ...
    array_type q , p ;
    init_state( q , p );
    stepper.do_step( ode() , std::make_pair( zr_boost_1_81::ref( q ) , zr_boost_1_81::ref( p ) ) , 0.0 , 0.01 );
...
BOOST_FIXTURE_TEST_CASE( test_with_unit_ref , velocity_verlet_fixture )
{
    ...
    stepper_type stepper;
    stepper.do_step( ode_units() , std::make_pair( zr_boost_1_81::ref( q ) , zr_boost_1_81::ref( p ) ) , 0.0 * si::second , 0.01 * si::second );
    BOOST_CHECK_EQUAL( ode_call_count , size_t( 2 ) );
}
