...
BOOST_AUTO_TEST_CASE_TEMPLATE( stepper_iterator_factory , Stepper , dummy_steppers )
{
    ...
    std::for_each(
        make_times_iterator_begin( stepper , zr_boost_1_81::ref( system ) , x , times.begin(), times.end() , 0.1 ) ,
        make_times_iterator_end<time_iterator_type>( stepper , zr_boost_1_81::ref( system ) , x ) ,
        dummy_observer() );
...
BOOST_AUTO_TEST_CASE_TEMPLATE( stepper_range , Stepper , dummy_steppers )
{
    ...
    zr_boost_1_81::for_each( make_times_range( stepper , zr_boost_1_81::ref( system ) , x , times.begin() , times.end() , 0.1 ) ,
                     dummy_observer() );
...
BOOST_AUTO_TEST_CASE_TEMPLATE( stepper_iterator_with_reference_wrapper_factory , Stepper , dummy_steppers )
{
    ...
    std::for_each(
        make_times_iterator_begin( zr_boost_1_81::ref( stepper ) , zr_boost_1_81::ref( system ) , x , times.begin() , times.end() , 0.1 ) ,
        make_times_iterator_end<time_iterator_type>( zr_boost_1_81::ref( stepper ) , zr_boost_1_81::ref( system ) , x ) ,
        dummy_observer() );
...
BOOST_AUTO_TEST_CASE_TEMPLATE( stepper_range_with_reference_wrapper , Stepper , dummy_steppers )
{
    ...
    zr_boost_1_81::for_each( make_times_range( zr_boost_1_81::ref( stepper ) , zr_boost_1_81::ref( system ) , x , times.begin() , times.end(), 0.1 ) ,
                     dummy_observer() );
...
BOOST_AUTO_TEST_CASE_TEMPLATE( copy_algorithm_with_range_factory , Stepper , dummy_steppers )
{
    state_type x = {{ 1.0 }};
    std::vector< state_type > res;
    zr_boost_1_81::range::copy( make_times_range( Stepper() , empty_system() , x , times.begin() , times.end() , 0.1 ) ,
                        std::back_insert_iterator< std::vector< state_type > >( res ) );
