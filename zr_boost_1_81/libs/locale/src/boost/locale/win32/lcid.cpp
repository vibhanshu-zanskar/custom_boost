...
namespace zr_boost_1_81 { namespace locale { namespace impl_win {

    typedef std::map<std::string, unsigned> table_type;

    static table_type* volatile table = 0;

    zr_boost_1_81::mutex& lcid_table_mutex()
    {
    ...
    const table_type& get_ready_lcid_table()
    {
        if(table)
            ...
            zr_boost_1_81::unique_lock<zr_boost_1_81::mutex> lock(lcid_table_mutex());
            if(table)
                return *table;
    ...
    unsigned locale_to_lcid(const std::string& locale_name)
    {
        ...
        zr_boost_1_81::locale::util::locale_data d;
        d.parse(locale_name);
...
}}} // namespace zr_boost_1_81::locale::impl_win
