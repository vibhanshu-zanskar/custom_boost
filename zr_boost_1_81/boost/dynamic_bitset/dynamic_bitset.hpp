...
public:
    ...
    :m_bits(alloc),
     m_num_bits(0)
    {
        using zr_boost_1_81::detail::dynamic_bitset_impl::value_to_type;
        using zr_boost_1_81::detail::dynamic_bitset_impl::is_numeric;
...
private:
                                 ...
                                 const Allocator& alloc*/)
    {
        ...
        typedef zr_boost_1_81::detail::dynamic_bitset_impl
...
dynamic_bitset(dynamic_bitset<Block, Allocator>&& b)
  : m_bits(zr_boost_1_81::move(b.m_bits)), m_num_bits(zr_boost_1_81::move(b.m_num_bits))
{
...
{
    if (zr_boost_1_81::addressof(b) == this) { return *this; }

    m_bits = zr_boost_1_81::move(b.m_bits);
    m_num_bits = zr_boost_1_81::move(b.m_num_bits);
    // Required so that assert(m_check_invariants()); works.
    assert((b.m_bits = buffer_type()).empty());
    b.m_num_bits = 0;
    return *this;
...
{
    if (pos >= m_num_bits)
        BOOST_THROW_EXCEPTION(std::out_of_range("zr_boost_1_81::dynamic_bitset::at out_of_range"));
...
to_ulong() const
{
  ...
  if (find_next(ulong_width - 1) != npos)
    BOOST_THROW_EXCEPTION(std::overflow_error("zr_boost_1_81::dynamic_bitset::to_ulong overflow"));
...
inline std::size_t hash_value(const dynamic_bitset<Block, Allocator>& a)
{
    ...
    zr_boost_1_81::hash_combine(res, a.m_bits);
    return res;
}
...
{
    template<typename Block, typename Allocator>
    struct hash< zr_boost_1_81::dynamic_bitset<Block, Allocator> >
    {
        typedef zr_boost_1_81::dynamic_bitset<Block, Allocator> argument_type;
        typedef std::size_t result_type;
        result_type operator()(const argument_type& a) const BOOST_NOEXCEPT
        {
            zr_boost_1_81::hash<argument_type> hasher;
            return hasher(a);
        }
    };
}
