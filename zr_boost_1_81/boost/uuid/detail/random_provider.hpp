...
public:
    ...
    random_provider(BOOST_RV_REF(random_provider) that) BOOST_NOEXCEPT :
        detail::random_provider_base(zr_boost_1_81::move(static_cast< detail::random_provider_base& >(that)))
    {
    }

    random_provider& operator= (BOOST_RV_REF(random_provider) that) BOOST_NOEXCEPT
    {
        static_cast< detail::random_provider_base& >(*this) = zr_boost_1_81::move(static_cast< detail::random_provider_base& >(that));
        return *this;
    }
    ...
    void generate(Iter first, Iter last)
    {
        ...
        for (; first != last; ++first)
        {
            ...
            *first &= (std::numeric_limits<zr_boost_1_81::uint32_t>::max)();
        }
    }
