...
namespace zr_boost_1_81 { namespace range_detail_microsoft {
    ...
    public:
        ...
        explicit mfc_ptr_array_iterator(ArrayT& arr, INT_PTR index) :
            super_t(index), m_parr(zr_boost_1_81::addressof(arr))
        { }
    ...
    public:
        ...
        explicit mfc_map_iterator(MapT const& map, POSITION pos) :
            m_pmap(zr_boost_1_81::addressof(map)), m_posNext(pos)
        {
            increment();
        }
    ...
    public:
        ...
        explicit mfc_cpair_map_iterator(MapT& map, PairT *pp) :
            m_pmap(zr_boost_1_81::addressof(map)), m_pp(pp)
        { }
    ...
    struct customization< ::CTypedPtrArray<BaseClass, PtrType> >
    {
        ...
        Iterator begin(X& x)
        {
            return Iterator(zr_boost_1_81::begin<BaseClass>(x), fun<X>());
        }

        template< class Iterator, class X >
        Iterator end(X& x)
        {
            return Iterator(zr_boost_1_81::end<BaseClass>(x), fun<X>());
        }
    };
...
BOOST_RANGE_DETAIL_MICROSOFT_CUSTOMIZATION_TYPE(
    zr_boost_1_81::range_detail_microsoft::using_type_as_tag,
