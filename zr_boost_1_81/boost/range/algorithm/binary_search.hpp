...
inline bool binary_search(const ForwardRange& rng, const Value& val)
{
    ...
    return std::binary_search(zr_boost_1_81::begin(rng), zr_boost_1_81::end(rng), val);
}
...
{
    BOOST_RANGE_CONCEPT_ASSERT(( ForwardRangeConcept<const ForwardRange> ));
    return std::binary_search(zr_boost_1_81::begin(rng), zr_boost_1_81::end(rng), val, pred);
}
