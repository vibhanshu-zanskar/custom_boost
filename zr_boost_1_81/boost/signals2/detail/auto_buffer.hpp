...
namespace detail
{
    ...
    public:
        ...
        typedef typename zr_boost_1_81::allocator_size_type<Allocator>::type size_type;
        typedef typename zr_boost_1_81::allocator_difference_type<Allocator>::type difference_type;
        typedef T*                                       pointer;
        typedef typename zr_boost_1_81::allocator_pointer<Allocator>::type allocator_pointer;
        ...
        typedef typename zr_boost_1_81::mpl::if_c< zr_boost_1_81::has_trivial_assign<T>::value
    ...
    private:
        ...
        {
            copy_rai( begin, end, where, zr_boost_1_81::has_trivial_assign<T>() );
        }

        static void copy_rai( const T* begin, const T* end,
                              pointer where, const zr_boost_1_81::true_type& )
        {
        ...
        static void copy_rai( I begin, I end,
                              pointer where, const zr_boost_1_81::integral_constant<bool, b>& )
        {
        ...
        {
            assign_impl( begin, end, where, zr_boost_1_81::has_trivial_assign<T>() );
        }

        template< class I, class I2 >
        static void assign_impl( I begin, I end, I2 where, const zr_boost_1_81::true_type& )
        {
            std::memcpy( where, begin, sizeof(T) * std::distance(begin,end) );
        }

        template< class I, class I2 >
        static void assign_impl( I begin, I end, I2 where, const zr_boost_1_81::false_type& )
        {
            for( ; begin != end; ++begin, ++where )
                *where = *begin;
        }

        void unchecked_push_back_n( size_type n, const zr_boost_1_81::true_type& )
        {
            std::uninitialized_fill( end(), end() + n, T() );
            size_ += n;
        }

        void unchecked_push_back_n( size_type n, const zr_boost_1_81::false_type& )
        {
            for( size_type i = 0u; i < n; ++i )
                unchecked_push_back();
        }

        void auto_buffer_destroy( pointer where, const zr_boost_1_81::false_type& )
        {
            (*where).~T();
        }

        void auto_buffer_destroy( pointer, const zr_boost_1_81::true_type& )
        { }

        void auto_buffer_destroy( pointer where )
        {
            auto_buffer_destroy( where, zr_boost_1_81::has_trivial_destructor<T>() );
        }
        ...
        void auto_buffer_destroy()
        {
            ...
            if( buffer_ ) // do we need this check? Yes, but only
                ...
                auto_buffer_destroy( zr_boost_1_81::has_trivial_destructor<T>() );
        }

        void destroy_back_n( size_type n, const zr_boost_1_81::false_type& )
        {
            BOOST_ASSERT( n > 0 );
        ...
        void destroy_back_n( size_type, const zr_boost_1_81::true_type& )
        { }

        void destroy_back_n( size_type n )
        {
            destroy_back_n( n, zr_boost_1_81::has_trivial_destructor<T>() );
        }

        void auto_buffer_destroy( const zr_boost_1_81::false_type& x )
        {
            if( size_ )
        ...
        void auto_buffer_destroy( const zr_boost_1_81::true_type& )
        {
            deallocate( buffer_, members_.capacity_ );
        }

        pointer move_to_new_buffer( size_type new_capacity, const zr_boost_1_81::false_type& )
        {
        ...
        pointer move_to_new_buffer( size_type new_capacity, const zr_boost_1_81::true_type& )
        {
            pointer new_buffer = allocate( new_capacity ); // strong
        ...
        void reserve_impl( size_type new_capacity )
        {
            pointer new_buffer = move_to_new_buffer( new_capacity,
                                                 zr_boost_1_81::has_nothrow_copy<T>() );
            auto_buffer_destroy();
            buffer_   = new_buffer;
                                 ...
                                 const zr_boost_1_81::true_type& )
        {
            ...
            zr_boost_1_81::swap( l.size_, r.size_ );
            zr_boost_1_81::swap( l.members_.capacity_, r.members_.capacity_ );
        }

        static void swap_helper( auto_buffer& l, auto_buffer& r,
                                 const zr_boost_1_81::false_type& )
        {
            BOOST_ASSERT( l.is_on_stack() && r.is_on_stack() );
            ...
            size_type i = 0u;
            for(  ; i < min_size; ++i )
                zr_boost_1_81::swap( (*smallest)[i], (*largest)[i] );
        ...
        void grow_back( size_type n, const zr_boost_1_81::true_type& )
        {
            BOOST_ASSERT( size_ + n <= members_.capacity_ );
            size_ += n;
        }

        void grow_back( size_type n, const zr_boost_1_81::false_type& )
        {
        ...
        {
            grow_back( n, zr_boost_1_81::has_trivial_constructor<T>() );
        }

        void grow_back_one( const zr_boost_1_81::true_type& )
        {
            BOOST_ASSERT( size_ + 1 <= members_.capacity_ );
            size_ += 1;
        }

        void grow_back_one( const zr_boost_1_81::false_type& )
        {
            unchecked_push_back();
        }
        ...
        void grow_back_one()
        {
            grow_back_one( zr_boost_1_81::has_trivial_constructor<T>() );
        }
    ...
    public:
        ...
        {
            BOOST_ASSERT( size_ + n <= members_.capacity_ );
            unchecked_push_back_n( n, zr_boost_1_81::has_trivial_assign<T>() );
        }
        ...
        {
            BOOST_ASSERT( !empty() );
            auto_buffer_destroy( buffer_ + size_ - 1, zr_boost_1_81::has_trivial_destructor<T>() );
            --size_;
        }
        ...
        void swap( auto_buffer& r )
        {
            ...
            if( both_on_heap )
            {
                zr_boost_1_81::swap( get_allocator(), r.get_allocator() );
                zr_boost_1_81::swap( members_.capacity_, r.members_.capacity_ );
                zr_boost_1_81::swap( buffer_, r.buffer_ );
                zr_boost_1_81::swap( size_, r.size_ );
                BOOST_ASSERT( is_valid() );
            ...
            if( exactly_one_on_stack )
            {
                ...
                zr_boost_1_81::swap( get_allocator(), r.get_allocator() );  // assume nothrow
                zr_boost_1_81::swap( members_.capacity_, r.members_.capacity_ );
                zr_boost_1_81::swap( size_, r.size_ );
            ...
            BOOST_ASSERT( on_stack && r_on_stack );
            swap_helper( *this, r, zr_boost_1_81::has_trivial_assign<T>() );
            BOOST_ASSERT( is_valid() );
            BOOST_ASSERT( r.is_valid() );
        }

    private:
        typedef zr_boost_1_81::aligned_storage< N * sizeof(T),
                                        zr_boost_1_81::alignment_of<T>::value >
                               storage;
