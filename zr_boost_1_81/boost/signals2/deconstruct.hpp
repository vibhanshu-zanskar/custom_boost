...
public:
    ...
    const shared_ptr<T>& postconstruct() const
    {
        if(!_postconstructed)
        {
            adl_postconstruct(_sp, const_cast<typename zr_boost_1_81::remove_const<T>::type *>(_sp.get()));
...
private:
    friend class zr_boost_1_81::signals2::deconstruct_access;
...
private:
    ...
    void destroy()
    {
        if( initialized_ )
        {
            ...
            using zr_boost_1_81::signals2::detail::adl_predestruct;
            adl_predestruct(const_cast<typename zr_boost_1_81::remove_const<T>::type *>(p));
            p->~T();
            initialized_ = false;
        }
    }
...
public:
    ...
    {
        zr_boost_1_81::shared_ptr< T > pt( static_cast< T* >( 0 ), detail::deconstruct_deleter< T >() );

        detail::deconstruct_deleter< T > * pd = zr_boost_1_81::get_deleter< detail::deconstruct_deleter< T > >( pt );
        ...
        zr_boost_1_81::shared_ptr< T > retval( pt, static_cast< T* >( pv ) );
        zr_boost_1_81::detail::sp_enable_shared_from_this(&retval, retval.get(), retval.get());
        return retval;
