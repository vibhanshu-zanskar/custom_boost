...
namespace zr_boost_1_81
{
   ...
   *
   * thread std/zr_boost_1_81::thread destructor terminates the program if the thread is not joinable.
   * Having a wrapper that can join the thread before destroying it seems a natural need.
   *
   * Example:
   *
   *     zr_boost_1_81::strict_scoped_thread<> t((zr_boost_1_81::thread(F)));
   *
   */
  template <class CallableThread = join_if_joinable, class Thread=::zr_boost_1_81::thread>
  ...
  public:
    ...
    explicit strict_scoped_thread(BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(Args)... args) :
      t_(zr_boost_1_81::forward<F>(f), zr_boost_1_81::forward<Args>(args)...) {}
#else
    template <class F>
    ...
    strict_scoped_thread(BOOST_THREAD_FWD_REF(F) f, BOOST_THREAD_FWD_REF(A1) a1, BOOST_THREAD_FWD_REF(A2) a2, BOOST_THREAD_FWD_REF(A3) a3) :
      t_(zr_boost_1_81::forward<F>(f), zr_boost_1_81::forward<A1>(a1), zr_boost_1_81::forward<A2>(a2), zr_boost_1_81::forward<A3>(a3)) {}
#endif
   ...
   * Example:
   *
   *     zr_boost_1_81::scoped_thread<> t((zr_boost_1_81::thread(F)));
   *     t.interrupt();
   *
   */
  template <class CallableThread = join_if_joinable, class Thread=::zr_boost_1_81::thread>
...
//    }
    ...
    t_(zr_boost_1_81::move(BOOST_THREAD_RV(x).t_))
    {}
