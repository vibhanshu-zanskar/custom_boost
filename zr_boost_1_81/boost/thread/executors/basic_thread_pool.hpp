    ...
    void worker_thread()
    {
      try
      {
        for(;;)
        {
          ...
          catch (zr_boost_1_81::thread_interrupted&)
          {
            return;
          }
        }
  ...
  public:
    ...
    basic_thread_pool(unsigned const thread_count = thread::hardware_concurrency()+1)
    {
      try
      {
        ...
        for (unsigned i = 0; i < thread_count; ++i)
        {
          ...
          threads.push_back(thread_t(zr_boost_1_81::move(th)));
#else
    ...
    basic_thread_pool( unsigned const thread_count, BOOST_THREAD_FWD_REF(AtThreadEntry) at_thread_entry)
    {
      try
      {
        ...
        for (unsigned i = 0; i < thread_count; ++i)
        {
          thread th (&basic_thread_pool::worker_thread3<AtThreadEntry>, this, zr_boost_1_81::forward<AtThreadEntry>(at_thread_entry));
          threads.push_back(thread_t(zr_boost_1_81::move(th)));
          //threads.push_back(thread_t(&basic_thread_pool::worker_thread, this)); // do not compile
        }
      }
    ...
    void submit(BOOST_THREAD_RV_REF(work) closure)  {
      work_queue.push(zr_boost_1_81::move(closure));
    }
    ...
    {
      //submit(work(zr_boost_1_81::forward<Closure>(closure)));
      work w((zr_boost_1_81::forward<Closure>(closure)));
      submit(zr_boost_1_81::move(w));
    }
