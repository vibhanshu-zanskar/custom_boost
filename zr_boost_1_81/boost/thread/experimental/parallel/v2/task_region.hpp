  ...
  namespace detail
  {
    void handle_task_region_exceptions(exception_list& errors)
    {
      ...
      catch (exception_list const& el)
      {
        ...
        {
          zr_boost_1_81::exception_ptr const& e = *it;
          try {
            rethrow_exception(e);
          }
          catch (...)
      ...
      {
        errors.add(zr_boost_1_81::current_exception());
      }
    }
    ...
    void wait_all()
    {
      ...
      for (group_type::iterator it = group.begin(); it != group.end(); ++it)
      {
        ...
        if (f.has_exception())
        {
          try
          {
            zr_boost_1_81::rethrow_exception(f.get_exception_ptr());
          }
          catch (...)
          {
      ...
      if (exs.size() != 0)
      {
        zr_boost_1_81::throw_exception(exs);
      }
    }
protected:
  ...
  public:
    ...
    void run(BOOST_THREAD_FWD_REF(F) f)
    {
#if defined BOOST_THREAD_TASK_REGION_HAS_SHARED_CANCELED
      {
        ...
        if (canceled) {
          zr_boost_1_81::throw_exception(task_canceled_exception());
        }
      }
#if defined BOOST_THREAD_PROVIDES_EXECUTORS
