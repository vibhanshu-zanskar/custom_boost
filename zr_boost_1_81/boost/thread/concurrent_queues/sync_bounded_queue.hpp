...
namespace concurrent
{
  ...
  private:
    ...
    {
      elem = zr_boost_1_81::move(data_[out_]);
      out_ = inc(out_);
      notify_not_full_if_needed(lk);
    }
    inline value_type pull(unique_lock<mutex>& lk)
    {
      value_type elem = zr_boost_1_81::move(data_[out_]);
      out_ = inc(out_);
      notify_not_full_if_needed(lk);
      return zr_boost_1_81::move(elem);
    }
    inline zr_boost_1_81::shared_ptr<value_type> ptr_pull(unique_lock<mutex>& lk)
    {
      shared_ptr<value_type> res = make_shared<value_type>(zr_boost_1_81::move(data_[out_]));
      out_ = inc(out_);
      notify_not_full_if_needed(lk);
      return res;
    }
    ...
    {
      data_[in_] = zr_boost_1_81::move(elem);
      set_in(in_p_1, lk);
    }
  };
...
//      iterator_t first = zr_boost_1_81::begin(range);
//      iterator_t end = zr_boost_1_81::end(range);
//      size_type in = 0;
...
//  }
  ...
  template <typename ValueType>
  zr_boost_1_81::shared_ptr<ValueType> sync_bounded_queue<ValueType>::try_pull()
  {
      unique_lock<mutex> lk(mtx_);
      return try_pull(lk);
  }
...
//  }
  ...
  template <typename ValueType>
  zr_boost_1_81::shared_ptr<ValueType> sync_bounded_queue<ValueType>::ptr_pull()
  {
      unique_lock<mutex> lk(mtx_);
      wait_until_not_empty(lk);
      return ptr_pull(lk);
  ...
  bool sync_bounded_queue<ValueType>::try_push(BOOST_THREAD_RV_REF(ValueType) elem, unique_lock<mutex>& lk)
  {
    ...
    push_at(zr_boost_1_81::move(elem), in_p_1, lk);
    return true;
  }
  ...
  {
      unique_lock<mutex> lk(mtx_);
      return try_push(zr_boost_1_81::move(elem), lk);
  }
#endif
  ...
  {
      unique_lock<mutex> lk(mtx_);
      return try_push_back(zr_boost_1_81::move(elem), lk);
  }
  ...
  {
    if (closed(lk)) return queue_op_status::closed;
    push_at(zr_boost_1_81::move(elem), wait_until_not_full(lk), lk);
    return queue_op_status::success;
  }
  template <typename ValueType>
  ...
  {
      unique_lock<mutex> lk(mtx_);
      return wait_push_back(zr_boost_1_81::move(elem), lk);
  }
  ...
  {
    sbq.push_back(zr_boost_1_81::move(elem));
    return sbq;
  }
