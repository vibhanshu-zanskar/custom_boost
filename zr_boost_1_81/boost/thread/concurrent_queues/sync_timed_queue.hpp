...
namespace detail
{
  ...
  struct scheduled_type
  {
    ...
    scheduled_type(BOOST_THREAD_RV_REF(T) pdata, time_point tp) : data(zr_boost_1_81::move(pdata)), time(tp) {}
    ...
    scheduled_type(BOOST_THREAD_RV_REF(scheduled_type) other) : data(zr_boost_1_81::move(other.data)), time(other.time) {}
    scheduled_type& operator=(BOOST_THREAD_RV_REF(scheduled_type) other) {
      data = zr_boost_1_81::move(other.data);
      time = other.time;
      return *this;
    }
  ...
  {
    super::push(stype(zr_boost_1_81::move(elem),tp));
  }
  ...
  {
    push(zr_boost_1_81::move(elem), clock::now() + dura);
  }
  ...
  {
    return super::try_push(stype(zr_boost_1_81::move(elem), tp));
  }
  ...
  {
    return try_push(zr_boost_1_81::move(elem), clock::now() + dura);
  }
  ...
  T sync_timed_queue<T, Clock, TimePoint>::pull(unique_lock<mutex>&)
  {
    ...
    return zr_boost_1_81::move(super::data_.pull().data);
#else
  ...
  {
#if ! defined  BOOST_NO_CXX11_RVALUE_REFERENCES
    elem = zr_boost_1_81::move(super::data_.pull().data);
#else
