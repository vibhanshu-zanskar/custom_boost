...
namespace spreadsort {
  namespace detail {
              ...
              , size_t *bin_sizes)
    {
      ...
      for (unsigned u = cache_offset; u < cache_end; lastPos = bin_cache[u],
        ...
        if (count < 2)
          continue;
        if (count < max_count)
          zr_boost_1_81::sort::pdqsort(lastPos, bin_cache[u]);
        else
                        ...
                        unsigned cache_offset, size_t *bin_sizes)
    {
      ...
      for (int ii = cache_end - 1; ii >= static_cast<int>(cache_offset);
        ...
        if (count < 2)
          continue;
        if (count < max_count)
          zr_boost_1_81::sort::pdqsort(lastPos, bin_cache[ii]);
        else
            ...
            size_t *bin_sizes, Right_shift rshift, Compare comp)
    {
      ...
      for (int ii = cache_end - 1; ii >= static_cast<int>(cache_offset);
        ...
        if (count < 2)
          continue;
        if (count < max_count)
          zr_boost_1_81::sort::pdqsort(lastPos, bin_cache[ii], comp);
        else
          negative_float_sort_rec<RandomAccessIter, Div_type, Right_shift,
            ...
            size_t *bin_sizes, Right_shift rshift, Compare comp)
    {
      ...
      for (unsigned u = cache_offset + first_positive; u < cache_end;
        ...
        if (count < 2)
          continue;
        if (count < max_count)
          zr_boost_1_81::sort::pdqsort(lastPos, bin_cache[u], comp);
        //sort positive values using normal spreadsort
        else
    ...
    inline typename zr_boost_1_81::enable_if_c< sizeof(zr_boost_1_81::uint32_t) ==
    ...
    float_sort(RandomAccessIter first, RandomAccessIter last)
    {
      ...
      std::vector<RandomAccessIter> bin_cache;
      float_sort_rec<RandomAccessIter, zr_boost_1_81::int32_t, zr_boost_1_81::uint32_t>
        (first, last, bin_cache, 0, bin_sizes);
    }

    //Checking whether the value type is a double, and using a 64-bit integer
    template <class RandomAccessIter>
    inline typename zr_boost_1_81::enable_if_c< sizeof(zr_boost_1_81::uint64_t) ==
      sizeof(typename std::iterator_traits<RandomAccessIter>::value_type)
      && std::numeric_limits<typename
      std::iterator_traits<RandomAccessIter>::value_type>::is_iec559,
      void >::type
    float_sort(RandomAccessIter first, RandomAccessIter last)
    {
      size_t bin_sizes[1 << max_finishing_splits];
      std::vector<RandomAccessIter> bin_cache;
      float_sort_rec<RandomAccessIter, zr_boost_1_81::int64_t, zr_boost_1_81::uint64_t>
        (first, last, bin_cache, 0, bin_sizes);
    }

    template <class RandomAccessIter>
    inline typename zr_boost_1_81::disable_if_c< (sizeof(zr_boost_1_81::uint64_t) ==
      sizeof(typename std::iterator_traits<RandomAccessIter>::value_type)
      || sizeof(zr_boost_1_81::uint32_t) ==
      sizeof(typename std::iterator_traits<RandomAccessIter>::value_type))
      && std::numeric_limits<typename
      std::iterator_traits<RandomAccessIter>::value_type>::is_iec559,
      void >::type
    float_sort(RandomAccessIter first, RandomAccessIter last)
    {
      BOOST_STATIC_ASSERT(!(sizeof(zr_boost_1_81::uint64_t) ==
      sizeof(typename std::iterator_traits<RandomAccessIter>::value_type)
      || sizeof(zr_boost_1_81::uint32_t) ==
      sizeof(typename std::iterator_traits<RandomAccessIter>::value_type))
      || !std::numeric_limits<typename
      std::iterator_traits<RandomAccessIter>::value_type>::is_iec559);
      zr_boost_1_81::sort::pdqsort(first, last);
    }

    //These approaches require the user to do the typecast
    //with rshift but default comparision
    template <class RandomAccessIter, class Div_type, class Right_shift>
    inline typename zr_boost_1_81::enable_if_c< sizeof(size_t) >= sizeof(Div_type),
      void >::type
    ...
    template <class RandomAccessIter, class Div_type, class Right_shift>
    inline typename zr_boost_1_81::enable_if_c< sizeof(size_t) < sizeof(Div_type)
      && sizeof(zr_boost_1_81::uintmax_t) >= sizeof(Div_type), void >::type
    float_sort(RandomAccessIter first, RandomAccessIter last, Div_type,
               Right_shift rshift)
    {
      size_t bin_sizes[1 << max_finishing_splits];
      std::vector<RandomAccessIter> bin_cache;
      float_sort_rec<RandomAccessIter, Div_type, Right_shift, zr_boost_1_81::uintmax_t>
        (first, last, bin_cache, 0, bin_sizes, rshift);
    }

    //sizeof(Div_type) doesn't match, so use zr_boost_1_81::sort::pdqsort
    template <class RandomAccessIter, class Div_type, class Right_shift>
    inline typename zr_boost_1_81::disable_if_c< sizeof(zr_boost_1_81::uintmax_t) >=
      sizeof(Div_type), void >::type
    float_sort(RandomAccessIter first, RandomAccessIter last, Div_type,
               Right_shift rshift)
    {
      BOOST_STATIC_ASSERT(sizeof(zr_boost_1_81::uintmax_t) >= sizeof(Div_type));
      zr_boost_1_81::sort::pdqsort(first, last);
    }
    ...
    {
      BOOST_STATIC_ASSERT(sizeof(zr_boost_1_81::uintmax_t) >= sizeof(Div_type));
      zr_boost_1_81::sort::pdqsort(first, last, comp);
    }
  }
}
}
