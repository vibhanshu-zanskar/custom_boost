  ...
  namespace detail {
              ...
              , size_t *bin_sizes)
    {
      ...
      for (unsigned u = cache_offset; u < cache_end; lastPos = bin_cache[u],
        ...
        if (count < 2)
          continue;
        //using zr_boost_1_81::sort::pdqsort if its worst-case is better
        if (count < max_count)
    ...
    inline typename zr_boost_1_81::enable_if_c< sizeof(Div_type) <= sizeof(size_t),
                                                            void >::type
    ...
    inline typename zr_boost_1_81::enable_if_c< (sizeof(Div_type) > sizeof(size_t))
      && sizeof(Div_type) <= sizeof(zr_boost_1_81::uintmax_t), void >::type
    integer_sort(RandomAccessIter first, RandomAccessIter last, Div_type)
    {
      size_t bin_sizes[1 << max_finishing_splits];
      std::vector<RandomAccessIter> bin_cache;
      spreadsort_rec<RandomAccessIter, Div_type, zr_boost_1_81::uintmax_t>(first,
          last, bin_cache, 0, bin_sizes);
    }

    template <class RandomAccessIter, class Div_type>
    inline typename zr_boost_1_81::disable_if_c< sizeof(Div_type) <= sizeof(size_t)
      || sizeof(Div_type) <= sizeof(zr_boost_1_81::uintmax_t), void >::type
    //defaulting to zr_boost_1_81::sort::pdqsort when integer_sort won't work
    integer_sort(RandomAccessIter first, RandomAccessIter last, Div_type)
    {
      //Warning that we're using zr_boost_1_81::sort::pdqsort, even though integer_sort was called
                ...
                Right_shift shift, Compare comp)
    {
      ...
      std::vector<RandomAccessIter> bin_cache;
      spreadsort_rec<RandomAccessIter, Div_type, Right_shift, Compare,
                        zr_boost_1_81::uintmax_t, int_log_mean_bin_size,
