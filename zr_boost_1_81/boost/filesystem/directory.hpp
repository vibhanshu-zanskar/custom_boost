...
{
public:
    typedef zr_boost_1_81::filesystem::path::value_type value_type; // enables class path ctor taking directory_entry

    directory_entry() BOOST_NOEXCEPT {}

    explicit directory_entry(zr_boost_1_81::filesystem::path const& p) :
        m_path(p), m_status(file_status()), m_symlink_status(file_status())
    {
    }

    directory_entry(zr_boost_1_81::filesystem::path const& p, file_status st, file_status symlink_st = file_status()) :
    ...
    directory_entry(directory_entry&& rhs) BOOST_NOEXCEPT :
        m_path(static_cast< zr_boost_1_81::filesystem::path&& >(rhs.m_path)),
    ...
    void assign(zr_boost_1_81::filesystem::path&& p, file_status st = file_status(), file_status symlink_st = file_status())
    {
        m_path = static_cast< zr_boost_1_81::filesystem::path&& >(p);
        m_status = static_cast< file_status&& >(st);
        m_symlink_status = static_cast< file_status&& >(symlink_st);
    }
#endif

    void assign(zr_boost_1_81::filesystem::path const& p, file_status st = file_status(), file_status symlink_st = file_status())
    {
        m_path = p;
#if !defined(BOOST_NO_CXX11_RVALUE_REFERENCES)
        m_status = static_cast< file_status&& >(st);
    ...
    void replace_filename(zr_boost_1_81::filesystem::path const& p, file_status st = file_status(), file_status symlink_st = file_status())
    {
    ...
    void replace_leaf(zr_boost_1_81::filesystem::path const& p, file_status st, file_status symlink_st)
    {
        replace_filename(p, st, symlink_st);
    }
#endif

    zr_boost_1_81::filesystem::path const& path() const BOOST_NOEXCEPT
    {
        return m_path;
    }
    operator zr_boost_1_81::filesystem::path const&() const BOOST_NOEXCEPT { return m_path; }
...
private:
    zr_boost_1_81::filesystem::path m_path;
...
{
    zr_boost_1_81::filesystem::path::string_type const& source = de.path().native();
...
//  - a conversion to 'path' using 'operator zr_boost_1_81::filesystem::path const&()',
...
struct dir_itr_imp :
    public zr_boost_1_81::intrusive_ref_counter< dir_itr_imp >
{
...
public:
    ...
    directory_iterator(directory_iterator&& that) BOOST_NOEXCEPT :
        m_imp(static_cast< zr_boost_1_81::intrusive_ptr< detail::dir_itr_imp >&& >(that.m_imp))
    {
    }
...
private:
    ...
    zr_boost_1_81::intrusive_ptr< detail::dir_itr_imp > m_imp;
};
...
//  namespace zr_boost_1_81 template specializations
template< typename C, typename Enabler >
struct range_mutable_iterator;

template<>
struct range_mutable_iterator< zr_boost_1_81::filesystem::directory_iterator, void >
{
    typedef zr_boost_1_81::filesystem::directory_iterator type;
};
...
template<>
struct range_const_iterator< zr_boost_1_81::filesystem::directory_iterator, void >
{
    typedef zr_boost_1_81::filesystem::directory_iterator type;
};
...
struct recur_dir_itr_imp :
    public zr_boost_1_81::intrusive_ref_counter< recur_dir_itr_imp >
{
...
public:
    ...
    recursive_directory_iterator(recursive_directory_iterator&& that) BOOST_NOEXCEPT :
        m_imp(static_cast< zr_boost_1_81::intrusive_ptr< detail::recur_dir_itr_imp >&& >(that.m_imp))
    {
    }
...
private:
    // intrusive_ptr provides the shallow-copy semantics required for single pass iterators
    // (i.e. InputIterators). The end iterator is indicated by is_end().
    zr_boost_1_81::intrusive_ptr< detail::recur_dir_itr_imp > m_imp;
};
...
//  namespace zr_boost_1_81 template specializations
template<>
struct range_mutable_iterator< zr_boost_1_81::filesystem::recursive_directory_iterator, void >
{
    typedef zr_boost_1_81::filesystem::recursive_directory_iterator type;
};

template<>
struct range_const_iterator< zr_boost_1_81::filesystem::recursive_directory_iterator, void >
{
    typedef zr_boost_1_81::filesystem::recursive_directory_iterator type;
};
