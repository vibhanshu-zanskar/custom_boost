...
struct sp_array_element {
    typedef typename zr_boost_1_81::remove_cv<typename
        zr_boost_1_81::remove_extent<T>::type>::type type;
};
...
struct sp_array_alignment {
    enum {
        value = sp_max_size<zr_boost_1_81::alignment_of<T>::value,
            zr_boost_1_81::alignment_of<U>::value>::value
    };
};
...
class sp_array_creator {
    ...
    typedef typename zr_boost_1_81::type_with_alignment<sp_array_alignment<T,
...
private:
    typename zr_boost_1_81::allocator_rebind<A, type>::type other_;
    std::size_t size_;
};
...
public:
    ...
    sp_array_base(const A& other, type* start, std::size_t size)
        : state_(other, size) {
        zr_boost_1_81::alloc_construct_n(state_.allocator(),
            zr_boost_1_81::first_scalar(start),
    ...
    sp_array_base(const A& other, type* start, std::size_t size, const U& list)
        ...
        zr_boost_1_81::alloc_construct_n(state_.allocator(),
            zr_boost_1_81::first_scalar(start), state_.size() * count,
            zr_boost_1_81::first_scalar(&list), count);
    }
    ...
    void dispose() BOOST_SP_NOEXCEPT BOOST_OVERRIDE {
        zr_boost_1_81::alloc_destroy_n(state_.allocator(),
            zr_boost_1_81::first_scalar(sp_array_start<type>(this)),
            state_.size() * sp_array_count<type>::value);
    }
...
{
    return zr_boost_1_81::allocate_shared<T>(zr_boost_1_81::noinit_adapt(allocator), count);
}
...
{
    return zr_boost_1_81::allocate_shared<T>(zr_boost_1_81::noinit_adapt(allocator));
}
