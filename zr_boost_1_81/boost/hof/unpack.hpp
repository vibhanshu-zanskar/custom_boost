...
(
    zr_boost_1_81::hof::pack_join(unpack_simple(zr_boost_1_81::hof::pack_forward, BOOST_HOF_FORWARD(Sequences)(s))...)(BOOST_HOF_FORWARD(F)(f))
);
...
struct unpack_adaptor : detail::callable_base<F>
{
    ...
    struct unpack_failure
    {
        ...
        struct apply
        {
            ...
            BOOST_HOF_RETURNS
            (
                zr_boost_1_81::hof::detail::unpack_simple(deducer(), BOOST_HOF_FORWARD(T)(x))
            );
            ...
            (
                zr_boost_1_81::hof::detail::unpack_join(deducer(), BOOST_HOF_FORWARD(T)(x), BOOST_HOF_FORWARD(Ts)(xs)...)
            );
#ifdef _MSC_VER
            template<class... Ts>
            struct nop_failure;
    ...
    BOOST_HOF_RETURNS
    (
        zr_boost_1_81::hof::detail::unpack_simple(BOOST_HOF_MANGLE_CAST(const detail::callable_base<F>&)(BOOST_HOF_CONST_THIS->base_function(x)), BOOST_HOF_FORWARD(T)(x))
    );
    ...
    (
        zr_boost_1_81::hof::detail::unpack_join(BOOST_HOF_MANGLE_CAST(const detail::callable_base<F>&)(BOOST_HOF_CONST_THIS->base_function(x)), BOOST_HOF_FORWARD(T)(x), BOOST_HOF_FORWARD(Ts)(xs)...)
    );
};
