...
struct implicit
{
    ...
    {
        template<class Pack>
        constexpr invoker<Pack> operator()(Pack p) const BOOST_HOF_NOEXCEPT(noexcept(invoker<Pack>(zr_boost_1_81::hof::move(p))))
        {
            return invoker<Pack>(zr_boost_1_81::hof::move(p));
        }
    ...
    BOOST_HOF_RETURNS
    (
        BOOST_HOF_RETURNS_CONSTRUCT(make_invoker)()(zr_boost_1_81::hof::pack_basic(BOOST_HOF_FORWARD(Ts)(xs)...))
    );
};
