...
struct ref_transformer
{
    ...
    BOOST_HOF_SFINAE_RETURNS(zr_boost_1_81::hof::always_ref(x.get()));
};
...
(
    p(zr_boost_1_81::hof::detail::pick_transformer(BOOST_HOF_FORWARD(T)(x)))
);
...
: detail::compressed_pair<F, Pack>
{
    ...
    (
        BOOST_HOF_MANGLE_CAST(const Pack&)(BOOST_HOF_CONST_THIS->get_pack(xs...))(
            zr_boost_1_81::hof::detail::make_lazy_unpack(
                BOOST_HOF_MANGLE_CAST(const F&)(BOOST_HOF_CONST_THIS->base_function(xs...)), 
                zr_boost_1_81::hof::pack_forward(BOOST_HOF_FORWARD(Ts)(xs)...)
            )
        )
    );
};
...
struct lazy_adaptor : detail::callable_base<F>
{
    ...
    (
        zr_boost_1_81::hof::detail::make_lazy_invoker(BOOST_HOF_RETURNS_C_CAST(detail::callable_base<F>&&)(BOOST_HOF_CONST_THIS->base_function(x, xs...)), 
            zr_boost_1_81::hof::pack_basic(BOOST_HOF_RETURNS_STATIC_CAST(T&&)(x), BOOST_HOF_RETURNS_STATIC_CAST(Ts&&)(xs)...))
    );
    ...
    BOOST_HOF_RETURNS_DEDUCE_NOEXCEPT(
        zr_boost_1_81::hof::detail::make_lazy_nullary_invoker(BOOST_HOF_RETURNS_C_CAST(detail::callable_base<F>&&)(
            BOOST_HOF_CONST_THIS->base_function(BOOST_HOF_RETURNS_CONSTRUCT(Unused)())
        ))
    )
    {
        return zr_boost_1_81::hof::detail::make_lazy_nullary_invoker((detail::callable_base<F>&&)(
    ...
    // constexpr auto operator()(Ts... xs) const& BOOST_HOF_RETURNS
    // (
    //     zr_boost_1_81::hof::detail::make_lazy_invoker(this->base_function(xs...), 
    //         pack(zr_boost_1_81::hof::move(xs)...))
    // );

    // template<class... Ts>
    // constexpr auto operator()(Ts... xs) && BOOST_HOF_RETURNS
    // (
    //     zr_boost_1_81::hof::detail::make_lazy_invoker((F&&)this->base_function(xs...), 
    //         pack(zr_boost_1_81::hof::move(xs)...))
    // );
...
namespace std {
    template<class F, class Pack>
    struct is_bind_expression<zr_boost_1_81::hof::detail::lazy_invoker<F, Pack>>
    : std::true_type
    {};

    template<class F>
    struct is_bind_expression<zr_boost_1_81::hof::detail::lazy_nullary_invoker<F>>
    : std::true_type
    {};
}
