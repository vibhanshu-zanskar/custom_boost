...
struct partial_adaptor_invoke
{
    ...
    (
        typename result_of<decltype(zr_boost_1_81::hof::pack_join), 
            id_<const Pack&>, 
            result_of<decltype(zr_boost_1_81::hof::pack_forward), id_<Ts>...> 
        >::type,
        id_<F&&>
    ) 
    operator()(Ts&&... xs) const BOOST_HOF_SFINAE_RETURNS
    (
        zr_boost_1_81::hof::pack_join
        (
...
struct partial_adaptor_join
{
    ...
    noexcept(BOOST_HOF_IS_NOTHROW_CONSTRUCTIBLE(F, F&&) && noexcept(zr_boost_1_81::hof::pack_join(std::declval<const Pack&>(), zr_boost_1_81::hof::pack(BOOST_HOF_FORWARD(Ts)(xs)...))))
#endif
    BOOST_HOF_PARTIAL_RETURNS
    (
        zr_boost_1_81::hof::partial
        (
            BOOST_HOF_RETURNS_C_CAST(F&&)(BOOST_HOF_CONST_THIS->get_function(xs...)), 
            zr_boost_1_81::hof::pack_join(BOOST_HOF_MANGLE_CAST(const Pack&)(BOOST_HOF_CONST_THIS->get_pack(xs...)), zr_boost_1_81::hof::pack(BOOST_HOF_FORWARD(Ts)(xs)...))
        )
    );
};
...
#ifdef _MSC_VER
    // Workaround ICE on MSVC
    noexcept(BOOST_HOF_IS_NOTHROW_CONSTRUCTIBLE(F, F&&) && noexcept(zr_boost_1_81::hof::pack(BOOST_HOF_FORWARD(Ts)(xs)...)))
#endif
    BOOST_HOF_PARTIAL_RETURNS
    (
        zr_boost_1_81::hof::partial
        (
            BOOST_HOF_RETURNS_C_CAST(F&&)(BOOST_HOF_CONST_THIS->get_function(xs...)), 
            zr_boost_1_81::hof::pack(BOOST_HOF_FORWARD(Ts)(xs)...)
        )
    );
};
