...
(detail::make_postfix_adaptor(BOOST_HOF_FORWARD(T)(x), zr_boost_1_81::hof::move(i.base_function(x))));
...
template<class T, class F>
auto operator<(T&& x, const zr_boost_1_81::hof::detail::static_function_wrapper<F>& f) BOOST_HOF_RETURNS
(
    detail::make_postfix_adaptor(BOOST_HOF_FORWARD(T)(x), zr_boost_1_81::hof::move(f.base_function().infix_base_function()))
);
...
template<class T, class F>
auto operator<(T&& x, const zr_boost_1_81::hof::detail::static_default_function<F>&) BOOST_HOF_RETURNS
(
    detail::make_postfix_adaptor(BOOST_HOF_FORWARD(T)(x), zr_boost_1_81::hof::move(F().infix_base_function()))
);
}
// This overload is needed for gcc
template<class T, class F>
constexpr auto operator<(T&& x, const zr_boost_1_81::hof::reveal_adaptor<F>& f) BOOST_HOF_RETURNS
(
