...
///     #include <cassert>
///     using namespace zr_boost_1_81::hof;
/// 
///     int main() {
///         auto sum = _1 + _2;
///         assert(3 == sum(1, 2));
...
namespace std {
    template<int N>
    struct is_placeholder<zr_boost_1_81::hof::detail::simple_placeholder<N>>
    : std::integral_constant<int, N>
    {};
}


namespace zr_boost_1_81 { namespace hof {
...
struct placeholder
{
    ...
    ( zr_boost_1_81::hof::lazy(operators::call())(detail::simple_placeholder<N>(), BOOST_HOF_FORWARD(Ts)(xs)...) );
#else
    template<class... Ts>
    struct result_call
    { typedef decltype(zr_boost_1_81::hof::lazy(operators::call())(detail::simple_placeholder<N>(), std::declval<Ts>()...)) type; };
    template<class... Ts>
    constexpr typename result_call<Ts...>::type operator()(Ts&&... xs) const 
    { return zr_boost_1_81::hof::lazy(operators::call())(detail::simple_placeholder<N>(), BOOST_HOF_FORWARD(Ts)(xs)...); };

#endif

#define BOOST_HOF_PLACEHOLDER_UNARY_OP(op, name) \
    constexpr auto operator op () const BOOST_HOF_RETURNS \
    ( zr_boost_1_81::hof::lazy(operators::name())(detail::simple_placeholder<N>()) );
...
#define BOOST_HOF_PLACEHOLDER_ASSIGN_OP(op, name) \
    ...
    ( zr_boost_1_81::hof::lazy(operators::name())(detail::simple_placeholder<N>(), BOOST_HOF_FORWARD(T)(x)) );
...
#define BOOST_HOF_PLACEHOLDER_BINARY_OP(op, name) \
    ...
    ( zr_boost_1_81::hof::lazy(operators::name())(BOOST_HOF_FORWARD(T)(x), detail::simple_placeholder<N>()) ); \
    template<int N, int M> \
    constexpr inline auto operator op (const placeholder<N>&, const placeholder<M>&) BOOST_HOF_RETURNS \
    ( zr_boost_1_81::hof::lazy(operators::name())(detail::simple_placeholder<N>(), detail::simple_placeholder<M>()) );
...
#define BOOST_HOF_PLACEHOLDER_BINARY_OP(op, name) \
    template<class T, class U> \
    struct result_ ## name \
    { typedef decltype(zr_boost_1_81::hof::lazy(operators::name())(std::declval<T>(), std::declval<U>())) type; }; \
    template<class T, int N> \
    constexpr inline typename result_ ## name<detail::simple_placeholder<N>, T>::type operator op (const placeholder<N>&, T&& x) \
    { return zr_boost_1_81::hof::lazy(operators::name())(detail::simple_placeholder<N>(), BOOST_HOF_FORWARD(T)(x)); } \
    template<class T, int N> \
    constexpr inline typename result_ ## name<T, detail::simple_placeholder<N>>::type operator op (T&& x, const placeholder<N>&) \
    { return zr_boost_1_81::hof::lazy(operators::name())(BOOST_HOF_FORWARD(T)(x), detail::simple_placeholder<N>()); } \
    template<int N, int M> \
    ...
    { return zr_boost_1_81::hof::lazy(operators::name())(detail::simple_placeholder<N>(), detail::simple_placeholder<M>()); }
...
#define BOOST_HOF_UNAMED_PLACEHOLDER_BINARY_OP(op, name) \
    ...
    ( unamed_placeholder::make_partial_ap<unamed_placeholder::right<operators::name>>(zr_boost_1_81::hof::decay(x)) ); \
    template<class T> \
    constexpr inline auto operator op (const T& x, const unamed_placeholder&) BOOST_HOF_RETURNS \
    ( unamed_placeholder::make_partial_ap<unamed_placeholder::left<operators::name>>(zr_boost_1_81::hof::decay(x)) ); \
...
namespace std {
    template<int N>
    struct is_placeholder<zr_boost_1_81::hof::placeholder<N>>
    : std::integral_constant<int, N>
    {};
}
