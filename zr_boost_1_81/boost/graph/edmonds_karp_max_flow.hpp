    ...
    PredEdgeMap pred)
{
    ...
    for (zr_boost_1_81::tie(u_iter, u_end) = vertices(g); u_iter != u_end; ++u_iter)
        for (zr_boost_1_81::tie(ei, e_end) = out_edges(*u_iter, g); ei != e_end; ++ei)
            put(res, *ei, get(cap, *ei));

    put(color, sink, Color::gray());
    while (get(color, sink) != Color::white())
    {
        zr_boost_1_81::queue< vertex_t > Q;
    ...
    for (zr_boost_1_81::tie(ei, e_end) = out_edges(src, g); ei != e_end; ++ei)
