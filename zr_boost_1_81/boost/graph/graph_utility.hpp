    ...
    const IncidenceGraph& G, Name name, std::ostream& os = std::cout)
{
    ...
    for (zr_boost_1_81::tie(ui, ui_end) = vertices(G); ui != ui_end; ++ui)
    {
        ...
        for (zr_boost_1_81::tie(ei, ei_end) = in_edges(*ui, G); ei != ei_end; ++ei)
    ...
    std::ostream& os = std::cout)
{
    ...
    for (zr_boost_1_81::tie(ui, ui_end) = vertices(G); ui != ui_end; ++ui)
    {
        ...
        for (zr_boost_1_81::tie(ei, ei_end) = out_edges(*ui, G); ei != ei_end; ++ei)
...
template < class Graph, class Vertex > bool in_vertex_set(Graph& g, Vertex v)
{
    ...
    zr_boost_1_81::tie(vi, vi_end) = vertices(g);
...
inline bool is_connected(const VertexListGraph& g, VertexColorMap color)
{
    ...
    for (zr_boost_1_81::tie(ui, ui_end) = vertices(g); ui != ui_end; ++ui)
        for (zr_boost_1_81::tie(vi, vi_end) = vertices(g); vi != vi_end; ++vi)
            if (*ui != *vi)
            {
                for (zr_boost_1_81::tie(ci, ci_end) = vertices(g); ci != ci_end; ++ci)
...
namespace graph
{
    ...
    {
        typedef typename zr_boost_1_81::graph_traits< Graph >::vertex_iterator vi;
    ...
    {
        typedef typename zr_boost_1_81::graph_traits< Graph >::edge_iterator ei;
    ...
    bool has_no_out_edges(
        const typename zr_boost_1_81::graph_traits< Graph >::vertex_descriptor& v,
        const Graph& g)
    {
        typedef typename zr_boost_1_81::graph_traits< Graph >::out_edge_iterator ei;
