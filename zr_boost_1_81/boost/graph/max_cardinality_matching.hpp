...
public:
    ...
    {
        typedef zr_boost_1_81::iterator_property_map<
    ...
    bool augment_matching()
    {
        ...
        for (zr_boost_1_81::tie(vi, vi_end) = vertices(g); vi != vi_end; ++vi)
        {
            ...
            if (mate[u] == graph_traits< Graph >::null_vertex())
            {
                ...
                for (zr_boost_1_81::tie(ei, ei_end) = out_edges(u, g); ei != ei_end;
                     ++ei)
                {
                    if (target(*ei, g) != u)
                    {
        ...
        while (!even_edges.empty() && !found_alternating_path)
        {
            ...
            if (vertex_state[w_prime] == graph::detail::V_UNREACHED)
            {
                ...
                out_edge_iterator_t ei, ei_end;
                for (zr_boost_1_81::tie(ei, ei_end) = out_edges(w_prime_mate, g);
                     ei != ei_end; ++ei)
                {
        ...
        vertex_descriptor_t stop_vertex, vertex_pair_t the_bridge)
    {
        for (vertex_descriptor_t v = x; v != stop_vertex; v = parent(v))
        {
            ...
            if (vertex_state[v] == graph::detail::V_ODD)
            {
                ...
                for (zr_boost_1_81::tie(oei, oei_end) = out_edges(v, g); oei != oei_end;
                     ++oei)
                {
                    if (target(*oei, g) != v)
                    {
