...
namespace detail
{
    ...
    struct add_reverse_edge_descriptor< Desc, zr_boost_1_81::reverse_graph< G, GR > >
    {
    ...
    struct add_reverse_edge_descriptor< reverse_graph_edge_descriptor< Desc >,
        zr_boost_1_81::reverse_graph< G, GR > >
    {
        typedef reverse_graph_edge_descriptor< Desc > type;
    ...
    template <> struct copy_graph_impl< 0 >
    {
            ...
            Orig2CopyVertexIndexMap orig2copy, IndexMap)
        {
            ...
            for (zr_boost_1_81::tie(vi, vi_end) = vertices(g_in); vi != vi_end; ++vi)
            {
            ...
            for (zr_boost_1_81::tie(ei, ei_end) = edges(g_in); ei != ei_end; ++ei)
            {
                typename graph_traits< MutableGraph >::edge_descriptor new_e;
                bool inserted;
                zr_boost_1_81::tie(new_e, inserted)
    ...
    template <> struct copy_graph_impl< 1 >
    {
            ...
            Orig2CopyVertexIndexMap orig2copy, IndexMap)
        {
            ...
            for (zr_boost_1_81::tie(vi, vi_end) = vertices(g_in); vi != vi_end; ++vi)
            {
                ...
                for (zr_boost_1_81::tie(ei, ei_end) = out_edges(*vi, g_in);
                     ei != ei_end; ++ei)
                {
