...
namespace detail
{
    ...
    public:
            ...
            typename graph_traits< Graph >::vertex_descriptor v, const Graph& g)
        {
            ...
            for (zr_boost_1_81::tie(ei, ei_end) = out_edges(v, g); ei != ei_end; ++ei)
            {
                w = target(*ei, g);
    ...
    Graph& G, ComponentsMap c, FinishTime finish_time, ColorMap color)
{
    ...
    for (zr_boost_1_81::tie(ui, ui_end) = vertices(G_T); ui != ui_end; ++ui)
    ...
    zr_boost_1_81::tie(i, iend) = vertices(G_T);
    zr_boost_1_81::tie(j, jend) = vertices(G);
    for (; i != iend; ++i, ++j)
    {
