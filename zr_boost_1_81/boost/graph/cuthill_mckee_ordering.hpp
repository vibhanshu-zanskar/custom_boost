    ...
    OutputIterator permutation, ColorMap color, DegreeMap degree)
{
    ...
    typedef typename zr_boost_1_81::sparse::sparse_ordering_queue< Vertex > queue;
...
OutputIterator cuthill_mckee_ordering(const Graph& G,
    OutputIterator permutation, ColorMap color, DegreeMap degree)
{
    if (zr_boost_1_81::graph::has_no_vertices(G))
        return permutation;

    typedef typename zr_boost_1_81::graph_traits< Graph >::vertex_descriptor Vertex;
    typedef typename property_traits< ColorMap >::value_type ColorValue;
    typedef color_traits< ColorValue > Color;
