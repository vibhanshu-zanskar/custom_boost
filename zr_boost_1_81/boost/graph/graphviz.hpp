...
{
    using namespace zr_boost_1_81::xpressive;
    static sregex valid_unquoted_id = (((alpha | '_') >> *_w)
        | (!as_xpr('-') >> (('.' >> *_d) | (+_d >> !('.' >> *_d)))));
    std::string s(zr_boost_1_81::lexical_cast< std::string >(obj));
    ...
    {
        zr_boost_1_81::algorithm::replace_all(s, "\"", "\\\"");
        return "\"" + s + "\"";
    }
}
        ...
        Graph, vertex_list_graph_tag))
{
    ...
    for (zr_boost_1_81::tie(i, end) = vertices(g); i != end; ++i)
    {
    ...
    for (zr_boost_1_81::tie(ei, edge_end) = edges(g); ei != edge_end; ++ei)
    {
...
namespace detail
{
        ...
        VertexID vertex_id)
    {
        ...
        // print subgraph
        for (zr_boost_1_81::tie(i_child, j_child) = g.children(); i_child != j_child;
             ++i_child)
...
namespace detail
{
    namespace graph
    {
        ...
        public:
            ...
            {
                return zr_boost_1_81::is_convertible<
                    typename zr_boost_1_81::graph_traits<
                        MutableGraph >::directed_category,
                    zr_boost_1_81::directed_tag >::value;
            }
                ...
                const node_t& target) BOOST_OVERRIDE
            {
                ...
                if (!result.second)
                {
                    ...
                    zr_boost_1_81::throw_exception(bad_parallel_edge(source, target));
                }
                else
                {
        ...
        public:
            ...
            void finish_building_graph() BOOST_OVERRIDE
            {
                ...
                typedef zr_boost_1_81::tuple< id_t, bgl_vertex_t, id_t > v_prop;
                BOOST_FOREACH (const v_prop& t, vertex_props)
                {
                    put(zr_boost_1_81::get< 0 >(t), dp_, zr_boost_1_81::get< 1 >(t),
                        zr_boost_1_81::get< 2 >(t));
                }
                typedef zr_boost_1_81::tuple< id_t, bgl_edge_t, id_t > e_prop;
                BOOST_FOREACH (const e_prop& t, edge_props)
                {
                    put(zr_boost_1_81::get< 0 >(t), dp_,
                        edge_permutation_from_sorting[zr_boost_1_81::get< 1 >(t)],
                        zr_boost_1_81::get< 2 >(t));
                }
            }
            ...
            {
                return zr_boost_1_81::is_convertible<
                    typename zr_boost_1_81::graph_traits< CSRGraph >::directed_category,
                    zr_boost_1_81::directed_tag >::value;
            }
            ...
            void do_add_vertex(const node_t& node) BOOST_OVERRIDE
            {
                ...
                vertex_props.push_back(
                    zr_boost_1_81::make_tuple(node_id_prop_, v, node));
            }
            ...
            {
                vertex_props.push_back(
                    zr_boost_1_81::make_tuple(key, bgl_nodes[node], value));
            }
                ...
                const id_t& value) BOOST_OVERRIDE
            {
                edge_props.push_back(
                    zr_boost_1_81::make_tuple(key, bgl_edges[edge], value));
            }
        ...
        protected:
            ...
            std::string node_id_prop_;
            std::vector< zr_boost_1_81::tuple< id_t, bgl_vertex_t, id_t > >
                vertex_props;
            std::vector< zr_boost_1_81::tuple< id_t, bgl_edge_t, id_t > > edge_props;
...
} // end namespace zr_boost_1_81::detail::graph
    ...
    std::string const& node_id = "node_id")
{
    ...
    typedef zr_boost_1_81::spirit::classic::multi_pass< is_t > iterator_t;

    iterator_t first(zr_boost_1_81::spirit::classic::make_multi_pass(user_first));
    iterator_t last(zr_boost_1_81::spirit::classic::make_multi_pass(user_last));
