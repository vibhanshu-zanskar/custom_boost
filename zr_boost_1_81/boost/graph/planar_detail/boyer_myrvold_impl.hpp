...
class boyer_myrvold_impl
{
    ...
    typedef zr_boost_1_81::shared_ptr< face_handle_list_t > face_handle_list_ptr_t;
    typedef zr_boost_1_81::shared_ptr< vertex_list_t > vertex_list_ptr_t;
    typedef zr_boost_1_81::tuple< vertex_t, bool, bool > merge_stack_frame_t;
...
private:
    void walkup(vertex_t v)
    {
        ...
        for (zr_boost_1_81::tie(oi, oi_end) = out_edges(v, g); oi != oi_end; ++oi)
        {
            edge_t e(*oi);
        ...
        while (!pertinent_roots[v]->empty())
        {
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    if (first_side_vertex == second_side_vertex)
                    {
                        if (first_tail != v)
                        {
                            ...
                            zr_boost_1_81::tie(first_side_vertex, first_tail)
                        ...
                        else if (second_tail != v)
                        {
                            ...
                            zr_boost_1_81::tie(second_side_vertex, second_tail)
                ...
                while (!merge_stack.empty())
                {
                    ...
                    zr_boost_1_81::tie(merge_point, next_bottom_follows_first,
    ...
    void clean_up_embedding(graph::detail::store_embedding)
    {
        ...
        for (zr_boost_1_81::tie(xi, xi_end) = vertices(g); xi != xi_end; ++xi)
        {
    ...
    void remove_vertex_from_separated_dfs_child_list(vertex_t v)
    {
        ...
        garbage.splice(garbage.end(), *separated_dfs_child_list[dfs_parent[v]],
            to_delete, zr_boost_1_81::next(to_delete));
    }
...
public:
    ...
    void extract_kuratowski_subgraph(OutputIterator o_itr, EdgeIndexMap em)
    {
        ...
        for (zr_boost_1_81::tie(ei, ei_end) = edges(g); ei != ei_end; ++ei)
        {
            edge_t e(*ei);
        ...
        if (bicomp_root != v)
        {
            ...
            for (zr_boost_1_81::tie(vi, vi_end) = vertices(g); vi != vi_end; ++vi)
                if (lower_face_vertex[*vi])
                    for (zr_boost_1_81::tie(oei, oei_end) = out_edges(*vi, g);
                         oei != oei_end; ++oei)
        ...
        else
        {
                 ...
                 ++of_itr)
            {
                ...
                for (; old_face_itr != old_face_end; ++old_face_itr)
                {
                    ...
                    if (w == graph_traits< Graph >::null_vertex()
                        ...
                        && outer_face_edge[e]
                        && outer_face_edge[*zr_boost_1_81::next(old_face_itr)]
                        && !seen_x_or_y)
                    {
                        w = current_vertex;
                    }
            ...
            for (zr_boost_1_81::tie(v_edge_itr, v_edge_end) = out_edges(v, g);
            ...
            && chosen_case != detail::BM_CASE_A)
        {
            ...
            if (chosen_case == detail::BM_CASE_E)
            {
                ...
                for (zr_boost_1_81::tie(oei, oei_end) = out_edges(w, g); oei != oei_end;
                     ++oei)
                {
                    if (!outer_face_edge[*oei])
                        goal_edge[*oei] = true;
        ...
        while (child != parent)
        {
            ...
            zr_boost_1_81::tie(parent, child)
