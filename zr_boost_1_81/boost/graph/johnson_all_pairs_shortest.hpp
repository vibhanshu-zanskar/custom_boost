    ...
    const BinaryFunction& combine, const Infinity& inf, DistanceZero zero)
{
    ...
    typename Traits2::vertex_descriptor s = *vertices(g2).first;
    {
        ...
        int i = 1;
        for (zr_boost_1_81::tie(v, v_end) = vertices(g1); v != v_end; ++v, ++i)
        {
            typename Traits2::edge_descriptor e;
            bool z;
            zr_boost_1_81::tie(e, z) = add_edge(s, get(id1, *v) + 1, g2);
            put(w, e, zero);
            verts1[i] = *v;
        }
        typename Traits1::edge_iterator e, e_end;
        for (zr_boost_1_81::tie(e, e_end) = edges(g1); e != e_end; ++e)
        {
            typename Traits2::edge_descriptor e2;
            bool z;
            zr_boost_1_81::tie(e2, z) = add_edge(
                get(id1, source(*e, g1)) + 1, get(id1, target(*e, g1)) + 1, g2);
            put(w, e2, get(w1, *e));
            if (is_undirected)
            {
                zr_boost_1_81::tie(e2, z) = add_edge(get(id1, target(*e, g1)) + 1,
    ...
    for (zr_boost_1_81::tie(v, v_end) = vertices(g2); v != v_end; ++v)
        put(d, *v, inf);
            ...
            g2, num_vertices(g2), w, pred, d, combine, compare, bvis))
    {
        ...
        for (zr_boost_1_81::tie(e, e_end) = edges(g2); e != e_end; ++e)
        {
            typename Traits2::vertex_descriptor a = source(*e, g2),
                                                b = target(*e, g2);
            put(w_hat, *e, combine((get(h, a) - get(h, b)), get(w, *e)));
        }
        for (zr_boost_1_81::tie(u, u_end) = vertices(g2); u != u_end; ++u)
        {
