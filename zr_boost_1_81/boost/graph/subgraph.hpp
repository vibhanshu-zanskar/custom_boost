...
public:
    ...
    : m_graph(n, p), m_parent(0), m_edge_counter(0), m_global_vertex(n)
    {
        ...
        for (zr_boost_1_81::tie(v, v_end) = vertices(m_graph); v != v_end; ++v)
            m_global_vertex[i++] = *v;
    }
    ...
    subgraph(const subgraph& x) : m_parent(x.m_parent), m_edge_counter(0)
    {
        ...
        else
        {
            ...
            zr_boost_1_81::tie(vi, vi_end) = vertices(x);
            for (; vi != vi_end; ++vi)
            {
        ...
        zr_boost_1_81::tie(i, i_end) = x.children();
        for (; i != i_end; ++i)
        {
    ...
    vertex_descriptor global_to_local(vertex_descriptor u_global) const
    {
        ...
        if (is_root())
            return u_global;
        zr_boost_1_81::tie(u_local, in_subgraph) = this->find_vertex(u_global);
    ...
    BOOST_STATIC_ASSERT((!is_same< edge_index_type,
                         zr_boost_1_81::detail::error_property_not_found >::value));
...
public: // Probably shouldn't be public....
        ...
        vertex_descriptor v_local, edge_descriptor e_global)
    {
        ...
        bool inserted;
        zr_boost_1_81::tie(e_local, inserted) = add_edge(u_local, v_local, m_graph);
    ...
    typename subgraph< G >::vertex_descriptor u_global, subgraph< G >& g)
{
    ...
    bool exists_local;
    zr_boost_1_81::tie(u_local, exists_local) = g.find_vertex(u_global);
    ...
    if (!exists_local)
    {
        ...
        // remember edge global and local maps
        {
            ...
            for (zr_boost_1_81::tie(ei, ei_end) = out_edges(u_global, r); ei != ei_end;
                 ++ei)
            {
                e_global = *ei;
        ...
        { // not necessary for undirected graph
            ...
            typename subgraph< G >::out_edge_iterator ei, ei_end;
            for (zr_boost_1_81::tie(vi, vi_end) = vertices(r); vi != vi_end; ++vi)
            {
                v_global = *vi;
                ...
                for (zr_boost_1_81::tie(ei, ei_end) = out_edges(*vi, r); ei != ei_end;
                     ++ei)
                {
                    e_global = *ei;
...
namespace detail
{
        ...
        subgraph< Graph >& g, subgraph< Graph >* orig)
    {
        if (&g != orig)
        {
            ...
            zr_boost_1_81::tie(u_local, u_in_subgraph) = g.find_vertex(u_global);
            zr_boost_1_81::tie(v_local, v_in_subgraph) = g.find_vertex(v_global);
        ...
        subgraph< Graph >* orig)
    {
        if (g.is_root())
        {
            ...
            bool inserted;
            zr_boost_1_81::tie(e_global, inserted)
    ...
    const typename G::edge_property_type& ep, subgraph< G >& g)
{
    ...
    else
    {
        ...
        bool inserted;
        zr_boost_1_81::tie(e_global, inserted) = detail::add_edge_recur_up(
