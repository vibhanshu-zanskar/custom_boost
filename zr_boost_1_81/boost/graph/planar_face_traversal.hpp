    ...
    Visitor& visitor, EdgeIndexMap em)
{
    ...
    typedef typename zr_boost_1_81::iterator_property_map<
    ...
    for (zr_boost_1_81::tie(vi, vi_end) = vertices(g); vi != vi_end; ++vi)
    {
        ...
        for (pi = pi_begin; pi != pi_end; ++pi)
        {
            ...
            m[v] = zr_boost_1_81::next(pi) == pi_end ? *pi_begin : *zr_boost_1_81::next(pi);
            put(next_edge, e, m);
        }
    }
    ...
    for (zr_boost_1_81::tie(fi, fi_end) = edges(g); fi != fi_end; ++fi)
    {
        edge_t e(*fi);
