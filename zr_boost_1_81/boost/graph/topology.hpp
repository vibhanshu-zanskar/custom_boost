...
public:
    ...
    double distance(point a, point b) const
    {
        ...
        {
            double diff = b[i] - a[i];
            dist = zr_boost_1_81::math::hypot(dist, diff);
        }
    ...
    double norm(point_difference delta) const
    {
        double n = 0.;
        for (std::size_t i = 0; i < Dims; ++i)
            n = zr_boost_1_81::math::hypot(n, delta[i]);
        return n;
    }
...
public:
    ...
    point_type bound(point_type a) const
    {
        ...
        double r = 0.;
        for (std::size_t i = 0; i < Dims; ++i)
            r = zr_boost_1_81::math::hypot(r, a[i]);
        if (r <= radius)
            return a;
...
template < typename RandomNumberGenerator = minstd_rand > class heart_topology
{
    ...
    bool in_heart(point p) const
    {
        ...
        if (zr_boost_1_81::math::hypot(p[0] - -500, p[1] - -500)
            <= 500. * zr_boost_1_81::math::constants::root_two< double >())
...
public:
    ...
    point random_point() const
    {
        ...
        do
        {
            result[0] = (*rand)()
                    * (1000
                        + 1000 * zr_boost_1_81::math::constants::root_two< double >())
                - (500 + 500 * zr_boost_1_81::math::constants::root_two< double >());
            result[1] = (*rand)()
                    * (2000
                        + 500
                            * (zr_boost_1_81::math::constants::root_two< double >()
                                - 1))
                - 2000;
    ...
    double distance(point a, point b) const
    {
        ...
        {
            // Straight line
            return zr_boost_1_81::math::hypot(b[0] - a[0], b[1] - a[1]);
        }
        else
        {
            // Straight line bending around (0, 0)
            return zr_boost_1_81::math::hypot(a[0], a[1])
                + zr_boost_1_81::math::hypot(b[0], b[1]);
        }
    }
    ...
    point move_position_toward(point a, double fraction, point b) const
    {
        ...
        {
            double distance_to_point_a = zr_boost_1_81::math::hypot(a[0], a[1]);
            double distance_to_point_b = zr_boost_1_81::math::hypot(b[0], b[1]);
