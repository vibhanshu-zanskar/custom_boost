...
private:
  ...
  {
    ar & zr_boost_1_81::serialization::base_object<inherited>(*this) 
...
{
  using zr_boost_1_81::parallel::inplace_all_to_all;
  ...
  for (zr_boost_1_81::tie(vi, vi_end) = vertices(*this); vi != vi_end; ++vi) {
  ...
  // an all-to-all transfer.
  for (zr_boost_1_81::tie(ei, ei_end) = edges(*this); ei != ei_end; ++ei) {
  ...
  if (has_stable_descriptors) {
    ...
    for (zr_boost_1_81::tie(vi, vi_end) = vertices(*this); vi != vi_end; /* in loop */) {
