...
namespace zr_boost_1_81 { namespace graph { namespace distributed {
...
struct mpi_process_group::impl
{
   ...
   */
  zr_boost_1_81::mpi::communicator             comm;

  /**
   * The MPI communicator used to transmit out-of-band replies.
   */
  zr_boost_1_81::mpi::communicator             oob_reply_comm;
                             ...
                             mpl::true_ /*is_mpi_datatype*/) const
{
  ...
  zr_boost_1_81::mpi::packed_oarchive oa(impl_->comm, outgoing.buffer);
  oa << value;
                             ...
                             mpl::false_ /*is_mpi_datatype*/) const
{
  ...
  zr_boost_1_81::mpi::packed_oarchive out(impl_->comm, outgoing.buffer);
  out << value;
     ...
     int tag, const T& value)
{
  pg.send_impl(dest, pg.encode_tag(pg.my_block_number(), tag), value,
               zr_boost_1_81::mpi::is_mpi_datatype<T>());
}

template<typename T>
typename enable_if<zr_boost_1_81::mpi::is_mpi_datatype<T>, void>::type
send(const mpi_process_group& pg, mpi_process_group::process_id_type dest,
     int tag, const T values[], std::size_t n)
{
  pg.send_impl(dest, pg.encode_tag(pg.my_block_number(), tag),
                 zr_boost_1_81::serialization::make_array(values,n), 
                 zr_boost_1_81::mpl::true_());
}

template<typename T>
typename disable_if<zr_boost_1_81::mpi::is_mpi_datatype<T>, void>::type
mpi_process_group::
                                ...
                                mpl::true_ /*is_mpi_datatype*/) const
{
  ...
  // Unpack the data
  if (header->bytes > 0) {
    zr_boost_1_81::mpi::packed_iarchive ia(impl_->comm, incoming.buffer, 
                                ...
                                mpl::false_ /*is_mpi_datatype*/) const
{
  ...
  zr_boost_1_81::mpi::packed_iarchive in(impl_->comm, incoming.buffer, 
                                 archive::no_header, header->offset);
  in >> value;
...
template<typename T>
typename disable_if<zr_boost_1_81::mpi::is_mpi_datatype<T>, bool>::type
mpi_process_group::
...
{
  using zr_boost_1_81::mpi::get_mpi_datatype;
    ...
    int source, int tag, Type& data, mpl::false_ /*is_mpi_datatype*/) 
{
  ...
  zr_boost_1_81::mpi::status status;
  status = self.impl_->comm.probe(source, tag);

#if BOOST_VERSION >= 103600
  int size = status.count<zr_boost_1_81::mpi::packed>().get();
#else
  int size;
  ...
  zr_boost_1_81::mpi::packed_iarchive in(self.impl_->comm);
  in.resize(size);
...
void do_oob_receive(mpi_process_group const& self, int source, int tag, Type& data) 
{
  do_oob_receive(self, source, tag, data,
                           zr_boost_1_81::mpi::is_mpi_datatype<Type>());
}
        ...
        trigger_receive_context context, int block) const
{
  ...
  if (context == trc_out_of_band) {
    ...
    zr_boost_1_81::graph::distributed::receive(self, source, tag, data);
        ...
        trigger_receive_context context, int block) const
{
  ...
  zr_boost_1_81::parallel::detail::untracked_pair<int, Type> data;
        ...
        trigger_receive_context context, int block) const
{
  ...
  zr_boost_1_81::mpi::packed_iarchive ia(self.impl_->comm,self.impl_->buffers[tag]);
  ia >> data;
  // Start a new receive
...
receive(const mpi_process_group& pg, int tag, T& value)
{
  for (std::size_t source = 0; source < pg.impl_->incoming.size(); ++source) {
    if (pg.receive_impl(source, pg.encode_tag(pg.my_block_number(), tag),
                        value, zr_boost_1_81::mpi::is_mpi_datatype<T>()))
      return source;
  }
  BOOST_ASSERT (false);
}
...
template<typename T>
typename 
  enable_if<zr_boost_1_81::mpi::is_mpi_datatype<T>, 
...
template<typename T>
typename 
  disable_if<zr_boost_1_81::mpi::is_mpi_datatype<T>, 
             std::pair<mpi_process_group::process_id_type, std::size_t> >::type
receive(const mpi_process_group& pg, int tag, T values[], std::size_t n)
{
  for (std::size_t source = 0; source < pg.impl_->incoming.size(); ++source) {
           ...
           BinaryOperation bin_op)
{
  ...
  zr_boost_1_81::mpi::all_reduce(zr_boost_1_81::mpi::communicator(communicator(pg),
                                                  zr_boost_1_81::mpi::comm_attach), 
...
{
  // broadcast the seed  
  zr_boost_1_81::mpi::communicator comm(communicator(pg),zr_boost_1_81::mpi::comm_attach);
  zr_boost_1_81::mpi::broadcast(comm,val,root);
}
           ...
           BinaryOperation bin_op)
{
  ...
  zr_boost_1_81::mpi::scan(communicator(pg), first, last-first, out, bin_op);
...
{
/*
  zr_boost_1_81::mpi::group current_group = communicator(pg).group();
  zr_boost_1_81::mpi::group new_group = current_group.include(first,last);
  zr_boost_1_81::mpi::communicator new_comm(communicator(pg),new_group);
...
*/
  ...
  if (new_comm != MPI_COMM_NULL) {
    mpi_process_group result_pg(zr_boost_1_81::mpi::communicator(new_comm,zr_boost_1_81::mpi::comm_attach));
...
template<typename T>
typename enable_if<zr_boost_1_81::mpi::is_mpi_datatype<T> >::type
            ...
            mpi_process_group::process_id_type source, int tag, T& value, int block)
{
  ...
  std::pair<zr_boost_1_81::mpi::communicator, int> actual
...
template<typename T>
typename disable_if<zr_boost_1_81::mpi::is_mpi_datatype<T> >::type
receive_oob(const mpi_process_group& pg, 
            mpi_process_group::process_id_type source, int tag, T& value, int block)
{
  // Determine the actual message we expect to receive, and which
  ...
  zr_boost_1_81::optional<zr_boost_1_81::mpi::status> status;
  do {
  ...
  zr_boost_1_81::mpi::packed_iarchive in(actual.first);

#if BOOST_VERSION >= 103600
  in.resize(status->count<zr_boost_1_81::mpi::packed>().get());
#else
  int size;
  MPI_Status mpi_status = *status;
  MPI_Get_count(&mpi_status, MPI_PACKED, &size);
...
typename enable_if<zr_boost_1_81::mpi::is_mpi_datatype<ReplyT> >::type
                    ...
                    int block)
{
  ...
  send_oob(pg, dest, tag, zr_boost_1_81::parallel::detail::make_untracked_pair(
        (int)reply_tag, send_value), block);
  receive_oob(pg, dest, reply_tag, reply_value);
}

template<typename SendT, typename ReplyT>
typename disable_if<zr_boost_1_81::mpi::is_mpi_datatype<ReplyT> >::type
send_oob_with_reply(const mpi_process_group& pg, 
                    mpi_process_group::process_id_type dest,
                    int tag, const SendT& send_value, ReplyT& reply_value,
                    int block)
{
  ...
  send_oob(pg, dest, tag, 
           zr_boost_1_81::parallel::detail::make_untracked_pair((int)reply_tag, 
                                                        send_value), block);
  receive_oob(pg, dest, reply_tag, reply_value);
}

} } } // end namespace zr_boost_1_81::graph::distributed
