...
using zr_boost_1_81::parallel::trigger_receive_context;
using zr_boost_1_81::detail::parallel::pair_with_property;
...
public:
   ...
   *  function uses the @c zr_boost_1_81::hash value of the name, modulo @c p.
   */
                           ...
                           ProcessGroup, void>
{
  ...
  typedef typename mpl::if_<is_same<InDistribution, defaultS>,
                            zr_boost_1_81::parallel::variant_distribution<ProcessGroup, 
                                                                  VertexSize>,
  ...
  typedef typename mpl::if_<is_same<InDistribution, defaultS>,
                            zr_boost_1_81::parallel::block, type>::type
    default_type;
};
...
protected:
  ...
  zr_boost_1_81::parallel::detail::untracked_pair<vertex_descriptor, bool>
  handle_find_vertex(int source, int tag, const vertex_name_type& msg,
                     trigger_receive_context);

  template<typename U, typename V>
  void handle_add_edge(int source, int tag, const zr_boost_1_81::parallel::detail::untracked_pair<U, V>& msg,
                       trigger_receive_context);

  template<typename U, typename V>
  zr_boost_1_81::parallel::detail::untracked_pair<edge_descriptor, bool> 
  handle_add_edge_with_reply(int source, int tag, const zr_boost_1_81::parallel::detail::untracked_pair<U, V>& msg,
                             trigger_receive_context);
...
{
  using zr_boost_1_81::parallel::detail::make_untracked_pair;
  ...
  if (vertex_name_type* v_name = zr_boost_1_81::get<vertex_name_type>(&v)) {
    ...
    if (vertex_name_type* u_name = zr_boost_1_81::get<vertex_name_type>(&u))
      ...
      send(self.process_group(), self.distribution_(*v_name),
           ...
           make_untracked_pair(zr_boost_1_81::get<vertex_descriptor>(u), *v_name));
  } else {
    if (vertex_name_type* u_name = zr_boost_1_81::get<vertex_name_type>(&u))
      ...
      send(self.process_group(), self.distribution_(*u_name),
           ...
           make_untracked_pair(*u_name, zr_boost_1_81::get<vertex_descriptor>(v)));
    else
      ...
      add_edge(zr_boost_1_81::get<vertex_descriptor>(u), 
               zr_boost_1_81::get<vertex_descriptor>(v), 
               self.derived());
  }
}
...
BGL_NAMED_GRAPH::lazy_add_edge::commit() const
{
  ...
  if (const vertex_name_type* u_name = zr_boost_1_81::get<vertex_name_type>(&u)) {
    /// We haven't resolved the source vertex to a descriptor yet, so
    ...
    if (const vertex_name_type* v_name = zr_boost_1_81::get<vertex_name_type>(&v))
    ...
    u_owner 
      = get(vertex_owner, self.derived(),
            zr_boost_1_81::get<vertex_descriptor>(u));
    if (u_owner == rank) {
      ...
      return add_edge(zr_boost_1_81::get<vertex_descriptor>(u), 
                      zr_boost_1_81::get<vertex_descriptor>(v), 
                      self.derived());
    } else {
...
{
  using zr_boost_1_81::detail::parallel::make_pair_with_property;
  ...
  if (vertex_name_type* v_name = zr_boost_1_81::get<vertex_name_type>(&v)) {
    ...
    if (vertex_name_type* u_name = zr_boost_1_81::get<vertex_name_type>(&u))
      ...
      send(self.process_group(), self.distribution_(*v_name),
           ...
           make_pair_with_property(zr_boost_1_81::get<vertex_descriptor>(u), *v_name, 
                                   property));
  } else {
    if (vertex_name_type* u_name = zr_boost_1_81::get<vertex_name_type>(&u))
      // We haven't resolved the source vertex to a descriptor yet, so
      ...
      send(self.process_group(), self.distribution_(*u_name),
           ...
           make_pair_with_property(*u_name, zr_boost_1_81::get<vertex_descriptor>(v), 
                                   property));
    else
      // We have descriptors for both of the vertices, either of which
      // may be remote or local. Tell the owner of the source vertex
...
{
  using zr_boost_1_81::graph::parallel::simple_trigger;
            ...
            const BGL_NAMED_GRAPH& g)
{
  ...
  if (owner == process_id(g.process_group())) {
    ...
    zr_boost_1_81::parallel::detail::untracked_pair<vertex_descriptor, bool> result;
...
template<BGL_NAMED_GRAPH_PARAMS>
zr_boost_1_81::parallel::detail::untracked_pair<typename BGL_NAMED_GRAPH::vertex_descriptor, bool>
BGL_NAMED_GRAPH::
handle_find_vertex(int source, int /*tag*/, const vertex_name_type& msg, 
                   trigger_receive_context)
{
...
void
BGL_NAMED_GRAPH::
handle_add_edge(int source, int /*tag*/, const zr_boost_1_81::parallel::detail::untracked_pair<U, V>& msg, 
                trigger_receive_context)
{
...
template<BGL_NAMED_GRAPH_PARAMS>
template<typename U, typename V>
zr_boost_1_81::parallel::detail::untracked_pair<typename BGL_NAMED_GRAPH::edge_descriptor, bool>
BGL_NAMED_GRAPH::
handle_add_edge_with_reply(int source, int /*tag*/, const zr_boost_1_81::parallel::detail::untracked_pair<U, V>& msg,
                           trigger_receive_context)
{
