...
namespace timer
{
  ...
  typedef zr_boost_1_81::int_least64_t nanosecond_type;
...
//  cpu_timer  -------------------------------------------------------------------------//
  ...
  public:
    ...
    std::string   format(short places, const std::string& format) const
                        { return ::zr_boost_1_81::timer::format(elapsed(), places, format); }
    std::string   format(short places = default_places) const
                        { return ::zr_boost_1_81::timer::format(elapsed(), places); }
    //  actions
