...
private:
    ...
    static inline void analyze_point_coordinates(MultiPoint const& multipoint,
                                                 ...
                                                 OutputIterator oit)
    {
        typedef typename zr_boost_1_81::range_value<MultiPoint>::type point_type;
        ...
        for (iterator_type it = zr_boost_1_81::begin(multipoint);
             it != zr_boost_1_81::end(multipoint);
             ++it)
        {
            point_type point;
                                    ...
                                    Value& max_gap_right)
    {
        ...
        iterator_type it1 = zr_boost_1_81::begin(sorted_range), it2 = it1;
        ++it2;
        max_gap_left = geometry::get<0>(*it1);
        max_gap_right = geometry::get<0>(*it2);

        Value max_gap = max_gap_right - max_gap_left;
        for (++it1, ++it2; it2 != zr_boost_1_81::end(sorted_range); ++it1, ++it2)
        {
                                              ...
                                              CoordinateType& lon_max)
    {
        ...
        std::pair<iterator_type, iterator_type> min_max_longitudes
            = zr_boost_1_81::minmax_element(zr_boost_1_81::begin(range),
        ...
        {
            std::sort(zr_boost_1_81::begin(range), zr_boost_1_81::end(range), lon_less);
                                             ...
                                             CoordinateType& lat_max)
    {
        ...
        else
        {
            std::pair<Iterator, Iterator> min_max_latitudes
                = zr_boost_1_81::minmax_element(first, last, lat_less);
...
public:
    ...
    static inline void apply(MultiPoint const& multipoint, Box& mbr)
    {
        ...
        if (zr_boost_1_81::empty(multipoint))
        {
        ...
        if (zr_boost_1_81::size(multipoint) == 1)
        {
        ...
        iterator_type it = zr_boost_1_81::begin(multipoint);
        geometry::detail::envelope::envelope_one_point<2, dimension<Box>::value>::apply(*it, mbr);

        for (++it; it != zr_boost_1_81::end(multipoint); ++it)
        {
