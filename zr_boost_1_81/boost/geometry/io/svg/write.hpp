...
struct svg_range
{
    ...
    {
        typedef typename zr_boost_1_81::range_iterator<Range const>::type iterator;

        bool first = true;

        os << "<" << Policy::prefix() << " points=\"";

        for (iterator it = zr_boost_1_81::begin(range);
...
struct svg_poly
{
        ...
        Polygon const& polygon, std::string const& style, double)
    {
        ...
        typedef typename zr_boost_1_81::range_iterator<ring_type const>::type iterator_type;

        bool first = true;
        os << "<g fill-rule=\"evenodd\"><path d=\"";

        ring_type const& ring = geometry::exterior_ring(polygon);
        for (iterator_type it = zr_boost_1_81::begin(ring);
            it != zr_boost_1_81::end(ring);
            ++it, first = false)
        {
            os << (first ? "M" : " L") << " "
                << geometry::get<0>(*it)
        ...
        // Inner rings:
        {
            ...
            for (typename detail::interior_iterator<Polygon const>::type
                    rit = zr_boost_1_81::begin(rings); rit != zr_boost_1_81::end(rings); ++rit)
            {
                first = true;
                for (typename detail::interior_ring_iterator<Polygon const>::type
                        it = zr_boost_1_81::begin(*rit); it != zr_boost_1_81::end(*rit);
                    ++it, first = false)
                {
                    os << (first ? "M" : " L") << " "
                        << geometry::get<0>(*it)
...
struct svg_multi
{
    ...
    {
        for (typename zr_boost_1_81::range_iterator<MultiGeometry const>::type
                    it = zr_boost_1_81::begin(multi);
    ...
    : detail::svg::svg_multi
        <
            ...
            detail::svg::svg_point
                <
                    typename zr_boost_1_81::range_value<MultiPoint>::type
                >
    ...
    : detail::svg::svg_multi
        <
            ...
            detail::svg::svg_range
                <
                    typename zr_boost_1_81::range_value<MultiLinestring>::type,
...
struct svg<MultiPolygon, multi_polygon_tag>
    : detail::svg::svg_multi
        <
            ...
            detail::svg::svg_poly
                <
                    typename zr_boost_1_81::range_value<MultiPolygon>::type
                >
...
struct devarianted_svg<variant<BOOST_VARIANT_ENUM_PARAMS(T)> >
{
    ...
    {
        zr_boost_1_81::apply_visitor(visitor<OutputStream>(os, style, size), geometry);
    }
};
