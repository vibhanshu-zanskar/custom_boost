...
// Silence warning C4512: 'zr_boost_1_81::geometry::wkt_manipulator<Geometry>' : assignment operator could not be generated
#if defined(_MSC_VER)
#pragma warning(push)  
...
struct wkt_range
{
                ...
                Range const& range, bool force_closure = ForceClosurePossible)
    {
        ...
        iterator_type begin = zr_boost_1_81::begin(range);
...
struct wkt_multi
{
                ...
                Multi const& geometry, bool force_closure)
    {
        ...
        for (typename zr_boost_1_81::range_iterator<Multi const>::type
                    it = zr_boost_1_81::begin(geometry);
            it != zr_boost_1_81::end(geometry);
            ++it)
        {
            if (it != zr_boost_1_81::begin(geometry))
            {
                os << ",";
            }
...
struct wkt_segment
{
                ...
                Segment const& segment, bool)
    {
        ...
        typedef zr_boost_1_81::array<point_type, 2> sequence;
...
private:
    ...
    static void output_or_recursive_call(OutputStream& os, Geom const& geom, bool force_closure)
    {
        ...
        bool first = true;
        auto const end = zr_boost_1_81::end(geom);
        for (auto it = zr_boost_1_81::begin(geom); it != end; ++it)
        {
            if (first)
                first = false;
            else
...
{
    std::stringstream ss;
    ss << zr_boost_1_81::geometry::wkt(geometry);
    return ss.str();
}
