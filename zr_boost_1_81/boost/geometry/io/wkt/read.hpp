...
typedef zr_boost_1_81::tokenizer<zr_boost_1_81::char_separator<char> > tokenizer;
...
struct parsing_assigner
{
                             ...
                             std::string const& wkt)
    {
        ...
        catch(zr_boost_1_81::bad_lexical_cast const& blc)
        {
...
{
    typedef typename geometry::point_type<Geometry>::type point_type;
    typedef typename zr_boost_1_81::range_size
        <
...
inline bool one_of(tokenizer::iterator const& it,
                   ...
                   bool& is_present)
{
    if (zr_boost_1_81::iequals(*it, value))
    {
        is_present = true;
        return true;
    }
...
{
    if (it == end || ! zr_boost_1_81::iequals(*it++, geometry_name))
    {
...
struct geometry_parser
{
    static inline void apply(std::string const& wkt, Geometry& geometry)
    {
        ...
        tokenizer tokens(wkt, zr_boost_1_81::char_separator<char>(" ", ",()"));
...
struct multi_parser
{
                             ...
                             MultiGeometry& geometry)
    {
        if (initialize<MultiGeometry>(it, end, wkt, PrefixPolicy::apply()))
        {
            ...
            while(it != end && *it != ")")
            {
                traits::resize<MultiGeometry>::apply(geometry, zr_boost_1_81::size(geometry) + 1);
                Parser
                    <
                        typename zr_boost_1_81::range_value<MultiGeometry>::type
                    >::apply(it, end, wkt, *(zr_boost_1_81::end(geometry) - 1));
                if (it != end && *it == ",")
                {
...
struct box_parser
{
                             ...
                             Box& box)
    {
        ...
        if (it != end && zr_boost_1_81::iequals(*it, "POLYGON"))
        {
            ++it;
            bool has_empty, has_z, has_m;
            handle_empty_z_m(it, end, has_empty, has_z, has_m);
        ...
        else if (it != end && zr_boost_1_81::iequals(*it, "BOX"))
        {
            ++it;
        }
        else
        ...
        std::size_t n = zr_boost_1_81::size(points);
        if (n == 2)
        {
            index = 1;
        }
...
struct segment_parser
{
    ...
    static inline void apply(tokenizer::iterator& it,
                             ...
                             Segment& segment)
    {
        if (it != end && (zr_boost_1_81::iequals(*it, "SEGMENT") || zr_boost_1_81::iequals(*it, "LINESTRING")))
        {
            ++it;
        }
        else
        ...
        if (zr_boost_1_81::size(points) == 2)
        {
...
struct dynamic_readwkt_caller
{
                             ...
                             Geometry& geometry)
    {
        ...
        else if (zr_boost_1_81::iequals(*it, "MULTIPOLYGON"))
        {
            parse_geometry<util::is_multi_polygon>("MULTIPOLYGON", it, end, wkt, geometry);
        }
        else if (zr_boost_1_81::iequals(*it, "GEOMETRYCOLLECTION"))
        {
...
struct read_wkt<DynamicGeometry, dynamic_geometry_tag>
{
    ...
    {
        detail::wkt::tokenizer tokens(wkt, zr_boost_1_81::char_separator<char>(" ", ",()"));
