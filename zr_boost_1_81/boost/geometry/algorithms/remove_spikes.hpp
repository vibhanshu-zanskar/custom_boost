...
struct range_remove_spikes
{
    ...
    static inline void apply(Range& range, SideStrategy const& strategy)
    {
        ...
        std::size_t n = zr_boost_1_81::size(range);
        ...
        for (typename zr_boost_1_81::range_iterator<Range const>::type it = zr_boost_1_81::begin(range);
            it != zr_boost_1_81::end(range); ++it)
        {
            // Add point
...
struct multi_remove_spikes
{
    ...
    {
        for (typename zr_boost_1_81::range_iterator<MultiGeometry>::type
...
struct remove_spikes<zr_boost_1_81::variant<BOOST_VARIANT_ENUM_PARAMS(T)> >
{
    template <typename Strategy>
    struct visitor: zr_boost_1_81::static_visitor<void>
    {
    ...
    template <typename Strategy>
    static inline void apply(zr_boost_1_81::variant<BOOST_VARIANT_ENUM_PARAMS(T)>& geometry,
                             Strategy const& strategy)
    {
        zr_boost_1_81::apply_visitor(visitor<Strategy>(strategy), geometry);
    }
};
