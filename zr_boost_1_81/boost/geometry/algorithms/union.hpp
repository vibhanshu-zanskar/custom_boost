    ...
    >
{
    ...
    using multi_polygon_t = typename util::sequence_find_if
        ...
        >::type;
    using tuple_out_t = zr_boost_1_81::tuple<multi_point_t, multi_linestring_t, multi_polygon_t>;
                             ...
                             Strategy const& strategy)
    {
        ...
        detail::gc_group_elements(gc1_view, gc2_view, strategy,
            [&](auto const& inters_group)
            {
                ...
                detail::intersection::gc_move_multi_back(geometry_out, zr_boost_1_81::get<0>(out));
                detail::intersection::gc_move_multi_back(geometry_out, zr_boost_1_81::get<1>(out));
                detail::intersection::gc_move_multi_back(geometry_out, zr_boost_1_81::get<2>(out));
                return true;
            },
...
private:
                                   ...
                                   tuple_out_t& out)
    {
        ...
        // L = L \ A
        {
            ...
            subtract_greater_topodim(zr_boost_1_81::get<1>(out), zr_boost_1_81::get<2>(out), l, strategy);
            zr_boost_1_81::get<1>(out) = std::move(l);
        }
        // P = P \ A
        {
            multi_point_t p;
            subtract_greater_topodim(zr_boost_1_81::get<0>(out), zr_boost_1_81::get<2>(out), p, strategy);
            zr_boost_1_81::get<0>(out) = std::move(p);
        }
        // P = P \ L
        {
            multi_point_t p;
            subtract_greater_topodim(zr_boost_1_81::get<0>(out), zr_boost_1_81::get<1>(out), p, strategy);
            zr_boost_1_81::get<0>(out) = std::move(p);
        }
        */
    }
    ...
    {
        multi_point_t p;
        union_<multi_point_t, G, multi_point_t>::apply(zr_boost_1_81::get<0>(out), g, p, strategy);
        zr_boost_1_81::get<0>(out) = std::move(p);
    }
    ...
    {
        multi_linestring_t l;
        union_<multi_linestring_t, G, multi_linestring_t>::apply(zr_boost_1_81::get<1>(out), g, l, strategy);
        zr_boost_1_81::get<1>(out) = std::move(l);
    }
    ...
    {
        multi_polygon_t a;
        union_<multi_polygon_t, G, multi_polygon_t>::apply(zr_boost_1_81::get<2>(out), g, a, strategy);
        zr_boost_1_81::get<2>(out) = std::move(a);
    }
...
struct union_
{
                             ...
                             Strategy const& strategy)
    {
        concepts::check<Geometry1 const>();
        concepts::check<Geometry2 const>();
        //concepts::check<typename zr_boost_1_81::range_value<Collection>::type>();
