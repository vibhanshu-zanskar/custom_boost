                             ...
                             PointInGeometryStrategy const& strategy)
{
    ...
    if (zr_boost_1_81::size(original.m_sections) == 0
        || zr_boost_1_81::size(original.m_ring) - zr_boost_1_81::size(original.m_sections) < 16)
    {
    ...
    typedef typename zr_boost_1_81::range_iterator<sections_type const>::type iterator_type;
    typedef typename zr_boost_1_81::range_value<sections_type const>::type section_type;
    typedef typename geometry::coordinate_type<Point>::type coordinate_type;

    coordinate_type const point_x = geometry::get<0>(point);

    // Walk through all monotonic sections of this original
    for (iterator_type it = zr_boost_1_81::begin(original.m_sections);
        it != zr_boost_1_81::end(original.m_sections);
        ++it)
    {
            ...
            && point_x <= geometry::get<max_corner, 0>(section.bounding_box))
        {
            ...
            if (! point_in_section(strategy, state, point, point_x,
                    zr_boost_1_81::begin(original.m_ring) + section.begin_index,
                    zr_boost_1_81::begin(original.m_ring) + section.end_index + 1,
...
public:
    ...
    {
        if (zr_boost_1_81::empty(original.m_ring))
        {
            // Skip empty rings
            return true;
        }
