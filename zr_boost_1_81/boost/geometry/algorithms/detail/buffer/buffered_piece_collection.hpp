...
struct buffered_piece_collection
{
    ...
    inline void check_linear_endpoints(buffer_turn_info_type& turn) const
    {
        ...
        // much higher)
        for (typename zr_boost_1_81::range_iterator<std::vector<point_type> const>::type it
             = zr_boost_1_81::begin(m_linear_end_points);
             it != zr_boost_1_81::end(m_linear_end_points);
             ++it)
        {
    ...
    inline void deflate_check_turns()
    {
        ...
        for (typename zr_boost_1_81::range_iterator<turn_vector_type>::type it =
            zr_boost_1_81::begin(m_turns); it != zr_boost_1_81::end(m_turns); ++it)
        {
    ...
    inline void update_turn_administration()
    {
        std::size_t index = 0;
        for (typename zr_boost_1_81::range_iterator<turn_vector_type>::type it =
            zr_boost_1_81::begin(m_turns); it != zr_boost_1_81::end(m_turns); ++it, ++index)
        {
            buffer_turn_info_type& turn = *it;
    ...
    {
        for (typename piece_vector_type::iterator it = zr_boost_1_81::begin(m_pieces);
            it != zr_boost_1_81::end(m_pieces);
            ++it)
        {
            piece& pc = *it;
    ...
    inline void start_new_ring(bool deflate)
    {
        ...
        m_first_piece_index = static_cast<signed_size_type>(zr_boost_1_81::size(m_pieces));
        m_deflate = deflate;
        if (deflate)
        {
            ...
            buffered_ring<Ring>& ring)
    {
        ...
        BOOST_GEOMETRY_ASSERT(zr_boost_1_81::size(m_pieces) > 0);
        if (! ring.empty()
    ...
    inline bool finish_ring(strategy::buffer::result_code code)
    {
        ...
        signed_size_type const last_piece_index
                = static_cast<signed_size_type>(zr_boost_1_81::size(m_pieces)) - 1;

        if (first_piece_index < zr_boost_1_81::size(m_pieces))
        {
        ...
        for (std::size_t i = first_piece_index; i < zr_boost_1_81::size(m_pieces); i++)
        {
                            ...
                            bool is_interior, bool has_interiors)
    {
        ...
        if (! zr_boost_1_81::empty(input_ring))
        {
            ...
            for (auto it = zr_boost_1_81::begin(view); it != zr_boost_1_81::end(view); ++it)
            {
    ...
    {
        BOOST_GEOMETRY_ASSERT(zr_boost_1_81::size(offsetted_rings) > 0);
            ...
            bool decrease_segment_index_by_one)
    {
        ...
        piece pc;
        pc.type = type;
        pc.index = static_cast<signed_size_type>(zr_boost_1_81::size(m_pieces));
        ...
        std::size_t const n = zr_boost_1_81::size(offsetted_rings.back());
    ...
    inline void sectionalize(piece const& pc, buffered_ring<Ring> const& ring)
    {
        ...
        sectionalizer::apply(monotonic_sections,
            zr_boost_1_81::begin(ring) + pc.first_seg_id.segment_index,
            zr_boost_1_81::begin(ring) + pc.beyond_last_segment_index,
            m_robust_policy,
            m_strategy,
            ring_id, 10);
    }
    ...
    {
        BOOST_GEOMETRY_ASSERT(zr_boost_1_81::size(range) != 0u);

        typename Range::const_iterator it = zr_boost_1_81::begin(range);
        ...
        for (++it; it != zr_boost_1_81::end(range); ++it)
        {
            ...
            bool decrease_segment_index_by_one)
    {
        ...
        if (zr_boost_1_81::size(range) > 0u)
        {
    ...
    {
        BOOST_GEOMETRY_ASSERT(zr_boost_1_81::size(range) >= 2u);
    ...
    {
        zr_boost_1_81::ignore_unused(strategy);
    ...
    {
        for (typename zr_boost_1_81::range_iterator<turn_vector_type const>::type it =
            zr_boost_1_81::begin(m_turns); it != zr_boost_1_81::end(m_turns); ++it)
        {
    ...
    inline void discard_nonintersecting_deflated_rings()
    {
        for(typename buffered_ring_collection<buffered_ring<Ring> >::iterator it
            = zr_boost_1_81::begin(offsetted_rings);
            it != zr_boost_1_81::end(offsetted_rings);
            ++it)
        {
            buffered_ring<Ring>& ring = *it;
            if (! ring.has_intersections()
                && zr_boost_1_81::size(ring) > 0u
    ...
    inline void reverse()
    {
        for(typename buffered_ring_collection<buffered_ring<Ring> >::iterator it = zr_boost_1_81::begin(offsetted_rings);
            it != zr_boost_1_81::end(offsetted_rings);
            ++it)
        {
        ...
        for (typename zr_boost_1_81::range_iterator<buffered_ring_collection<Ring> >::type
                it = zr_boost_1_81::begin(traversed_rings);
                it != zr_boost_1_81::end(traversed_rings);
                ++it)
        {
            std::reverse(it->begin(), it->end());
        }
    ...
    inline OutputIterator assign(OutputIterator out) const
    {
        ...
        for(typename buffered_ring_collection<buffered_ring<Ring> >::const_iterator it = zr_boost_1_81::begin(offsetted_rings);
            it != zr_boost_1_81::end(offsetted_rings);
            ++it, ++index)
        {
            if (! it->has_intersections()
        ...
        index = 0;
        for (typename zr_boost_1_81::range_iterator<buffered_ring_collection<Ring> const>::type
                it = zr_boost_1_81::begin(traversed_rings);
                it != zr_boost_1_81::end(traversed_rings);
                ++it, ++index)
        {
