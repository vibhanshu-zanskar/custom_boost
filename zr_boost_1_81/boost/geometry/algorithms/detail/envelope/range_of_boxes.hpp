...
struct envelope_range_of_longitudes
{
                             ...
                             Longitude& lon_min, Longitude& lon_max)
    {
        ...
        {
            lon_min = std::min_element(zr_boost_1_81::begin(range),
                                       zr_boost_1_81::end(range),
                                       longitude_less<0>())->template get<0>();
            lon_max = std::max_element(zr_boost_1_81::begin(range),
                                       zr_boost_1_81::end(range),
...
struct envelope_range_of_boxes_by_expansion
{
    ...
    {
        typedef typename zr_boost_1_81::range_value<RangeOfBoxes>::type box_type;
        ...
        iterator_type it = zr_boost_1_81::begin(range_of_boxes);
        for (++it; it != zr_boost_1_81::end(range_of_boxes); ++it)
        {
...
struct envelope_range_of_boxes
{
    ...
    static inline void apply(RangeOfBoxes const& range_of_boxes, Box& mbr)
    {
        ...
        BOOST_GEOMETRY_ASSERT(! zr_boost_1_81::empty(range_of_boxes));

        iterator_type it_min = std::min_element(zr_boost_1_81::begin(range_of_boxes),
                                                zr_boost_1_81::end(range_of_boxes),
                                                latitude_less<min_corner>());
        iterator_type it_max = std::max_element(zr_boost_1_81::begin(range_of_boxes),
                                                zr_boost_1_81::end(range_of_boxes),
        ...
        for (iterator_type it = zr_boost_1_81::begin(range_of_boxes);
             it != zr_boost_1_81::end(range_of_boxes);
             ++it)
        {
