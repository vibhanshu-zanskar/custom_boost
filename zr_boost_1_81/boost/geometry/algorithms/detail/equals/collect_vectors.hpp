...
struct collected_vector_cartesian
{
    ...
    bool normalize()
    {
        T magnitude = math::sqrt(zr_boost_1_81::numeric_cast<T>(dx * dx + dy * dy));
...
{
    typedef typename zr_boost_1_81::range_value<Collection>::type item_type;
...
private:
    ...
    {
        if (zr_boost_1_81::size(range) < 2)
        {
            return;
        }

        auto c_old_size = zr_boost_1_81::size(collection);
        bool is_first = true;
        auto it = zr_boost_1_81::begin(range);

        for (auto prev = it++; it != zr_boost_1_81::end(range); prev = it++)
        {
            typename zr_boost_1_81::range_value<Collection>::type v(*prev, *it);
        ...
        if (zr_boost_1_81::size(collection) > c_old_size + 1)
        {
...
struct box_collect_vectors
{
    ...
    static inline void apply(Collection& collection, Box const& box)
    {
        ...
        typedef typename zr_boost_1_81::range_value<Collection>::type item;
