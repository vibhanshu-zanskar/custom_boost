...
{
    typedef typename zr_boost_1_81::range_value<TurnPoints>::type turn_point_type;

    bool error = false;
    int index = 0;

    std::vector<meta_turn<turn_point_type> > meta_turns;
    for (typename zr_boost_1_81::range_iterator<TurnPoints const>::type
            it = zr_boost_1_81::begin(turn_points);
         it != zr_boost_1_81::end(turn_points);
         ++it, ++index)
    {
    ...
    int cycle = 0;
    for (typename zr_boost_1_81::range_iterator<std::vector<meta_turn<turn_point_type> > > ::type
            it = zr_boost_1_81::begin(meta_turns);
         it != zr_boost_1_81::end(meta_turns);
         ++it)
    {
