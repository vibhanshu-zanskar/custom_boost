...
struct copy_points<PointOut, MultiPointIn, multi_point_tag>
{
    ...
    static inline void apply(MultiPointIn const& multi_point_in,
                             OutputIterator& oit)
    {
        for (typename zr_boost_1_81::range_iterator<MultiPointIn const>::type
                 it = zr_boost_1_81::begin(multi_point_in);
             it != zr_boost_1_81::end(multi_point_in); ++it)
        {
            PointOut point_out;
...
struct multipoint_point_point
{
                                       ...
                                       Strategy const& strategy)
    {
        ...
        for (typename zr_boost_1_81::range_iterator<MultiPoint const>::type
                 it = zr_boost_1_81::begin(multipoint);
             it != zr_boost_1_81::end(multipoint); ++it)
        {
            action_selector_pl
                <
                    PointOut, OverlayType
...
struct multipoint_multipoint_point
{
                                       ...
                                       Strategy const& strategy)
    {
        ...
        if ( OverlayType != overlay_difference
             && zr_boost_1_81::size(multipoint1) > zr_boost_1_81::size(multipoint2) )
        {
        ...
        typedef typename zr_boost_1_81::range_value<MultiPoint2>::type point2_type;

        std::vector<point2_type> points2(zr_boost_1_81::begin(multipoint2),
                                         zr_boost_1_81::end(multipoint2));

        less_type const less = less_type();
        std::sort(points2.begin(), points2.end(), less);

        for (typename zr_boost_1_81::range_iterator<MultiPoint1 const>::type
                 it1 = zr_boost_1_81::begin(multipoint1);
             it1 != zr_boost_1_81::end(multipoint1); ++it1)
        {
