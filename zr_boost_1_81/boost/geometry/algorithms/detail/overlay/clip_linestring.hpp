...
// TODO: Temporary, this strategy should be moved, it is cartesian-only
    ...
    {
        if (!zr_boost_1_81::empty(line_out))
        {
            *out = line_out;
            ++out;
...
{
    if (zr_boost_1_81::begin(range) == zr_boost_1_81::end(range))
    {
        return out;
    }
    ...
    iterator_type vertex = zr_boost_1_81::begin(range);
    for(iterator_type previous = vertex++;
            vertex != zr_boost_1_81::end(range);
            ++previous, ++vertex)
    {
        point_type p1, p2;
