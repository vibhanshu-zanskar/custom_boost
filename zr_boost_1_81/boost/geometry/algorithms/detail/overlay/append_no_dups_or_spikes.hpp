                                     ...
                                     RobustPolicy const& robust_policy)
{
    ...
    if ( zr_boost_1_81::size(range) == 1
      && points_equal_or_close(*(zr_boost_1_81::begin(range)), point, strategy,
                               robust_policy) )
    {
        return;
    }
    ...
    while(zr_boost_1_81::size(range) >= 3
            && point_is_spike_or_equal(point,
                *(zr_boost_1_81::end(range) - 3),
                *(zr_boost_1_81::end(range) - 2),
                strategy.side(), // TODO: Pass strategy?
                robust_policy))
    {
        // Use the Concept/traits, so resize and append again
        traits::resize<Range>::apply(range, zr_boost_1_81::size(range) - 2);
        traits::push_back<Range>::apply(range, point);
    }
}
                                ...
                                RobustPolicy const& robust_policy)
{
    ...
    // So we have to check the first potential duplicate differently
    if ( zr_boost_1_81::size(range) == 1
      && points_equal_or_close(*(zr_boost_1_81::begin(range)), point,
                               strategy,
                               robust_policy) )
    {
        return;
                                          ...
                                          RobustPolicy const& robust_policy)
{
    ...
    if (zr_boost_1_81::size(range) <= minsize)
    {
        return;
    }

    typedef typename zr_boost_1_81::range_iterator<Range>::type iterator_type;
...
// and then erase some number of points from the beginning of the Range
    ...
    {
        found = false;
        iterator_type first = zr_boost_1_81::begin(range);
        iterator_type second = first + 1;
        iterator_type ultimate = zr_boost_1_81::end(range) - 1;
                               ...
                               robust_policy))
        {
            ...
            if (BOOST_GEOMETRY_CONDITION(closed))
            {
                ...
                range::resize(range, zr_boost_1_81::size(range) - 1);
                // Add new closing point
                range::push_back(range, range::front(range));
            }
            found = true;
        }
    } while(found && zr_boost_1_81::size(range) > minsize);
}
