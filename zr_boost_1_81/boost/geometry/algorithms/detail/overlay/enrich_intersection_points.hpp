...
{
    std::sort(zr_boost_1_81::begin(operations),
            zr_boost_1_81::end(operations),
            less_by_segment_ratio
                <
                    Turns,
                    typename zr_boost_1_81::range_value<Operations>::type,
                    Geometry1, Geometry2,
                    RobustPolicy,
                    SideStrategy,
                    Reverse1, Reverse2
...
inline void enrich_assign(Operations& operations, Turns& turns,
                          bool check_turns)
{
    typedef typename zr_boost_1_81::range_value<Turns>::type turn_type;
    typedef typename turn_type::turn_operation_type op_type;
    typedef typename zr_boost_1_81::range_iterator<Operations>::type iterator_type;
    ...
    if (operations.size() > 0)
    {
        ...
        for (iterator_type it = zr_boost_1_81::begin(operations);
             it != zr_boost_1_81::end(operations); ++it)
        {
    ...
    {
        for (iterator_type it = zr_boost_1_81::begin(operations);
             it != zr_boost_1_81::end(operations);
             ++it)
        {
...
inline void enrich_adapt(Operations& operations, Turns& turns)
{
    typedef typename zr_boost_1_81::range_value<Turns>::type turn_type;
    typedef typename turn_type::turn_operation_type op_type;
    typedef typename zr_boost_1_81::range_value<Operations>::type indexed_turn_type;
    ...
    next_phase = false;
    for (typename zr_boost_1_81::range_iterator<Turns>::type
            it = zr_boost_1_81::begin(turns);
         it != zr_boost_1_81::end(turns);
         ++it)
    {
        turn_type& turn = *it;
    ...
    operations.erase(std::remove_if(zr_boost_1_81::begin(operations),
        zr_boost_1_81::end(operations), predicate), zr_boost_1_81::end(operations));
}
                       ...
                       IncludePolicy const& include_policy)
{
    ...
    typedef typename zr_boost_1_81::range_value<mapped_type>::type indexed_type;

    std::size_t index = 0;
    for (typename zr_boost_1_81::range_iterator<Turns const>::type
            it = zr_boost_1_81::begin(turns);
         it != zr_boost_1_81::end(turns);
         ++it, ++index)
    {
        ...
        for (typename zr_boost_1_81::range_iterator<container_type const>::type
                op_it = zr_boost_1_81::begin(turn.operations);
            op_it != zr_boost_1_81::end(turn.operations);
            ++op_it, ++op_index)
        {
...
{
    using turn_type = typename zr_boost_1_81::range_value<Turns>::type;
    using op_type = typename turn_type::turn_operation_type;

    typename op_type::comparable_distance_type const zero_distance = 0;

    for (auto it = zr_boost_1_81::begin(turns); it != zr_boost_1_81::end(turns); ++it)
    {
        turn_type& turn = *it;
