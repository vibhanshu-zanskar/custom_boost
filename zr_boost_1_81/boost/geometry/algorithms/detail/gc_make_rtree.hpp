...
inline auto gc_make_rtree_iterators(GC& gc, Strategy const& strategy)
{
    ...
    using iter_t = typename zr_boost_1_81::range_iterator<GC>::type;
    ...
    auto const size = zr_boost_1_81::size(gc);
...
inline auto gc_make_rtree_indexes(GCView const& gc, Strategy const& strategy)
{
    ...
    // TODO: get rid of the temporary vector
    std::size_t const size = zr_boost_1_81::size(gc);
    std::vector<value_t> values;
    values.reserve(size);

    auto const begin = zr_boost_1_81::begin(gc);
