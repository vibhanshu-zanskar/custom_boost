...
struct has_spikes
{
                                        ...
                                        bool is_linear)
    {
        ...
        typedef typename zr_boost_1_81::range_iterator<View const>::type iterator;

        iterator cur = zr_boost_1_81::begin(view);
        typename zr_boost_1_81::range_reverse_iterator
            <
                View const
            >::type prev = find_different_from_first(zr_boost_1_81::rbegin(view),
                                                     zr_boost_1_81::rend(view),
                                                     strategy);

        iterator next = find_different_from_first(cur, zr_boost_1_81::end(view),
                                                  strategy);
                             ...
                             Strategy const& strategy)
    {
        ...
        auto prev = zr_boost_1_81::begin(view);
        auto const end = zr_boost_1_81::end(view);

        auto cur = find_different_from_first(prev, zr_boost_1_81::end(view), strategy);
        if (cur == end)
        {
        ...
        auto next = find_different_from_first(cur, zr_boost_1_81::end(view), strategy);
        if (next == end)
        {
        ...
        while (next != end)
        {
            ...
            prev = cur;
            cur = next;
            next = find_different_from_first(cur, zr_boost_1_81::end(view), strategy);
        }
