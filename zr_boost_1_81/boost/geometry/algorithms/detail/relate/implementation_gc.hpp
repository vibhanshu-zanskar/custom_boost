...
struct gc_gc
{
    ...
    using mpo1_t = std::conditional_t
        <
            ...
            mpo1_found_t
        >;
    using tuple1_t = zr_boost_1_81::tuple<mpt1_t, mls1_t, mpo1_t>;
    ...
    using mpo2_t = std::conditional_t
        <
            ...
            mpo2_found_t
        >;
    using tuple2_t = zr_boost_1_81::tuple<mpt2_t, mls2_t, mpo2_t>;
            ...
            [&](auto const& inters_group)
            {
                ...
                for (auto const& id : inters_group)
                {
                    ...
                    if (id.source_id == 0)
                    {
                        ...
                        }, zr_boost_1_81::begin(gc1_view) + id.gc_id);
                    }
                    else
                    {
                        ...
                        }, zr_boost_1_81::begin(gc2_view) + id.gc_id);
                    }
                }
                ...
                // Helpers
                auto const& mpt1 = zr_boost_1_81::get<0>(tuple1);
                auto const& mls1 = zr_boost_1_81::get<1>(tuple1);
                auto const& mpo1 = zr_boost_1_81::get<2>(tuple1);
                auto const& mpt2 = zr_boost_1_81::get<0>(tuple2);
                auto const& mls2 = zr_boost_1_81::get<1>(tuple2);
                auto const& mpo2 = zr_boost_1_81::get<2>(tuple2);
...
private:
    ...
    static inline void merge_geometry(Tuple& tuple, Geometry const& geometry, Strategy const& strategy)
    {
        ...
        typename zr_boost_1_81::tuples::element<index, Tuple>::type temp_out;
        geometry::union_(zr_boost_1_81::get<index>(tuple), geometry, temp_out, strategy);
        zr_boost_1_81::get<index>(tuple) = std::move(temp_out);
    }

    template <typename Tuple, typename Strategy>
    static inline void subtract_elements(Tuple& tuple, Strategy const& strategy)
    {
        if (! geometry::is_empty(zr_boost_1_81::get<1>(tuple)))
        {
            if (! geometry::is_empty(zr_boost_1_81::get<2>(tuple)))
            {
                typename zr_boost_1_81::tuples::element<1, Tuple>::type mls;
                geometry::difference(zr_boost_1_81::get<1>(tuple), zr_boost_1_81::get<2>(tuple), mls, strategy);
                zr_boost_1_81::get<1>(tuple) = std::move(mls);
            }
        }
        if (! geometry::is_empty(zr_boost_1_81::get<0>(tuple)))
        {
            if (! geometry::is_empty(zr_boost_1_81::get<2>(tuple)))
            {
                typename zr_boost_1_81::tuples::element<0, Tuple>::type mpt;
                geometry::difference(zr_boost_1_81::get<0>(tuple), zr_boost_1_81::get<2>(tuple), mpt, strategy);
                zr_boost_1_81::get<0>(tuple) = std::move(mpt);
            }
            if (! geometry::is_empty(zr_boost_1_81::get<1>(tuple)))
            {
                typename zr_boost_1_81::tuples::element<0, Tuple>::type mpt;
                geometry::difference(zr_boost_1_81::get<0>(tuple), zr_boost_1_81::get<1>(tuple), mpt, strategy);
                zr_boost_1_81::get<0>(tuple) = std::move(mpt);
            }
        }
    }
