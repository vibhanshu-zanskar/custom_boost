...
private:
    void init() const
    {
        ...
        std::size_t count = zr_boost_1_81::size(m_ls);
...
private:
    ...
    void init() const
    {
        ...
        m_endpoints.reserve(zr_boost_1_81::size(m_mls) * 2);

        m_has_interior = false;

        typedef typename zr_boost_1_81::range_iterator<MultiLinestring const>::type ls_iterator;
        for ( ls_iterator it = zr_boost_1_81::begin(m_mls) ; it != zr_boost_1_81::end(m_mls) ; ++it )
        {
            typename zr_boost_1_81::range_reference<MultiLinestring const>::type
                ls = *it;

            std::size_t count = zr_boost_1_81::size(ls);
            ...
            if (count > 1)
            {
                typedef typename zr_boost_1_81::range_reference
                    <
                        typename zr_boost_1_81::range_value<MultiLinestring const>::type const
