...
public:
    ...
    {
        zr_boost_1_81::ignore_unused(pt);
#ifdef BOOST_GEOMETRY_DEBUG_RELATE_BOUNDARY_CHECKER
...
inline void copy_boundary_points_of_mls(MultiLinestring const& mls, Strategy const& strategy,
                                        Out & boundary_points)
{
    for (auto it = zr_boost_1_81::begin(mls); it != zr_boost_1_81::end(mls); ++it)
    {
...
public:
    ...
    bool is_endpoint_boundary(Point const& pt) const
    {
        ...
        auto const multi_count = zr_boost_1_81::size(m_geometry);
        ...
        std::size_t const equal_points_count = zr_boost_1_81::size(equal_range);
