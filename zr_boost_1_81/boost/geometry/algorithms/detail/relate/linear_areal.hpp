...
public:
    ...
    {
        std::size_t const count = zr_boost_1_81::size(linestring);
                                  ...
                                  Strategy const& strategy)
{
    ...
    BOOST_GEOMETRY_ASSERT(zr_boost_1_81::size(range1));
    std::size_t const s2 = zr_boost_1_81::size(range2);
    ...
    BOOST_GEOMETRY_ASSERT(p_seg_ij + 1 < zr_boost_1_81::size(range1));
    ...
    if (is_ip_qj)
    {
        ...
        range2_iterator qk_it = find_next_non_duplicated(zr_boost_1_81::begin(range2),
                                                         range::pos(range2, q_seg_jk),
                                                         zr_boost_1_81::end(range2),
                                                         strategy);
...
//       Some benchmarking should probably be done to check if only one std::sort should be used
            ...
            for (auto it = turns.begin() ; it != turns.end() ; )
            {
                ...
                prev_seg_id_ptr = zr_boost_1_81::addressof(it->operations[1].seg_id);
...
//       this way only the first enter will be handled
                        if ( !m_interior_detected )
                        {
                            ...
                            m_first_interior_other_id_ptr = zr_boost_1_81::addressof(other_id);
                        }
                    }
                }
...
// TODO: calculate_from_inside() is only needed if the current Linestring is not closed
            ...
            m_previous_turn_ptr = zr_boost_1_81::addressof(*it);
        ...
        void apply(Result & res,
                   ...
                   BoundaryChecker const& boundary_checker)
        {
            zr_boost_1_81::ignore_unused(first, last);
            //BOOST_GEOMETRY_ASSERT( first != last );
