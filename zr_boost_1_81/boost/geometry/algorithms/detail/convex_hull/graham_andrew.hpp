                         ...
                         Less const& less)
{
    ...
    in_proxy.for_each_range([&](auto const& range)
    {
        ...
        auto left_it = zr_boost_1_81::begin(range);
        auto right_it = zr_boost_1_81::begin(range);
                          ...
                          SideStrategy const& side)
{
    in_proxy.for_each_range([&](auto const& range)
    {
        ...
        for (auto it = zr_boost_1_81::begin(range); it != zr_boost_1_81::end(range); ++it)
        {
...
private:
    ...
    static void apply(InputProxy const& in_proxy, partitions& state, Strategy& strategy)
    {
        ...
        std::sort(zr_boost_1_81::begin(lower_points), zr_boost_1_81::end(lower_points), less);
        std::sort(zr_boost_1_81::begin(upper_points), zr_boost_1_81::end(upper_points), less);
    ...
    static inline void output_ranges(container_type const& first,
                                     ...
                                     bool closed)
    {
        std::copy(zr_boost_1_81::begin(first), zr_boost_1_81::end(first), out);

        BOOST_GEOMETRY_ASSERT(closed ? !zr_boost_1_81::empty(second) : zr_boost_1_81::size(second) > 1);
        std::copy(++zr_boost_1_81::rbegin(second), // skip the first Point
                  closed ? zr_boost_1_81::rend(second) : --zr_boost_1_81::rend(second), // skip the last Point if open
                  out);

        typedef typename zr_boost_1_81::range_size<container_type>::type size_type;
        size_type const count = zr_boost_1_81::size(first) + zr_boost_1_81::size(second) - 1;
