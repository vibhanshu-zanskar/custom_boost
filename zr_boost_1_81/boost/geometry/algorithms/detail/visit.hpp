    ...
    static bool apply(F function, Geom && geom)
    {
        using iter_t = std::conditional_t
            <
                ...
                std::move_iterator<typename zr_boost_1_81::range_iterator<Geom>::type>,
                typename zr_boost_1_81::range_iterator<Geom>::type
            >;

        std::deque<iter_t> queue;

        iter_t it = iter_t{ zr_boost_1_81::begin(geom) };
        iter_t end = iter_t{ zr_boost_1_81::end(geom) };
        for(;;)
        {
            for (; it != end; ++it)
            {
...
private:
    ...
    {
        first = Iterator{ zr_boost_1_81::begin(g) };
        last = Iterator{ zr_boost_1_81::end(g) };
    }
    template
    <
        typename Geom, typename Iterator,
