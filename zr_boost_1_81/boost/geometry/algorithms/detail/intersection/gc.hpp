...
{
    template <typename G>
    using is_same_as_single = std::is_same<G, typename zr_boost_1_81::range_value<Multi>::type>;
...
struct gc_can_convert_element
{
    ...
    using has_same_tag_as_single = std::is_same
        <
            ...
            typename geometry::tag<typename zr_boost_1_81::range_value<Multi>::type>::type
        >;
    using gc_types = typename traits::geometry_types<GC>::type;
...
{
    range::emplace_back(gc, std::move(*zr_boost_1_81::begin(multi)));
}
...
inline void gc_move_one_elem_multi_back(GC& gc, Multi&& multi)
{
    ...
    geometry::convert(*zr_boost_1_81::begin(multi), single_out);
...
private:
    ...
    {
        auto const& multi_result = zr_boost_1_81::get<Index>(inters_result);
        auto& multi_out = zr_boost_1_81::get<Index>(out);
        if (! zr_boost_1_81::empty(multi_result))
        {
    ...
    static void merge_two(Out const& g1, Out const& g2, Out& out, Strategy const& strategy)
    {
        ...
        geometry::dispatch::intersection_insert
            <
                Out, Out, typename zr_boost_1_81::range_value<Out>::type,
                overlay_union
            >::apply(g1,
                     g2,
                     robust_policy,
