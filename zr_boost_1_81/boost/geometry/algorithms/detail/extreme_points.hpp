...
struct extreme_points_on_ring
{
    ...
    typedef typename zr_boost_1_81::range_iterator<Ring const>::type range_iterator;
    ...
    {
        std::size_t const n = zr_boost_1_81::size(ring);
    ...
    {
        if (zr_boost_1_81::size(extremes) < 3)
        {
            return;
        }
        coordinate_type const min_value = geometry::get<Dimension>(*std::min_element(zr_boost_1_81::begin(extremes), zr_boost_1_81::end(extremes), compare<Dimension>()));

        // Also select left/right (if Dimension=1)
        coordinate_type const other_min = geometry::get<1 - Dimension>(*std::min_element(zr_boost_1_81::begin(extremes), zr_boost_1_81::end(extremes), compare<1 - Dimension>()));
        coordinate_type const other_max = geometry::get<1 - Dimension>(*std::max_element(zr_boost_1_81::begin(extremes), zr_boost_1_81::end(extremes), compare<1 - Dimension>()));
        ...
        while (left != right && defensive_check_index < n)
        {
            ...
            if (coordinate > min_value && other_coordinate > other_min && other_coordinate < other_max)
            {
                ...
                if (first_side != 1 && last_side != 1)
                {
                    ...
                    typename zr_boost_1_81::range_value<Intruders>::type intruder;
    ...
    static inline bool right_turn(Ring const& ring, Iterator it, SideStrategy const& strategy)
    {
        typename std::iterator_traits<Iterator>::difference_type const index
            = std::distance(zr_boost_1_81::begin(ring), it);
        geometry::ever_circling_range_iterator<Ring const> left(ring);
        geometry::ever_circling_range_iterator<Ring const> right(ring);
        left += index;
        right += index;
...
//std::cout << "Candidate at " << geometry::wkt(*it) << " first=" << first_side << " last=" << last_side << std::endl;
                             ...
                             SideStrategy const& strategy)
    {
        ...
        range_iterator max_it = zr_boost_1_81::begin(ring);
        compare<Dimension> smaller;
        for (range_iterator it = max_it + 1; it != zr_boost_1_81::end(ring); ++it)
        {
        ...
        if (max_it == zr_boost_1_81::end(ring))
        {
            return false;
        }

        typename std::iterator_traits<range_iterator>::difference_type const
            index = std::distance(zr_boost_1_81::begin(ring), max_it);
...
struct extreme_points<Polygon, Dimension, polygon_tag>
{
                             ...
                             SideStrategy const& strategy)
    {
        ...
        for (typename detail::interior_iterator<Polygon const>::type
                it = zr_boost_1_81::begin(rings); it != zr_boost_1_81::end(rings); ++it)
        {
...
struct extreme_points<MultiPolygon, Dimension, multi_polygon_tag>
{
                             ...
                             Intruders& intruders, SideStrategy const& strategy)
    {
        ...
        if (zr_boost_1_81::size(multi) >= 1)
        {
            return extreme_points
                <
                    typename zr_boost_1_81::range_value<MultiPolygon const>::type,
                    Dimension,
                    polygon_tag
                >::apply(*zr_boost_1_81::begin(multi), extremes, intruders, strategy);
        }
                           ...
                           SideStrategy const& strategy)
{
    ...
    concepts::check<typename zr_boost_1_81::range_value<Extremes>::type>();
    ...
    concepts::check
        <
            typename zr_boost_1_81::range_value
                <
                    typename zr_boost_1_81::range_value<Intruders>::type
                >::type
            const
        >();
