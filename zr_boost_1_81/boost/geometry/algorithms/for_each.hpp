...
struct fe_point_type_is_referencable
{
    static const bool value =
     ...
     || std::is_same
            <
                typename zr_boost_1_81::range_reference<Range>::type,
...
struct fe_point_range
{
    ...
    static inline bool apply(Range& range, Functor&& f)
    {
        ...
        for (auto it = zr_boost_1_81::begin(range); it != end; ++it)
        {
...
struct fe_segment_range_with_closure<open>
{
    ...
    static inline bool apply(Range& range, Functor&& f)
    {
        ...
        auto const begin = zr_boost_1_81::begin(range);
        auto end = zr_boost_1_81::end(range);
        if (begin == end)
        {
            return true;
        }
...
struct for_each_polygon
{
    ...
    static inline bool apply(Polygon& poly, Functor&& f)
    {
        ...
        auto const end = zr_boost_1_81::end(rings);
        for (auto it = zr_boost_1_81::begin(rings); it != end; ++it)
        {
...
struct for_each_multi
{
    ...
    {
        auto const end = zr_boost_1_81::end(multi);
        for (auto it = zr_boost_1_81::begin(multi); it != end; ++it)
        {
            // NOTE: Currently lvalue iterator required
