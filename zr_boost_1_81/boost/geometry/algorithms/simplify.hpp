...
class douglas_peucker
{
                                        ...
                                        PSDistanceStrategy const& ps_distance_strategy)
    {
        ...
        typedef typename zr_boost_1_81::range_value<Range>::type point_type;
        typedef douglas_peucker_point<point_type> dp_point_type;

        // Copy coordinates, a vector of references to all points
        std::vector<dp_point_type> ref_candidates(zr_boost_1_81::begin(range),
                                                  zr_boost_1_81::end(range));
        ...
        consider(zr_boost_1_81::begin(ref_candidates), zr_boost_1_81::end(ref_candidates), max_distance, n,
                 ps_distance_strategy);

        // Copy included elements to the output
        for (auto it = zr_boost_1_81::begin(ref_candidates); it != zr_boost_1_81::end(ref_candidates); ++it)
        {
            if (it->included)
            {
...
{
    return zr_boost_1_81::size(range) == 2
...
struct simplify_range_insert
{
                             ...
                             Strategies const& strategies)
    {
        ...
        {
            std::copy(zr_boost_1_81::begin(range), zr_boost_1_81::begin(range) + 1, out);
        }
        else if (zr_boost_1_81::size(range) <= 2 || max_distance < 0)
        {
            std::copy(zr_boost_1_81::begin(range), zr_boost_1_81::end(range), out);
        }
        else
        {
...
struct simplify_copy
{
    ...
    static inline void apply(RangeIn const& range, RangeOut& out,
                             ...
                             Strategies const& )
    {
        std::copy(zr_boost_1_81::begin(range), zr_boost_1_81::end(range),
...
struct simplify_range
{
                             ...
                             Strategies const& strategies)
    {
        ...
        if (zr_boost_1_81::size(range) <= MinimumToUseStrategy || max_distance < 0)
        {
...
private :
                                    ...
                                    Strategies const& strategies)
    {
        ...
        std::size_t i = 0;
        for (auto it = zr_boost_1_81::begin(ring); it != zr_boost_1_81::end(ring); ++it, ++i)
        {
...
public :
    ...
    {
        std::size_t const size = zr_boost_1_81::size(ring);
        if (size == 0)
        {
            return;
        }
        ...
        for (std::size_t iteration = 0; iteration < 4u; iteration++)
        {
            ...
            auto rot_end = zr_boost_1_81::end(ring);
            ...
            std::rotate_copy(zr_boost_1_81::begin(ring), range::pos(ring, rot_index),
            ...
            if (! is_closed_out && zr_boost_1_81::size(out) > 1)
            {
                range::pop_back(out);
            }
        ...
        {
            std::reverse(zr_boost_1_81::begin(out), zr_boost_1_81::end(out));
        }
    }
};
...
private:
    ...
    static inline void iterate(IteratorIn begin, IteratorIn end,
                               ...
                               Impl const& impl, Strategies const& strategies)
    {
        typedef typename zr_boost_1_81::range_value<InteriorRingsOut>::type single_type;
                                            ...
                                            Impl const& impl, Strategies const& strategies)
    {
        ...
        iterate(zr_boost_1_81::begin(interior_rings_in), zr_boost_1_81::end(interior_rings_in),
                interior_rings_out,
                max_distance,
                impl, strategies);
    }
...
struct simplify_multi
{
                             ...
                             Impl const& impl, Strategies const& strategies)
    {
        ...
        using single_type = typename zr_boost_1_81::range_value<MultiGeometryOut>::type;

        for (auto it = zr_boost_1_81::begin(multi); it != zr_boost_1_81::end(multi); ++it)
        {
