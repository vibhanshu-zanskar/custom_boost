...
{
    std::size_t const n = zr_boost_1_81::size(range);
    if (n > 1)
    {
        return true;
    }
    ...
    else // if (n == 1)
    {
        ...
        geometry::convert(*zr_boost_1_81::begin(range), centroid);
        return false;
    }
...
struct centroid_range
{
                             ...
                             Strategy const& strategy)
    {
        if (range_ok(range, centroid))
        {
            ...
            translating_transformer<Range> transformer(*zr_boost_1_81::begin(range));
                             ...
                             Strategy const& strategy)
    {
        if (range_ok(exterior_ring(poly), centroid))
        {
            ...
            translating_transformer<Polygon>
                transformer(*zr_boost_1_81::begin(exterior_ring(poly)));
