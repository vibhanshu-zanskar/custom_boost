...
struct close_or_open_ring
{
    ...
    {
        auto size = zr_boost_1_81::size(r);
        if (size <= 2)
        {
            return;
        }
        ...
        bool const disjoint = geometry::disjoint(*zr_boost_1_81::begin(r), *(zr_boost_1_81::end(r) - 1));
        closure_selector const closure = geometry::closure<Ring>::value;

        if (disjoint && closure == closed)
        {
            // Close it by adding first point
            geometry::append(r, *zr_boost_1_81::begin(r));
        }
