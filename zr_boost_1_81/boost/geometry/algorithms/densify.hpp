...
{
    typename zr_boost_1_81::range_value<Range>::type p2;
...
struct densify_range
{
    ...
    {
        typedef typename zr_boost_1_81::range_iterator<FwdRng const>::type iterator_t;
        typedef typename zr_boost_1_81::range_value<FwdRng>::type point_t;

        iterator_t it = zr_boost_1_81::begin(rng);
        iterator_t end = zr_boost_1_81::end(rng);
...
struct densify<Geometry, GeometryOut, multi_linestring_tag, multi_linestring_tag>
{
    ...
    {
        std::size_t count = zr_boost_1_81::size(mls);
...
struct densify<Geometry, GeometryOut, polygon_tag, polygon_tag>
{
                      ...
                      T const& len, Strategy const& strategy)
    {
        ...
        std::size_t count = zr_boost_1_81::size(interior_rings(poly));
...
struct densify<Geometry, GeometryOut, multi_polygon_tag, multi_polygon_tag>
{
    ...
    static void apply(Geometry const& mpoly, GeometryOut & mpoly_out,
                      T const& len, Strategy const& strategy)
    {
        std::size_t count = zr_boost_1_81::size(mpoly);
