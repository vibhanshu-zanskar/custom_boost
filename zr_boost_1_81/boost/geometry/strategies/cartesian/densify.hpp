...
public:
    ...
    static inline void apply(Point const& p0, Point const& p1, AssignPolicy & policy, T const& length_threshold)
    {
        ...
        {
            calc_t const coord0 = zr_boost_1_81::numeric_cast<calc_t>(get<index>(p0));
            calc_t const coord1 = zr_boost_1_81::numeric_cast<calc_t>(get<index>(p1));
        ...
        for (signed_size_type i = 0 ; i < n ; ++i)
        {
            ...
            geometry::detail::for_each_dimension<out_point_t>([&](auto index)
            {
                ...
                set<index>(out, zr_boost_1_81::numeric_cast<out_coord_t>(coord));
            });
