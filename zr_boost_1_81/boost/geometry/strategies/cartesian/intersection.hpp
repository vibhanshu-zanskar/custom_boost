    ...
    private :
                    ...
                    SegmentRatio const& ratio) const
        {
            ...
            calc_type const numerator
                = zr_boost_1_81::numeric_cast<calc_type>(ratio.numerator());
            calc_type const denominator
                = zr_boost_1_81::numeric_cast<calc_type>(ratio.denominator());
            calc_type const dx_calc = zr_boost_1_81::numeric_cast<calc_type>(dx);
            calc_type const dy_calc = zr_boost_1_81::numeric_cast<calc_type>(dy);

            set<0>(point, get<0, 0>(segment)
                   + zr_boost_1_81::numeric_cast<CoordinateType>(
