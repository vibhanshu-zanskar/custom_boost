    ...
    : std::vector<proj4_parameter>
{
    ...
    proj4_parameters(std::string const& proj4_str)
    {
        ...
        // zr_boost_1_81::split splits on one character, here it should be on " +", so implementation below
        // todo: put in different routine or sort out
        std::string def = zr_boost_1_81::trim_copy(proj4_str);
        zr_boost_1_81::trim_left_if(def, zr_boost_1_81::is_any_of(sep));

        std::string::size_type loc = def.find(sep);
        while (loc != std::string::npos)
        {
            std::string par = def.substr(0, loc);
            zr_boost_1_81::trim(par);
            if (! par.empty())
            {
                this->add(par);
            }
    ...
    void add(std::string const& str)
    {
        std::string name = str;
        std::string value;
        zr_boost_1_81::trim_left_if(name, zr_boost_1_81::is_any_of("+"));
