        ...
        for (;;)
        {
            ...
            else
            {
                ...
                for (auto const& v : rtree::elements(n))
                {
                    ...
                    {
                        // store value
                        store_value(value_distance, zr_boost_1_81::addressof(v));
                    }
                }
            }
...
//        , m_strategy()
    ...
    inline distance_query_incremental(MembersHolder const& members, Predicates const& pred)
        : m_tr(::zr_boost_1_81::addressof(members.translator()))
        , m_strategy(index::detail::get_strategy(members.parameters()))
        , m_pred(pred)
        , m_neighbors_count(0)
        , m_neighbor_ptr(nullptr)        
        ...
        else
        {
            ...
            for (auto const& v : rtree::elements(n))
            {
                    ...
                    && ! ignore_branch_or_value(value_distance))
                {
                    ...
                    m_neighbors.push(std::make_pair(value_distance, zr_boost_1_81::addressof(v)));
