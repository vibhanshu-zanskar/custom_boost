...
struct checker
{
    ...
    static inline void check_capacity(Varray const& v, size_type s)
    {
        ...
        ::zr_boost_1_81::ignore_unused(v, s);
    }
    ...
    static inline void throw_out_of_bounds(Varray const& v, size_type i)
    {
        ...
        ::zr_boost_1_81::ignore_unused(v, i);
    }
    ...
    static inline void check_iterator_end_neq(Varray const& v, const_iterator position)
    {
        ...
        ::zr_boost_1_81::ignore_unused(v, position);
    }
...
varray is a sequence container like zr_boost_1_81::container::vector with contiguous storage that can
change in size, along with the static allocation, low overhead, and fixed capacity of zr_boost_1_81::array.
...
class varray
{
    ...
    BOOST_GEOMETRY_STATIC_ASSERT(
        ( std::is_unsigned<typename vt::size_type>::value &&
          sizeof(typename zr_boost_1_81::uint_value_t<Capacity>::least) <= sizeof(typename vt::size_type) ),
    ...
    typedef zr_boost_1_81::aligned_storage<
        ...
        zr_boost_1_81::alignment_of<Value[Capacity]>::value
...
public:
    ...
    varray & operator=(varray<Value, C> & sv)
    {
        ...
        this->operator=(static_cast<const ::zr_boost_1_81::rv<other> &>(const_cast<const other &>(sv)));
        return *this;
    }
#endif
...
public:
    ...
    typedef zr_boost_1_81::reverse_iterator<iterator> reverse_iterator;
    //! @brief The const reverse iterator.
    typedef zr_boost_1_81::reverse_iterator<const_iterator> const_reverse_iterator;
    ...
    varray(Iterator first, Iterator last)
        : m_size(0)
    {
        BOOST_CONCEPT_ASSERT((zr_boost_1_81_concepts::ForwardTraversal<Iterator>)); // Make sure you passed a ForwardIterator
    ...
    //!
    //! @par Throws
    //!   @li If \c zr_boost_1_81::has_nothrow_move<Value>::value is \c true and Value's move constructor throws.
    //!   @li If \c zr_boost_1_81::has_nothrow_move<Value>::value is \c false and Value's copy constructor throws.
    //! @internal
    ...
    //!
    //! @par Throws
    //!   @li If \c zr_boost_1_81::has_nothrow_move<Value>::value is \c true and Value's move constructor or move assignment throws.
    //!   @li If \c zr_boost_1_81::has_nothrow_move<Value>::value is \c false and Value's copy constructor or copy assignment throws.
    //! @internal
    //!   @li It throws only if \c use_memop_in_swap_and_move is \c false_type - default.
    //! @endinternal
    //!
    ...
    void push_back(BOOST_RV_REF(value_type) value)
    {
        ...
        namespace sv = varray_detail;
        sv::construct(dti(), this->end(), ::zr_boost_1_81::move(value));                           // may throw
        ++m_size; // update end
    }
    ...
    iterator insert(iterator position, value_type const& value)
    {
        ...
        else
        {
            ...
            sv::construct(dti(), this->end(), zr_boost_1_81::move(r));                      // may throw
            ++m_size; // update end
    ...
    iterator insert(iterator position, BOOST_RV_REF(value_type) value)
    {
        ...
        if ( position == this->end() )
        {
            sv::construct(dti(), position, zr_boost_1_81::move(value));                     // may throw
            ++m_size; // update end
        }
        else
        {
            ...
            ++m_size; // update end
            sv::move_backward(position, this->end() - 2, this->end() - 1);          // may throw
            sv::assign(position, zr_boost_1_81::move(value));                               // may throw
        }
    ...
    iterator insert(iterator position, Iterator first, Iterator last)
    {
        ...
        typedef typename zr_boost_1_81::iterator_traversal<Iterator>::type traversal;
    ...
    void emplace_back(BOOST_FWD_REF(Args) ...args)
    {
        ...
        namespace sv = varray_detail;
        sv::construct(dti(), this->end(), ::zr_boost_1_81::forward<Args>(args)...);                // may throw
        ++m_size; // update end
    }
    ...
    iterator emplace(iterator position, BOOST_FWD_REF(Args) ...args)
    {
        ...
        if ( position == this->end() )
        {
            sv::construct(dti(), position, ::zr_boost_1_81::forward<Args>(args)...);               // may throw
            ++m_size; // update end
        }
        else
        {
            ...
            sv::construct(dti(), val_p, ::zr_boost_1_81::forward<Args>(args)...);                  // may throw
            sv::scoped_destructor<value_type> d(val_p);
            sv::assign(position, ::zr_boost_1_81::move(*val_p));                            // may throw
        }
    ...
    Value * data()
    {
        return zr_boost_1_81::addressof(*(this->ptr()));
    }
...
private:
    ...
    // @par Throws
    //   @li If zr_boost_1_81::has_nothrow_move<Value>::value is true and Value's move constructor throws
    //   @li If zr_boost_1_81::has_nothrow_move<Value>::value is false and Value's copy constructor throws.
    // @par Complexity
    //   Linear O(N).
    template <std::size_t C>
    ...
    // @par Throws
    //   @li If zr_boost_1_81::has_nothrow_move<Value>::value is true and Value's move constructor or move assignment throws
    //   @li If zr_boost_1_81::has_nothrow_move<Value>::value is false and Value's copy constructor or move assignment throws.
    // @par Complexity
    //   Linear O(N).
    template <std::size_t C>
    ...
    void swap_dispatch_impl(iterator first_sm, iterator last_sm, iterator first_la, iterator last_la, std::true_type /*use_memop*/)
    {
        ...
        {
            zr_boost_1_81::aligned_storage<
                sizeof(value_type),
                zr_boost_1_81::alignment_of<value_type>::value
            > temp_storage;
            value_type * temp_ptr = reinterpret_cast<value_type*>(temp_storage.address());

            ::memcpy(temp_ptr, zr_boost_1_81::addressof(*first_sm), sizeof(value_type));
            ::memcpy(zr_boost_1_81::addressof(*first_sm), zr_boost_1_81::addressof(*first_la), sizeof(value_type));
            ::memcpy(zr_boost_1_81::addressof(*first_la), temp_ptr, sizeof(value_type));
        }
    ...
    void swap_dispatch_impl(iterator first_sm, iterator last_sm, iterator first_la, iterator last_la, std::false_type /*use_memop*/)
    {
        ...
        for (; first_sm != last_sm ; ++first_sm, ++first_la)
        {
            //zr_boost_1_81::swap(*first_sm, *first_la);                                    // may throw
            value_type temp(zr_boost_1_81::move(*first_sm));                                // may throw
            *first_sm = zr_boost_1_81::move(*first_la);                                     // may throw
            *first_la = zr_boost_1_81::move(temp);                                          // may throw
        }
    ...
    //   Linear O(N).
    template <typename Iterator>
    void insert_dispatch(iterator position, Iterator first, Iterator last, zr_boost_1_81::random_access_traversal_tag const&)
    {
        BOOST_CONCEPT_ASSERT((zr_boost_1_81_concepts::RandomAccessTraversal<Iterator>)); // Make sure you passed a RandomAccessIterator
        
        errh::check_iterator_end_eq(*this, position);
        
        typename zr_boost_1_81::iterator_difference<Iterator>::type
    ...
    //   Linear O(N).
    template <typename Iterator>
    void assign_dispatch(Iterator first, Iterator last, zr_boost_1_81::random_access_traversal_tag const& /*not_random_access*/)
    {
        namespace sv = varray_detail;
...
public:
    ...
    // nothrow
    Value * data() { return zr_boost_1_81::addressof(*(this->ptr())); }
    const Value * data() const { return zr_boost_1_81::addressof(*(this->ptr())); }
