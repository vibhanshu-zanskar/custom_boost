...
void save_iterator(std::ostream &os, InputIterator begin, InputIterator end){
    typedef zr_boost_1_81::archive::iterators::mb_from_wchar<
        zr_boost_1_81::archive::iterators::xml_escape<InputIterator>
    > translator;
    std::copy(
        translator(begin),
        translator(end),
        zr_boost_1_81::archive::iterators::ostream_iterator<char>(os)
    );
}
...
//  at least one library doesn't typedef value_type for strings
//  so rather than using string directly make a pointer iterator out of it
    typedef zr_boost_1_81::archive::iterators::xml_escape<
        const char * 
...
BOOST_ARCHIVE_DECL
xml_oarchive_impl<Archive>::~xml_oarchive_impl(){
    if(zr_boost_1_81::core::uncaught_exceptions() > 0)
        return;
