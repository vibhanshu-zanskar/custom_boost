                                   ...
                                   const interval<T, Policies>& y)
{
  ...
  const T& yl = y.lower();
  const T& yu = y.upper();
  if (::zr_boost_1_81::numeric::interval_lib::user::is_neg(xu))
    if (::zr_boost_1_81::numeric::interval_lib::user::is_neg(yu))
      return I(rnd.div_down(xu, yl), rnd.div_up(xl, yu), true);
    else
      return I(rnd.div_down(xl, yl), rnd.div_up(xu, yu), true);
  else if (::zr_boost_1_81::numeric::interval_lib::user::is_neg(xl))
    if (::zr_boost_1_81::numeric::interval_lib::user::is_neg(yu))
...
interval<T, Policies> div_non_zero(const T& x, const interval<T, Policies>& y)
{
  ...
  const T& yl = y.lower();
  const T& yu = y.upper();
  if (::zr_boost_1_81::numeric::interval_lib::user::is_neg(x))
...
{
  // assert(::zr_boost_1_81::numeric::interval_lib::user::is_pos(yu));
  if (::zr_boost_1_81::numeric::interval_lib::user::is_zero(x.lower()) &&
      ::zr_boost_1_81::numeric::interval_lib::user::is_zero(x.upper()))
    return x;
  typename Policies::rounding rnd;
  typedef interval<T, Policies> I;
  const T& xl = x.lower();
...
interval<T, Policies> div_positive(const T& x, const T& yu)
{
  // assert(::zr_boost_1_81::numeric::interval_lib::user::is_pos(yu));
  typedef interval<T, Policies> I;
  if (::zr_boost_1_81::numeric::interval_lib::user::is_zero(x))
...
{
  // assert(::zr_boost_1_81::numeric::interval_lib::user::is_neg(yl));
  if (::zr_boost_1_81::numeric::interval_lib::user::is_zero(x.lower()) &&
      ::zr_boost_1_81::numeric::interval_lib::user::is_zero(x.upper()))
    return x;
  typename Policies::rounding rnd;
...
{
  // assert(::zr_boost_1_81::numeric::interval_lib::user::is_neg(y.lower()) && ::zr_boost_1_81::numeric::interval_lib::user::is_pos(y.upper()));
  if (::zr_boost_1_81::numeric::interval_lib::user::is_zero(x.lower()) && ::zr_boost_1_81::numeric::interval_lib::user::is_zero(x.upper()))
    { b = false; return x; }
  typename Policies::rounding rnd;
  typedef interval<T, Policies> I;
  const T& xl = x.lower();
...
interval<T, Policies> div_zero_part2(const interval<T, Policies>& x,
                                     const interval<T, Policies>& y)
{
  // assert(::zr_boost_1_81::numeric::interval_lib::user::is_neg(y.lower()) && ::zr_boost_1_81::numeric::interval_lib::user::is_pos(y.upper()) && (div_zero_part1(x, y, b), b));
  typename Policies::rounding rnd;
  typedef interval<T, Policies> I;
  typedef typename Policies::checking checking;
  if (::zr_boost_1_81::numeric::interval_lib::user::is_neg(x.upper()))
