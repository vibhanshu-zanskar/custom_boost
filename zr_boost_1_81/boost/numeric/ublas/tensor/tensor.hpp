		...
		public detail::tensor_expression<tensor<T, F, A>,tensor<T, F, A>>
{
	...
	static_assert( std::is_same<F,first_order>::value ||
								 std::is_same<F,last_order >::value, "zr_boost_1_81::numeric::tensor template class only supports first- or last-order storage formats.");
...
//	static_assert(std::is_same_v<tensor_expression_type<self_type>, detail::tensor_expression<tensor<T,F,A>,tensor<T,F,A>>>, "tensor_expression_type<self_type>");
		...
		, data_    (a)
	{
		if(this->extents_.product() != this->data_.size())
			throw std::runtime_error("Error in zr_boost_1_81::numeric::ublas::tensor: size of provided data and specified extents do not match.");
	}
		...
		, data_    ( extents_.product() )
	{
		static_assert( detail::has_tensor_types<self_type, tensor_expression_type<derived_type>>::value,
									 "Error in zr_boost_1_81::numeric::ublas::tensor: expression does not contain a tensor. cannot retrieve shape.");
	...
	decltype(auto) operator() (index::index_type<I> p, index_types ... ps) const
	{
		...
		if( N != this->rank() )
			throw std::runtime_error("Error in zr_boost_1_81::numeric::ublas::operator(): size of provided index_types does not match with the rank.");
