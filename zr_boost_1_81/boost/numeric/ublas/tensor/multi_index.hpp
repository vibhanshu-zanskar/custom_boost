...
public:
	  ...
	  : _base{i(), is()... }
	{
		static_assert( sizeof...(is)+1 == N,
		               "Static assert in zr_boost_1_81::numeric::ublas::multi_index: number of constructor arguments is not equal to the template parameter." );

		static_assert( valid_multi_index<std::tuple<index::index_type<I>, indexes ...> >::value,
		               "Static assert in zr_boost_1_81::numeric::ublas::multi_index: indexes occur twice in multi-index." );
	}
...
auto array_to_vector(multi_index<M> const& lhs, multi_index<N> const& rhs)
{
	...
	for(auto i = 0u; i < N; ++i)
		for(auto j = 0u; j < M; ++j)
			if ( lhs.at(i) == rhs.at(j) && lhs.at(i) != zr_boost_1_81::numeric::ublas::index::_())
