...
public:
	...
	static_assert( std::numeric_limits<typename base_type::value_type>::is_integer,
								 "Static error in zr_boost_1_81::numeric::ublas::basic_strides: type must be of type integer.");
	static_assert(!std::numeric_limits<typename base_type::value_type>::is_signed,
								"Static error in zr_boost_1_81::numeric::ublas::basic_strides: type must be of type unsigned integer.");
	static_assert(std::is_same<__layout,first_order>::value || std::is_same<__layout,last_order>::value,
								"Static error in zr_boost_1_81::numeric::ublas::basic_strides: layout type must either first or last order");
			...
			: _base(s.size(),1)
	{
		...
		if(!s.valid())
			throw std::runtime_error("Error in zr_boost_1_81::numeric::ublas::basic_strides() : shape is not valid.");		

		if(s.is_vector() || s.is_scalar())
			return;

		if(this->size() < 2)
			throw std::runtime_error("Error in zr_boost_1_81::numeric::ublas::basic_strides() : size of strides must be greater or equal 2.");
