...
auto prod(tensor<V,F,A1> const& a, vector<V,A2> const& b, const std::size_t m)
{
	...
	if( m == 0)
		throw std::length_error("error in zr_boost_1_81::numeric::ublas::prod(ttv): contraction mode must be greater than zero.");

	if( p < m )
		throw std::length_error("error in zr_boost_1_81::numeric::ublas::prod(ttv): rank of tensor must be greater than or equal to the modus.");

	if( p == 0)
		throw std::length_error("error in zr_boost_1_81::numeric::ublas::prod(ttv): rank of tensor must be greater than zero.");

	if( a.empty() )
		throw std::length_error("error in zr_boost_1_81::numeric::ublas::prod(ttv): first argument tensor should not be empty.");

	if( b.size() == 0)
		throw std::length_error("error in zr_boost_1_81::numeric::ublas::prod(ttv): second argument vector should not be empty.");
...
auto prod(tensor<V,F,A1> const& a, matrix<V,F,A2> const& b, const std::size_t m)
{
	...
	if( m == 0)
		throw std::length_error("error in zr_boost_1_81::numeric::ublas::prod(ttm): contraction mode must be greater than zero.");

	if( p < m || m > a.extents().size())
		throw std::length_error("error in zr_boost_1_81::numeric::ublas::prod(ttm): rank of the tensor must be greater equal the modus.");

	if( p == 0)
		throw std::length_error("error in zr_boost_1_81::numeric::ublas::prod(ttm): rank of the tensor must be greater than zero.");

	if( a.empty() )
		throw std::length_error("error in zr_boost_1_81::numeric::ublas::prod(ttm): first argument tensor should not be empty.");

	if( b.size1()*b.size2() == 0)
		throw std::length_error("error in zr_boost_1_81::numeric::ublas::prod(ttm): second argument matrix should not be empty.");
...
auto inner_prod(tensor<V,F,A1> const& a, tensor<V,F,A2> const& b)
{
	...
	if( a.rank() != b.rank() )
		throw std::length_error("error in zr_boost_1_81::numeric::ublas::inner_prod: Rank of both tensors must be the same.");

	if( a.empty() || b.empty())
		throw std::length_error("error in zr_boost_1_81::numeric::ublas::inner_prod: Tensors should not be empty.");

	if( a.extents() != b.extents())
		throw std::length_error("error in zr_boost_1_81::numeric::ublas::inner_prod: Tensor extents should be the same.");
...
auto outer_prod(tensor<V,F,A1> const& a, tensor<V,F,A2> const& b)
{
	...
	if( a.empty() || b.empty() )
		throw std::runtime_error("error in zr_boost_1_81::numeric::ublas::outer_prod: tensors should not be empty.");
...
auto norm(detail::tensor_expression<T,D> const& expr)
{
	...
	if( a.empty() )
		throw std::runtime_error("error in zr_boost_1_81::numeric::ublas::norm: tensors should not be empty.");
...
auto real(detail::tensor_expression<tensor<std::complex<V>,F,A>,D> const& expr)
{
	...
	if( detail::retrieve_extents( expr  ).empty() )
		throw std::runtime_error("error in zr_boost_1_81::numeric::ublas::real: tensors should not be empty.");
...
auto conj(detail::tensor_expression<T,D> const& expr)
{
	...
	if( detail::retrieve_extents( expr  ).empty() )
		throw std::runtime_error("error in zr_boost_1_81::numeric::ublas::conj: tensors should not be empty.");
