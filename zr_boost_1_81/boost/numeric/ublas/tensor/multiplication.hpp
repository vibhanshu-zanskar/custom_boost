         ...
         const PointerIn2 b, SizeType const*const nb, SizeType const*const wb)
{
	static_assert( std::is_pointer<PointerOut>::value & std::is_pointer<PointerIn1>::value & std::is_pointer<PointerIn2>::value,
	               "Static error in zr_boost_1_81::numeric::ublas::ttv: Argument types for pointers are not pointer types.");

	if( m == 0)
		throw std::length_error("Error in zr_boost_1_81::numeric::ublas::ttv: Contraction mode must be greater than zero.");

	if( p < m )
		throw std::length_error("Error in zr_boost_1_81::numeric::ublas::ttv: Rank must be greater equal the modus.");

	if( p == 0)
		throw std::length_error("Error in zr_boost_1_81::numeric::ublas::ttv: Rank must be greater than zero.");

	if(c == nullptr || a == nullptr || b == nullptr)
		throw std::length_error("Error in zr_boost_1_81::numeric::ublas::ttv: Pointers shall not be null pointers.");

	for(auto i = 0u; i < m-1; ++i)
		if(na[i] != nc[i])
			throw std::length_error("Error in zr_boost_1_81::numeric::ublas::ttv: Extents (except of dimension mode) of A and C must be equal.");
	...
	if(  na[m-1] != max)
		throw std::length_error("Error in zr_boost_1_81::numeric::ublas::ttv: Extent of dimension mode of A and b must be equal.");
         ...
         const PointerIn2 b, SizeType const*const nb, SizeType const*const wb)
{
	...
	static_assert( std::is_pointer<PointerOut>::value & std::is_pointer<PointerIn1>::value & std::is_pointer<PointerIn2>::value,
	               "Static error in zr_boost_1_81::numeric::ublas::ttm: Argument types for pointers are not pointer types.");

	if( m == 0 )
		throw std::length_error("Error in zr_boost_1_81::numeric::ublas::ttm: Contraction mode must be greater than zero.");

	if( p < m )
		throw std::length_error("Error in zr_boost_1_81::numeric::ublas::ttm: Rank must be greater equal than the specified mode.");

	if( p == 0)
		throw std::length_error("Error in zr_boost_1_81::numeric::ublas::ttm:Rank must be greater than zero.");

	if(c == nullptr || a == nullptr || b == nullptr)
		throw std::length_error("Error in zr_boost_1_81::numeric::ublas::ttm: Pointers shall not be null pointers.");

	for(auto i = 0u; i < m-1; ++i)
		if(na[i] != nc[i])
			throw std::length_error("Error in zr_boost_1_81::numeric::ublas::ttm: Extents (except of dimension mode) of A and C must be equal.");
	...
	if(na[m-1] != nb[1])
		throw std::length_error("Error in zr_boost_1_81::numeric::ublas::ttm: 2nd Extent of B and M-th Extent of A must be the equal.");

	if(nc[m-1] != nb[0])
		throw std::length_error("Error in zr_boost_1_81::numeric::ublas::ttm: 1nd Extent of B and M-th Extent of C must be the equal.");
         ...
         PointerIn2 b, SizeType const*const nb, SizeType const*const wb)
{
	...
	if( pa == 0 || pb == 0)
		throw std::length_error("Error in zr_boost_1_81::numeric::ublas::ttt: tensor order must be greater zero.");

	if( q > pa && q > pb)
		throw std::length_error("Error in zr_boost_1_81::numeric::ublas::ttt: number of contraction must be smaller than or equal to the tensor order.");
	...
	for(auto i = 0ul; i < r; ++i)
		if( na[phia[i]-1] != nc[i] )
			throw std::length_error("Error in zr_boost_1_81::numeric::ublas::ttt: dimensions of lhs and res tensor not correct.");

	for(auto i = 0ul; i < s; ++i)
		if( nb[phib[i]-1] != nc[r+i] )
			throw std::length_error("Error in zr_boost_1_81::numeric::ublas::ttt: dimensions of rhs and res not correct.");

	for(auto i = 0ul; i < q; ++i)
		if( nb[phib[s+i]-1] != na[phia[r+i]-1] )
			throw std::length_error("Error in zr_boost_1_81::numeric::ublas::ttt: dimensions of lhs and rhs not correct.");
           ...
           value_t v)
{
	static_assert( std::is_pointer<PointerIn1>::value && std::is_pointer<PointerIn2>::value,
	               "Static error in zr_boost_1_81::numeric::ublas::inner: Argument types for pointers must be pointer types.");
	if(p<2)
		throw std::length_error("Error in zr_boost_1_81::numeric::ublas::inner: Rank must be greater than zero.");
	if(a == nullptr || b == nullptr)
		throw std::length_error("Error in zr_boost_1_81::numeric::ublas::inner: Pointers shall not be null pointers.");
           ...
           const PointerIn2 b, SizeType const pb, SizeType const*const nb, SizeType const*const wb)
{
	static_assert( std::is_pointer<PointerIn1>::value & std::is_pointer<PointerIn2>::value & std::is_pointer<PointerOut>::value,
	               "Static error in zr_boost_1_81::numeric::ublas::outer: argument types for pointers must be pointer types.");
	if(pa < 2u || pb < 2u)
		throw std::length_error("Error in zr_boost_1_81::numeric::ublas::outer: number of extents of lhs and rhs tensor must be equal or greater than two.");
	if((pa + pb) != pc)
		throw std::length_error("Error in zr_boost_1_81::numeric::ublas::outer: number of extents of lhs plus rhs tensor must be equal to the number of extents of C.");
	if(a == nullptr || b == nullptr || c == nullptr)
		throw std::length_error("Error in zr_boost_1_81::numeric::ublas::outer: pointers shall not be null pointers.");
