...
std::ostream& operator << (std::ostream& out, zr_boost_1_81::numeric::ublas::tensor<V,F,A> const& t)
{

	if(t.extents().is_scalar()){
		out << '[';
		zr_boost_1_81::numeric::ublas::detail::print(out,t[0]);
		out << ']';
	}
		...
		for(auto i = 0u; i < t.size()-1; ++i){
			zr_boost_1_81::numeric::ublas::detail::print(out,t[i]);
			out << cat << ' ';
		}
		zr_boost_1_81::numeric::ublas::detail::print(out,t[t.size()-1]);
		out << ']';
	}
	else{
		zr_boost_1_81::numeric::ublas::detail::print(out, t.rank()-1, t.data(), t.strides().data(), t.extents().data());
	}
	return out;
}
