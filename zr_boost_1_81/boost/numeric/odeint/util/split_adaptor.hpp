...
{
    const std::pair<std::size_t, std::size_t> off = split_offsets(zr_boost_1_81::size(rng), index, parts);
    return make_iterator_range( zr_boost_1_81::begin(rng) + off.first, zr_boost_1_81::begin(rng) + off.second );
}
