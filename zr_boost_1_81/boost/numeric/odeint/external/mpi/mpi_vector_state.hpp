...
struct split_impl< Source, mpi_state< InnerState >,
    typename zr_boost_1_81::enable_if< zr_boost_1_81::has_range_const_iterator<Source> >::type >
{
    typedef typename zr_boost_1_81::range_iterator<const Source>::type iterator;
    ...
    static void split( const Source &from, mpi_state< InnerState > &to )
    {
        ...
        if(to.world.rank() == 0) {
            ...
            for(size_t i = 0 ; i < num ; i++) {
                ...
                zr_boost_1_81::numeric::odeint::resize(pieces[i], part);
                zr_boost_1_81::numeric::odeint::copy(part, pieces[i]);
            }
        }
        // send to nodes
        zr_boost_1_81::mpi::scatter(to.world, pieces, to(), 0);
    }
};
...
struct unsplit_impl< mpi_state< InnerState >, Target,
    typename zr_boost_1_81::enable_if< zr_boost_1_81::has_range_iterator<Target> >::type >
{
    typedef typename zr_boost_1_81::range_iterator<Target>::type iterator;

    static void unsplit( const mpi_state< InnerState > &from , Target &to )
    {
        std::vector< InnerState > pieces;
        // send data to root
        zr_boost_1_81::mpi::gather(from.world, from(), pieces, 0);
        if(from.world.rank() == 0) {
            // check target size
            size_t total_size = 0;
            for(size_t i = 0 ; i < pieces.size() ; i++)
                total_size += zr_boost_1_81::size(pieces[i]);
            BOOST_ASSERT( total_size <= zr_boost_1_81::size(to) );
            // copy parts
            iterator out = zr_boost_1_81::begin(to);
            for(size_t i = 0 ; i < pieces.size() ; i++)
                out = zr_boost_1_81::copy(pieces[i], out);
        }
    }
};
