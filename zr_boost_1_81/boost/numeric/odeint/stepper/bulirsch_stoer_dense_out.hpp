...
public:
    ...
    void initialize( const StateType &x0 , const time_type &t0 , const time_type &dt0 )
    {
        ...
        zr_boost_1_81::numeric::odeint::copy( x0 , get_current_state() );
        m_t = t0;
        m_dt = dt0;
        reset();
    }
...
private:
    ...
    /* k is the order to which the result was approximated */
    {
        ...
        {
            zr_boost_1_81::numeric::odeint::copy( m_diffs[2*k+1][0].m_v , m_err.m_v );
                ...
                if( i >= 0 )
                {
                    m_algebra.for_each3( m_diffs[kappa][j_diffs].m_v , m_diffs[kappa][j_diffs].m_v , m_derivs[j][i].m_v ,
                                         typename operations_type::template scale_sum2< value_type , value_type >( 1.0 ,
                                                                                                                   sign * fac * zr_boost_1_81::math::binomial_coefficient< value_type >( kappa , c ) ) );
                }
                else
                {
    ...
    void do_interpolation( time_type t , StateOut &out ) const
    {
        ...
        zr_boost_1_81::numeric::odeint::copy( m_mp_states[0].m_v , out );
