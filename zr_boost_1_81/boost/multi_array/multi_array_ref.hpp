...
public: 
  ...
  void assign(InputIterator begin, InputIterator end) {
    zr_boost_1_81::function_requires<InputIteratorConcept<InputIterator> >();
...
public:
    ...
    base_(base), storage_(so), origin_offset_(0), directional_offset_(0)
 {
   ...
   if(extents) {
     ...
     zr_boost_1_81::array<index,NumDims> extent_list;
...
private:
  ...
  void init_from_extent_gen(const
    ...
    typedef zr_boost_1_81::array<index,NumDims> extent_list;

    // get the index_base values
    std::transform(ranges.ranges_.begin(),ranges.ranges_.end(),
              index_base_list_.begin(),
              zr_boost_1_81::mem_fun_ref(&extent_range::start));

    // calculate the extents
    extent_list extents;
    std::transform(ranges.ranges_.begin(),ranges.ranges_.end(),
              extents.begin(),
              zr_boost_1_81::mem_fun_ref(&extent_range::size));
