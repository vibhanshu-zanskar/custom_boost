...
namespace zr_boost_1_81{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename U, typename = decltype(zr_boost_1_81::declval<T>() = zr_boost_1_81::declval<U>())>
   ...
   template <class T, class U> struct is_assignable : public integral_constant<bool, sizeof(detail::is_assignable_imp::test<T, U>(0)) == sizeof(zr_boost_1_81::type_traits::yes_type)>
   {
      BOOST_STATIC_ASSERT_MSG(zr_boost_1_81::is_complete<T>::value, "Arguments to is_assignable must be complete types");
   };
