...
// Boost.ResultOf not always able to deduce lambda result type (on MSVC).
    ...
    #define BOOST_CONTRACT_CALL_IF_RESULT_OF_(F) \
        decltype(zr_boost_1_81::declval<F>()())
#else
    #include <boost/utility/result_of.hpp>
    #define BOOST_CONTRACT_CALL_IF_RESULT_OF_(F) \
        typename zr_boost_1_81::result_of<F()>::type
#endif
...
@RefFunc{zr_boost_1_81::contract::call_if} and @RefFunc{zr_boost_1_81::contract::call_if_c}.
...
struct call_if_statement<true, Then, ThenResult> { // Copyable (as *).
    ...
    */
    explicit call_if_statement(Then f) :
            r_(zr_boost_1_81::make_shared<ThenResult>(f())) {}
...
private:
    zr_boost_1_81::shared_ptr<ThenResult> r_;
};
...
@code
zr_boost_1_81::contract::call_if_c<Pred1>(
...
This is equivalent to <c>zr_boost_1_81::contract::call_if_c<Pred::value>(f)</c>.
Create a call-if object with the specified then-branch functor template:

@code
zr_boost_1_81::contract::call_if<Pred1>(
    then_functor_template1
...
A call to <c>zr_boost_1_81::contract::condition_if_c<Pred>(f, else_)</c> is logically
equivalent to <c>zr_boost_1_81::contract::call_if_c<Pred>(f, [] { return else_; })</c>
...
*/
    ...
    template<bool Pred, typename Then>
    typename zr_boost_1_81::enable_if_c<Pred, bool>::type
    condition_if_c(Then f, bool /* else_ */ = true) { return f(); }

    template<bool Pred, typename Then>
    typename zr_boost_1_81::disable_if_c<Pred, bool>::type
...
<c>zr_boost_1_81::contract::condition_if_c<Pred::value>(f, else_)</c>.
Compile and execute the nullary boolean functor template call @c f() if and only
