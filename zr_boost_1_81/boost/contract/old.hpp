...
/** @cond */
    ...
    #define BOOST_CONTRACT_OLDOF_AUTO_TYPEOF_(value) \
        zr_boost_1_81::contract::old_ptr<BOOST_TYPEOF(value)>
#endif

#define BOOST_CONTRACT_ERROR_macro_OLDOF_has_invalid_number_of_arguments_2( \
        v, value) \
    BOOST_CONTRACT_OLDOF_AUTO_TYPEOF_(value)(zr_boost_1_81::contract::make_old(v, \
        zr_boost_1_81::contract::copy_old(v) ? (value) : zr_boost_1_81::contract::null_old() \
    ))

#define BOOST_CONTRACT_ERROR_macro_OLDOF_has_invalid_number_of_arguments_1( \
        value) \
    BOOST_CONTRACT_OLDOF_AUTO_TYPEOF_(value)(zr_boost_1_81::contract::make_old( \
        zr_boost_1_81::contract::copy_old() ? (value) : zr_boost_1_81::contract::null_old() \
    ))
...
pointer of type @RefClass{zr_boost_1_81::contract::old_ptr} or
@RefClass{zr_boost_1_81::contract::old_ptr_if_copyable}.
...
@arg    <c><b>v</b></c> is the extra parameter of type
        @RefClass{zr_boost_1_81::contract::virtual_}<c>*</c> and default value @c 0
...
@c zr_boost_1_81::is_copy_constructible<T> but programmers can chose to specialize it
...
<c>zr_boost_1_81::is_copy_constructible</c></a>):
...
namespace zr_boost_1_81 { namespace contract {
    ...
    struct is_old_value_copyable<u> : zr_boost_1_81::false_type {};
} } // namespace
@endcode

A given old value type @c T is copied only if
@c zr_boost_1_81::contract::is_old_value_copyable<T>::value is @c true.
A copyable old value type @c V is always copied using
@c zr_boost_1_81::contract::old_value_copy<V>.
A non-copyable old value type @c W generates a compile-time error when
@c zr_boost_1_81::contract::old_ptr<W> is dereferenced, but instead leaves
@c zr_boost_1_81::contract::old_ptr_if_copyable<W> always null (without generating
compile-time errors).
...
*/
template<typename T>
struct is_old_value_copyable : zr_boost_1_81::is_copy_constructible<T> {};

/** @cond */
class old_value;

template<> // Needed because `old_value` incomplete type when trait first used.
struct is_old_value_copyable<old_value> : zr_boost_1_81::true_type {};
/** @endcond */
...
<c>zr_boost_1_81::contract::is_old_value_copyable<T>::value</c> is @c true).
...
@RefMacro{BOOST_CONTRACT_OLDOF} or @RefFunc{zr_boost_1_81::contract::make_old} (that is
why this class does not have public non-default constructors):

@code
class u {
public:
    virtual void f(..., zr_boost_1_81::contract::virtual_* v = 0) {
        zr_boost_1_81::contract::old_ptr<old_type> old_var = // For copyable `old_type`.
                BOOST_CONTRACT_OLDOF(v, old_expr);
        ...
    }
...
@tparam T Type of the pointed old value.
        ...
        <c>zr_boost_1_81::contract::is_old_value_copyable<T>::value</c> must be
...
public:
    ...
    @c zr_boost_1_81::contract::is_old_value_copyable<T>::value is @c false).
    ...
    */
    T const& operator*() const {
        BOOST_STATIC_ASSERT_MSG(
            zr_boost_1_81::contract::is_old_value_copyable<T>::value,
    ...
    copyable (i.e., if @c zr_boost_1_81::contract::is_old_value_copyable<T>::value is
    @c false).
...
private:
        ...
        explicit old_ptr(zr_boost_1_81::shared_ptr<old_value_copy<T> > old)
                : typed_copy_(old) {}
    #endif

    zr_boost_1_81::shared_ptr<old_value_copy<T> > typed_copy_;
...
@RefMacro{BOOST_CONTRACT_OLDOF} or @RefFunc{zr_boost_1_81::contract::make_old}:
...
public:
    virtual void f(..., zr_boost_1_81::contract::virtual_* v = 0) {
        zr_boost_1_81::contract::old_ptr_if_copyable<T> old_var =
                BOOST_CONTRACT_OLDOF(v, old_expr);
        ...
...
@tparam T Type of the pointed old value.
        ...
        <c>zr_boost_1_81::contract::is_old_value_copyable<T>::value</c> is @c false),
...
public:
    ...
    */
    ...
    (i.e., if @c zr_boost_1_81::contract::is_old_value_copyable<T>::value is @c false).
...
private:
    #ifndef BOOST_CONTRACT_NO_OLDS
        explicit old_ptr_if_copyable(zr_boost_1_81::shared_ptr<old_value_copy<T> > old)
                : typed_copy_(old) {}
    #endif
...
@c zr_boost_1_81::contract::is_old_value_copyable trait used in the declaration of this
...
public:
    ...
    @c zr_boost_1_81::contract::old_value_copy, in which case the related old value
    ...
    */
    ...
    ,
        typename zr_boost_1_81::enable_if<zr_boost_1_81::contract::is_old_value_copyable<T>
                >::type* = 0
    )
        #ifndef BOOST_CONTRACT_NO_OLDS
    ...
    */
    ...
    /* implicit */ old_value(
        ...
        #endif // Else, no name (avoid unused param warning).
    ,
        typename zr_boost_1_81::disable_if<zr_boost_1_81::contract::is_old_value_copyable<T>
                >::type* = 0
...
private:
    ...
    #ifndef BOOST_CONTRACT_NO_OLDS
        zr_boost_1_81::shared_ptr<void> untyped_copy_; // Type erasure.
    #endif
...
public:
    ...
    value pointers of type @c zr_boost_1_81::contract::old_ptr_if_copyable.
    
    @tparam T   Type of the pointed old value.
                The old value pointer will always be null if this type is not
                copyable (see
                @c zr_boost_1_81::contract::is_old_value_copyable), but this library
    ...
    */
    ...
    For example, this is implicitly called when assigning or initializing old
    value pointers of type @c zr_boost_1_81::contract::old_ptr.
    
    @tparam T   Type of the pointed old value. This type must be copyable
                (see @c zr_boost_1_81::contract::is_old_value_copyable),
...
private:
    ...
    Ptr get() {
        #ifndef BOOST_CONTRACT_NO_OLDS
            if(!zr_boost_1_81::contract::is_old_value_copyable<typename
                    Ptr::element_type>::value) {
                BOOST_CONTRACT_DETAIL_DEBUG(!untyped_copy_);
                return Ptr(); // Non-copyable so no old value and return null.
        #ifndef BOOST_CONTRACT_ALL_DISABLE_NO_ASSERTION
            } else if(!v_ && zr_boost_1_81::contract::detail::checking::already()) {
                return Ptr(); // Not checking (so return null).
        #endif
            } else if(!v_) {
                BOOST_CONTRACT_DETAIL_DEBUG(untyped_copy_);
                typedef old_value_copy<typename Ptr::element_type> copied_type;
                zr_boost_1_81::shared_ptr<copied_type> typed_copy = // Un-erase type.
                        zr_boost_1_81::static_pointer_cast<copied_type>(untyped_copy_);
                BOOST_CONTRACT_DETAIL_DEBUG(typed_copy);
                return Ptr(typed_copy);
            } else if(
                v_->action_ == zr_boost_1_81::contract::virtual_::push_old_init_copy ||
                v_->action_ == zr_boost_1_81::contract::virtual_::push_old_ftor_copy
            ) {
                BOOST_CONTRACT_DETAIL_DEBUG(untyped_copy_);
                std::queue<zr_boost_1_81::shared_ptr<void> >& copies = v_->action_ ==
                        zr_boost_1_81::contract::virtual_::push_old_ftor_copy ?
                    v_->old_ftor_copies_
                :
                    v_->old_init_copies_
                ;
                ...
                zr_boost_1_81::contract::virtual_::pop_old_init_copy(v_->action_) ||
                v_->action_ == zr_boost_1_81::contract::virtual_::pop_old_ftor_copy
            ) {
                // Copy not null, but still pop it from the queue.
                BOOST_CONTRACT_DETAIL_DEBUG(!untyped_copy_);

                std::queue<zr_boost_1_81::shared_ptr<void> >& copies = v_->action_ ==
                        zr_boost_1_81::contract::virtual_::pop_old_ftor_copy ?
                    v_->old_ftor_copies_
                :
                    v_->old_init_copies_
                ;
                zr_boost_1_81::shared_ptr<void> untyped_copy = copies.front();
                BOOST_CONTRACT_DETAIL_DEBUG(untyped_copy);
                copies.pop();

                typedef old_value_copy<typename Ptr::element_type> copied_type;
                zr_boost_1_81::shared_ptr<copied_type> typed_copy = // Un-erase type.
                        zr_boost_1_81::static_pointer_cast<copied_type>(untyped_copy);
                BOOST_CONTRACT_DETAIL_DEBUG(typed_copy);
                return Ptr(typed_copy);
            }
            BOOST_CONTRACT_DETAIL_DEBUG(!untyped_copy_);
...
@code
zr_boost_1_81::contract::make_old(zr_boost_1_81::contract::copy_old() ? old_expr :
        zr_boost_1_81::contract::null_old())
@endcode
...
@param old  Old value which is usually implicitly constructed from the user old
            ...
            @c zr_boost_1_81::contract::copy_old() is @c false).

@return Old value pointer (usually implicitly converted to either
        @RefClass{zr_boost_1_81::contract::old_ptr} or
        @RefClass{zr_boost_1_81::contract::old_ptr_if_copyable} in user code).
*/
/** @cond */ BOOST_CONTRACT_DETAIL_DECLSPEC /** @endcond */
...
@code
zr_boost_1_81::contract::make_old(v, zr_boost_1_81::contract::copy_old(v) ? old_expr :
        zr_boost_1_81::contract::null_old())
@endcode
...
@param old  Old value which is usually implicitly constructed from the user old
            value expression to be copied (use the ternary operator <c>?:</c>
            to completely avoid to evaluate the old value expression when
            @c zr_boost_1_81::contract::copy_old(v) is @c false).
...
inline bool copy_old() {
    #ifndef BOOST_CONTRACT_NO_OLDS
        #ifndef BOOST_CONTRACT_ALL_DISABLE_NO_ASSERTION
            return !zr_boost_1_81::contract::detail::checking::already();
        #else
            return true;
        #endif
    #else
...
*/
    ...
    inline bool copy_old(virtual_* v) {
        ...
        return v->action_ == zr_boost_1_81::contract::virtual_::push_old_init_copy ||
                v->action_ == zr_boost_1_81::contract::virtual_::push_old_ftor_copy;
    }
#else
