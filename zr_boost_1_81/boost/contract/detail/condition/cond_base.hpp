...
// in instances of zr_boost_1_81::contract::check while polymorphically calling
// init and destructor functions to check contracts at entry and exit. This
// could be possible without inheritance only if zr_boost_1_81::contract::check was made
...
class cond_base : // Base to hold all contract objects for RAII.
    private zr_boost_1_81::noncopyable // Avoid copying possible user's ftor captures.
{
public:
    explicit cond_base(zr_boost_1_81::contract::from from) :
    ...
    // Return true if actually checked calling user ftor.
        ...
        bool check_pre(bool throw_on_failure = false) {
            ...
            try { if(pre_) pre_(); else return false; }
                ...
                fail(&zr_boost_1_81::contract::precondition_failure);
            }
            return true;
        }
    #endif
        ...
        void copy_old() {
            if(failed()) return;
            try { if(old_) old_(); }
            catch(...) { fail(&zr_boost_1_81::contract::old_failure); }
        }
    #endif
        ...
        void check_except() {
            ...
            catch(...) { fail(&zr_boost_1_81::contract::except_failure); }
        }
    #endif
    
    #ifndef BOOST_CONTRACT_NO_CONDITIONS
        void fail(void (*h)(zr_boost_1_81::contract::from)) {
            failed(true);
            if(h) h(from_);
        }
...
private:
    ...
    #ifndef BOOST_CONTRACT_NO_CONDITIONS
        zr_boost_1_81::contract::from from_;
        bool failed_;
    #endif
    // Following use Boost.Function to handle also lambdas, binds, etc.
    #ifndef BOOST_CONTRACT_NO_PRECONDITIONS
        zr_boost_1_81::function<void ()> pre_;
    #endif
    #ifndef BOOST_CONTRACT_NO_OLDS
        zr_boost_1_81::function<void ()> old_;
    #endif
    #ifndef BOOST_CONTRACT_NO_EXCEPTS
        zr_boost_1_81::function<void ()> except_;
    #endif
};
