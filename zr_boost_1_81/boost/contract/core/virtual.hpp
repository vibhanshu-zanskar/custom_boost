...
class u {
public:
    virtual void f(int x, zr_boost_1_81::contract::virtual_* v = 0) { // Declare `v`.
...
@RefFunc{zr_boost_1_81::contract::public_function}, @RefMacro{BOOST_CONTRACT_OLDOF}, and
all other operations of this library that accept a pointer to
@RefClass{zr_boost_1_81::contract::virtual_}.
...
private: // No public API (so users cannot use it directly by mistake).
    ...
    // No zr_boost_1_81::noncopyable to avoid its overhead when contracts disabled.
    virtual_(virtual_&);
    ...
    #endif
    #ifndef BOOST_CONTRACT_NO_OLDS
        std::queue<zr_boost_1_81::shared_ptr<void> > old_init_copies_;
        std::queue<zr_boost_1_81::shared_ptr<void> > old_ftor_copies_;
    #endif
    #ifndef BOOST_CONTRACT_NO_POSTCONDITIONS
        zr_boost_1_81::any result_ptr_; // Result for virtual and overriding functions.
