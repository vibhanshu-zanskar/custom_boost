...
namespace pdqsort_detail {
   ...
   struct pair
   {
    ...
    inline bool partial_insertion_sort(Iter begin, Iter end, Compare comp) {
        typedef typename zr_boost_1_81::movelib::iterator_traits<Iter>::value_type T;
        typedef typename zr_boost_1_81::movelib:: iter_size<Iter>::type  size_type;
        ...
        for (Iter cur = begin + 1; cur != end; ++cur) {
            ...
            if (comp(*sift, *sift_1)) {
                T tmp = zr_boost_1_81::move(*sift);

                do { *sift-- = zr_boost_1_81::move(*sift_1); }
                while (sift != begin && comp(tmp, *--sift_1));

                *sift = zr_boost_1_81::move(tmp);
    ...
    inline void sort2(Iter a, Iter b, Compare comp) {
        if (comp(*b, *a)) zr_boost_1_81::adl_move_iter_swap(a, b);
    }
    ...
    pdqsort_detail::pair<Iter, bool> partition_right(Iter begin, Iter end, Compare comp) {
        ...
        T pivot(zr_boost_1_81::move(*begin));
        ...
        // above.
        while (first < last) {
            zr_boost_1_81::adl_move_iter_swap(first, last);
            while (comp(*++first, pivot));
        ...
        if(begin != pivot_pos)   //Avoid potential self-move
            *begin = zr_boost_1_81::move(*pivot_pos);
        *pivot_pos = zr_boost_1_81::move(pivot);
   ...
   void pdqsort_loop( Iter begin, Iter end, Compare comp
                    , typename zr_boost_1_81::movelib:: iter_size<Iter>::type bad_allowed
                    , bool leftmost = true)
   {
        typedef typename zr_boost_1_81::movelib:: iter_size<Iter>::type size_type;
        ...
        while (true) {
            ...
            if (size > ninther_threshold) {
                ...
                zr_boost_1_81::adl_move_iter_swap(begin, begin + s2);
            ...
            if (highly_unbalanced) {
                ...
                if (--bad_allowed == 0) {
                    zr_boost_1_81::movelib::heap_sort(begin, end, comp);
                    return;
                }

                if (l_size >= insertion_sort_threshold) {
                    zr_boost_1_81::adl_move_iter_swap(begin,             begin + l_size / 4);
                    zr_boost_1_81::adl_move_iter_swap(pivot_pos - 1, pivot_pos - l_size / 4);

                    if (l_size > ninther_threshold) {
                        zr_boost_1_81::adl_move_iter_swap(begin + 1,         begin + (l_size / 4 + 1));
                        zr_boost_1_81::adl_move_iter_swap(begin + 2,         begin + (l_size / 4 + 2));
                        zr_boost_1_81::adl_move_iter_swap(pivot_pos - 2, pivot_pos - (l_size / 4 + 1));
                        zr_boost_1_81::adl_move_iter_swap(pivot_pos - 3, pivot_pos - (l_size / 4 + 2));
                    }
                }
                
                if (r_size >= insertion_sort_threshold) {
                    zr_boost_1_81::adl_move_iter_swap(pivot_pos + 1, pivot_pos + (1 + r_size / 4));
                    zr_boost_1_81::adl_move_iter_swap(end - 1,                   end - r_size / 4);
                    
                    if (r_size > ninther_threshold) {
                        zr_boost_1_81::adl_move_iter_swap(pivot_pos + 2, pivot_pos + (2 + r_size / 4));
                        zr_boost_1_81::adl_move_iter_swap(pivot_pos + 3, pivot_pos + (3 + r_size / 4));
                        zr_boost_1_81::adl_move_iter_swap(end - 2,             end - (1 + r_size / 4));
                        zr_boost_1_81::adl_move_iter_swap(end - 3,             end - (2 + r_size / 4));
                    }
                }
            } else {
