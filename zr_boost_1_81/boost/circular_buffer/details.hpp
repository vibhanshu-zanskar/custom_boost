...
struct const_traits {
    ...
    typedef typename zr_boost_1_81::allocator_const_pointer<Alloc>::type pointer;
...
struct assign_range {
    ...
    template <class Pointer>
    void operator () (Pointer p) const {
        zr_boost_1_81::cb_details::uninitialized_copy(m_first, m_last, p, m_alloc);
    }
};
...
inline ForwardIterator uninitialized_copy(InputIterator first, InputIterator last, ForwardIterator dest, Alloc& a) {
    ...
    BOOST_TRY {
        for (; first != last; ++first, ++dest)
            zr_boost_1_81::allocator_construct(a, zr_boost_1_81::to_address(dest), *first);
    } BOOST_CATCH(...) {
        for (; next != dest; ++next)
            zr_boost_1_81::allocator_destroy(a, zr_boost_1_81::to_address(next));
        BOOST_RETHROW
    }
    BOOST_CATCH_END
    return dest;
...
ForwardIterator uninitialized_move_if_noexcept_impl(InputIterator first, InputIterator last, ForwardIterator dest, Alloc& a,
    true_type) {
    for (; first != last; ++first, ++dest)
        zr_boost_1_81::allocator_construct(a, zr_boost_1_81::to_address(dest), zr_boost_1_81::move(*first));
    return dest;
}
...
template<class InputIterator, class ForwardIterator, class Alloc>
ForwardIterator uninitialized_move_if_noexcept(InputIterator first, InputIterator last, ForwardIterator dest, Alloc& a) {
    typedef typename zr_boost_1_81::is_nothrow_move_constructible<typename Alloc::value_type>::type tag_t;
...
inline void uninitialized_fill_n_with_alloc(ForwardIterator first, Diff n, const T& item, Alloc& alloc) {
    ...
    BOOST_TRY {
        for (; n > 0; ++first, --n)
            zr_boost_1_81::allocator_construct(alloc, zr_boost_1_81::to_address(first), item);
    } BOOST_CATCH(...) {
        for (; next != first; ++next)
            zr_boost_1_81::allocator_destroy(alloc, zr_boost_1_81::to_address(next));
        BOOST_RETHROW
    }
    BOOST_CATCH_END
}
