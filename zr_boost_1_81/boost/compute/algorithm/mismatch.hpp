         ...
         command_queue &queue = system::default_queue())
{
    ...
    ::zr_boost_1_81::compute::equal_to<value_type> op;

    InputIterator2 last2 = first2 + std::distance(first1, last1);

    InputIterator1 iter =
        zr_boost_1_81::get<0>(
            ::zr_boost_1_81::compute::find(
                ::zr_boost_1_81::compute::make_transform_iterator(
                    ::zr_boost_1_81::compute::make_zip_iterator(
                        zr_boost_1_81::make_tuple(first1, first2)
                    ),
                    detail::unpack(op)
                ),
                ::zr_boost_1_81::compute::make_transform_iterator(
                    ::zr_boost_1_81::compute::make_zip_iterator(
                        zr_boost_1_81::make_tuple(last1, last2)
                    ),
                    detail::unpack(op)
                ),
                false,
         ...
         command_queue &queue = system::default_queue())
{
    ...
    if(std::distance(first1, last1) < std::distance(first2, last2)){
        return ::zr_boost_1_81::compute::mismatch(first1, last1, first2, queue);
    }
    else {
        return ::zr_boost_1_81::compute::mismatch(
