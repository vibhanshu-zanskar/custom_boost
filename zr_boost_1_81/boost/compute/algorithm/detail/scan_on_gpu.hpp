                                ...
                                command_queue &queue)
{
    ...
    ::zr_boost_1_81::compute::vector<input_type> block_sums(block_count, context);

    // zero block sums
    input_type zero;
    std::memset(&zero, 0, sizeof(input_type));
    ::zr_boost_1_81::compute::fill(block_sums.begin(), block_sums.end(), zero, queue);

    // local scan
    local_scan_kernel<InputIterator, OutputIterator, BinaryOperator>
        local_scan_kernel(first, last, result, exclusive, op);

    ::zr_boost_1_81::compute::kernel kernel = local_scan_kernel.compile(context);
