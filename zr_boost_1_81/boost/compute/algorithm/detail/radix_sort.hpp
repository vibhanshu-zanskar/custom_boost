...
struct is_radix_sortable :
    zr_boost_1_81::mpl::and_<
        typename ::zr_boost_1_81::compute::is_fundamental<T>::type,
        typename zr_boost_1_81::mpl::not_<typename is_vector_type<T>::type>::type
    >
{
};
                            ...
                            command_queue &queue)
{
    ...
    zr_boost_1_81::shared_ptr<parameter_cache> parameters =
    ...
    if(zr_boost_1_81::is_floating_point<value_type>::value){
        options << " -DIS_FLOATING_POINT";
    }

    if(zr_boost_1_81::is_signed<value_type>::value){
        options << " -DIS_SIGNED";
    }
    ...
    std::string custom_type_def = zr_boost_1_81::compute::type_definition<T2>() + "\n";
    ...
    for(uint_ i = 0; i < sizeof(sort_type) * CHAR_BIT / k; i++){
        ...
        if(k == 1){
            ...
            ::zr_boost_1_81::compute::exclusive_scan(
