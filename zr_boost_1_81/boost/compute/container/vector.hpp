...
/// \c int values:
/// \code
/// zr_boost_1_81::compute::vector<int> vec(10, context);
/// \endcode
///
...
/// zr_boost_1_81::compute::vector<int> vec(data, data + 4, queue);
/// \endcode
///
...
/// zr_boost_1_81::compute::vector<float> vec = ...
///
/// // copy data to the vector from a host std:vector
/// zr_boost_1_81::compute::copy(host_vec.begin(), host_vec.end(), vec.begin(), queue);
///
/// // copy data from the vector to a host std::vector
/// zr_boost_1_81::compute::copy(vec.begin(), vec.end(), host_vec.begin(), queue);
///
/// // sort the values in the vector
/// zr_boost_1_81::compute::sort(vec.begin(), vec.end(), queue);
///
/// // calculate the sum of the values in the vector (also see reduce())
/// float sum = zr_boost_1_81::compute::accumulate(vec.begin(), vec.end(), 0, queue);
///
/// // reverse the values in the vector
/// zr_boost_1_81::compute::reverse(vec.begin(), vec.end(), queue);
///
/// // fill the vector with ones
/// zr_boost_1_81::compute::fill(vec.begin(), vec.end(), 1, queue);
/// \endcode
///
...
public:
    ...
    /// zr_boost_1_81::compute::vector<int> vec(4, 9, queue);
    /// \endcode
          ...
          m_allocator(queue.get_context())
    {
        ...
        ::zr_boost_1_81::compute::fill_n(begin(), count, value, queue);
    }
          ...
          m_allocator(queue.get_context())
    {
        ...
        ::zr_boost_1_81::compute::copy(first, last, begin(), queue);
    }
          ...
          m_allocator(other.m_allocator)
    {
        ...
        if(!other.empty()){
            if(other.get_buffer().get_context() != queue.get_context()){
                ...
                ::zr_boost_1_81::compute::copy(other.begin(), other.end(), begin(), other_queue);
                other_queue.finish();
            }
            else {
                ::zr_boost_1_81::compute::copy(other.begin(), other.end(), begin(), queue);
                queue.finish();
            }
        }
    }
          ...
          m_allocator(queue.get_context())
    {
        ...
        ::zr_boost_1_81::compute::copy(vector.begin(), vector.end(), begin(), queue);
    }
          ...
          m_allocator(queue.get_context())
    {
        ...
        ::zr_boost_1_81::compute::copy(list.begin(), list.end(), begin(), queue);
    }
    ...
    iterator begin()
    {
        return ::zr_boost_1_81::compute::make_buffer_iterator<T>(m_data.get_buffer(), 0);
    }
    ...
    iterator end()
    {
        return ::zr_boost_1_81::compute::make_buffer_iterator<T>(m_data.get_buffer(), m_size);
    }
    ...
    void resize(size_type size, command_queue &queue)
    {
        if(size <= capacity()){
            ...
            if(capacity() > 0)
            {
                ...
                ::zr_boost_1_81::compute::copy(m_data, m_data + m_size, new_data, queue);
    ...
    void shrink_to_fit(command_queue &queue)
    {
        ...
        if(m_size > 0)
        {
            ...
            // copy old values to the new buffer
            ::zr_boost_1_81::compute::copy(old_data, old_data + m_size, m_data, queue);
        }
    ...
    void assign(size_type n, const T &value, command_queue &queue)
    {
        ...
        ::zr_boost_1_81::compute::fill_n(begin(), n, value, queue);
    }
    ...
    iterator insert(iterator position, const T &value, command_queue &queue)
    {
        if(position == end()){
            ...
            ::zr_boost_1_81::compute::copy_n(&value, 1, position, queue);
        }
        else {
            ::zr_boost_1_81::compute::vector<T, Alloc> tmp(position, end(), queue);
            resize(m_size + 1, queue);
            position = begin() + position.get_index();
            ::zr_boost_1_81::compute::copy_n(&value, 1, position, queue);
            ::zr_boost_1_81::compute::copy(tmp.begin(), tmp.end(), position + 1, queue);
        }
                ...
                command_queue &queue)
    {
        ...
        ::zr_boost_1_81::compute::fill_n(position, count, value, queue);
        ::zr_boost_1_81::compute::copy(
            tmp.begin(),
            tmp.end(),
                ...
                command_queue &queue)
    {
        ...
        ::zr_boost_1_81::compute::copy(first, last, position, queue);
        ::zr_boost_1_81::compute::copy(
            tmp.begin(),
            tmp.end(),
            position + static_cast<difference_type>(count),
    ...
    {
        if(last != end()){
            ::zr_boost_1_81::compute::vector<T, Alloc> tmp(last, end(), queue);
            ::zr_boost_1_81::compute::copy(tmp.begin(), tmp.end(), first, queue);
        }
...
struct capture_traits<vector<T, Alloc> >
{
    ...
    {
        return std::string("__global ") + ::zr_boost_1_81::compute::type_name<T>() + "*";
    }
};
