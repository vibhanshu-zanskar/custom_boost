...
public:
    ...
    template<class T>
    zr_boost_1_81::optional<T> get_sub_group_info(const device &device, cl_kernel_sub_group_info info,
                                          const size_t input_size, const void * input) const
    {
        ...
        {
            return zr_boost_1_81::optional<T>();
        }
    ...
    /// \overload
    template<class T>
    zr_boost_1_81::optional<T> get_sub_group_info(const device &device, cl_kernel_sub_group_info info) const
    {
...
{
    typename zr_boost_1_81::enable_if<is_fundamental<T> >::type
