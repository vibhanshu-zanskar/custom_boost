...
/// \code
/// zr_boost_1_81::optional<program> p = cache.get("foo");
/// if(p){
...
///
/// \see program
class program_cache : zr_boost_1_81::noncopyable
{
public:
    ...
    zr_boost_1_81::optional<program> get(const std::string &key)
    {
    ...
    zr_boost_1_81::optional<program> get(const std::string &key, const std::string &options)
    {
    ...
    /// Equivalent to:
    /// \code
    /// zr_boost_1_81::optional<program> p = get(key, options);
    /// if(!p){
    ...
    static zr_boost_1_81::shared_ptr<program_cache> get_global_cache(const context &context)
    {
        typedef detail::lru_cache<cl_context, zr_boost_1_81::shared_ptr<program_cache> > cache_map;

        BOOST_COMPUTE_DETAIL_GLOBAL_STATIC(cache_map, caches, (8));

        zr_boost_1_81::optional<zr_boost_1_81::shared_ptr<program_cache> > cache = caches.get(context.get());
        if(!cache){
            cache = zr_boost_1_81::make_shared<program_cache>(64);
