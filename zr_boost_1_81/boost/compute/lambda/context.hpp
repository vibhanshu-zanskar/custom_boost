...
struct context : proto::callable_context<context<Args> >
{
    ...
    context(zr_boost_1_81::compute::detail::meta_kernel &kernel, const Args &args_)
        : stream(kernel),
          args(args_)
    {
    }
    ...
    {
        stream << zr_boost_1_81::get<I>(args);
    }
    ...
    zr_boost_1_81::compute::detail::meta_kernel &stream;
    Args args;
};
...
{
    typedef typename ::zr_boost_1_81::compute::result_of<Expr(Arg)>::type result_type;
...
template<class Expr, class Arg>
zr_boost_1_81::compute::detail::meta_kernel&
operator<<(zr_boost_1_81::compute::detail::meta_kernel &kernel,
           const invoked_unary_expression<Expr, Arg> &expr)
{
    context<zr_boost_1_81::tuple<Arg> > ctx(kernel, zr_boost_1_81::make_tuple(expr.m_arg));
...
{
    typedef typename ::zr_boost_1_81::compute::result_of<Expr(Arg1, Arg2)>::type result_type;
...
{
    context<zr_boost_1_81::tuple<Arg1, Arg2> > ctx(
        kernel,
        zr_boost_1_81::make_tuple(expr.m_arg1, expr.m_arg2)
    );
    proto::eval(expr.m_expr, ctx);
...
struct expression : proto::extends<Expr, expression<Expr>, domain>
{
    ...
    {
        typedef
            typename ::zr_boost_1_81::compute::lambda::result_of<Expr>::type type;
    };
    ...
    {
        typedef
            typename ::zr_boost_1_81::compute::lambda::result_of<
                Expr,
                typename zr_boost_1_81::tuple<Arg>
            >::type type;
    };
    ...
    {
        typedef typename
            ::zr_boost_1_81::compute::lambda::result_of<
                Expr,
                typename zr_boost_1_81::tuple<Arg1, Arg2>
            >::type type;
    };
    ...
    {
        using ::zr_boost_1_81::compute::detail::meta_kernel;

        std::stringstream source;

        ::zr_boost_1_81::compute::detail::meta_kernel_variable<A1> arg1("x");

        source << "inline " << type_name<R>() << " lambda"
               << ::zr_boost_1_81::compute::detail::generate_argument_list<R(A1)>('x')
               << "{\n"
    ...
    operator function<R(A1, A2)>() const
    {
        ...
        ::zr_boost_1_81::compute::detail::meta_kernel_variable<A1> arg1("x");
        ::zr_boost_1_81::compute::detail::meta_kernel_variable<A1> arg2("y");

        source << "inline " << type_name<R>() << " lambda"
               << ::zr_boost_1_81::compute::detail::generate_argument_list<R(A1, A2)>('x')
               << "{\n"
