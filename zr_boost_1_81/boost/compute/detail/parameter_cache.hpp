...
class parameter_cache : zr_boost_1_81::noncopyable
{
public:
          ...
          m_device_name(device.name())
    {
        ...
        if(zr_boost_1_81::filesystem::exists(m_file_name)){
            read_from_disk();
        }
    #endif // BOOST_COMPUTE_USE_OFFLINE_CACHE
    }
    ...
    static zr_boost_1_81::shared_ptr<parameter_cache> get_global_cache(const device &device)
    {
        // device name -> parameter cache
        typedef std::map<std::string, zr_boost_1_81::shared_ptr<parameter_cache> > cache_map;

        BOOST_COMPUTE_DETAIL_GLOBAL_STATIC(cache_map, caches, ((std::less<std::string>())));

        cache_map::iterator iter = caches.find(device.name());
        if(iter == caches.end()){
            zr_boost_1_81::shared_ptr<parameter_cache> cache =
                zr_boost_1_81::make_shared<parameter_cache>(device);
...
private:
    ...
    {
        zr_boost_1_81::algorithm::trim(name);
    ...
    void write_to_disk()
    {
        ...
        if(m_dirty){
            ...
            zr_boost_1_81::property_tree::ptree pt;
    ...
    void read_from_disk()
    {
        ...
        catch(zr_boost_1_81::property_tree::json_parser::json_parser_error&){
        ...
        if(stored_device == m_device_name && stored_version == version_string()){
            typedef zr_boost_1_81::property_tree::ptree::const_iterator pt_iter;
            for(pt_iter iter = pt.begin(); iter != pt.end(); ++iter){
                ...
                zr_boost_1_81::property_tree::ptree child_pt = pt.get_child(iter->first);
                for(pt_iter child_iter = child_pt.begin(); child_iter != child_pt.end(); ++child_iter){
                    set(iter->first, child_iter->first, zr_boost_1_81::lexical_cast<uint_>(child_iter->second.data()));
                }
            }
        }
