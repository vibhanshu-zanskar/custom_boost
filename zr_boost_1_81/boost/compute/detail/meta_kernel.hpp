...
struct meta_kernel_stored_arg
{
    ...
    {
        set_value(sizeof(T), zr_boost_1_81::addressof(value));
    }
...
public:
    ...
    kernel compile(const context &context, const std::string &options = std::string())
    {
        ...
        // load program cache
        zr_boost_1_81::shared_ptr<program_cache> cache =
            program_cache::get_global_cache(context);

        std::string compile_options = m_options + options;

        // load (or build) program from cache
        ::zr_boost_1_81::compute::program program =
            cache->get_or_build(cache_key, compile_options, source, context);

        // create kernel
        ::zr_boost_1_81::compute::kernel kernel = program.create_kernel(name());
    ...
    std::string type() const
    {
        ...
        // const qualifier
        if(zr_boost_1_81::is_const<T>::value){
            stream << "const ";
        }

        // volatile qualifier
        if(zr_boost_1_81::is_volatile<T>::value){
            stream << "volatile ";
        }
        ...
        // pointer
        if(zr_boost_1_81::is_pointer<T>::value){
            stream << "*";
        }
                  ...
                  const wait_list &events = wait_list())
    {
        ...
        ::zr_boost_1_81::compute::kernel kernel = compile(context);
    ...
    detail::invoked_function<bool, zr_boost_1_81::tuple<Predicate> > if_(Predicate pred) const
    {
        return detail::invoked_function<bool, zr_boost_1_81::tuple<Predicate> >(
            "if", std::string(), zr_boost_1_81::make_tuple(pred)
        );
    }

    template<class Predicate>
    detail::invoked_function<bool, zr_boost_1_81::tuple<Predicate> > else_if_(Predicate pred) const
    {
        return detail::invoked_function<bool, zr_boost_1_81::tuple<Predicate> >(
            "else if", std::string(), zr_boost_1_81::make_tuple(pred)
        );
    }
    ...
    void insert_function_call_args(const zr_boost_1_81::tuple<>&)
    {
    }
