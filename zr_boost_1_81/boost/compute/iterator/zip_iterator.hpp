...
public:
    typedef ::zr_boost_1_81::iterator_facade<
        ::zr_boost_1_81::compute::zip_iterator<IteratorTuple>,
...
#define BOOST_COMPUTE_PRINT_ELEM(z, n, unused)                                 \
        BOOST_PP_EXPR_IF(n, << ", ")                                           \
        << zr_boost_1_81::get<n>(expr.m_iterators)[expr.m_index_expr]
...
inline meta_kernel& operator<<(                                                \
        ...
        const zip_iterator_index_expr<                                         \
                  zr_boost_1_81::tuple<BOOST_PP_ENUM_PARAMS(n, Iterator)>,             \
                  IndexExpr                                                    \
              > &expr)                                                         \
{                                                                              \
    typedef typename                                                           \
...
private:
    ...
    void increment()
    {
        zr_boost_1_81::fusion::for_each(m_iterators, detail::increment_iterator);
    }

    void decrement()
    {
        zr_boost_1_81::fusion::for_each(m_iterators, detail::decrement_iterator);
    }

    void advance(difference_type n)
    {
        zr_boost_1_81::fusion::for_each(m_iterators, detail::iterator_advancer(n));
    }

    difference_type distance_to(const zip_iterator<IteratorTuple> &other) const
    {
        return std::distance(zr_boost_1_81::get<0>(m_iterators),
                             zr_boost_1_81::get<0>(other.m_iterators));
    }
...
/// \p c):
/// \code
/// auto zipped = zr_boost_1_81::compute::make_zip_iterator(
///     zr_boost_1_81::make_tuple(a.begin(), b.begin(), c.begin())
/// );
/// \endcode
template<class IteratorTuple>
...
template<class IteratorTuple>
struct is_device_iterator<zip_iterator<IteratorTuple> > : zr_boost_1_81::true_type {};
...
operator<<(meta_kernel &kernel,                                                \
           const invoked_get<                                                  \
               ...
               zr_boost_1_81::tuple<BOOST_PP_ENUM_PARAMS(n, T)>                        \
            > &expr)                                                           \
{                                                                              \
    typedef typename zr_boost_1_81::tuple<BOOST_PP_ENUM_PARAMS(n, T)> Tuple;           \
    typedef typename zr_boost_1_81::tuples::element<N, Tuple>::type T;                 \
    BOOST_STATIC_ASSERT(N < size_t(zr_boost_1_81::tuples::length<Tuple>::value));      \
    kernel.inject_type<T>();                                                   \
    return kernel                                                              \
        << zr_boost_1_81::get<N>(expr.m_arg.m_iterators)[expr.m_arg.m_index_expr];     \
}
