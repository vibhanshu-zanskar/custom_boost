...
protected:
    ...
    token_type const &expand_tokensequence_worker(ContainerT &pending,
        ...
        bool& seen_newline, bool expand_operator_defined,
        bool expand_operator_has_include,
        zr_boost_1_81::optional<position_type> expanding_pos);
...
private:
    ...
    zr_boost_1_81::shared_ptr<defined_macros_type> defined_macros; // global symbol table
...
#if BOOST_WAVE_SUPPORT_HAS_INCLUDE != 0
    if (zr_boost_1_81::wave::need_has_include(ctx.get_language()) &&
...
#if BOOST_WAVE_SUPPORT_HAS_INCLUDE != 0
    return (zr_boost_1_81::wave::need_has_include(ctx.get_language()) &&
            (name == "__has_include"));
#else
    return false;
#endif
...
#if BOOST_WAVE_SUPPORT_HAS_INCLUDE != 0
            else if (zr_boost_1_81::wave::need_has_include(ctx.get_language()) &&
...
#endif
            else if (zr_boost_1_81::wave::need_variadics(ctx.get_language()) &&
    ...
    for (;cit != cend; ++cit)
    {
        ...
        if (IS_CATEGORY((*cit), ParameterTokenType)) {
            ...
            if (IS_EXTCATEGORY((*cit), ExtParameterTokenType)) {
                BOOST_ASSERT(zr_boost_1_81::wave::need_variadics(ctx.get_language()));
            ...
            if (IS_EXTCATEGORY((*cit), OptParameterTokenType)) {
                BOOST_ASSERT(zr_boost_1_81::wave::need_va_opt(ctx.get_language()));
...
#if BOOST_WAVE_SUPPORT_VARIADICS_PLACEMARKERS != 0
                if (is_ellipsis && zr_boost_1_81::wave::need_variadics(ctx.get_language())) {
...
#if BOOST_WAVE_SUPPORT_CPP2A != 0
                    else if (zr_boost_1_81::wave::need_cpp2a(ctx.get_language())) {
                        BOOST_ASSERT(i == arguments.size());
    ...
    defined_macros_type *scope, ContainerT *queue_symbol)
{
    ...
    if (T_LEFTPAREN == impl::next_token<IteratorT>::peek(first, last)) {
        ...
        if (macro_def.is_functionlike) {
            ...
            if (zr_boost_1_81::wave::need_cpp2a(ctx.get_language())) {
...
{
    using namespace zr_boost_1_81::wave;
    using namespace zr_boost_1_81::wave::grammars;

ContainerT result;
IteratorT start = first;
zr_boost_1_81::spirit::classic::parse_info<IteratorT> hit =
    ...
    position_type const &pos, ContainerT &rescanned)
{
    ...
    zr_boost_1_81::wave::language_support lang =
        zr_boost_1_81::wave::enable_prefer_pp_numbers(ctx.get_language());
    lang = zr_boost_1_81::wave::enable_single_line(lang);
    ...
    defined_macros_type *scope, bool at_global_scope)
{
...
#if BOOST_WAVE_SUPPORT_VARIADICS_PLACEMARKERS != 0
    if (zr_boost_1_81::wave::need_c99(ctx.get_language())) {
...
#endif
    {
        ...
        if (zr_boost_1_81::wave::need_cpp0x(ctx.get_language())) {
...
template<typename ContextT>
struct version<zr_boost_1_81::wave::util::macromap<ContextT> >
{
    typedef zr_boost_1_81::wave::util::macromap<ContextT> target_type;
