...
#define BOOST_WAVE_LEXER_THROW(cls, code, msg, line, column, name)            \
    {                                                                         \
        ...
        zr_boost_1_81::throw_exception(cls(throwmsg.c_str(), cls::code, line, column, \
            name));                                                           \
    }                                                                         \
    /**/
#else
...
#define BOOST_WAVE_LEXER_THROW(cls, code, msg, line, column, name)                \
    {                                                                             \
        ...
        if ((msg)[0] != 0) stream << ": " << (msg);                               \
        stream << std::ends;                                                      \
        zr_boost_1_81::throw_exception(cls(stream.str().c_str(), cls::code, line, column, \
            name));                                                               \
    }                                                                             \
    /**/
...
#define BOOST_WAVE_LEXER_THROW_VAR(cls, codearg, msg, line, column, name) \
    {                                                                     \
        ...
        stream << std::ends;                                              \
        std::string throwmsg = stream.str(); stream.freeze(false);        \
        zr_boost_1_81::throw_exception(cls(throwmsg.c_str(), code, line, column,  \
            name));                                                       \
    }                                                                     \
    /**/
#else
...
#define BOOST_WAVE_LEXER_THROW_VAR(cls, codearg, msg, line, column, name)    \
    {                                                                        \
        ...
        if ((msg)[0] != 0) stream << ": " << (msg);                          \
        stream << std::ends;                                                 \
        zr_boost_1_81::throw_exception(cls(stream.str().c_str(), code, line, column, \
            name));                                                          \
    }                                                                        \
    /**/
#endif // BOOST_NO_STRINGSTREAM
...
public:
    ...
    {
        return "zr_boost_1_81::wave::lexing_exception";
    }
