...
template <typename IteratorT,
    typename PositionT = zr_boost_1_81::wave::util::file_position_type,
...
public:
    ...
    lexer(IteratorT const &first, IteratorT const &last,
        PositionT const &pos, zr_boost_1_81::wave::language_support language_);
    ~lexer();
...
private:
    ...
    string_type value;
    bool at_eof;
    zr_boost_1_81::wave::language_support language;
#if BOOST_WAVE_SUPPORT_PRAGMA_ONCE != 0
...
lexer<IteratorT, PositionT, TokenT>::lexer(IteratorT const &first,
        ...
        zr_boost_1_81::wave::language_support language_)
    : scanner(first, last),
...
#endif
{
    ...
    scanner.act_in_c99_mode = zr_boost_1_81::wave::need_c99(language_);
#endif

#if BOOST_WAVE_SUPPORT_IMPORT_KEYWORD != 0
    scanner.enable_import_keyword = !zr_boost_1_81::wave::need_c99(language_);
#else
    scanner.enable_import_keyword = false;
#endif

    scanner.detect_pp_numbers = zr_boost_1_81::wave::need_prefer_pp_numbers(language_);
    scanner.single_line_only = zr_boost_1_81::wave::need_single_line(language_);

#if BOOST_WAVE_SUPPORT_CPP0X != 0
    scanner.act_in_cpp0x_mode = zr_boost_1_81::wave::need_cpp0x(language_);
#else
    scanner.act_in_cpp0x_mode = false;
#endif

#if BOOST_WAVE_SUPPORT_CPP2A != 0
    scanner.act_in_cpp2a_mode = zr_boost_1_81::wave::need_cpp2a(language_);
    scanner.act_in_cpp0x_mode = zr_boost_1_81::wave::need_cpp2a(language_)
        || zr_boost_1_81::wave::need_cpp0x(language_);
#else
...
lexer<IteratorT, PositionT, TokenT>::get(TokenT& result)
{
    ...
    // test identifier characters for validity (throws if invalid chars found)
        ...
        if (!zr_boost_1_81::wave::need_no_character_validation(language))
    ...
    // test literal characters for validity (throws if invalid chars found)
        value = string_type((char const *)scanner.tok,
            scanner.cur-scanner.tok);
        if (zr_boost_1_81::wave::need_convert_trigraphs(language))
    ...
    case T_LONGINTLIT:  // supported in C++11, C99 and long_long mode
        value = string_type((char const *)scanner.tok,
            scanner.cur-scanner.tok);
        if (!zr_boost_1_81::wave::need_long_long(language)) {
...
//     std::cerr << zr_boost_1_81::wave::get_token_name(id) << ": " << value << std::endl;
...
public:
    ...
    lex_functor(IteratorT const &first, IteratorT const &last,
            PositionT const &pos, zr_boost_1_81::wave::language_support language)
