...
public:
    ...
    const zr_boost_1_81::core::typeinfo& component_type(int n) const
    {
        if (static_cast<size_type>(n) >= size())
            zr_boost_1_81::throw_exception(std::out_of_range("bad chain offset"));
        return (*zr_boost_1_81::next(list().begin(), n))->component_type();
    }

    // Deprecated.
    template<int N>
    const zr_boost_1_81::core::typeinfo& component_type() const { return component_type(N); }

    template<typename T>
    T* component(int n) const { return component(n, zr_boost_1_81::type<T>()); }
...
#endif
    template<typename T>
    T* component(int n, zr_boost_1_81::type<T>) const
    {
        if (static_cast<size_type>(n) >= size())
            zr_boost_1_81::throw_exception(std::out_of_range("bad chain offset"));
        streambuf_type* link = *zr_boost_1_81::next(list().begin(), n);
...
private:
                   ...
                   std::streamsize pback_size = -1)
    {
        ...
        if (is_complete())
            zr_boost_1_81::throw_exception(std::logic_error("chain complete"));
        ...
        void close()
            {
                if ((flags_ & f_open) != 0) {
                    ...
                    try {
                        zr_boost_1_81::iostreams::detail::execute_foreach(
...
#define BOOST_IOSTREAMS_DECL_CHAIN(name_, default_char_) \
    ...
    class name_ : public zr_boost_1_81::iostreams::detail::chain_base< \
    ...
    public: \
        ...
        typedef Mode                                   mode; \
    private: \
        typedef zr_boost_1_81::iostreams::detail::chain_base< \
                    name_<Mode, Ch, Tr, Alloc>, \
                    Ch, Tr, Alloc, Mode \
                >                                      base_type; \
    public: \
...
public:
    ...
    // Deprecated.
    template<int N>
    const zr_boost_1_81::core::typeinfo& component_type() const
