...
protected:
    ...
    const zr_boost_1_81::core::typeinfo& component_type() const { return BOOST_CORE_TYPEID(T); }
...
{
    if (!ibeg_) 
        zr_boost_1_81::throw_exception(cant_read());
    if (!gptr()) 
        init_get_area();
    return gptr() != iend_ ? 
...
direct_streambuf<T, Tr>::pbackfail(int_type c)
{
    ...
    zr_boost_1_81::throw_exception(bad_putback());
}
...
direct_streambuf<T, Tr>::overflow(int_type c)
{
    using namespace std;
    if (!obeg_)
        zr_boost_1_81::throw_exception(BOOST_IOSTREAMS_FAILURE("no write access"));
...
void direct_streambuf<T, Tr>::close_impl(BOOST_IOS::openmode which)
{
    ...
    zr_boost_1_81::iostreams::close(*storage_, which);
}
