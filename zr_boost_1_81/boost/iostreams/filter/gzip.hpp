...
namespace zr_boost_1_81 { namespace iostreams {
                    
//------------------Definitions of constants----------------------------------//

namespace gzip {

using namespace zr_boost_1_81::iostreams::zlib;
...
public:
    ...
    std::streamsize write(Sink& snk, const char_type* s, std::streamsize n)
    {
        if (!(flags_ & f_header_done)) {
            ...
            offset_ += zr_boost_1_81::iostreams::write_if(snk, header_.data() + offset_, amt);
...
private:
    ...
    template<typename Sink>
    static void write_long(long n, Sink& next, zr_boost_1_81::mpl::true_)
    {
        zr_boost_1_81::iostreams::put(next, static_cast<char>(0xFF & n));
        zr_boost_1_81::iostreams::put(next, static_cast<char>(0xFF & (n >> 8)));
        zr_boost_1_81::iostreams::put(next, static_cast<char>(0xFF & (n >> 16)));
        zr_boost_1_81::iostreams::put(next, static_cast<char>(0xFF & (n >> 24)));
    }
    template<typename Sink>
    static void write_long(long, Sink&, zr_boost_1_81::mpl::false_)
    {
    }
    template<typename Sink>
...
} // End namespace zr_boost_1_81::iostreams::detail.
...
public:
        ...
        while(result < n) {
            ...
            if (state_ == s_header) {
                ...
                try {
                    ...
                    zr_boost_1_81::throw_exception(gzip_error(e));
                }
            } else { // state_ == s_footer
                if (footer_.done()) {
                    if (footer_.crc() != this->crc())
                        zr_boost_1_81::throw_exception(gzip_error(gzip::bad_crc));
    ...
    std::streamsize read(Source& src, char_type* s, std::streamsize n)
    {
        ...
        while (result < n && state_ != s_done) {
            ...
            if (state_ == s_header) {
                int c = zr_boost_1_81::iostreams::get(peek);
                if (traits_type::is_eof(c)) {
                    zr_boost_1_81::throw_exception(gzip_error(gzip::bad_header));
                ...
                int c = zr_boost_1_81::iostreams::get(peek);
                if (traits_type::is_eof(c)) {
                    zr_boost_1_81::throw_exception(gzip_error(gzip::bad_footer));
                } else if (traits_type::would_block(c)) {
                    break;
                }
                footer_.process(c);
                if (footer_.done()) {
                    if (footer_.crc() != this->crc())
                        zr_boost_1_81::throw_exception(gzip_error(gzip::bad_crc));
                    c = zr_boost_1_81::iostreams::get(peek);
                    if (traits_type::is_eof(c)) {
                        state_ = s_done;
                    } else {
                        peek.putback(c);
...
private:
    ...
    struct peekable_source {
        ...
        std::streamsize read(char* s, std::streamsize n)
        {
            ...
            std::streamsize amt = 
                zr_boost_1_81::iostreams::read(src_, s + result, n - result);
            return amt != -1 ? 
                result + amt : 
                result ? result : -1;
        }
        bool putback(char c)
        {
            if (offset_) {
                ...
                zr_boost_1_81::throw_exception(
                    zr_boost_1_81::iostreams::detail::bad_putback());
            }
            return true;
        }
...
{
    zr_boost_1_81::iostreams::back_insert_device<std::string> out(footer_);
