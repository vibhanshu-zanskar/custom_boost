...
#define BOOST_IOSTREAMS_PIPABLE(filter, arity) \
    ...
    ::zr_boost_1_81::iostreams::pipeline< \
        ::zr_boost_1_81::iostreams::detail::pipeline_segment< \
    ...
    > operator|( const filter BOOST_IOSTREAMS_TEMPLATE_ARGS(arity, T)& f, \
                 const Component& c ) \
    { \
        typedef ::zr_boost_1_81::iostreams::detail::pipeline_segment< \
                    filter BOOST_IOSTREAMS_TEMPLATE_ARGS(arity, T) \
                > segment; \
        return ::zr_boost_1_81::iostreams::pipeline<segment, Component> \
                   (segment(f), c); \
    } \
    /**/
