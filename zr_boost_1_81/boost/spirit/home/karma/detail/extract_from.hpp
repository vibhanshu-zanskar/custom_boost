...
namespace zr_boost_1_81 { namespace spirit { namespace traits
{
    ...
    template <typename Attribute, typename Exposed>
    struct extract_from_attribute<zr_boost_1_81::optional<Attribute>, Exposed>
    {
        typedef Attribute const& type;

        template <typename Context>
        static type call(zr_boost_1_81::optional<Attribute> const& attr, Context& ctx)
        {
            return extract_from<Exposed>(zr_boost_1_81::get<Attribute>(attr), ctx);
        }
    };

    template <typename Attribute, typename Exposed>
    struct extract_from_attribute<zr_boost_1_81::optional<Attribute const>, Exposed>
    {
        typedef Attribute const& type;

        template <typename Context>
        static type call(zr_boost_1_81::optional<Attribute const> const& attr, Context& ctx)
        {
            return extract_from<Exposed>(zr_boost_1_81::get<Attribute const>(attr), ctx);
        }
    };

    // This handles attributes wrapped inside a zr_boost_1_81::ref().
    template <typename Attribute, typename Exposed>
