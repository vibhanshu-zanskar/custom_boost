...
namespace zr_boost_1_81 { namespace spirit { namespace karma
{
            ...
            symbols<Attribute, T, Lookup, CharEncoding, Tag> >
    {
        ...
        symbols(Symbols const& syms, Data const& data
          ...
          , name_(name)
        {
            typename range_const_iterator<Symbols>::type si = zr_boost_1_81::begin(syms);
            typename range_const_iterator<Data>::type di = zr_boost_1_81::begin(data);
            while (si != zr_boost_1_81::end(syms))
                add(*si++, *di++);
        }
