...
namespace zr_boost_1_81 { namespace spirit { namespace ucd
{
    ...
    inline properties::category get_category(::zr_boost_1_81::uint32_t ch)
    {
        return static_cast<properties::category>(detail::category_lookup(ch) & 0x3F);
    }

    inline properties::major_category get_major_category(::zr_boost_1_81::uint32_t ch)
    {
        return static_cast<properties::major_category>(get_category(ch) >> 3);
    }

    inline bool is_punctuation(::zr_boost_1_81::uint32_t ch)
    {
        return get_major_category(ch) == properties::punctuation;
    }

    inline bool is_decimal_number(::zr_boost_1_81::uint32_t ch)
    {
        return get_category(ch) == properties::decimal_number;
    }

    inline bool is_hex_digit(::zr_boost_1_81::uint32_t ch)
    {
        return (detail::category_lookup(ch) & properties::hex_digit) != 0;
    }

    inline bool is_control(::zr_boost_1_81::uint32_t ch)
    {
        return get_category(ch) == properties::control;
    }

    inline bool is_alphabetic(::zr_boost_1_81::uint32_t ch)
    {
        return (detail::category_lookup(ch) & properties::alphabetic) != 0;
    }

    inline bool is_alphanumeric(::zr_boost_1_81::uint32_t ch)
    {
        return is_decimal_number(ch) || is_alphabetic(ch);
    }

    inline bool is_uppercase(::zr_boost_1_81::uint32_t ch)
    {
        return (detail::category_lookup(ch) & properties::uppercase) != 0;
    }

    inline bool is_lowercase(::zr_boost_1_81::uint32_t ch)
    {
        return (detail::category_lookup(ch) & properties::lowercase) != 0;
    }

    inline bool is_white_space(::zr_boost_1_81::uint32_t ch)
    {
        return (detail::category_lookup(ch) & properties::white_space) != 0;
    }

    inline bool is_blank(::zr_boost_1_81::uint32_t ch)
    {
        switch (ch)
        {
    ...
    inline bool is_graph(::zr_boost_1_81::uint32_t ch)
    {
        return !(   is_white_space(ch)
    ...
    inline bool is_print(::zr_boost_1_81::uint32_t ch)
    {
        return (is_graph(ch) || is_blank(ch)) && !is_control(ch);
    }

    inline bool is_noncharacter_code_point(::zr_boost_1_81::uint32_t ch)
    {
        return (detail::category_lookup(ch) & properties::noncharacter_code_point) != 0;
    }

    inline bool is_default_ignorable_code_point(::zr_boost_1_81::uint32_t ch)
    {
        return (detail::category_lookup(ch) & properties::default_ignorable_code_point) != 0;
    }

    inline properties::script get_script(::zr_boost_1_81::uint32_t ch)
    {
        return static_cast<properties::script>(detail::script_lookup(ch) & 0x7F);
    }

    inline ::zr_boost_1_81::uint32_t to_lowercase(::zr_boost_1_81::uint32_t ch)
    {
        // The table returns 0 to signal that this code maps to itself
        ::zr_boost_1_81::uint32_t r = detail::lowercase_lookup(ch);
        return (r == 0)? ch : r;
    }

    inline ::zr_boost_1_81::uint32_t to_uppercase(::zr_boost_1_81::uint32_t ch)
    {
        // The table returns 0 to signal that this code maps to itself
        ::zr_boost_1_81::uint32_t r = detail::uppercase_lookup(ch);
        return (r == 0)? ch : r;
    }
}}}
