...
namespace zr_boost_1_81 { namespace spirit { namespace lex { namespace lexertl
{
    ...
    namespace detail
    {
        ...
        //
        ///////////////////////////////////////////////////////////////////////
        inline zr_boost_1_81::lexer::regex_flags map_flags(unsigned int flags)
        {
            unsigned int retval = zr_boost_1_81::lexer::none;
            if (flags & match_flags::match_not_dot_newline)
                retval |= zr_boost_1_81::lexer::dot_not_newline;
            if (flags & match_flags::match_icase)
                retval |= zr_boost_1_81::lexer::icase;

            return zr_boost_1_81::lexer::regex_flags(retval);
        }
    }
    ...
    public:
        ...
        typedef zr_boost_1_81::lexer::basic_rules<char_type> basic_rules_type;
...
#  pragma warning(disable: 4512) // assignment operator could not be generated.
        ...
        struct iterator_data_type
        {
            ...
            iterator_data_type(
                    zr_boost_1_81::lexer::basic_state_machine<char_type> const& sm
                  , zr_boost_1_81::lexer::basic_rules<char_type> const& rules
                  , semantic_actions_type const& actions)
              : state_machine_(sm), rules_(rules), actions_(actions)
            {}

            zr_boost_1_81::lexer::basic_state_machine<char_type> const& state_machine_;
            zr_boost_1_81::lexer::basic_rules<char_type> const& rules_;
...
#  pragma warning(pop)
    ...
    public:
        ...
        zr_boost_1_81::lexer::rules const& get_rules() const { return rules_; }

        void clear(char_type const* state)
        {
            std::size_t s = rules_.state(state);
            if (zr_boost_1_81::lexer::npos != s)
                rules_.clear(state);
            initialized_dfa_ = false;
        }
        std::size_t add_state(char_type const* state)
        {
            ...
            if (zr_boost_1_81::lexer::npos == stateid) {
...
//         }
        ...
        bool init_dfa(bool minimize = false) const
        {
            if (!initialized_dfa_) {
                ...
                typedef zr_boost_1_81::lexer::basic_generator<char_type> generator;
                generator::build (rules_, state_machine_);
                if (minimize)
                    generator::minimise (state_machine_);

#if defined(BOOST_SPIRIT_LEXERTL_DEBUG)
                zr_boost_1_81::lexer::debug::dump(state_machine_, std::cerr);
#endif
...
//                 std::swap(rules, rules_);
    ...
    private:
        ...
        mutable zr_boost_1_81::lexer::basic_state_machine<char_type> state_machine_;
        zr_boost_1_81::lexer::regex_flags flags_;
