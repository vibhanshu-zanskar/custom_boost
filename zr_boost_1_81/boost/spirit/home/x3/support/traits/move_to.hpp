...
namespace zr_boost_1_81 { namespace spirit { namespace x3 { namespace traits
{
    ...
    namespace detail
    {
        ...
        template <typename Iterator>
        inline void
        move_to(Iterator first, Iterator last, zr_boost_1_81::iterator_range<Iterator>& rng, range_attribute)
        {
            rng = {first, last};
        }
    }
    ...
    inline void move_to(T& src, T& dest)
    {
        BOOST_ASSERT(zr_boost_1_81::addressof(src) != zr_boost_1_81::addressof(dest));
        dest = std::move(src);
    }
