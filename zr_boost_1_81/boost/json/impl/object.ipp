...
std::size_t
std::hash<::zr_boost_1_81::json::object>::operator()(
    ::zr_boost_1_81::json::object const& jo) const noexcept
{
    std::size_t seed = jo.size();
    for (const auto& kv_pair : jo) {
        auto const hk = ::zr_boost_1_81::json::detail::digest(
            kv_pair.key().begin(), kv_pair.key().end(), 0);
        auto const hkv = ::zr_boost_1_81::json::detail::hash_combine(
            hk,
            std::hash<::zr_boost_1_81::json::value>{}(kv_pair.value()));
        seed = ::zr_boost_1_81::json::detail::hash_combine_commutative(seed, hkv);
    }
    return seed;
}
