...
template<class T> class remove_reference_if_valid
{
  ...
  typedef typename zr_boost_1_81::remove_reference<T>::type plainT;
public:
  typedef typename IF<
    zr_boost_1_81::is_function<plainT>::value,
    T,
    plainT
  >::RET type;
...
template<int N, class T> struct tuple_element_as_reference {   
  typedef typename
     zr_boost_1_81::tuples::access_traits<
       typename zr_boost_1_81::tuples::element<N, T>::type
     >::non_const_type type;
};
...
struct parameter_traits_<zr_boost_1_81::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<const zr_boost_1_81::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<volatile zr_boost_1_81::reference_wrapper<T>, Any >{
  typedef T& type;
};

template<class T, class Any> 
struct parameter_traits_<const volatile zr_boost_1_81::reference_wrapper<T>, Any >{
  typedef T& type;
};
...
struct const_copy_argument {
  typedef typename 
    detail::parameter_traits_<
      T,
      typename detail::IF<zr_boost_1_81::is_function<T>::value, T&, const T>::RET
    >::type type;
};
...
class bind_type_generator {
  ...
  BOOST_STATIC_CONSTANT(int, nof_elems = zr_boost_1_81::tuples::length<args_t>::value);
