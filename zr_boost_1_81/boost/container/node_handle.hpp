...
class node_handle
{
   ...
   typedef ::zr_boost_1_81::aligned_storage
      ...
      , zr_boost_1_81::alignment_of<nallocator_type>::value>              nalloc_storage_t;

   node_pointer      m_ptr;
   nalloc_storage_t  m_nalloc_storage;

   void move_construct_alloc(nallocator_type &al)
   {  ::new(m_nalloc_storage.address(), boost_container_new_t()) nallocator_type(::zr_boost_1_81::move(al));   }

   void destroy_deallocate_node()
   {
      zr_boost_1_81::movelib::to_raw_pointer(m_ptr)->destructor(this->node_alloc());
   ...
   {
      if(m_ptr){
         ::new (m_nalloc_storage.address(), boost_container_new_t()) nallocator_type(::zr_boost_1_81::move(nh.node_alloc()));
      ...
      if(was_nh_non_null){
         if(was_this_non_null){
            ...
            if(nator_traits::propagate_on_container_move_assignment::value){
               this->node_alloc() = ::zr_boost_1_81::move(nh.node_alloc());
            }
         }
         else{
      ...
      BOOST_NOEXCEPT_IF(nator_traits::propagate_on_container_swap::value || nator_traits::is_always_equal::value)
   {
      ...
      if(was_nh_non_null){
         if(was_this_non_null){
            if(nator_traits::propagate_on_container_swap::value){
               ::zr_boost_1_81::adl_move_swap(this->node_alloc(), nh.node_alloc());
            }
         }
         else{
            this->move_construct_alloc(nh.node_alloc());
      ...
      ::zr_boost_1_81::adl_move_swap(m_ptr, nh.m_ptr);
   }
...
struct insert_return_type_base
{
   ...
   insert_return_type_base(BOOST_RV_REF(insert_return_type_base) other)
      : inserted(other.inserted), position(other.position), node(zr_boost_1_81::move(other.node))
   {}

   template<class RelatedIt, class RelatedNode>
   insert_return_type_base(bool insert, RelatedIt it, BOOST_RV_REF(RelatedNode) n)
      : inserted(insert), position(it), node(zr_boost_1_81::move(n))
   {}
   ...
   insert_return_type_base & operator=(BOOST_RV_REF(insert_return_type_base) other)
   {
      ...
      node = zr_boost_1_81::move(other.node);
      return *this;
   }
