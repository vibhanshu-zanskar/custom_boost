   ...
   :  public HookDefiner::type
{
   ...
   explicit base_node(Alloc &a, Args &&...args)
      : hook_type()
   {
      ::zr_boost_1_81::container::allocator_traits<Alloc>::construct
         (a, &this->get_real_data(), ::zr_boost_1_81::forward<Args>(args)...);
   }
   ...
   explicit base_node(iterator_arg_t, Alloc &a, It it)
      : hook_type()
   {
      ::zr_boost_1_81::container::construct_in_place(a, &this->get_real_data(), it);
   }
   ...
   {
      typedef typename Pair::first_type first_type;
      const_cast<first_type&>(this->get_real_data().first) = ::zr_boost_1_81::move(p.first);
      this->get_real_data().second = ::zr_boost_1_81::move(p.second);
   }
   ...
   typename dtl::disable_if< dtl::is_pair<V>, void >::type
      do_move_assign(V &v)
   {  this->get_real_data() = ::zr_boost_1_81::move(v); }
            ...
            portable_rebind_alloc<typename ICont::value_type>::type   //NodeAlloc
{
   ...
   explicit node_alloc_holder(BOOST_RV_REF(node_alloc_holder) x)
      : NodeAlloc(zr_boost_1_81::move(BOOST_MOVE_TO_LV(x).node_alloc()))
   ...
   explicit node_alloc_holder(BOOST_RV_REF(node_alloc_holder) x, const intrusive_key_compare &c)
      : NodeAlloc(zr_boost_1_81::move(BOOST_MOVE_TO_LV(x).node_alloc())), m_icont(c)
   {  this->icont().swap(x.icont());  }
   ...
   NodePtr create_node(Args &&...args)
   {
      ...
      ::new(zr_boost_1_81::movelib::iterator_to_raw_pointer(p), boost_container_new_t())
         Node(nalloc, zr_boost_1_81::forward<Args>(args)...);
   ...
   NodePtr create_node_from_key(BOOST_FWD_REF(KeyConvertible) key)
   {
      NodePtr p = this->allocate_one();
      BOOST_TRY{
         ::new(zr_boost_1_81::movelib::iterator_to_raw_pointer(p), boost_container_new_t()) Node;
         NodeAlloc &na = this->node_alloc();
         node_allocator_traits_type::construct
            (na, dtl::addressof(p->get_real_data().first), zr_boost_1_81::forward<KeyConvertible>(key));
         BOOST_TRY{
   ...
   {
      zr_boost_1_81::movelib::to_raw_pointer(nodep)->destructor(this->node_alloc());
      ...
      (FwdIterator beg, size_type n, Inserter inserter)
   {
      if(n){
         ...
         Node *p = 0;
            ...
            while(n){
               --n;
               p = zr_boost_1_81::movelib::iterator_to_raw_pointer(itbeg);
   ...
   void clear(version_2)
   {
      ...
      //BOOST_STATIC_ASSERT((::zr_boost_1_81::has_move_emulation_enabled<typename NodeAlloc::multiallocation_chain>::value == true));
      if(!chain.empty())
