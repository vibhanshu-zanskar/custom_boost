...
// forward declaration
namespace zr_boost_1_81 { template<class T> class optional; }
...
namespace zr_boost_1_81 { namespace program_options { 
    ...
    template<class T, class charT>
    void 
    typed_value<T, charT>::notify(const zr_boost_1_81::any& value_store) const
    {
        const T* value = zr_boost_1_81::any_cast<T>(&value_store);
        if (m_store_to) {
            *m_store_to = *value;
        }
        if (m_notifier) {
    ...
    namespace validators {
            ...
            bool allow_empty = false)
        {
            ...
            if (v.size() > 1)
                zr_boost_1_81::throw_exception(validation_error(validation_error::multiple_values_not_allowed));
            else if (v.size() == 1)
                return v.front();
            else if (!allow_empty)
                zr_boost_1_81::throw_exception(validation_error(validation_error::at_least_one_value_required));
            return empty;
        }

        /* Throws multiple_occurrences if 'value' is not empty. */
        BOOST_PROGRAM_OPTIONS_DECL void 
        check_first_occurrence(const zr_boost_1_81::any& value);
    }
    ...
    */
    template<class T, class charT>
    void validate(zr_boost_1_81::any& v, 
                  ...
                  T*, long)
    {
        ...
        try {
            ...
            zr_boost_1_81::throw_exception(invalid_option_value(s));
        }
    }

    BOOST_PROGRAM_OPTIONS_DECL void validate(zr_boost_1_81::any& v, 
    ...
    {
        if (v.empty()) {
            v = zr_boost_1_81::any(std::vector<T>());
        }
        std::vector<T>* tv = zr_boost_1_81::any_cast< std::vector<T> >(&v);
        assert(NULL != tv);
        for (unsigned i = 0; i < s.size(); ++i)
        {
            try {
                ...
                cv.push_back(s[i]);
                validate(a, cv, (T*)0, 0);                
                tv->push_back(zr_boost_1_81::any_cast<T>(a));
            }
            catch(const bad_lexical_cast& /*e*/) {
                zr_boost_1_81::throw_exception(invalid_option_value(s[i]));
            }
        }
    }
                  ...
                  int)
    {
        ...
        zr_boost_1_81::any a;
        validate(a, s, (T*)0, 0);
        v = zr_boost_1_81::any(zr_boost_1_81::optional<T>(zr_boost_1_81::any_cast<T>(a)));
    }

    template<class T, class charT>
    void 
    typed_value<T, charT>::
    xparse(zr_boost_1_81::any& value_store, 
    ...
    value()
    {
        ...
        return zr_boost_1_81::program_options::value<T>(0);
    }
