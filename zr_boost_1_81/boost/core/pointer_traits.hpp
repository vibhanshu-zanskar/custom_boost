...
struct ptr_to<T*, T, true> {
    static T* pointer_to(T& v) BOOST_NOEXCEPT {
        return zr_boost_1_81::addressof(v);
    }
};
...
-> decltype(zr_boost_1_81::pointer_traits<T>::to_address(v))
{
    return zr_boost_1_81::pointer_traits<T>::to_address(v);
}
...
{
    return zr_boost_1_81::detail::ptr_address(v.operator->(), 0);
}
...
{
    return zr_boost_1_81::detail::ptr_address(v, 0);
}
#else
template<class T>
...
to_address(const T& v) BOOST_NOEXCEPT
{
    return zr_boost_1_81::to_address(v.operator->());
}
#endif
