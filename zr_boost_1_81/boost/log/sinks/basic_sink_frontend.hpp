...
protected:
    //! Mutex type
    typedef zr_boost_1_81::log::aux::light_rw_mutex mutex_type;
...
public:
    ...
    {
        BOOST_LOG_EXPR_IF_MT(zr_boost_1_81::log::aux::exclusive_lock_guard< mutex_type > lock(m_Mutex);)
        m_Filter = filter;
    }
    /*!
    ...
    {
        BOOST_LOG_EXPR_IF_MT(zr_boost_1_81::log::aux::shared_lock_guard< mutex_type > lock(m_Mutex);)
        try
        {
...
protected:
    ...
    void feed_record(record_view const& rec, BackendMutexT& backend_mutex, BackendT& backend)
    {
        try
        {
            BOOST_LOG_EXPR_IF_MT(zr_boost_1_81::log::aux::exclusive_lock_guard< BackendMutexT > lock(backend_mutex);)
            backend.consume(rec);
        }
#if !defined(BOOST_LOG_NO_THREADS)
        catch (thread_interrupted&)
    ...
    bool try_feed_record(record_view const& rec, BackendMutexT& backend_mutex, BackendT& backend)
    {
        ...
        {
            zr_boost_1_81::log::aux::shared_lock_guard< mutex_type > frontend_lock(this->frontend_mutex());
        ...
        zr_boost_1_81::log::aux::exclusive_auto_unlocker< BackendMutexT > unlocker(backend_mutex);
#endif
        // No need to lock anything in the feed_record method
        zr_boost_1_81::log::aux::fake_mutex m;
...
private:
    ...
    template< typename BackendMutexT, typename BackendT >
    void flush_backend_impl(BackendMutexT& backend_mutex, BackendT& backend, zr_boost_1_81::true_type)
    {
        try
        {
            BOOST_LOG_EXPR_IF_MT(zr_boost_1_81::log::aux::exclusive_lock_guard< BackendMutexT > lock(backend_mutex);)
    ...
    template< typename BackendMutexT, typename BackendT >
    void flush_backend_impl(BackendMutexT&, BackendT&, zr_boost_1_81::false_type)
    {
    }
};
...
private:
    ...
    //! State version
    zr_boost_1_81::atomic< unsigned int > m_Version;
...
public:
    ...
    {
#if !defined(BOOST_LOG_NO_THREADS)
        zr_boost_1_81::log::aux::exclusive_lock_guard< mutex_type > lock(this->frontend_mutex());
        m_Formatter = formatter;
        m_Version.opaque_add(1u, zr_boost_1_81::memory_order_relaxed);
#else
...
protected:
    ...
    void feed_record(record_view const& rec, BackendMutexT& backend_mutex, BackendT& backend)
    {
        ...
        if (!context || context->m_Version != m_Version.load(zr_boost_1_81::memory_order_relaxed))
        {
            {
                zr_boost_1_81::log::aux::shared_lock_guard< mutex_type > lock(this->frontend_mutex());
                context = new formatting_context(m_Version.load(zr_boost_1_81::memory_order_relaxed), m_Locale, m_Formatter);
            }
        ...
        {
            BOOST_LOG_EXPR_IF_MT(zr_boost_1_81::log::aux::shared_lock_guard< mutex_type > lock(this->frontend_mutex());)
            if (this->exception_handler().empty())
                throw;
            this->exception_handler()();
        }
