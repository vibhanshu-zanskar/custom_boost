...
{
private:
    typedef zr_boost_1_81::log::aux::threadsafe_queue< record_view > queue_type;

private:
    //! Thread-safe queue
    queue_type m_queue;
    //! Event object to block on
    zr_boost_1_81::log::aux::event m_event;
    //! Interruption flag
    zr_boost_1_81::atomic< bool > m_interruption_requested;
...
protected:
    ...
    bool dequeue_ready(record_view& rec)
    {
        ...
        {
            m_event.wait();
            if (m_interruption_requested.exchange(false, zr_boost_1_81::memory_order_acquire))
                return false;
            if (m_queue.try_pop(rec))
                return true;
        }
    ...
    {
        m_interruption_requested.store(true, zr_boost_1_81::memory_order_release);
        m_event.set_signalled();
    }
};
