...
#define BOOST_LOG_SINK_CTOR_FORWARD_INTERNAL_1(z, n, data)\
    ...
    explicit asynchronous_sink(T0 const& arg0, typename zr_boost_1_81::log::aux::enable_if_named_parameters< T0, zr_boost_1_81::log::aux::sfinae_dummy >::type = zr_boost_1_81::log::aux::sfinae_dummy()) :\
        base_type(true),\
        queue_base_type(arg0),\
        m_pBackend(zr_boost_1_81::make_shared< sink_backend_type >(arg0)),\
...
#define BOOST_LOG_SINK_CTOR_FORWARD_INTERNAL_N(z, n, data)\
    ...
    explicit asynchronous_sink(BOOST_PP_ENUM_BINARY_PARAMS_Z(z, n, T, const& arg)) :\
        ...
        m_pBackend(zr_boost_1_81::make_shared< sink_backend_type >(BOOST_PP_ENUM_PARAMS_Z(z, n, arg))),\
        m_ActiveOperation(idle),\
        m_StopRequested(false),\
        m_FlushRequested(false)\
    {\
...
private:
    ...
    typedef zr_boost_1_81::recursive_mutex backend_mutex_type;
    ...
    private:
        ...
        zr_boost_1_81::atomic< bool >& m_Flag;

    public:
        explicit scoped_flag(frontend_mutex_type& mut, condition_variable_any& cond, zr_boost_1_81::atomic< bool >& f) :
        ...
        ~scoped_flag()
        {
            try
            {
                ...
                m_Flag.store(false, zr_boost_1_81::memory_order_relaxed);
                m_Cond.notify_all();
            }
            catch (...)
            {
...
public:
    ...
    typedef zr_boost_1_81::log::aux::locking_ptr< sink_backend_type, backend_mutex_type > locked_backend_ptr;
...
private:
    ...
    zr_boost_1_81::atomic< bool > m_StopRequested;
    //! The flag indicates that queue flush has been requested
    zr_boost_1_81::atomic< bool > m_FlushRequested;
...
public:
    ...
    explicit asynchronous_sink(bool start_thread = true) :
        base_type(true),
        m_pBackend(zr_boost_1_81::make_shared< sink_backend_type >()),
        m_ActiveOperation(idle),
        m_StopRequested(false),
        m_FlushRequested(false)
    {
    ...
    ~asynchronous_sink() BOOST_NOEXCEPT BOOST_OVERRIDE
    {
        try
        {
            zr_boost_1_81::this_thread::disable_interruption no_interrupts;
            stop();
        }
        catch (...)
        {
    ...
    {
        if (BOOST_UNLIKELY(m_FlushRequested.load(zr_boost_1_81::memory_order_acquire)))
        {
            unique_lock< frontend_mutex_type > lock(base_type::frontend_mutex());
            // Wait until flush is done
            while (m_FlushRequested.load(zr_boost_1_81::memory_order_acquire))
    ...
    {
        if (!m_FlushRequested.load(zr_boost_1_81::memory_order_acquire))
        {
    ...
    void run()
    {
        ...
        {
            do_feed_records();
            if (!m_StopRequested.load(zr_boost_1_81::memory_order_acquire))
            {
    ...
    void stop()
    {
        zr_boost_1_81::thread feeding_thread;
        {
            lock_guard< frontend_mutex_type > lock(base_type::frontend_mutex());

            m_StopRequested.store(true, zr_boost_1_81::memory_order_release);
    ...
    {
        {
            ...
            if (static_cast< unsigned int >(m_ActiveOperation & feeding_records) != 0u)
            {
                ...
                m_FlushRequested.store(true, zr_boost_1_81::memory_order_release);
                queue_base_type::interrupt_dequeue();
                while (!m_StopRequested.load(zr_boost_1_81::memory_order_acquire) && m_FlushRequested.load(zr_boost_1_81::memory_order_acquire))
                    m_BlockCond.wait(lock);
            ...
            m_FlushRequested.store(true, zr_boost_1_81::memory_order_relaxed);
        }
...
private:
    ...
    {
        zr_boost_1_81::thread(run_func(this)).swap(m_DedicatedFeedingThread);
    }
    ...
    bool start_feeding_operation(unique_lock< frontend_mutex_type >& lock, operation op)
    {
        while (m_ActiveOperation != idle)
        {
            ...
            if (BOOST_UNLIKELY(m_StopRequested.load(zr_boost_1_81::memory_order_relaxed)))
            {
                m_StopRequested.store(false, zr_boost_1_81::memory_order_relaxed);
                return true;
            }
    ...
    {
        while (!m_StopRequested.load(zr_boost_1_81::memory_order_acquire))
        {
            record_view rec;
            bool dequeued = false;
            if (BOOST_LIKELY(!m_FlushRequested.load(zr_boost_1_81::memory_order_acquire)))
