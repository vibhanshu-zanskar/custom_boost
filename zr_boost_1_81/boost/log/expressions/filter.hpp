...
private:
    ...
    typedef zr_boost_1_81::log::aux::light_function< bool (attribute_value_set const&) > filter_type;
...
public:
     ...
     */
    filter(BOOST_RV_REF(filter) that) BOOST_NOEXCEPT : m_Filter(zr_boost_1_81::move(that.m_Filter))
    {
    }
    ...
    filter(FunT&& fun) : m_Filter(zr_boost_1_81::forward< FunT >(fun))
    {
    }
#elif !defined(BOOST_MSVC) || BOOST_MSVC >= 1600
    template< typename FunT >
    filter(FunT const& fun, typename zr_boost_1_81::disable_if_c< move_detail::is_rv< FunT >::value, zr_boost_1_81::log::aux::sfinae_dummy >::type = zr_boost_1_81::log::aux::sfinae_dummy()) : m_Filter(fun)
    {
    }
#else
...
#else
    template< typename FunT >
    typename zr_boost_1_81::disable_if_c< is_same< typename remove_cv< FunT >::type, filter >::value, filter& >::type
