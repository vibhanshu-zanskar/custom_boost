...
public:
    ...
    typedef zr_boost_1_81::log::aux::basic_format< char_type > format_type;
    //! String type
...
public:
    ...
    {
        return m_format.make_pump(fusion::at_c< 1 >(zr_boost_1_81phoenix::env(ctx).args()));
    }
 ...
 */
template< typename CharT >
BOOST_FORCEINLINE zr_boost_1_81phoenix::actor< format_terminal< CharT > > format(const CharT* fmt)
{
    typedef format_terminal< CharT > terminal_type;
    zr_boost_1_81phoenix::actor< terminal_type > act = {{ terminal_type(fmt) }};
    return act;
}
...
BOOST_FORCEINLINE zr_boost_1_81phoenix::actor< format_terminal< CharT > > format(std::basic_string< CharT, TraitsT, AllocatorT > const& fmt)
{
    typedef format_terminal< CharT > terminal_type;
    zr_boost_1_81phoenix::actor< terminal_type > act = {{ terminal_type(fmt.c_str()) }};
    return act;
}
...
template< typename CharT >
struct is_nullary< custom_terminal< zr_boost_1_81::log::expressions::format_terminal< CharT > > > :
    public mpl::false_
{
};
