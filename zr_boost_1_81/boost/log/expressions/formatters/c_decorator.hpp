...
{
    static zr_boost_1_81::iterator_range< const char* const* > get_patterns()
    {
        static const char* const patterns[] =
        {
            "\\", "\a", "\b", "\f", "\n", "\r", "\t", "\v", "'", "\"", "?"
        };
        return zr_boost_1_81::make_iterator_range(patterns);
    }
    static zr_boost_1_81::iterator_range< const char* const* > get_replacements()
    {
        static const char* const replacements[] =
        {
            "\\\\", "\\a", "\\b", "\\f", "\\n", "\\r", "\\t", "\\v", "\\'", "\\\"", "\\?"
        };
        return zr_boost_1_81::make_iterator_range(replacements);
    }
    template< unsigned int N >
    static std::size_t print_escaped(char (&buf)[N], char c)
    {
        int n = zr_boost_1_81::log::aux::snprintf(buf, N, "\\x%.2X", static_cast< unsigned int >(static_cast< uint8_t >(c)));
        if (n < 0)
        {
            n = 0;
            buf[0] = '\0';
...
{
    static zr_boost_1_81::iterator_range< const wchar_t* const* > get_patterns()
    {
    ...
    static zr_boost_1_81::iterator_range< const wchar_t* const* > get_replacements()
    {
    ...
    static std::size_t print_escaped(wchar_t (&buf)[N], wchar_t c)
    {
        ...
        int n = zr_boost_1_81::log::aux::swprintf(buf, N, format, val);
        if (n < 0)
        {
            n = 0;
            buf[0] = L'\0';
