...
public:
    ...
    typename result< this_type(ContextT const&) >::type operator() (ContextT const& ctx)
    {
        ...
        if (zr_boost_1_81phoenix::eval(m_cond, ctx))
            zr_boost_1_81phoenix::eval(m_then, ctx);
        return strm;
    }
...
public:
    ...
    typename result< this_type(ContextT const&) >::type operator() (ContextT const& ctx)
    {
        ...
        if (zr_boost_1_81phoenix::eval(m_cond, ctx))
            ...
            zr_boost_1_81phoenix::eval(m_else, ctx);
        return strm;
    }
...
#define BOOST_LOG_AUX_OVERLOAD(left_ref, right_ref)\
    ...
    BOOST_FORCEINLINE zr_boost_1_81phoenix::actor< if_else_output_terminal< zr_boost_1_81phoenix::actor< LeftExprT >, CondT, ThenT, ElseT > >\
    operator<< (zr_boost_1_81phoenix::actor< LeftExprT > left_ref left, if_then_else_gen< CondT, ThenT, ElseT > right_ref right)\
    {\
        typedef if_else_output_terminal< zr_boost_1_81phoenix::actor< LeftExprT >, CondT, ThenT, ElseT > terminal_type;\
        zr_boost_1_81phoenix::actor< terminal_type > actor = {{ terminal_type(left, right.m_cond, right.m_then, right.m_else) }};\
        return actor;\
    }
...
#define BOOST_LOG_AUX_OVERLOAD(left_ref, right_ref)\
    ...
    BOOST_FORCEINLINE zr_boost_1_81phoenix::actor< if_output_terminal< zr_boost_1_81phoenix::actor< LeftExprT >, CondT, ThenT > >\
    operator<< (zr_boost_1_81phoenix::actor< LeftExprT > left_ref left, if_then_gen< CondT, ThenT > right_ref right)\
    {\
        typedef if_output_terminal< zr_boost_1_81phoenix::actor< LeftExprT >, CondT, ThenT > terminal_type;\
        zr_boost_1_81phoenix::actor< terminal_type > actor = {{ terminal_type(left, right.else_.m_cond, right.else_.m_then) }};\
        return actor;\
    }
...
template< typename LeftT, typename CondT, typename ThenT >
struct is_nullary< custom_terminal< zr_boost_1_81::log::expressions::aux::if_output_terminal< LeftT, CondT, ThenT > > > :
    public mpl::false_
{
};

template< typename LeftT, typename CondT, typename ThenT, typename ElseT >
struct is_nullary< custom_terminal< zr_boost_1_81::log::expressions::aux::if_else_output_terminal< LeftT, CondT, ThenT, ElseT > > > :
    public mpl::false_
{
};
