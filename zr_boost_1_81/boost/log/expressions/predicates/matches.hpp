...
class attribute_matches :
    public aux::attribute_predicate< T, typename zr_boost_1_81::log::aux::match_traits< RegexT >::compiled_type, matches_fun, FallbackPolicyT >
{
    typedef aux::attribute_predicate< T, typename zr_boost_1_81::log::aux::match_traits< RegexT >::compiled_type, matches_fun, FallbackPolicyT > base_type;
...
public:
     ...
     */
    attribute_matches(attribute_name const& name, RegexT const& rex) : base_type(name, zr_boost_1_81::log::aux::match_traits< RegexT >::compile(rex))
    {
    }
     ...
     */
    template< typename U >
    attribute_matches(attribute_name const& name, RegexT const& rex, U const& arg) : base_type(name, zr_boost_1_81::log::aux::match_traits< RegexT >::compile(rex), arg)
    {
    }
};

#if defined(BOOST_MSVC) && BOOST_MSVC == 1925
// MSVC 14.2 has a codegen bug that makes inlined `matches` functions below crash on copy constructing the zr_boost_1_81phoenix::actor on return.
...
BOOST_LOG_AUX_FORCEINLINE_MSVC_BUG982738 zr_boost_1_81phoenix::actor< aux::unary_function_terminal< attribute_matches< T, RegexT > > >
matches(attribute_name const& name, RegexT const& rex)
{
    ...
    zr_boost_1_81phoenix::actor< terminal_type > act = {{ terminal_type(name, rex) }};
    return act;
}
