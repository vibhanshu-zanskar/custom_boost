...
private:
    ...
    {
        typedef zr_boost_1_81::true_type result_type;
        ...
        template< typename T >
        result_type operator() (zr_boost_1_81::true_type, T const& elem) const
        {
            m_stream << m_delimiter;
            return operator()(zr_boost_1_81::false_type(), elem);
        }

        template< typename T >
        result_type operator() (zr_boost_1_81::false_type, T const& elem) const
        {
            m_stream << elem;
            return result_type();
        }
...
public:
    ...
    {
        zr_boost_1_81::fusion::fold(m_tuple, zr_boost_1_81::false_type(), output_visitor< StreamT >(stream, m_delimiter));
    }
};
...
public:
    ...
    void output(StreamT& stream) const
    {
        zr_boost_1_81::fusion::for_each(m_tuple, output_visitor< StreamT >(stream));
    }
};
...
inline typename zr_boost_1_81::enable_if_c< log::aux::is_ostream< StreamT >::value, StreamT& >::type operator<< (StreamT& strm, tuple_manipulator< TupleT, DelimiterT > const& manip)
{
