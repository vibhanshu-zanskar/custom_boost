...
struct filter_factory
{
    ...
    {
        BOOST_LOG_THROW_DESCR_PARAMS(parse_error, "The custom attribute value relation \"" + zr_boost_1_81::log::aux::to_narrow(rel) + "\" is not supported", (name));
        BOOST_LOG_UNREACHABLE_RETURN(filter());
    }
...
public:
    ...
    {
        return zr_boost_1_81::lexical_cast< value_type >(arg);
    }
};
...
>::type register_filter_factory(attribute_name const& name, shared_ptr< FactoryT > const& factory)
{
    ...
    register_filter_factory(name, zr_boost_1_81::static_pointer_cast< factory_base >(factory));
}
...
inline void register_simple_filter_factory(attribute_name const& name)
{
    shared_ptr< filter_factory< CharT > > factory =
        zr_boost_1_81::make_shared< basic_filter_factory< CharT, AttributeValueT > >();
