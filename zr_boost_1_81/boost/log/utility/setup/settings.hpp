...
private:
    ...
    public:
        ...
        template< typename T >
        BOOST_LOG_TYPEOF(property_tree_type().get(typename property_tree_type::path_type(), zr_boost_1_81::declval< T >())) or_default(T const& def_value) const
        {
            if (m_section.m_ptree)
                return m_section.m_ptree->get(m_path, def_value);
            else
        ...
        template< typename T >
        typename zr_boost_1_81::enable_if_c< zr_boost_1_81::property_tree::detail::is_character< T >::value, std::basic_string< T > >::type
