...
public:
    ...
    //! Returns a pointer to the referred value
    template< typename U >
    typename zr_boost_1_81::enable_if_c< compatibility_traits::is_compatible< value_type, U >::value, const U* >::type get_ptr() const BOOST_NOEXCEPT
    {
        return m_ptr;
    }
    ...
    //! Returns a reference to the value
    template< typename U >
    typename zr_boost_1_81::enable_if_c< compatibility_traits::is_compatible< value_type, U >::value, U const& >::type get() const BOOST_NOEXCEPT
    {
        BOOST_ASSERT(m_ptr != NULL);
        return *m_ptr;
    }
    ...
    //! Applies a visitor function object to the referred value
    template< typename VisitorT >
    typename zr_boost_1_81::enable_if_c< is_void< typename VisitorT::result_type >::value, bool >::type apply_visitor_optional(VisitorT visitor) const
    {
        if (m_ptr)
        {
            visitor(*m_ptr);
    ...
    //! Applies a visitor function object to the referred value
    template< typename VisitorT >
    typename zr_boost_1_81::disable_if_c< is_void< typename VisitorT::result_type >::value, optional< typename VisitorT::result_type > >::type apply_visitor_optional(VisitorT visitor) const
    {
...
// MSVC-8 can't handle SFINAE in this case properly and often wrongly disables this constructor
        ...
        , typename zr_boost_1_81::enable_if_c< compatibility_traits::BOOST_NESTED_TEMPLATE is_compatible< value_type, U >::value, zr_boost_1_81::log::aux::sfinae_dummy >::type = zr_boost_1_81::log::aux::sfinae_dummy()
#endif
#endif
    ) BOOST_NOEXCEPT :
        base_type(zr_boost_1_81::addressof(val))
    {
    }
...
{
    if (!!val)
        val.apply_visitor(zr_boost_1_81::log::bind_output(strm));
    return strm;
}
...
{
    if (!!val)
        val.apply_visitor(zr_boost_1_81::log::bind_to_log< TagT >(strm));
    return strm;
}
