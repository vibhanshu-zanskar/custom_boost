...
public:
    ...
    typename result< this_type(ContextT const&) >::type operator() (ContextT const& ctx)
    {
        ...
        m_visitor_invoker(m_name, fusion::at_c< 0 >(zr_boost_1_81phoenix::env(ctx).args()), binder1st< impl_type&, result_type >(m_impl, strm));
        return strm;
    }
    ...
    typename result< const this_type(ContextT const&) >::type operator() (ContextT const& ctx) const
    {
        ...
        result_type strm = zr_boost_1_81phoenix::eval(m_left, ctx);
        m_visitor_invoker(m_name, fusion::at_c< 0 >(zr_boost_1_81phoenix::env(ctx).args()), binder1st< impl_type const&, result_type >(m_impl, strm));
        return strm;
    }
...
template< typename LeftT, typename T, typename FallbackPolicyT, typename ImplT >
struct is_nullary< custom_terminal< zr_boost_1_81::log::expressions::aux::attribute_output_terminal< LeftT, T, FallbackPolicyT, ImplT > > > :
    public mpl::false_
{
};
