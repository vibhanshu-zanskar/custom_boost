...
template< typename T, typename ThisT >
struct is_rv_or_same< zr_boost_1_81::rv< T >, ThisT > { enum _ { value = true }; };
...
private:
    ...
    public:
        ...
        explicit impl(FunT&& fun) :
            impl_base(&this_type::invoke_impl, &this_type::clone_impl, &this_type::destroy_impl),
            m_Function(zr_boost_1_81::move(fun))
        {
        }
...
public:
    ...
    light_function(FunT&& fun) :
        m_pImpl(new impl< typename remove_cv< typename remove_reference< FunT >::type >::type >(zr_boost_1_81::forward< FunT >(fun)))
    {
    }
#else
    template< typename FunT >
    light_function(FunT const& fun, typename zr_boost_1_81::disable_if_c< is_rv_or_same< FunT, this_type >::value, zr_boost_1_81::log::aux::sfinae_dummy >::type = zr_boost_1_81::log::aux::sfinae_dummy()) :
        m_pImpl(new impl< FunT >(fun))
    {
    }
    template< typename FunT >
    light_function(BOOST_RV_REF(FunT) fun, typename zr_boost_1_81::disable_if_c< is_cv_same< FunT, this_type >::value, zr_boost_1_81::log::aux::sfinae_dummy >::type = zr_boost_1_81::log::aux::sfinae_dummy()) :
    ...
    {
        light_function tmp(zr_boost_1_81::forward< FunT >(fun));
        this->swap(tmp);
        return *this;
    }
#else
    template< typename FunT >
    typename zr_boost_1_81::disable_if_c< is_rv_or_same< FunT, this_type >::value, this_type& >::type
