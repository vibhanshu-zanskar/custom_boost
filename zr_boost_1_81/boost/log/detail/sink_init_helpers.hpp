...
{
    return zr_boost_1_81::log::parse_filter(filter);
}
...
inline filter acquire_filter(std::basic_string_view< CharT, TraitsT > const& filter)
{
    ...
    return zr_boost_1_81::log::parse_filter(p, p + filter.size());
}
#endif // !defined(BOOST_NO_CXX17_HDR_STRING_VIEW)

template< typename CharT, typename TraitsT >
inline filter acquire_filter(zr_boost_1_81::basic_string_view< CharT, TraitsT > const& filter)
{
    const CharT* p = filter.data();
    return zr_boost_1_81::log::parse_filter(p, p + filter.size());
}
...
inline typename zr_boost_1_81::disable_if_c<
    zr_boost_1_81::is_array< FilterT >::value,
    FilterT const&
...
inline void setup_filter(SinkT&, ArgsT const&, zr_boost_1_81::true_type)
{
}

template< typename SinkT, typename ArgsT >
inline void setup_filter(SinkT& s, ArgsT const& args, zr_boost_1_81::false_type)
{
...
{
    return zr_boost_1_81::log::parse_formatter(formatter);
}
...
inline basic_formatter< CharT > acquire_formatter(std::basic_string_view< CharT, TraitsT > const& formatter)
{
    ...
    return zr_boost_1_81::log::parse_formatter(p, p + formatter.size());
}
#endif // !defined(BOOST_NO_CXX17_HDR_STRING_VIEW)

template< typename CharT, typename TraitsT >
inline basic_formatter< CharT > acquire_formatter(zr_boost_1_81::basic_string_view< CharT, TraitsT > const& formatter)
{
    const CharT* p = formatter.data();
    return zr_boost_1_81::log::parse_formatter(p, p + formatter.size());
}
...
inline typename zr_boost_1_81::disable_if_c<
    zr_boost_1_81::is_array< FormatterT >::value,
    FormatterT const&
...
// The function installs filter into the sink, if provided in the arguments pack
template< typename SinkT, typename ArgsT >
inline void setup_formatter(SinkT&, ArgsT const&, zr_boost_1_81::true_type)
{
}

template< typename SinkT, typename ArgsT >
inline void setup_formatter(SinkT& s, ArgsT const& args, zr_boost_1_81::false_type)
{
