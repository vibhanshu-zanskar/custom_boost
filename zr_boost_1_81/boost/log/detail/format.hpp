...
public:
    //! Initializing constructor
    pump(basic_format& owner, stream_type& strm) BOOST_NOEXCEPT : m_owner(&owner), m_stream(&strm), m_exception_count(zr_boost_1_81::core::uncaught_exceptions())
    {
    }
    ...
    ~pump() BOOST_NOEXCEPT_IF(false)
    {
        if (m_owner)
        {
            ...
            zr_boost_1_81::log::aux::cleanup_guard< basic_format< char_type > > cleanup1(*m_owner);

            BOOST_ASSERT(m_stream != NULL);
            if (m_exception_count >= zr_boost_1_81::core::uncaught_exceptions())
            {
    ...
    pump operator% (T const& val)
    {
        ...
        return zr_boost_1_81::move(*this);
    }
};
