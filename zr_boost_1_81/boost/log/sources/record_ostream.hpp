...
struct enable_record_ostream_generic_operator< basic_record_ostream< CharT >, T, false, R > :
    public zr_boost_1_81::disable_if_c< zr_boost_1_81::is_scalar< typename zr_boost_1_81::remove_cv< T >::type >::value, R >
{
};

template< typename CharT, typename T, typename R >
struct enable_record_ostream_generic_operator< basic_record_ostream< CharT >, T, true, R > :
    public zr_boost_1_81::enable_if_c< zr_boost_1_81::is_enum< typename zr_boost_1_81::remove_cv< T >::type >::value, R >
{
};

template< typename CharT, typename T, typename R >
struct enable_record_ostream_generic_operator< basic_record_ostream< CharT >, T*, true, R > :
    public disable_if_streamable_char_type< typename zr_boost_1_81::remove_cv< T >::type, R >
{
};
...
inline typename zr_boost_1_81::log::aux::enable_record_ostream_generic_operator< StreamT, T, true, StreamT& >::type
...
template< typename StreamT, typename T >
inline typename zr_boost_1_81::log::aux::enable_record_ostream_generic_operator< StreamT, T, false, StreamT& >::type
...
public:
    ...
    explicit record_pump(logger_type& lg, record& rec) :
        m_pLogger(zr_boost_1_81::addressof(lg)),
        m_pStreamCompound(stream_provider_type::allocate_compound(rec)),
        m_ExceptionCount(zr_boost_1_81::core::uncaught_exceptions())
    {
    }
    //! Move constructor
    ...
    ~record_pump() BOOST_NOEXCEPT_IF(false)
    {
        if (m_pLogger)
        {
            ...
            if (m_ExceptionCount >= zr_boost_1_81::core::uncaught_exceptions())
                m_pLogger->push_record(zr_boost_1_81::move(m_pStreamCompound->stream.get_record()));
        }
    }
...
#define BOOST_LOG_STREAM_INTERNAL(logger, rec_var)\
    for (::zr_boost_1_81::log::record rec_var = (logger).open_record(); !!rec_var;)\
        ::zr_boost_1_81::log::aux::make_record_pump((logger), rec_var).stream()

#define BOOST_LOG_STREAM_WITH_PARAMS_INTERNAL(logger, rec_var, params_seq)\
    for (::zr_boost_1_81::log::record rec_var = (logger).open_record((BOOST_PP_SEQ_ENUM(params_seq))); !!rec_var;)\
        ::zr_boost_1_81::log::aux::make_record_pump((logger), rec_var).stream()
