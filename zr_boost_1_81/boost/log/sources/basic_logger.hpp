...
public:
    ...
    typedef zr_boost_1_81::log::aux::multiple_unique_lock2< threading_model, threading_model > swap_lock;
    //! Lock requirement for the add_attribute_unlocked method
    typedef zr_boost_1_81::log::aux::exclusive_lock_guard< threading_model > add_attribute_lock;
    //! Lock requirement for the remove_attribute_unlocked method
    typedef zr_boost_1_81::log::aux::exclusive_lock_guard< threading_model > remove_attribute_lock;
    //! Lock requirement for the remove_all_attributes_unlocked method
    typedef zr_boost_1_81::log::aux::exclusive_lock_guard< threading_model > remove_all_attributes_lock;
    //! Lock requirement for the get_attributes method
    typedef zr_boost_1_81::log::aux::shared_lock_guard< const threading_model > get_attributes_lock;
    //! Lock requirement for the open_record_unlocked method
    typedef zr_boost_1_81::log::aux::shared_lock_guard< threading_model > open_record_lock;
    //! Lock requirement for the set_attributes method
    typedef zr_boost_1_81::log::aux::exclusive_lock_guard< threading_model > set_attributes_lock;
#else
...
public:
     ...
     * \param that Source logger
     */
    basic_logger(BOOST_RV_REF(basic_logger) that) BOOST_NOEXCEPT_IF(zr_boost_1_81::is_nothrow_move_constructible< threading_model >::value &&
                                                                    zr_boost_1_81::is_nothrow_move_constructible< core_ptr >::value &&
                                                                    zr_boost_1_81::is_nothrow_move_constructible< attribute_set >::value) :
        threading_model(zr_boost_1_81::move(static_cast< threading_model& >(that))),
        m_pCore(zr_boost_1_81::move(that.m_pCore)),
        m_Attributes(zr_boost_1_81::move(that.m_Attributes))
    {
    }
    /*!
...
protected:
    ...
    {
        m_pCore->push_record(zr_boost_1_81::move(rec));
    }
...
inline void swap(
    ...
    basic_logger< CharT, FinalT, ThreadingModelT >& right) BOOST_NOEXCEPT_IF(zr_boost_1_81::is_nothrow_swappable< FinalT >::value)
{
...
class basic_composite_logger :
    public zr_boost_1_81::log::sources::aux::inherit_features<
...
private:
    ...
    typedef typename zr_boost_1_81::log::sources::aux::inherit_features<
        basic_logger< CharT, FinalT, ThreadingModelT >,
        FeaturesT
    >::type base_type;
...
public:
    ...
    basic_composite_logger(basic_composite_logger const& that) :
        base_type
        ((
            zr_boost_1_81::log::aux::shared_lock_guard< const threading_model >(that.get_threading_model()),
     ...
     * Move constructor
     */
    basic_composite_logger(BOOST_RV_REF(logger_base) that) BOOST_NOEXCEPT_IF(zr_boost_1_81::is_nothrow_move_constructible< base_type >::value) :
        base_type(zr_boost_1_81::move(static_cast< base_type& >(that)))
    {
    }
    /*!
    ...
    record open_record()
    {
        ...
        if (this->core()->get_logging_enabled())
        {
            ...
            return base_type::open_record_unlocked(zr_boost_1_81::log::aux::empty_arg_list());
        }
        else
            return record();
    }
    ...
    void push_record(BOOST_RV_REF(record) rec)
    {
        ...
        base_type::push_record_unlocked(zr_boost_1_81::move(rec));
    }
    /*!
...
protected:
    ...
    {
        BOOST_LOG_ASSUME(this != NULL);
        if (static_cast< FinalT* >(this) != zr_boost_1_81::addressof(that))
        {
            // We'll have to explicitly create the copy in order to make sure it's unlocked when we attempt to lock *this
            FinalT tmp(that);
            zr_boost_1_81::log::aux::exclusive_lock_guard< threading_model > lock(base_type::get_threading_model());
...
#define BOOST_LOG_FORWARD_LOGGER_CONSTRUCTORS_IMPL(class_type, typename_keyword)\
    public:\
        ...
        class_type(BOOST_RV_REF(class_type) that) BOOST_NOEXCEPT_IF(zr_boost_1_81::is_nothrow_move_constructible< typename_keyword() class_type::logger_base >::value) : class_type::logger_base(\
            ::zr_boost_1_81::move(static_cast< typename_keyword() class_type::logger_base& >(that))) {}\
...
#define BOOST_LOG_FORWARD_LOGGER_ASSIGNMENT(class_type)\
    public:\
        ...
        {\
            BOOST_LOG_EXPR_IF_MT(::zr_boost_1_81::log::aux::exclusive_lock_guard< class_type::threading_model > lock(this->get_threading_model());)\
...
#define BOOST_LOG_FORWARD_LOGGER_ASSIGNMENT_TEMPLATE(class_type)\
    public:\
        ...
        class_type& operator= (BOOST_RV_REF(class_type) that)\
        {\
            BOOST_LOG_EXPR_IF_MT(::zr_boost_1_81::log::aux::exclusive_lock_guard< typename class_type::threading_model > lock(this->get_threading_model());)\
            this->swap_unlocked(that);\
            return *this;\
        }
...
#define BOOST_LOG_DECLARE_LOGGER_TYPE(type_name, char_type, base_seq, threading)\
    class type_name :\
        public ::zr_boost_1_81::log::sources::basic_composite_logger<\
            char_type,\
            type_name,\
            threading,\
            ::zr_boost_1_81::log::sources::features< BOOST_PP_SEQ_ENUM(base_seq) >\
        >\
    {\
...
#define BOOST_LOG_DECLARE_LOGGER(type_name, base_seq)\
    BOOST_LOG_DECLARE_LOGGER_TYPE(type_name, char, base_seq, ::zr_boost_1_81::log::sources::single_thread_model)
...
#define BOOST_LOG_DECLARE_LOGGER_MT(type_name, base_seq)\
    BOOST_LOG_DECLARE_LOGGER_TYPE(type_name, char, base_seq,\
        ::zr_boost_1_81::log::sources::multi_thread_model< ::zr_boost_1_81::shared_mutex >)
...
#define BOOST_LOG_DECLARE_WLOGGER(type_name, base_seq)\
    BOOST_LOG_DECLARE_LOGGER_TYPE(type_name, wchar_t, base_seq, ::zr_boost_1_81::log::sources::single_thread_model)
