...
struct logger_singleton :
    public zr_boost_1_81::log::aux::lazy_singleton<
        logger_singleton< TagT >,
        shared_ptr< logger_holder< typename TagT::logger_type > >
    >
{
    //! Base type
    typedef zr_boost_1_81::log::aux::lazy_singleton<
        logger_singleton< TagT >,
        shared_ptr< logger_holder< typename TagT::logger_type > >
    > base_type;
    //! Logger type
    ...
    static void init_instance()
    {
        ...
        if (holder->m_LoggerType == logger_type_index)
        {
            ...
            instance = zr_boost_1_81::static_pointer_cast< logger_holder< logger_type > >(holder);
        }
        else
        {
            ...
            zr_boost_1_81::log::sources::aux::throw_odr_violation(tag_type_index, logger_type_index, *holder);
        }
    }
...
private:
    ...
    {
        return zr_boost_1_81::make_shared< logger_holder< logger_type > >(
...
#define BOOST_LOG_GLOBAL_LOGGER(tag_name, logger)\
    ...
    {\
        ...
        {\
            return ::zr_boost_1_81::log::sources::aux::logger_singleton< tag_name >::get();\
        }\
    };
