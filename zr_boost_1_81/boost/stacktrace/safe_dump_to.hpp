...
/// so you could read them by using 'od -tx8 -An stacktrace_dump_failename' Linux command or using zr_boost_1_81::stacktrace::stacktrace::from_dump functions.
...
struct this_thread_frames { // struct is required to avoid warning about usage of inline+BOOST_NOINLINE
    ...
    BOOST_NOINLINE static std::size_t safe_dump_to_impl(void* memory, std::size_t size, std::size_t skip) BOOST_NOEXCEPT {
        ...
        const std::size_t frames_count = zr_boost_1_81::stacktrace::detail::this_thread_frames::collect(mem, size / sizeof(native_frame_ptr_t) - 1, skip + 1);
        mem[frames_count] = 0;
    ...
    BOOST_NOINLINE static std::size_t safe_dump_to_impl(T file, std::size_t skip, std::size_t max_depth) BOOST_NOEXCEPT {
        ...
        native_frame_ptr_t buffer[zr_boost_1_81::stacktrace::detail::max_frames_dump + 1];
        if (max_depth > zr_boost_1_81::stacktrace::detail::max_frames_dump) {
            max_depth = zr_boost_1_81::stacktrace::detail::max_frames_dump;
        }

        const std::size_t frames_count = zr_boost_1_81::stacktrace::detail::this_thread_frames::collect(buffer, max_depth, skip + 1);
        buffer[frames_count] = 0;
        return zr_boost_1_81::stacktrace::detail::dump(file, buffer, frames_count + 1);
    }
};
...
///
/// @returns Stored call sequence depth including terminating zero frame. To get the actually consumed bytes multiply this value by the sizeof(zr_boost_1_81::stacktrace::frame::native_frame_ptr_t)
///
...
BOOST_FORCEINLINE std::size_t safe_dump_to(void* memory, std::size_t size) BOOST_NOEXCEPT {
    return  zr_boost_1_81::stacktrace::detail::this_thread_frames::safe_dump_to_impl(memory, size, 0);
}
...
BOOST_FORCEINLINE std::size_t safe_dump_to(std::size_t skip, void* memory, std::size_t size) BOOST_NOEXCEPT {
    return  zr_boost_1_81::stacktrace::detail::this_thread_frames::safe_dump_to_impl(memory, size, skip);
}
...
BOOST_FORCEINLINE std::size_t safe_dump_to(const char* file) BOOST_NOEXCEPT {
    return zr_boost_1_81::stacktrace::detail::this_thread_frames::safe_dump_to_impl(file, 0, zr_boost_1_81::stacktrace::detail::max_frames_dump);
}
...
BOOST_FORCEINLINE std::size_t safe_dump_to(std::size_t skip, std::size_t max_depth, const char* file) BOOST_NOEXCEPT {
    return zr_boost_1_81::stacktrace::detail::this_thread_frames::safe_dump_to_impl(file, skip, max_depth);
}
...
BOOST_FORCEINLINE std::size_t safe_dump_to(void* fd) BOOST_NOEXCEPT {
    return zr_boost_1_81::stacktrace::detail::this_thread_frames::safe_dump_to_impl(fd, 0, zr_boost_1_81::stacktrace::detail::max_frames_dump);
}

BOOST_FORCEINLINE std::size_t safe_dump_to(std::size_t skip, std::size_t max_depth, void* fd) BOOST_NOEXCEPT {
    return zr_boost_1_81::stacktrace::detail::this_thread_frames::safe_dump_to_impl(fd, skip, max_depth);
}
