...
class basic_stacktrace {
    std::vector<zr_boost_1_81::stacktrace::frame, Allocator> impl_;
    typedef zr_boost_1_81::stacktrace::detail::native_frame_ptr_t native_frame_ptr_t;
    ...
    BOOST_NOINLINE void init(std::size_t frames_to_skip, std::size_t max_depth) {
        ...
        BOOST_TRY {
            {   // Fast path without additional allocations
                ...
                const std::size_t frames_count = zr_boost_1_81::stacktrace::detail::this_thread_frames::collect(buffer, buffer_size < max_depth ? buffer_size : max_depth, frames_to_skip + 1);
            ...
            do {
                const std::size_t frames_count = zr_boost_1_81::stacktrace::detail::this_thread_frames::collect(&buf[0], buf.size() < max_depth ? buf.size() : max_depth, frames_to_skip + 1);
...
public:
    typedef typename std::vector<zr_boost_1_81::stacktrace::frame, Allocator>::value_type             value_type;
    typedef typename std::vector<zr_boost_1_81::stacktrace::frame, Allocator>::allocator_type         allocator_type;
    typedef typename std::vector<zr_boost_1_81::stacktrace::frame, Allocator>::const_pointer          pointer;
    typedef typename std::vector<zr_boost_1_81::stacktrace::frame, Allocator>::const_pointer          const_pointer;
    typedef typename std::vector<zr_boost_1_81::stacktrace::frame, Allocator>::const_reference        reference;
    typedef typename std::vector<zr_boost_1_81::stacktrace::frame, Allocator>::const_reference        const_reference;
    typedef typename std::vector<zr_boost_1_81::stacktrace::frame, Allocator>::size_type              size_type;
    typedef typename std::vector<zr_boost_1_81::stacktrace::frame, Allocator>::difference_type        difference_type;
    typedef typename std::vector<zr_boost_1_81::stacktrace::frame, Allocator>::const_iterator         iterator;
    typedef typename std::vector<zr_boost_1_81::stacktrace::frame, Allocator>::const_iterator         const_iterator;
    typedef typename std::vector<zr_boost_1_81::stacktrace::frame, Allocator>::const_reverse_iterator reverse_iterator;
    typedef typename std::vector<zr_boost_1_81::stacktrace::frame, Allocator>::const_reverse_iterator const_reverse_iterator;
...
#ifndef BOOST_NO_CXX11_HDR_TYPE_TRAITS
        BOOST_NOEXCEPT_IF(( std::is_nothrow_move_assignable< std::vector<zr_boost_1_81::stacktrace::frame, Allocator> >::value ))
#else
        BOOST_NOEXCEPT
#endif
    {
    ...
    const std::vector<zr_boost_1_81::stacktrace::frame, Allocator>& as_vector() const BOOST_NOEXCEPT {
        return impl_;
    }
    ...
    ///
    /// @param begin Begining of the memory where the stacktrace was saved using the zr_boost_1_81::stacktrace::safe_dump_to
    ///
    /// @param buffer_size_in_bytes Size of the memory. Usually the same value that was passed to the zr_boost_1_81::stacktrace::safe_dump_to
    ///
...
std::size_t hash_value(const basic_stacktrace<Allocator>& st) BOOST_NOEXCEPT {
    return zr_boost_1_81::hash_range(st.as_vector().begin(), st.as_vector().end());
}
...
std::string to_string(const basic_stacktrace<Allocator>& bt) {
    ...
    return zr_boost_1_81::stacktrace::detail::to_string(&bt.as_vector()[0], bt.size());
}
...
std::basic_ostream<CharT, TraitsT>& operator<<(std::basic_ostream<CharT, TraitsT>& os, const basic_stacktrace<Allocator>& bt) {
    return os << zr_boost_1_81::stacktrace::to_string(bt);
}

/// This is the typedef to use unless you'd like to provide a specific allocator to zr_boost_1_81::stacktrace::basic_stacktrace.
