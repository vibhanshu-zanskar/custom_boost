...
inline void libbacktrace_syminfo_callback(void *data, uintptr_t pc, const char *symname, uintptr_t symval) {
    zr_boost_1_81::stacktrace::detail::libbacktrace_syminfo_callback(data, pc, symname, symval, 0);
}
...
BOOST_SYMBOL_VISIBLE inline ::backtrace_state* construct_state(const program_location& prog_location) BOOST_NOEXCEPT {
    ...
    //    zr_boost_1_81::stacktrace::detail::libbacktrace_error_callback,
...
struct to_string_using_backtrace {
    std::string res;
    zr_boost_1_81::stacktrace::detail::program_location prog_location;
    ::backtrace_state* state;
    std::string filename;
    std::size_t line;

    void prepare_function_name(const void* addr) {
        zr_boost_1_81::stacktrace::detail::pc_data data = {&res, &filename, 0};
        if (state) {
            ::backtrace_pcinfo(
                state,
                reinterpret_cast<uintptr_t>(addr),
                zr_boost_1_81::stacktrace::detail::libbacktrace_full_callback,
                zr_boost_1_81::stacktrace::detail::libbacktrace_error_callback,
                &data
            ) 
            ||
            ::backtrace_syminfo(
                state,
                reinterpret_cast<uintptr_t>(addr),
                zr_boost_1_81::stacktrace::detail::libbacktrace_syminfo_callback,
                zr_boost_1_81::stacktrace::detail::libbacktrace_error_callback,
                &data
            );
        }
    ...
    bool prepare_source_location(const void* /*addr*/) {
        ...
        res += filename;
        res += ':';
        res += zr_boost_1_81::stacktrace::detail::to_dec_array(line).data();
        return true;
    }

    to_string_using_backtrace() BOOST_NOEXCEPT {
        state = zr_boost_1_81::stacktrace::detail::construct_state(prog_location);
    }
};
...
inline std::string name_impl(const void* addr) {
    ...
    zr_boost_1_81::stacktrace::detail::program_location prog_location;
    ::backtrace_state* state = zr_boost_1_81::stacktrace::detail::construct_state(prog_location);

    zr_boost_1_81::stacktrace::detail::pc_data data = {&res, 0, 0};
    if (state) {
        ::backtrace_pcinfo(
            state,
            reinterpret_cast<uintptr_t>(addr),
    ...
    if (!res.empty()) {
        res = zr_boost_1_81::core::demangle(res.c_str());
    }
...
std::string frame::source_file() const {
    ...
    zr_boost_1_81::stacktrace::detail::pc_data data = {0, &res, 0};
    if (state) {
        ::backtrace_pcinfo(
            state,
...
std::size_t frame::source_line() const {
    ...
    zr_boost_1_81::stacktrace::detail::pc_data data = {0, 0, 0};
    if (state) {
        ::backtrace_pcinfo(
            state,
            reinterpret_cast<uintptr_t>(addr_),
