...
template <class T>
class com_holder: zr_boost_1_81::noncopyable {
    T* holder_;
...
inline std::string mingw_demangling_workaround(const std::string& s) {
    ...
    if (s[0] != '_') {
        return zr_boost_1_81::core::demangle(('_' + s).c_str());
    }

    return zr_boost_1_81::core::demangle(s.c_str());
#else
    return s;
#endif
}
...
class debugging_symbols: zr_boost_1_81::noncopyable {
...
public:
    ...
    void to_string_impl(const void* addr, std::string& res) const {
        ...
        if (!source_line.first.empty() && source_line.second) {
            ...
            res += ':';
            res += zr_boost_1_81::stacktrace::detail::to_dec_array(source_line.second).data();
...
std::string to_string(const frame* frames, std::size_t size) {
    zr_boost_1_81::stacktrace::detail::debugging_symbols idebug;
    if (!idebug.is_inited()) {
        return std::string();
    }
