...
public:
    std::string operator()(zr_boost_1_81::stacktrace::detail::native_frame_ptr_t addr) {
        Base::res.clear();
        Base::prepare_function_name(addr);
        if (!Base::res.empty()) {
            Base::res = zr_boost_1_81::core::demangle(Base::res.c_str());
        } else {
...
std::string to_string(const frame* frames, std::size_t size) {
    ...
    for (std::size_t i = 0; i < size; ++i) {
        ...
        res += zr_boost_1_81::stacktrace::detail::to_dec_array(i).data();
        res += '#';
        res += ' ';
...
std::string frame::name() const {
    ...
    if (dl_ok && dli.dli_sname) {
        return zr_boost_1_81::core::demangle(dli.dli_sname);
    }
#endif
    return zr_boost_1_81::stacktrace::detail::name_impl(addr_);
}
...
std::string to_string(const frame& f) {
    ...
    zr_boost_1_81::stacktrace::detail::to_string_impl impl;
