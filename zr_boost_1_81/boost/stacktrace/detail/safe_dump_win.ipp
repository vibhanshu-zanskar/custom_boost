...
std::size_t dump(void* /*fd*/, const native_frame_ptr_t* /*frames*/, std::size_t /*frames_count*/) BOOST_NOEXCEPT {
    ...
    zr_boost_1_81::winapi::DWORD_ written;
    const zr_boost_1_81::winapi::DWORD_ bytes_to_write = static_cast<zr_boost_1_81::winapi::DWORD_>(
        sizeof(native_frame_ptr_t) * frames_count
    );
    if (!zr_boost_1_81::winapi::WriteFile(fd, frames, bytes_to_write, &written, 0)) {
        return 0;
    }
...
std::size_t dump(const char* /*file*/, const native_frame_ptr_t* /*frames*/, std::size_t /*frames_count*/) BOOST_NOEXCEPT {
    ...
    void* const fd = zr_boost_1_81::winapi::CreateFileA(
        file,
        zr_boost_1_81::winapi::GENERIC_WRITE_,
        0,
        0,
        zr_boost_1_81::winapi::CREATE_ALWAYS_,
        zr_boost_1_81::winapi::FILE_ATTRIBUTE_NORMAL_,
        0
    );

    if (fd == zr_boost_1_81::winapi::invalid_handle_value) {
        return 0;
    }

    const std::size_t size = zr_boost_1_81::stacktrace::detail::dump(fd, frames, frames_count);
    zr_boost_1_81::winapi::CloseHandle(fd);
    return size;
#endif
    return 0;
}
