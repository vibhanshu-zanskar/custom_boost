...
template <class T>
inline zr_boost_1_81::array<char, 2 + sizeof(void*) * 2 + 1> to_hex_array(T addr) BOOST_NOEXCEPT {
    zr_boost_1_81::array<char, 2 + sizeof(void*) * 2 + 1> ret = {"0x"};
    ret.back() = '\0';
    BOOST_STATIC_ASSERT_MSG(!zr_boost_1_81::is_pointer<T>::value, "");
...
inline zr_boost_1_81::array<char, 2 + sizeof(void*) * 2 + 1> to_hex_array(const void* addr) BOOST_NOEXCEPT {
    return to_hex_array(
        reinterpret_cast< zr_boost_1_81::make_unsigned<std::ptrdiff_t>::type >(addr)
    );
}
