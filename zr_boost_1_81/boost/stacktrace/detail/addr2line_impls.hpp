...
public:
        ...
        , pid(0)
    {
        ...
        static_assert(
            zr_boost_1_81::stacktrace::detail::is_abs_path( BOOST_STRINGIZE( BOOST_STACKTRACE_ADDR2LINE_LOCATION ) ),
...
inline std::string addr2line(const char* flag, const void* addr) {
    ...
    zr_boost_1_81::stacktrace::detail::location_from_symbol loc(addr);
    if (!loc.empty()) {
        res = loc.name();
    } else {
        res.resize(16);
...
struct to_string_using_addr2line {
    ...
    void prepare_function_name(const void* addr) {
        res = zr_boost_1_81::stacktrace::frame(addr).name();
    }

    bool prepare_source_location(const void* addr) {
        //return addr2line("-Cfipe", addr); // Does not seem to work in all cases
        std::string source_line = zr_boost_1_81::stacktrace::detail::addr2line("-Cpe", addr);
...
inline std::string name_impl(const void* addr) {
    std::string res = zr_boost_1_81::stacktrace::detail::addr2line("-fe", addr);
...
std::string frame::source_file() const {
    std::string res;
    res = zr_boost_1_81::stacktrace::detail::addr2line("-e", addr_);
    res = res.substr(0, res.find_last_of(':'));
    if (res == "??") {
        res.clear();
    }
...
std::size_t frame::source_line() const {
    ...
    std::string res = zr_boost_1_81::stacktrace::detail::addr2line("-e", addr_);
    ...
    if (!zr_boost_1_81::stacktrace::detail::try_dec_convert(res.c_str(), line_num)) {
        return 0;
    }
