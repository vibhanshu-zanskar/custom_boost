...
push_coroutine< T >::control_block::destroy( control_block * cb) noexcept {
    zr_boost_1_81::context::fiber c = std::move( cb->c);
...
push_coroutine< T >::control_block::control_block( context::preallocated palloc, StackAllocator && salloc,
    ...
    c{ std::allocator_arg, palloc, std::forward< StackAllocator >( salloc),
       wrap( [this](typename std::decay< Fn >::type & fn_,zr_boost_1_81::context::fiber && c) mutable {
               ...
               if ( state_t::none == ( state & state_t::destroy) ) {
                   try {
                       ...
                       fn( synthesized);
                   } catch ( zr_boost_1_81::context::detail::forced_unwind const&) {
                       throw;
...
#else
    c{ std::allocator_arg, palloc, std::forward< StackAllocator >( salloc),
       [this,fn_=std::forward< Fn >( fn)](zr_boost_1_81::context::fiber && c) mutable {
          // create synthesized pull_coroutine< T >
          typename pull_coroutine< T >::control_block synthesized_cb{ this, c };
          pull_coroutine< T > synthesized{ & synthesized_cb };
          other = & synthesized_cb;
...
push_coroutine< T >::control_block::control_block( typename pull_coroutine< T >::control_block * cb,
                                                   zr_boost_1_81::context::fiber & c_) noexcept :
    c{ std::move( c_) },
    other{ cb },
    state{ state_t::none },
    except{} {
