...
It also implements the necessary traits for passing an zr_boost_1_81::asio::io_context,
which is needed for asynchronous communication.

It also pulls the [zr_boost_1_81::asio::buffer](http://www.boost.org/doc/libs/release/doc/html/boost_asio/reference/buffer.html)
into the zr_boost_1_81::process namespace for convenience.
...
namespace zr_boost_1_81 {
  namespace process {
    ...
    <emphasis>unspecified</emphasis> <globalname alt="zr_boost_1_81::process::on_exit">on_exit</globalname>;
  }
}
</programlisting>
...
{
    auto& ref = *zr_boost_1_81::fusion::find_if<is_io_context<zr_boost_1_81::mpl::_>>(tup);
    return ref.get();
}
...
struct async_builder
{
    ...
    void operator()(zr_boost_1_81::asio::io_context & ios_) {this->ios = &ios_;};
...
using ::zr_boost_1_81::asio::buffer;
...
\warning `on_exit` uses `zr_boost_1_81::asio::signal_set` to listen for `SIGCHLD` on posix, and so has the
same restrictions as that class (do not register a handler for `SIGCHLD` except by using
`zr_boost_1_81::asio::signal_set`).
 */
constexpr static ::zr_boost_1_81::process::detail::on_exit_ on_exit{};
#endif
