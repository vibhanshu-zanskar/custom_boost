...
inline bool compare_handles(int lhs, int rhs)
{
    ...
    if(fstat(lhs, &stat1) < 0) ::zr_boost_1_81::process::detail::throw_last_error("fstat() failed");
    if(fstat(rhs, &stat2) < 0) ::zr_boost_1_81::process::detail::throw_last_error("fstat() failed");
