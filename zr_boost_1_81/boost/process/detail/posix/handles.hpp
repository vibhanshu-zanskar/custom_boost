...
inline std::vector<native_handle_type> get_handles()
{
    ...
    if (ec)
        zr_boost_1_81::process::detail::throw_error(ec, "open_dir(\"/dev/fd\") failed");
...
inline bool is_stream_handle(native_handle_type handle)
{
    ...
    if (ec)
        zr_boost_1_81::process::detail::throw_error(ec, "fstat() failed");
...
struct limit_handles_ : handler_base_ext
{
    ...
    void on_exec_setup(Executor & exec) const
    {
        ...
        if (!dir)
        {
            exec.set_error(::zr_boost_1_81::process::detail::get_last_error(), "opendir(\"/dev/fd\")");
            return;
        }
        ...
        while ((ent_p = readdir(dir)) != nullptr)
        {
            ...
            if (::close(conv) != 0)
            {
                exec.set_error(::zr_boost_1_81::process::detail::get_last_error(), "close() failed");
                return;
            }
        }
        ::closedir(dir);
