...
{
    io_context_ref(zr_boost_1_81::asio::io_context & ios) : ios(ios)
    {
    ...
    {
        ios.notify_fork(zr_boost_1_81::asio::io_context::fork_parent);
        ...
        auto wh = [funcs, es](int val, const std::error_code & ec)
                {
                    es->store(val);
                    for (auto & func : funcs)
                        func(::zr_boost_1_81::process::detail::posix::eval_exit_status(val), ec);
                };
    ...
    template<typename Executor>
    void on_setup (Executor &) const {/*ios.notify_fork(zr_boost_1_81::asio::io_context::fork_prepare);*/}

    template<typename Executor>
    void on_exec_setup  (Executor &) const {/*ios.notify_fork(zr_boost_1_81::asio::io_context::fork_child);*/}

    template <class Executor>
    void on_error(Executor&, const std::error_code &) const {/*ios.notify_fork(zr_boost_1_81::asio::io_context::fork_parent);*/}

private:
    zr_boost_1_81::asio::io_context &ios;
    zr_boost_1_81::process::detail::posix::sigchld_service &sigchld_service = zr_boost_1_81::asio::use_service<zr_boost_1_81::process::detail::posix::sigchld_service>(ios);
};
