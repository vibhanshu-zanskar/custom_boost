...
class native_environment_impl
{
    static std::vector<std::basic_string<Char>>  _load()
    {
        ...
        {
            std::string str = *p;
            val.push_back(::zr_boost_1_81::process::detail::convert(str));
            p++;
        }
        return val;
    }
...
public:
    ...
    {
        std::string id_c = ::zr_boost_1_81::process::detail::convert(id);
        std::string g = ::getenv(id_c.c_str());
        return ::zr_boost_1_81::process::detail::convert(g.c_str());
    }
    void        set(const string_type & id, const string_type & value)
    {
        std::string id_c    = ::zr_boost_1_81::process::detail::convert(id.c_str());
        std::string value_c = ::zr_boost_1_81::process::detail::convert(value.c_str());
...
public:
        ...
        : _data(rhs._data.size())
    {
        std::transform(rhs._data.begin(), rhs._data.end(), _data.begin(),
                ...
                {
                    return ::zr_boost_1_81::process::detail::convert(st, cv);
                }
