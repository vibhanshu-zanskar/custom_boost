...
class executor
{
    ...
    void internal_error_handle(const std::error_code&, const char*, HasHandler, zr_boost_1_81::mpl::true_, UseVFork) {}
    ...
    void write_error(const std::error_code & ec, const char * msg)
    {
        ...
        zr_boost_1_81::ignore_unused(::write(_pipe_sink, &data[0], sizeof(int) * 2));
        zr_boost_1_81::ignore_unused(::write(_pipe_sink, msg, len));
    }

    void internal_error_handle(const std::error_code &ec, const char* msg, zr_boost_1_81::mpl::true_ , zr_boost_1_81::mpl::false_, zr_boost_1_81::mpl::false_)
    {
    ...
    void internal_error_handle(const std::error_code &ec, const char* msg, zr_boost_1_81::mpl::false_, zr_boost_1_81::mpl::false_, zr_boost_1_81::mpl::false_)
    {
        if (this->pid == 0)
            write_error(ec, msg);
        else
    ...
    void internal_error_handle(const std::error_code &ec, const char* msg, zr_boost_1_81::mpl::true_ , zr_boost_1_81::mpl::false_, zr_boost_1_81::mpl::true_)
    {
        this->_ec  = ec;
        this->_msg = msg;
    }
    void internal_error_handle(const std::error_code &ec, const char* msg, zr_boost_1_81::mpl::false_, zr_boost_1_81::mpl::false_, zr_boost_1_81::mpl::true_)
    {
        if (this->pid == 0)
        {
            this->_ec  = ec;
    ...
    void check_error(zr_boost_1_81::mpl::true_) {};
    void check_error(zr_boost_1_81::mpl::false_)
    {
        if (_ec)
            throw process_error(_ec, _msg);
    }

    typedef typename ::zr_boost_1_81::process::detail::has_error_handler<Sequence>::type has_error_handler;
    typedef typename ::zr_boost_1_81::process::detail::has_ignore_error <Sequence>::type has_ignore_error;
    typedef typename ::zr_boost_1_81::process::detail::posix::shall_use_vfork<Sequence>::type shall_use_vfork;

    inline child invoke(zr_boost_1_81::mpl::true_ , zr_boost_1_81::mpl::true_ );
    inline child invoke(zr_boost_1_81::mpl::false_, zr_boost_1_81::mpl::true_ );
    inline child invoke(zr_boost_1_81::mpl::true_ , zr_boost_1_81::mpl::false_ );
    inline child invoke(zr_boost_1_81::mpl::false_, zr_boost_1_81::mpl::false_ );
    ...
    inline void prepare_cmd_style() //this does what execvpe does - but we execute it in the father process, to avoid allocations.
    {
        ...
        if ((prepare_cmd_style_fn.find('/') == std::string::npos) && ::access(prepare_cmd_style_fn.c_str(), X_OK))
        {
            ...
            while ((e != nullptr) && (*e != nullptr) && !zr_boost_1_81::starts_with(*e, "PATH="))
                e++;
            ...
            if ((e != nullptr) && (*e != nullptr))
            {
                ...
                zr_boost_1_81::split(path, (*e) + 5, zr_boost_1_81::is_any_of(":"));
...
template<typename Sequence>
child executor<Sequence>::invoke(zr_boost_1_81::mpl::true_, zr_boost_1_81::mpl::false_) //ignore errors
{
    zr_boost_1_81::fusion::for_each(seq, call_on_setup(*this));
    if (_ec)
        return child();
    if (cmd_style)
        prepare_cmd_style();
    ...
    if (pid == -1)
    {
        auto ec = zr_boost_1_81::process::detail::get_last_error();
        zr_boost_1_81::fusion::for_each(seq, call_on_fork_error(*this, ec));
        return child();
    }
    else if (pid == 0)
    {
        zr_boost_1_81::fusion::for_each(seq, call_on_exec_setup(*this));
        ::execve(exe, cmd_line, env);
        auto ec = zr_boost_1_81::process::detail::get_last_error();
        zr_boost_1_81::fusion::for_each(seq, call_on_exec_error(*this, ec));
        _exit(EXIT_FAILURE);
    }

    child c(child_handle(pid), exit_status);

    zr_boost_1_81::fusion::for_each(seq, call_on_success(*this));

    return c;
}

template<typename Sequence>
child executor<Sequence>::invoke(zr_boost_1_81::mpl::false_, zr_boost_1_81::mpl::false_)
{
    {
        struct pipe_guard
        {
        ...
        if (::pipe(p.p) == -1)
        {
            set_error(::zr_boost_1_81::process::detail::get_last_error(), "pipe(2) failed");
            return child();
        }
        if (::fcntl(p.p[1], F_SETFD, FD_CLOEXEC) == -1)
        {
            auto err = ::zr_boost_1_81::process::detail::get_last_error();
        ...
        if (_ec)
        {
            zr_boost_1_81::fusion::for_each(seq, call_on_error(*this, _ec));
            return child();
        }
        ...
        if (pid == -1)
        {
            _ec = zr_boost_1_81::process::detail::get_last_error();
            _msg = "fork() failed";
            zr_boost_1_81::fusion::for_each(seq, call_on_fork_error(*this, _ec));
            zr_boost_1_81::fusion::for_each(seq, call_on_error(*this, _ec));
            return child();
        }
        else if (pid == 0)
        {
            ...
            _ec = zr_boost_1_81::process::detail::get_last_error();
            _msg = "execve failed";
            zr_boost_1_81::fusion::for_each(seq, call_on_exec_error(*this, _ec));
...
template<typename Sequence>
child executor<Sequence>::invoke(zr_boost_1_81::mpl::true_, zr_boost_1_81::mpl::true_) //ignore errors
{
    zr_boost_1_81::fusion::for_each(seq, call_on_setup(*this));
    if (_ec)
        return child();
...
template<typename Sequence>
child executor<Sequence>::invoke(zr_boost_1_81::mpl::false_, zr_boost_1_81::mpl::true_)
{
    zr_boost_1_81::fusion::for_each(seq, call_on_setup(*this));
