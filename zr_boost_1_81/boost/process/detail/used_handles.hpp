...
struct foreach_handle_invocator
{
    ...
    void invoke(::zr_boost_1_81::process::detail::api::native_handle_type handle) const {func(handle);};
...
{
    zr_boost_1_81::fusion::for_each(zr_boost_1_81::fusion::filter_if<does_use_handle<zr_boost_1_81::mpl::_>>(exec.seq),
                            foreach_handle_invocator<Function>(func));
}

template<typename Executor>
std::vector<::zr_boost_1_81::process::detail::api::native_handle_type>
        get_used_handles(Executor &exec)
{
    std::vector<::zr_boost_1_81::process::detail::api::native_handle_type> res = exec.get_used_handles();
    foreach_used_handle(exec, [&](::zr_boost_1_81::process::detail::api::native_handle_type handle){res.push_back(handle);});
    return res;
}
