...
using ::zr_boost_1_81::process::detail::api::pid_t;

class child
{
    ::zr_boost_1_81::process::detail::api::child_handle _child_handle;
    std::shared_ptr<std::atomic<int>> _exit_status = std::make_shared<std::atomic<int>>(::zr_boost_1_81::process::detail::api::still_active);
    bool _attached = true;
    bool _terminated = false;

    bool _exited()
    {
        return _terminated || !::zr_boost_1_81::process::detail::api::is_running(_exit_status->load());
    };
public:
    typedef ::zr_boost_1_81::process::detail::api::child_handle child_handle;
    ...
    int exit_code() const {return ::zr_boost_1_81::process::detail::api::eval_exit_status(_exit_status->load());}
    ...
    bool running()
    {
        std::error_code ec;
        bool b = running(ec);
        zr_boost_1_81::process::detail::throw_error(ec, "running error");
        return b;
    }
    ...
    void terminate()
    {
        std::error_code ec;
        terminate(ec);
        zr_boost_1_81::process::detail::throw_error(ec, "terminate error");
    }
    ...
    bool running(std::error_code & ec) noexcept
    {
        ...
        {
            int exit_code = 0;
            auto res = zr_boost_1_81::process::detail::api::is_running(_child_handle, exit_code, ec);
            if (!ec && !res && !_exited())
    ...
    void terminate(std::error_code & ec) noexcept
    {
        if (valid() && running(ec) && !ec)
            zr_boost_1_81::process::detail::api::terminate(_child_handle, ec);
    ...
    void wait(std::error_code & ec) noexcept
    {
        ...
        {
            int exit_code = 0;
            zr_boost_1_81::process::detail::api::wait(_child_handle, exit_code, ec);
            if (!ec)
                _exit_status->store(exit_code);
        }
    }
    ...
    bool wait_until(const std::chrono::time_point<Clock, Duration>& timeout_time, std::error_code & ec) noexcept
    {
        ...
        {
            int exit_code = 0;
            auto b = zr_boost_1_81::process::detail::api::wait_until(_child_handle, exit_code, timeout_time, ec);
            if (!b || ec)
                return false;
            _exit_status->store(exit_code);
        }
