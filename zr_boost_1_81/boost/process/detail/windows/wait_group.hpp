...
{
    ::zr_boost_1_81::winapi::DWORD_ completion_code;
    ::zr_boost_1_81::winapi::ULONG_PTR_ completion_key;
    ::zr_boost_1_81::winapi::LPOVERLAPPED_ overlapped;

    auto start_time = std::chrono::system_clock::now();

    while (workaround::get_queued_completion_status(
                                       p._io_port, &completion_code,
                                       &completion_key, &overlapped, static_cast<::zr_boost_1_81::winapi::DWORD_>(wait_time)))
    {
        if (reinterpret_cast<::zr_boost_1_81::winapi::HANDLE_>(completion_key) == p._job_object &&
        ...
        if (wait_time != static_cast<int>(::zr_boost_1_81::winapi::infinite))
        {
    ...
    if (ec_.value() == ::zr_boost_1_81::winapi::wait_timeout)
        return true; //timeout
...
{
    wait_impl(p, ec, ::zr_boost_1_81::winapi::infinite);
}
