...
{
    static void _deleter(Char* p) {zr_boost_1_81::winapi::free_environment_strings(p);};
    std::unique_ptr<Char[], void(*)(Char*)> _buf{zr_boost_1_81::winapi::get_environment_strings<Char>(), &native_environment_impl::_deleter};
...
public:
    ...
    void reload()
    {
        _buf.reset(zr_boost_1_81::winapi::get_environment_strings<Char>());
...
{
    Char buf[4096];
    auto size = zr_boost_1_81::winapi::get_environment_variable(id, buf, sizeof(buf));
    if (size == 0) //failed
    {
        auto err =  ::zr_boost_1_81::winapi::GetLastError();
        if (err == ::zr_boost_1_81::winapi::ERROR_ENVVAR_NOT_FOUND_)//well, then we consider that an empty value
            return "";
        else
    ...
    if (size == sizeof(buf)) //the return size gives the size without the null, so I know this went wrong
    {
        ...
        while (buf_size <= max_size)
        {
            ...
            auto size = zr_boost_1_81::winapi::get_environment_variable(id, buf.data(), buf.size());

            if (size == buf_size) //buffer to small
                buf_size *= 2;
            else if (size == 0)
                ::zr_boost_1_81::process::detail::throw_last_error("GetEnvironmentVariable() failed");
            else
...
{
    zr_boost_1_81::winapi::set_environment_variable(id, value);
}

template<typename Char>
inline void  native_environment_impl<Char>::reset(const pointer_type id)
{
    zr_boost_1_81::winapi::set_environment_variable(id, nullptr);
}
...
public:
    ...
    explicit inline  basic_environment_impl(
                ...
                const ::zr_boost_1_81::process::codecvt_type & cv = ::zr_boost_1_81::process::codecvt())
        : _data(::zr_boost_1_81::process::detail::convert(rhs._data, cv))
    {
    }
    ...
    {
        _data = ::zr_boost_1_81::process::detail::convert(rhs._data);
...
inline int   get_id()         {return zr_boost_1_81::winapi::GetCurrentProcessId();}
inline void* native_handle()  {return zr_boost_1_81::winapi::GetCurrentProcess(); }
