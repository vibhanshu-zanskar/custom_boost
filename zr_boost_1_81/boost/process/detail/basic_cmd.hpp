...
struct char_converter<char, exe_setter_<wchar_t>>
{
    ...
    {
        return {::zr_boost_1_81::process::detail::convert(in.exe_)};
    }
};
...
struct char_converter<char, arg_setter_<wchar_t, true>>
{
    static arg_setter_<char, true> conv(const arg_setter_<wchar_t, true> & in)
    {
        ...
        std::transform(in._args.begin(), in._args.end(), vec.begin(),
                ...
                {
                    return ::zr_boost_1_81::process::detail::convert(ws);
                });
        return {vec};
    }
};
...
struct exe_builder
{
    ...
    void operator()(const zr_boost_1_81::process::filesystem::path & data)
    {
        not_cmd = true;
        if (exe.empty())
            exe = data.native();
