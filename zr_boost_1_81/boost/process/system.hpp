...
inline int system_impl(
        ...
        Args && ...args)
{
    IoService & ios = ::zr_boost_1_81::process::detail::get_io_context_var(args...);
    ...
    child c(std::forward<Args>(args)...,
            check,
            ::zr_boost_1_81::process::on_exit(
                [&](int, const std::error_code&)
                {
                    zr_boost_1_81::asio::post(ios.get_executor(), [&]{exited.store(true);});
                }));
        ...
        Args && ...args)
{
    child c(std::forward<Args>(args)...
#if defined(BOOST_POSIX_API)
            ,::zr_boost_1_81::process::posix::sig.dfl()
#endif
            );
    if (!c.valid())
        return -1;
...
{
    typedef typename ::zr_boost_1_81::process::detail::needs_io_context<Args...>::type
            need_ios;
    typedef typename ::zr_boost_1_81::process::detail::has_io_context<Args...>::type
            has_ios;
    return ::zr_boost_1_81::process::detail::system_impl<zr_boost_1_81::asio::io_context>(
            need_ios(), has_ios(),
