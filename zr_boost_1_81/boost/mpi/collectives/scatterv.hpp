...
{
  using detail::c_data;
  ::zr_boost_1_81::mpi::scatterv(comm, c_data(in_values), sizes, displs,
...
scatterv(const communicator& comm, const std::vector<T>& in_values,
         const std::vector<int>& sizes, T* out_values, int root)
{
  ::zr_boost_1_81::mpi::scatterv(comm, detail::c_data(in_values), sizes, out_values, root);
}
...
scatterv(const communicator& comm, const std::vector<T>& in_values,
         T* out_values, int out_size, int root)
{
  ::zr_boost_1_81::mpi::scatterv(comm, detail::c_data(in_values), out_values, out_size, root);
}
