...
class BOOST_MPI_DECL mpi_datatype_map
 : public zr_boost_1_81::noncopyable
{
...
public:
  ...
  template <class T>
  MPI_Datatype datatype(const T& x = T(), typename zr_boost_1_81::enable_if<is_mpi_builtin_datatype<T> >::type* =0)
  {
    return get_mpi_datatype<T>(x);
  }

  template <class T>
  MPI_Datatype datatype(const T& x =T(), typename zr_boost_1_81::disable_if<is_mpi_builtin_datatype<T> >::type* =0 )
  {
