...
//       It now makes no use of zr_boost_1_81::function or old FC++ code.
//
//       The functor type F must be an operator defined with
//       zr_boost_1_81::zr_boost_1_81phoenix::function.
...
struct reuser1 {
   ...
   typedef typename zr_boost_1_81::zr_boost_1_81phoenix::function<R> fun0_type;
   typedef Thunk1<V1,V2,F,X,R> M;
   typedef M result_type;
   zr_boost_1_81::intrusive_ptr<const M> ref;
   reuser1(a_unique_type_for_nil) {}
   reuser1(const M* m) : ref(m) {}
   M operator()( const F& f, const X& x ) {
      if( !ref )   ref = zr_boost_1_81::intrusive_ptr<const M>( new M(f,x) );
      else         ref->init(f,x);
      return *ref;
   }
...
struct reuser2 {
   ...
   M operator()( const F& f, const X& x, const Y& y ) {
      if( !ref )   ref = zr_boost_1_81::intrusive_ptr<const M>( new M(f,x,y) );
      else         ref->init(f,x,y);
      return *ref;
   }
...
struct reuser3 {
   ...
   M operator()( const F& f, const X& x, const Y& y, const Z& z ) {
      if( !ref )   ref = zr_boost_1_81::intrusive_ptr<const M>( new M(f,x,y,z) );
...
struct Thunk4 : public ThunkImpl<R> {
   ...
   M operator()( const F& f, const W& w, const X& x, const Y& y, const Z& z ) {
      if( !ref )   ref = zr_boost_1_81::intrusive_ptr<const M>( new M(f,w,x,y,z) );
