...
BOOST_PHOENIX_DEFINE_EXPRESSION(
    (zr_boost_1_81)(zr_boost_1_81phoenix)(let_)
    ...
    struct let_eval
    {
        ...
        operator()(Vars const & vars, Map, Expr const & expr, Context const & ctx) const
        {
            ...
            scoped_environment<
              ...
              , map_type
            >
            env(zr_boost_1_81phoenix::env(ctx), zr_boost_1_81phoenix::env(ctx), locals);

            // Fix for bugs (trial)
            // The idea is to do something which will not be optimised away.
            //int vsize = zr_boost_1_81::fusion::size(vars);
            //std::stringstream strm;
            //strm << vsize << std::endl;
            //int size = strm.str().length();
            //BOOST_ASSERT(size >= 0);
            return eval(expr, zr_boost_1_81phoenix::context(env, zr_boost_1_81phoenix::actions(ctx)));
