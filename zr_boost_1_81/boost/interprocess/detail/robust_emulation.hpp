...
{
   public:
   static const zr_boost_1_81::uint32_t correct_state = 0;
   static const zr_boost_1_81::uint32_t fixing_state  = 1;
   static const zr_boost_1_81::uint32_t broken_state  = 2;
   ...
   bool is_owner_dead(zr_boost_1_81::uint32_t own);
   void owner_to_filename(zr_boost_1_81::uint32_t own, std::string &s);
   //The real mutex
   Mutex mtx;
   //The pid of the owner
   volatile zr_boost_1_81::uint32_t owner;
   //The state of the mutex (correct, fixing, broken)
   volatile zr_boost_1_81::uint32_t state;
};

template<class Mutex>
inline robust_spin_mutex<Mutex>::robust_spin_mutex()
   : mtx(), owner((zr_boost_1_81::uint32_t)get_invalid_process_id()), state(correct_state)
{}
...
inline bool robust_spin_mutex<Mutex>::try_lock()
{
   ...
   if (mtx.try_lock()){
      atomic_write32(&this->owner, static_cast<zr_boost_1_81::uint32_t>(get_current_process_id()));
      return true;
   }
   else{
...
template<class Mutex>
inline void robust_spin_mutex<Mutex>::owner_to_filename(zr_boost_1_81::uint32_t own, std::string &s)
{
...
{
   zr_boost_1_81::uint32_t cur_owner = static_cast<zr_boost_1_81::uint32_t>(get_current_process_id());
   zr_boost_1_81::uint32_t old_owner = atomic_read32(&this->owner), old_owner2;
...
template<class Mutex>
inline bool robust_spin_mutex<Mutex>::is_owner_dead(zr_boost_1_81::uint32_t own)
{
   //If owner is an invalid id, then it's clear it's dead
   if(own == static_cast<zr_boost_1_81::uint32_t>(get_invalid_process_id())){
      return true;
   }
...
inline void robust_spin_mutex<Mutex>::consistent()
{
   ...
   if(atomic_read32(&this->state) != fixing_state &&
      atomic_read32(&this->owner) != (zr_boost_1_81::uint32_t)get_current_process_id()){
      throw interprocess_exception(lock_error, "Broken id");
   }
...
inline void robust_spin_mutex<Mutex>::unlock()
{
   ...
   atomic_write32(&this->owner, static_cast<zr_boost_1_81::uint32_t>(get_invalid_process_id()));
   mtx.unlock();
}
