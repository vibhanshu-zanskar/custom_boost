...
struct enable_if_ptime
   : enable_if_c< BOOST_INTRUSIVE_HAS_TYPE(zr_boost_1_81::interprocess::ipcdetail::, T, time_duration_type) >
{};
...
struct enable_if_ptime_duration
   : enable_if_c< BOOST_INTRUSIVE_HAS_TYPE(zr_boost_1_81::interprocess::ipcdetail::, T, rep_type) >
{};

template<class T>
struct enable_if_time_point
   : enable_if_c< BOOST_INTRUSIVE_HAS_TYPE(zr_boost_1_81::interprocess::ipcdetail::, T, clock) >
{};

template<class T>
struct enable_if_duration
   : enable_if_c< BOOST_INTRUSIVE_HAS_TYPE(zr_boost_1_81::interprocess::ipcdetail::, T, rep) >
{};
...
*/
inline zr_boost_1_81::uint64_t file_time_to_microseconds(const zr_boost_1_81::winapi::FILETIME_ & ft)
{
   // shift is difference between 1970-Jan-01 & 1601-Jan-01
   // in 100-nanosecond units
   const zr_boost_1_81::uint64_t shift = 116444736000000000ULL; // (27111902 << 32) + 3577643008

   // 100-nanos since 1601-Jan-01
   zr_boost_1_81::uint64_t ft_as_integer = (static_cast< zr_boost_1_81::uint64_t >(ft.dwHighDateTime) << 32) | static_cast< zr_boost_1_81::uint64_t >(ft.dwLowDateTime);
...
class usduration
{
   ...
   explicit usduration(zr_boost_1_81::uint64_t microsecs)
      : m_microsecs(microsecs)
   {}

   zr_boost_1_81::uint64_t get_microsecs() const
   {  return m_microsecs;  }
   ...
   private:
   zr_boost_1_81::uint64_t m_microsecs;
};

class ustime
{
   public:
   explicit ustime(zr_boost_1_81::uint64_t microsecs)
      : m_microsecs(microsecs)
   {}
...
inline usduration usduration_milliseconds(zr_boost_1_81::uint64_t millisec)
{  return usduration(millisec*1000u);   }

inline usduration usduration_seconds(zr_boost_1_81::uint64_t sec)
...
class microsec_clock<TimeType, typename enable_if_ptime<TimeType>::type>
{
   ...
   static TimeType universal_time()
   {
         ...
         zr_boost_1_81::uint32_t sub_sec = static_cast<zr_boost_1_81::uint32_t>(tv.tv_usec);
      #elif defined(BOOST_HAS_FTIME)
         zr_boost_1_81::winapi::FILETIME_ ft;
         zr_boost_1_81::winapi::GetSystemTimeAsFileTime(&ft);
         zr_boost_1_81::uint64_t micros = file_time_to_microseconds(ft); // it will not wrap, since ft is the current time
                                                                  // and cannot be before 1970-Jan-01
         std::time_t t = static_cast<std::time_t>(micros / 1000000UL); // seconds since epoch
         // microseconds -- static casts suppress warnings
         zr_boost_1_81::uint32_t sub_sec = static_cast<zr_boost_1_81::uint32_t>(micros % 1000000UL);
      #else
...
class microsec_clock<ustime>
{
   ...
   static ustime universal_time()
   {
         ...
         timeval tv;
         gettimeofday(&tv, 0); //gettimeofday does not support TZ adjust on Linux.
         zr_boost_1_81::uint64_t micros = zr_boost_1_81::uint64_t(tv.tv_sec)*1000000;
         micros += (zr_boost_1_81::uint64_t)tv.tv_usec;
      #elif defined(BOOST_HAS_FTIME)
         zr_boost_1_81::winapi::FILETIME_ ft;
         zr_boost_1_81::winapi::GetSystemTimeAsFileTime(&ft);
         zr_boost_1_81::uint64_t micros = file_time_to_microseconds(ft); // it will not wrap, since ft is the current time
...
template<class Duration>
inline zr_boost_1_81::uint64_t duration_to_milliseconds(const Duration &abs_time, typename enable_if_ptime_duration<Duration>::type* = 0)
{
   return static_cast<zr_boost_1_81::uint64_t>(abs_time.total_milliseconds());
}

template<class Duration>
inline zr_boost_1_81::uint64_t duration_to_milliseconds(const Duration &d, typename enable_if_duration<Duration>::type* = 0)
{
   const double factor = double(Duration::period::num)*1000.0/double(Duration::period::den);
   return static_cast<zr_boost_1_81::uint64_t>(double(d.count())*factor);
}

inline zr_boost_1_81::uint64_t duration_to_milliseconds(const usduration &d)
{
...
{
   return usduration(static_cast<zr_boost_1_81::uint64_t>(d.total_microseconds()));
}
...
inline usduration duration_to_usduration(const Duration &d, typename enable_if_duration<Duration>::type* = 0)
{
   ...
   return usduration(static_cast<zr_boost_1_81::uint64_t>(double(d.count())*factor));
}
