   ...
   :  private MemoryAlgorithm
{
   ...
   //!Allocates nbytes bytes. Throws zr_boost_1_81::interprocess::bad_alloc
   //!on failure
                           ...
                           size_type &prefer_in_recvd_out_size, T *&reuse)
   {
      T *ret = MemoryAlgorithm::allocation_command
         (command | zr_boost_1_81::interprocess::nothrow_allocation, limit_size, prefer_in_recvd_out_size, reuse);
      if(!(command & zr_boost_1_81::interprocess::nothrow_allocation) && !ret)
         throw bad_alloc();
      return ret;
   }

   void *raw_allocation_command  (zr_boost_1_81::interprocess::allocation_type command,   size_type limit_objects,
                           size_type &prefer_in_recvd_out_size, void *&reuse, size_type sizeof_object = 1)
   {
      void *ret = MemoryAlgorithm::raw_allocation_command
         ( command | zr_boost_1_81::interprocess::nothrow_allocation, limit_objects,
   ...
   :  public segment_manager_base<MemoryAlgorithm>
{
      ...
      ,  m_header(static_cast<segment_manager_base_t*>(get_this_pointer()))
   {
      ...
      const std::size_t void_ptr_alignment = zr_boost_1_81::move_detail::alignment_of<void_pointer>::value; (void)void_ptr_alignment;
      BOOST_ASSERT((0 == (std::size_t)this_addr % zr_boost_1_81::move_detail::alignment_of<segment_manager>::value));
   }
   ...
   //!Can throw zr_boost_1_81::interprocess::bad_alloc if there is no enough memory.
   ...
   struct allocator
   {
      typedef zr_boost_1_81::interprocess::allocator<T, segment_manager> type;
   };
   ...
   struct deleter
   {
      typedef zr_boost_1_81::interprocess::deleter<T, segment_manager> type;
   };
   ...
   {
      zr_boost_1_81::interprocess::allocation_type type = ctrl_data->alloc_type();
   ...
   scoped_lock<rmutex> priv_get_lock(bool use_lock)
   {
      ...
      return scoped_lock<rmutex>(zr_boost_1_81::move(local));
   }
