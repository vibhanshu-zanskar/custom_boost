...
class spin_condition
{
   ...
   {
      typedef zr_boost_1_81::interprocess::scoped_lock<spin_mutex> InternalLock;
      ...
      //no other thread can execute the do_timed_wait method.
      {
         ...
         atomic_inc32(const_cast<zr_boost_1_81::uint32_t*>(&m_num_waiters));
      ...
      while(1){
         ...
         if(TimeoutEnabled && timed_out){
            ...
            atomic_dec32(const_cast<zr_boost_1_81::uint32_t*>(&m_num_waiters));
            unlock_enter_mut = true;
            break;
         }
         else{
            zr_boost_1_81::uint32_t result = atomic_cas32
                           (const_cast<zr_boost_1_81::uint32_t*>(&m_command), SLEEP, NOTIFY_ONE);
            if(result == SLEEP){
               ...
               unlock_enter_mut = 1 == atomic_dec32(const_cast<zr_boost_1_81::uint32_t*>(&m_num_waiters));
               //Check if this is the last thread of notify_all waiters
               //Only the last thread will release the mutex
               if(unlock_enter_mut){
                  atomic_cas32(const_cast<zr_boost_1_81::uint32_t*>(&m_command), SLEEP, NOTIFY_ALL);
               }
               break;
            }
         }
   ...
   void notify(zr_boost_1_81::uint32_t command)
   {
      ...
      spin_wait swait;
      while(SLEEP != atomic_cas32(const_cast<zr_boost_1_81::uint32_t*>(&m_command), command, SLEEP)){
         swait.yield();
      }
   ...
   volatile zr_boost_1_81::uint32_t    m_command;
   volatile zr_boost_1_81::uint32_t    m_num_waiters;
};
