...
//    const std::list<T>::iterator prev = zr_boost_1_81::prior(p);
//    const std::list<T>::iterator next = zr_boost_1_81::next(prev, 2);
...
struct next_advance_impl< T, Distance, true >
{
    static T call(T x, Distance n)
    {
        zr_boost_1_81::iterators::advance(x, n);
        return x;
    }
};
...
struct prior_advance_impl< T, Distance, true >
{
    static T call(T x, Distance n)
    {
        ...
        zr_boost_1_81::iterators::reverse_iterator< T > rx(x);
        zr_boost_1_81::iterators::advance(rx, n);
        return rx.base();
    }
};
