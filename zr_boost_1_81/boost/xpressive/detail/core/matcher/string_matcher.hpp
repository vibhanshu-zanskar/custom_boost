...
namespace zr_boost_1_81 { namespace xpressive { namespace detail
{
      ...
      : quant_style_fixed_unknown_width
    {
        ...
        string_matcher(string_type const &str, Traits const &tr)
          ...
          , end_()
        {
            typename range_iterator<string_type>::type cur = zr_boost_1_81::begin(this->str_);
            typename range_iterator<string_type>::type end = zr_boost_1_81::end(this->str_);
            for(; cur != end; ++cur)
            {
        ...
        {
            return zr_boost_1_81::size(this->str_);
        }
    };
