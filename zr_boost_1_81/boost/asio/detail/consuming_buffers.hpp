...
struct prepared_buffers_max<zr_boost_1_81::array<Elem, N> >
{
  enum { value = N };
};
...
public:
  ...
  prepared_buffers_type prepare(std::size_t max_size)
  {
    ...
    Buffer_Iterator next = zr_boost_1_81::asio::buffer_sequence_begin(buffers_);
    Buffer_Iterator end = zr_boost_1_81::asio::buffer_sequence_end(buffers_);
    ...
    while (next != end && max_size > 0 && (result.count) < result.max_buffers)
    {
      ...
      result.elems[result.count] = zr_boost_1_81::asio::buffer(next_buf, max_size);
...
public:
  ...
  {
    return zr_boost_1_81::asio::buffer(buffer_ + total_consumed_, max_size);
  }
...
class consuming_buffers<Buffer, zr_boost_1_81::array<Elem, 2>,
    typename zr_boost_1_81::array<Elem, 2>::const_iterator>
{
public:
  // Construct to represent the entire list of buffers.
  explicit consuming_buffers(const zr_boost_1_81::array<Elem, 2>& buffers)
    : buffers_(buffers),
      total_consumed_(0)
  {
  }
  ...
  // Get the buffer for a single transfer, with a size.
  zr_boost_1_81::array<Buffer, 2> prepare(std::size_t max_size)
  {
    zr_boost_1_81::array<Buffer, 2> result = {{
      Buffer(buffers_[0]), Buffer(buffers_[1]) }};
    std::size_t buffer0_size = result[0].size();
    result[0] = zr_boost_1_81::asio::buffer(result[0] + total_consumed_, max_size);
    result[1] = zr_boost_1_81::asio::buffer(
...
private:
  zr_boost_1_81::array<Elem, 2> buffers_;
  std::size_t total_consumed_;
};
...
class consuming_buffers<Buffer, null_buffers, const mutable_buffer*>
  : public zr_boost_1_81::asio::null_buffers
{
public:
