...
zr_boost_1_81::system::error_code win_iocp_socket_service_base::close(
    win_iocp_socket_service_base::base_implementation_type& impl,
...
zr_boost_1_81::system::error_code win_iocp_socket_service_base::cancel(
    win_iocp_socket_service_base::base_implementation_type& impl,
    zr_boost_1_81::system::error_code& ec)
{
  if (!is_open(impl))
...
zr_boost_1_81::system::error_code win_iocp_socket_service_base::do_open(
    win_iocp_socket_service_base::base_implementation_type& impl,
    int family, int type, int protocol, zr_boost_1_81::system::error_code& ec)
{
  if (is_open(impl))
  {
...
zr_boost_1_81::system::error_code win_iocp_socket_service_base::do_assign(
    win_iocp_socket_service_base::base_implementation_type& impl,
    int type, socket_type native_socket, zr_boost_1_81::system::error_code& ec)
{
  if (is_open(impl))
  {
    ec = zr_boost_1_81::asio::error::already_open;
    ...
    socket_base::message_flags flags, bool noop, operation* op)
{
  ...
  if (noop)
    ...
    iocp_service_.on_completion(op, zr_boost_1_81::asio::error::bad_descriptor);
  else
  {
    ...
    int protocol, void* output_buffer, DWORD address_length, operation* op)
{
  ...
  if (!is_open(impl))
    ...
    iocp_service_.on_completion(op, zr_boost_1_81::asio::error::already_open);
  else
  {
    zr_boost_1_81::system::error_code ec;
    ...
    long* cancel_requested, operation* op)
{
  ...
  if (cancel_requested)
    if (::InterlockedExchangeAdd(cancel_requested, 0) == 1)
      iocp_service_.on_completion(op, zr_boost_1_81::asio::error::operation_aborted);
    ...
    win_iocp_socket_connect_op_base* op, operation* iocp_op)
{
      ...
      || family == BOOST_ASIO_OS_DEF(AF_INET6))
  {
    if (connect_ex_fn connect_ex = get_connect_ex(impl, type))
    {
      ...
      if (op->ec_ && op->ec_ != zr_boost_1_81::asio::error::invalid_argument)
      {
        iocp_service_.post_immediate_completion(op, false);
        return -1;
      }
        ...
        impl.socket_, impl.state_, true, op->ec_))
  {
    ...
    {
      if (op->ec_ == zr_boost_1_81::asio::error::in_progress
          || op->ec_ == zr_boost_1_81::asio::error::would_block)
      {
