...
kqueue_reactor::kqueue_reactor(zr_boost_1_81::asio::execution_context& ctx)
    ...
    registered_descriptors_mutex_(mutex_.enabled())
{
  ...
  {
    zr_boost_1_81::system::error_code error(errno,
        zr_boost_1_81::asio::error::get_system_category());
    zr_boost_1_81::asio::detail::throw_error(error);
  }
}
...
{
  if (fork_ev == zr_boost_1_81::asio::execution_context::fork_child)
  {
    ...
    if (::kevent(kqueue_fd_, events, 1, 0, 0, 0) == -1)
    {
      zr_boost_1_81::system::error_code ec(errno,
          zr_boost_1_81::asio::error::get_system_category());
      zr_boost_1_81::asio::detail::throw_error(ec, "kqueue interrupter registration");
    }
        ...
        state != 0; state = state->next_)
    {
      if (state->num_kevents_ > 0)
      {
        ...
        if (::kevent(kqueue_fd_, events, state->num_kevents_, 0, 0, 0) == -1)
        {
          zr_boost_1_81::system::error_code ec(errno,
              zr_boost_1_81::asio::error::get_system_category());
          zr_boost_1_81::asio::detail::throw_error(ec, "kqueue re-registration");
        }
      }
    }
  }
  ...
  if (descriptor_data->op_queue_[op_type].empty())
  {
          ...
          || descriptor_data->op_queue_[except_op].empty()))
    {
      ...
      if (descriptor_data->num_kevents_ < num_kevents[op_type])
      {
        ...
        {
          op->ec_ = zr_boost_1_81::system::error_code(errno,
              zr_boost_1_81::asio::error::get_system_category());
          scheduler_.post_immediate_completion(op, is_continuation);
          return;
        }
...
int kqueue_reactor::do_kqueue_create()
{
  ...
  if (fd == -1)
  {
    zr_boost_1_81::system::error_code ec(errno,
        zr_boost_1_81::asio::error::get_system_category());
    zr_boost_1_81::asio::detail::throw_error(ec, "kqueue");
  }
  return fd;
}
