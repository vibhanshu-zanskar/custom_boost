...
io_uring_service::io_uring_service(zr_boost_1_81::asio::execution_context& ctx)
...
void io_uring_service::notify_fork(
    zr_boost_1_81::asio::execution_context::fork_event fork_ev)
{
  switch (fork_ev)
  {
  case zr_boost_1_81::asio::execution_context::fork_prepare:
    {
  ...
  case zr_boost_1_81::asio::execution_context::fork_parent:
    // Restart the timeout and eventfd operations.
    update_timeout();
    register_with_reactor();
    break;

  case zr_boost_1_81::asio::execution_context::fork_child:
    {
    ...
    int op_type, io_uring_operation* op)
{
  ...
  {
    zr_boost_1_81::system::error_code ec(ENOBUFS,
        zr_boost_1_81::asio::error::get_system_category());
    zr_boost_1_81::asio::detail::throw_error(ec, "io_uring_get_sqe");
  }
}
...
void io_uring_service::register_buffers(const ::iovec* v, unsigned n)
{
  ...
  if (result < 0)
  {
    zr_boost_1_81::system::error_code ec(-result,
        zr_boost_1_81::asio::error::get_system_category());
    zr_boost_1_81::asio::detail::throw_error(ec, "io_uring_register_buffers");
  }
}
    ...
    int op_type, void* cancellation_key)
{
  ...
  while (io_uring_operation* op = io_obj->queues_[op_type].op_queue_.front())
  {
    ...
    if (op->cancellation_key_ == cancellation_key)
    {
      ...
      {
        op->ec_ = zr_boost_1_81::asio::error::operation_aborted;
        ops.push(op);
      }
    }
    else
...
void io_uring_service::init_ring()
{
  ...
  if (result < 0)
  {
    ...
    zr_boost_1_81::system::error_code ec(-result,
        zr_boost_1_81::asio::error::get_system_category());
    zr_boost_1_81::asio::detail::throw_error(ec, "io_uring_queue_init");
  }
  ...
  if (event_fd_ < 0)
  {
    zr_boost_1_81::system::error_code ec(-result,
        zr_boost_1_81::asio::error::get_system_category());
    ::io_uring_queue_exit(&ring_);
    zr_boost_1_81::asio::detail::throw_error(ec, "eventfd");
  }
...
public:
  event_fd_read_op(io_uring_service* s)
    : reactor_op(zr_boost_1_81::system::error_code(),
...
void io_uring_service::submit_sqes_op::do_complete(void* owner, operation* base,
    const zr_boost_1_81::system::error_code& /*ec*/, std::size_t /*bytes_transferred*/)
{
  if (owner)
  {
...
operation* io_uring_service::io_queue::perform_io(int result)
{
  ...
  if (result != -ECANCELED || cancel_requested_)
  {
    if (io_uring_operation* op = op_queue_.front())
    {
      if (result < 0)
      {
        op->ec_.assign(-result, zr_boost_1_81::asio::error::get_system_category());
  ...
  if (!op_queue_.empty())
  {
    ...
    else
    {
      ...
      while (io_uring_operation* op = op_queue_.front())
      {
        op->ec_ = zr_boost_1_81::asio::error::no_buffer_space;
        op_queue_.pop();
        io_cleanup.ops_.push(op);
      }
    }
