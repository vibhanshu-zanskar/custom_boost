...
public:
  ...
  typedef scheduler_task* (*get_task_func_type)(
      zr_boost_1_81::asio::execution_context&);

  // Constructor. Specifies the number of concurrent threads that are likely to
  // run the scheduler. If set to 1 certain optimisation are performed.
  BOOST_ASIO_DECL scheduler(zr_boost_1_81::asio::execution_context& ctx,
  ...
  BOOST_ASIO_DECL std::size_t run(zr_boost_1_81::system::error_code& ec);

  // Run until interrupted or one operation is performed.
  BOOST_ASIO_DECL std::size_t run_one(zr_boost_1_81::system::error_code& ec);

  // Run until timeout, interrupted, or one operation is performed.
  BOOST_ASIO_DECL std::size_t wait_one(
      long usec, zr_boost_1_81::system::error_code& ec);

  // Poll for operations without blocking.
  BOOST_ASIO_DECL std::size_t poll(zr_boost_1_81::system::error_code& ec);

  // Poll for one operation without blocking.
  BOOST_ASIO_DECL std::size_t poll_one(zr_boost_1_81::system::error_code& ec);
...
private:
  ...
  BOOST_ASIO_DECL std::size_t do_run_one(mutex::scoped_lock& lock,
      thread_info& this_thread, const zr_boost_1_81::system::error_code& ec);

  // Run at most one operation with a timeout. May block.
  BOOST_ASIO_DECL std::size_t do_wait_one(mutex::scoped_lock& lock,
      thread_info& this_thread, long usec, const zr_boost_1_81::system::error_code& ec);
  ...
  BOOST_ASIO_DECL static scheduler_task* get_default_task(
      zr_boost_1_81::asio::execution_context& ctx);
