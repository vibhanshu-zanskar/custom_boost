...
#if defined(BOOST_ASIO_HAS_BOOST_DATE_TIME) \
  && defined(BOOST_ASIO_USE_BOOST_DATE_TIME_FOR_SOCKET_IOSTREAM)
    typename Clock = zr_boost_1_81::posix_time::ptime,
...
# endif // !defined(BOOST_ASIO_NO_DEPRECATED)
      ...
      expiry_time_(other.expiry_time_)
  {
    ...
    other.ec_ = zr_boost_1_81::system::error_code();
  ...
  {
    init_buffers();
    ec_ = zr_boost_1_81::system::error_code();
   ...
   * buffer.
   */
  const zr_boost_1_81::system::error_code& error() const
  {
    return ec_;
  }
   ...
   * buffer.
   */
  const zr_boost_1_81::system::error_code& puberror() const
  {
    return error();
  }
   ...
   * zr_boost_1_81::asio::error::operation_aborted.
   *
...
protected:
  int_type underflow()
  {
    ...
    ec_ = zr_boost_1_81::asio::error::operation_not_supported;
    ...
    for (;;)
    {
      ...
      {
        ec_ = zr_boost_1_81::asio::error::timed_out;
        return traits_type::eof();
      }
      ...
      detail::buffer_sequence_adapter<mutable_buffer, mutable_buffer>
        bufs(zr_boost_1_81::asio::buffer(get_buffer_) + putback_max);
      ...
      if (bytes == 0)
      {
        ec_ = zr_boost_1_81::asio::error::eof;
        return traits_type::eof();
      }

      // Operation failed.
      if (ec_ != zr_boost_1_81::asio::error::would_block
          && ec_ != zr_boost_1_81::asio::error::try_again)
        return traits_type::eof();
  ...
  int_type overflow(int_type c)
  {
    ...
    if (put_buffer_.empty())
    {
      ...
      output_buffer = zr_boost_1_81::asio::buffer(&ch, sizeof(char_type));
    }
    else
    {
      output_buffer = zr_boost_1_81::asio::buffer(pbase(),
...
private:
  ...
  void connect_to_endpoints(EndpointIterator begin, EndpointIterator end)
  {
    ...
    ec_ = zr_boost_1_81::asio::error::not_found;
    for (EndpointIterator i = begin; i != end; ++i)
    {
      ...
      // Operation failed.
      if (ec_ != zr_boost_1_81::asio::error::in_progress
          && ec_ != zr_boost_1_81::asio::error::would_block)
        continue;
      ...
      ec_ = zr_boost_1_81::system::error_code(connect_error,
