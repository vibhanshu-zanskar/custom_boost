...
struct prefer_only_property<InnerProperty,
    typename void_type<
      decltype(zr_boost_1_81::asio::declval<const InnerProperty>().value())
    >::type>
{
  InnerProperty property;
  ...
  BOOST_ASIO_CONSTEXPR auto value() const
    BOOST_ASIO_NOEXCEPT_IF((
      noexcept(zr_boost_1_81::asio::declval<const InnerProperty>().value())))
    -> decltype(zr_boost_1_81::asio::declval<const InnerProperty>().value())
  {
  ...
  && !defined(__clang__) // Clang crashes if noexcept is used here.
    BOOST_ASIO_NOEXCEPT_IF((
      is_nothrow_prefer<const Executor&, const InnerProperty&>::value))
       ...
       //   && !defined(__clang__)
  {
    return zr_boost_1_81::asio::prefer(ex, p.property);
  }
  ...
  && !defined(__clang__) // Clang crashes if noexcept is used here.
    BOOST_ASIO_NOEXCEPT_IF((
      is_nothrow_query<const Executor&, const InnerProperty&>::value))
       ...
       //   && !defined(__clang__)
  {
    return zr_boost_1_81::asio::query(ex, p.property);
  }
};
