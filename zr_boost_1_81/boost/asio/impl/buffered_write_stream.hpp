...
template <typename Stream>
std::size_t buffered_write_stream<Stream>::flush(zr_boost_1_81::system::error_code& ec)
{
  std::size_t bytes_written = write(next_layer_,
      buffer(storage_.data(), storage_.size()),
      transfer_all(), ec);
...
{
  using zr_boost_1_81::asio::buffer_size;
...
namespace detail
{
  ...
  public:
    ...
    void operator()(const zr_boost_1_81::system::error_code& ec, std::size_t)
    {
      if (ec)
      {
      ...
      else
      {
        ...
        const std::size_t bytes_copied = zr_boost_1_81::asio::buffer_copy(
