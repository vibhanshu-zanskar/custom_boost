...
public:
  ...
  {
    return zr_boost_1_81::asio::query(system_executor(), execution::context);
  }
  ...
  void execute(BOOST_ASIO_MOVE_ARG(F) f) const
  {
    ...
    zr_boost_1_81::asio::require(system_executor(), Blocking()).execute(
        promise_invoker<T, F>(p_, BOOST_ASIO_MOVE_CAST(F)(f)));
#else // defined(BOOST_ASIO_NO_DEPRECATED)
    execution::execute(
        zr_boost_1_81::asio::require(system_executor(), Blocking()),
        promise_invoker<T, F>(p_, BOOST_ASIO_MOVE_CAST(F)(f)));
    ...
    if (ec)
    {
      this->p_->set_exception(
          std::make_exception_ptr(
            zr_boost_1_81::system::system_error(ec)));
    }
    else
    {
      this->p_->set_value();
...
template <>
class promise_handler_selector<void(zr_boost_1_81::system::error_code)>
...
template <typename Arg>
class promise_handler_selector<void(zr_boost_1_81::system::error_code, Arg)>
...
#define BOOST_ASIO_PRIVATE_PROMISE_SELECTOR_DEF(n) \
  ...
  template <typename Arg, BOOST_ASIO_VARIADIC_TPARAMS(n)> \
  class promise_handler_selector< \
    void(zr_boost_1_81::system::error_code, Arg, BOOST_ASIO_VARIADIC_TARGS(n))> \
    ...
    promise_handler<Signature, Allocator>* h)
{
  typename promise_handler<Signature, Allocator>::executor_type
    ex(h->get_executor());
  zr_boost_1_81::asio::dispatch(ex, promise_function_wrapper<Function>(f));
}
...
struct equality_comparable<
    zr_boost_1_81::asio::detail::promise_executor<T, Blocking> >
{
...
struct execute_member<
    zr_boost_1_81::asio::detail::promise_executor<T, Blocking>, Function>
{
  BOOST_ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  ...
  >
{
  BOOST_ASIO_STATIC_CONSTEXPR(bool, is_valid = true);
  BOOST_ASIO_STATIC_CONSTEXPR(bool, is_noexcept = true);
  typedef zr_boost_1_81::asio::detail::promise_executor<T,
