...
struct partial_promise : partial_promise_base<Allocator>
{
  ...
  {
    return zr_boost_1_81::asio::detail::suspend_always{};
  }
  ...
  auto final_suspend() noexcept
  {
    struct awaitable_t
    {
      ...
      auto await_suspend(zr_boost_1_81::asio::detail::coroutine_handle<>) noexcept
      {
...
struct coroutine_traits<
    zr_boost_1_81::asio::experimental::detail::partial_coro,
    Executor, Completion, Args...>
{
  using promise_type =
    zr_boost_1_81::asio::experimental::detail::partial_promise<
      zr_boost_1_81::asio::associated_allocator_t<Completion>>;
};
...
{
  post(exec, zr_boost_1_81::asio::append(std::move(token), std::move(args)...));
  co_return;
}
...
partial_coro post_coroutine(Context& ctx,
    CompletionToken token, Args&&... args) noexcept
{
  post(ctx, zr_boost_1_81::asio::append(std::move(token), std::move(args)...));
  co_return;
}
...
partial_coro dispatch_coroutine(Executor exec,
    CompletionToken token, Args&&... args) noexcept
{
  dispatch(exec, zr_boost_1_81::asio::append(std::move(token), std::move(args)...));
  co_return;
}
...
partial_coro dispatch_coroutine(Context& ctx,
    CompletionToken token, Args &&... args) noexcept
{
  dispatch(ctx, zr_boost_1_81::asio::append(std::move(token), std::move(args)...));
  co_return;
}
