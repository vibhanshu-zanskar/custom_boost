...
struct parallel_group_completion_handler
{
    ...
    : handler_(std::move(h)),
      executor_(
          zr_boost_1_81::asio::prefer(
            zr_boost_1_81::asio::get_associated_executor(handler_),
  ...
  void operator()()
  {
    this->invoke(zr_boost_1_81::asio::detail::make_index_sequence<sizeof...(Ops)>());
  }

  template <std::size_t... I>
  void invoke(zr_boost_1_81::asio::detail::index_sequence<I...>)
  {
  ...
  void invoke(std::tuple<Args...>&& args)
  {
    this->invoke(std::move(args),
        zr_boost_1_81::asio::detail::index_sequence_for<Args...>());
  }

  template <typename... Args, std::size_t... I>
  void invoke(std::tuple<Args...>&& args,
      zr_boost_1_81::asio::detail::index_sequence<I...>)
  {
...
struct parallel_group_state
{
  ...
  zr_boost_1_81::asio::cancellation_signal cancellation_signals_[sizeof...(Ops)];
...
struct parallel_group_op_handler
{
  ...
  void operator()(Args... args)
  {
    ...
    if (--state_->outstanding_ == 0)
      zr_boost_1_81::asio::dispatch(std::move(state_->handler_));
  }
  ...
  parallel_group_op_handler<I, Condition, Handler, Ops...>
{
  ...
  struct cancel_proxy
  {
    ...
    void operator()(cancellation_type_t type)
    {
      ...
      {
        zr_boost_1_81::asio::cancellation_signal* sig = &signal_;
        zr_boost_1_81::asio::dispatch(executor_,
            [state, sig, type]{ sig->emit(type); });
      }
    }

    std::weak_ptr<parallel_group_state<Condition, Handler, Ops...> > state_;
    zr_boost_1_81::asio::cancellation_signal signal_;
    executor_type executor_;
  };
...
struct parallel_group_op_launcher
{
      ...
      Condition, Handler, Ops...> >& state)
  {
    ...
    ex_type ex = zr_boost_1_81::asio::get_associated_executor(op);
    std::move(op)(
...
void parallel_group_launch(Condition cancellation_condition, Handler handler,
    std::tuple<Ops...>& ops, zr_boost_1_81::asio::detail::index_sequence<I...>)
{
  ...
  std::shared_ptr<state_type> state = std::allocate_shared<state_type>(
      zr_boost_1_81::asio::detail::recycling_allocator<state_type,
        zr_boost_1_81::asio::detail::thread_info_base::parallel_group_tag>(),
...
struct ranged_parallel_group_completion_handler
{
  ...
  {
    this->invoke(
        zr_boost_1_81::asio::detail::make_index_sequence<
...
struct ranged_parallel_group_state
{
    ...
    : cancellations_requested_(size),
      ...
      cancellation_signals_(
          BOOST_ASIO_REBIND_ALLOC(Allocator,
            zr_boost_1_81::asio::cancellation_signal)(allocator)),
      cancellation_condition_(std::move(c)),
  ...
  // The cancellation signals for each operation in the group.
  std::deque<zr_boost_1_81::asio::cancellation_signal,
    BOOST_ASIO_REBIND_ALLOC(Allocator, zr_boost_1_81::asio::cancellation_signal)>
      cancellation_signals_;
