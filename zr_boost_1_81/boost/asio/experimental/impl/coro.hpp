...
template <>
struct coro_error<zr_boost_1_81::system::error_code>
{
  static zr_boost_1_81::system::error_code invalid()
  {
    return zr_boost_1_81::asio::error::fault;
  }

  static zr_boost_1_81::system::error_code cancelled()
  {
    return zr_boost_1_81::asio::error::operation_aborted;
  }

  static zr_boost_1_81::system::error_code interrupted()
  {
    return zr_boost_1_81::asio::error::interrupted;
  }

  static zr_boost_1_81::system::error_code done()
  {
    return zr_boost_1_81::asio::error::broken_pipe;
  }
};
...
struct coro_error<std::exception_ptr>
{
  static std::exception_ptr invalid()
  {
    return std::make_exception_ptr(
        zr_boost_1_81::system::system_error(
          coro_error<zr_boost_1_81::system::error_code>::invalid()));
  }
  ...
  static std::exception_ptr cancelled()
  {
    return std::make_exception_ptr(
        zr_boost_1_81::system::system_error(
          coro_error<zr_boost_1_81::system::error_code>::cancelled()));
  }
  ...
  static std::exception_ptr interrupted()
  {
    return std::make_exception_ptr(
        zr_boost_1_81::system::system_error(
          coro_error<zr_boost_1_81::system::error_code>::interrupted()));
  }
  ...
  static std::exception_ptr done()
  {
    return std::make_exception_ptr(
        zr_boost_1_81::system::system_error(
          coro_error<zr_boost_1_81::system::error_code>::done()));
  }
};
      ...
      -> coroutine_handle<>
    {
      ...
      if constexpr (!coro_promise<Y, R, E, A>::is_noexcept)
      {
            ...
            && hp.cancel->throw_if_cancelled_)
        {
          zr_boost_1_81::asio::detail::throw_error(
              zr_boost_1_81::asio::error::operation_aborted, "coro-cancelled");
        }
      }
      ...
      else
      {
        coro.coro_->awaited_from =
          dispatch_coroutine(
              zr_boost_1_81::asio::prefer(hp.get_executor(),
        ...
        struct cancel_handler
        {
          ...
          {
            zr_boost_1_81::asio::dispatch(e, [ct, st = st]() mutable
            {
              auto & [sig, state] = *st;
              sig.emit(ct);
            });
      ...
      typename coro_traits<Yield, Return, Executor>::return_type>
{
  ...
  using cancellation_slot_type = zr_boost_1_81::asio::cancellation_slot;
  ...
  auto await_transform(this_coro::cancellation_state_t) const
  {
    struct exec_helper
    {
      const zr_boost_1_81::asio::cancellation_state& value;
      ...
      zr_boost_1_81::asio::cancellation_state await_resume() const noexcept
      {
        return value;
      }
    };
...
struct coro<Yield, Return, Executor, Allocator>::initiate_async_resume
{
  ...
  typedef zr_boost_1_81::asio::cancellation_slot cancellation_slot_type;
      ...
      std::true_type /* result is void */)
  {
    ...
    {
      if (!coro)
        return zr_boost_1_81::asio::post(exec,
            zr_boost_1_81::asio::append(std::move(h),
  ...
  {
    const auto exec = zr_boost_1_81::asio::prefer(
    ...
    zr_boost_1_81::asio::dispatch(get_executor(),
