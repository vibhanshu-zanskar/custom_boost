 ...
 * @code
 * awaitable<void> read_write_some(zr_boost_1_81::asio::ip::tcp::socket & sock,
 *     zr_boost_1_81::asio::mutable_buffer read_buf, zr_boost_1_81::asio::const_buffer to_write)
 * {
 *   auto p = zr_boost_1_81::asio::async_read(read_buf, zr_boost_1_81::asio::use_awaitable);
 *   co_await zr_boost_1_81::asio::async_write_some(to_write, zr_boost_1_81::asio::deferred);
 *   co_await p;
 * }
 * @endcode
 */
...
#endif // defined(GENERATING_DOCUMENTATION)
{
  ...
  void cancel(cancellation_type level = cancellation_type::all)
  {
    if (impl_ && !impl_->done)
    {
      zr_boost_1_81::asio::dispatch(impl_->executor,
...
private:
  ...
  struct initiate_async_wait
  {
    ...
    void operator()(WaitHandler&& handler) const
    {
      ...
      if (self_->done)
      {
        auto exec = zr_boost_1_81::asio::get_associated_executor(
            handler, self_->get_executor());

        zr_boost_1_81::asio::post(exec,
            [self = std::move(self_),
