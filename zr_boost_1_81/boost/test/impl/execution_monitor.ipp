...
extract( zr_boost_1_81::exception const* ex )
{
    if( !ex )
        return 0;

    typename ErrorInfo::value_type const * val = zr_boost_1_81::get_error_info<ErrorInfo>( *ex );
...
report_error( execution_exception::error_code ec, zr_boost_1_81::exception const* be, char const* format, va_list* args )
{
...
report_error( execution_exception::error_code ec, zr_boost_1_81::exception const* be, char const* format, ... )
{
    va_list args;
...
{
    return zr_boost_1_81::core::demangle(typeid(t).name());
}
#endif
...
// ************************************************************************** //
// **************    zr_boost_1_81::detail::system_signal_exception    ************** //
// ************************************************************************** //
...
// ************************************************************************** //
// **************         zr_boost_1_81::detail::signal_action         ************** //
// ************************************************************************** //
...
// ************************************************************************** //
// **************        zr_boost_1_81::detail::signal_handler         ************** //
// ************************************************************************** //
...
int
execution_monitor::catch_signals( zr_boost_1_81::function<int ()> const& F )
{
...
int
execution_monitor::execute( zr_boost_1_81::function<int ()> const& F )
{
    ...
    // zr_boost_1_81::exception (before std::exception, with extended diagnostic)
    catch( zr_boost_1_81::exception const& ex )
      { detail::report_error( execution_exception::cpp_exception_error,
                              &ex,
                              "%s", zr_boost_1_81::diagnostic_information(ex).c_str() ); }
...
#define CATCH_AND_REPORT_STD_EXCEPTION( ex_name )                           \
       ...
       { detail::report_error( execution_exception::cpp_exception_error,    \
                          current_exception_cast<zr_boost_1_81::exception const>(), \
                          #ex_name ": %s", ex.what() ); }                   \
/**/
#else
#define CATCH_AND_REPORT_STD_EXCEPTION( ex_name )                           \
...
struct forward {
    explicit    forward( zr_boost_1_81::function<void ()> const& F ) : m_F( F ) {}

    int         operator()() { m_F(); return 0; }

    zr_boost_1_81::function<void ()> const& m_F;
};

} // namespace detail
void
execution_monitor::vexecute( zr_boost_1_81::function<void ()> const& F )
{
...
{
    zr_boost_1_81::ignore_unused(mask);
