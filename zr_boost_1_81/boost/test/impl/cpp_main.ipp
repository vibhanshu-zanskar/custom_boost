...
prg_exec_monitor_main( int (*cpp_main)( int argc, char* argv[] ), int argc, char* argv[] )
{
    ...
    BOOST_TEST_I_TRY {
        zr_boost_1_81::unit_test::const_string p( std::getenv( "BOOST_TEST_CATCH_SYSTEM_ERRORS" ) );
        ...
        if( result == 0 )
            result = ::zr_boost_1_81::exit_success;
        else if( result != ::zr_boost_1_81::exit_success ) {
            std::cout << "\n**** error return code: " << result << std::endl;
            result = ::zr_boost_1_81::exit_failure;
        }
    }
    BOOST_TEST_I_CATCH( ::zr_boost_1_81::execution_exception, exex ) {
        std::cout << "\n**** exception(" << exex.code() << "): " << exex.what() << std::endl;
        result = ::zr_boost_1_81::exit_exception_failure;
    }
    BOOST_TEST_I_CATCH( ::zr_boost_1_81::system_error, ex ) {
    ...
    if( result != ::zr_boost_1_81::exit_success ) {
        ...
        ::zr_boost_1_81::unit_test::const_string p( std::getenv( "BOOST_PRG_MON_CONFIRM" ) );
        if( p != "no" ) {
...
{
    return ::zr_boost_1_81::prg_exec_monitor_main( &cpp_main, argc, argv );
}
