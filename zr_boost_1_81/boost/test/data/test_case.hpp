...
boost_bind_rvalue_holder_helper_impl(T&& value, zr_boost_1_81::false_type /* is copy constructible */) {
    return parameter_holder<typename std::remove_reference<T>::type>(std::forward<T>(value));
}

template <class T>
T&& boost_bind_rvalue_holder_helper_impl(T&& value, zr_boost_1_81::true_type /* is copy constructible */) {
...
auto boost_bind_rvalue_holder_helper(T&& value)
  -> decltype(boost_bind_rvalue_holder_helper_impl(
                  std::forward<T>(value),
                  typename zr_boost_1_81::is_copy_constructible<typename std::remove_reference<T>::type>::type()))
{
    // need to use zr_boost_1_81::is_copy_constructible because std::is_copy_constructible is broken on MSVC12
...
public:
    ...
    // there is also a limit on zr_boost_1_81::bind
#define TC_MAKE(z,arity,_)                                                              \
    ...
    {                                                                                   \
        m_test_cases.push_back( new test_case( genTestCaseName(), m_tc_file, m_tc_line, \
           zr_boost_1_81::bind( &TestCase::template test_method<BOOST_PP_ENUM_PARAMS(arity,Arg)>,\
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
template<typename TestCase,typename DataSet>
zr_boost_1_81::shared_ptr<test_unit_generator> //test_case_gen<TestCase,DataSet>
make_test_case_gen( const_string tc_name, const_string tc_file, std::size_t tc_line, DataSet&& ds )
{
    return zr_boost_1_81::shared_ptr<test_unit_generator>(new test_case_gen<TestCase,DataSet>( tc_name, tc_file, tc_line, std::forward<DataSet>(ds) ));
}
#else
template<typename TestCase,typename DataSet>
...
#define BOOST_DATA_TEST_CONTEXT(r, _, param)  << BOOST_STRINGIZE(param) << " = " << zr_boost_1_81::test_tools::tt_detail::print_helper(param) << "; "
...
BOOST_AUTO_TEST_SUITE( test_name,                                       \
                       *zr_boost_1_81::unit_test::decorator::stack_decorator()) \
                                                                        \
BOOST_AUTO_TU_REGISTRAR( BOOST_PP_CAT(test_name, case) )(               \
    zr_boost_1_81::unit_test::data::ds_detail::make_test_case_gen<              \
                                      BOOST_PP_CAT(test_name, case)>(   \
          BOOST_STRINGIZE( test_name ),                                 \
          __FILE__, __LINE__,                                           \
          zr_boost_1_81::unit_test::data::ds_detail::seed{} ->* dataset ),      \
