...
private:
    struct nil_t {};
    template<typename U> static auto  test( U* ) -> decltype(zr_boost_1_81::declval<U>().size());
...
private:
    struct nil_t {};
    template<typename U>  static auto  test( U* ) -> decltype(std::begin(zr_boost_1_81::declval<U&>())); // does not work with zr_boost_1_81::begin
    template<typename>    static nil_t test( ... );
public:
    static bool const value = !std::is_same< decltype(test<T>( nullptr )), nil_t>::value;
};
...
private:
    struct nil_t {};
    template<typename U>  static auto  test( U* ) -> decltype(std::end(zr_boost_1_81::declval<U&>())); // does not work with zr_boost_1_81::end
    template<typename>    static nil_t test( ... );
public:
    static bool const value = !std::is_same< decltype(test<T>( nullptr )), nil_t>::value;
};
...
#else
    typedef decltype(zr_boost_1_81::declval<
...
struct bt_iterator_traits< T [N], true > {
    typedef typename zr_boost_1_81::add_const<T>::type T_const;
    typedef typename zr_boost_1_81::add_pointer<T_const>::type const_iterator;
