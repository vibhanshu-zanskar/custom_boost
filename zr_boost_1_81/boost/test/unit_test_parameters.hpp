...
public:
    ...
    void            setup( const const_string& stream_name,
                           zr_boost_1_81::function<void ()> const &cleaner_callback = zr_boost_1_81::function<void ()>() )
    {
        ...
        if( stream_name == "stderr" ) {
            m_stream = &std::cerr;
            if(cleaner_callback) {
                m_cleaner = zr_boost_1_81::make_shared<callback_cleaner>(cleaner_callback);
            }
            else {
                m_cleaner.reset();
            }
...
private:
    struct callback_cleaner {
        callback_cleaner(zr_boost_1_81::function<void ()> cleaner_callback)
        ...
        zr_boost_1_81::function<void ()> m_cleaner_callback;
        std::ofstream m_file;
    };

    // Data members
    zr_boost_1_81::shared_ptr<callback_cleaner>   m_cleaner;
