...
#   define DEDUCE_RESULT_TYPE( oper )                                   \
    decltype(zr_boost_1_81::declval<Lhs>() oper zr_boost_1_81::declval<Rhs>() ) optype; \
    typedef typename zr_boost_1_81::remove_reference<optype>::type              \
/**/
#else
...
#define ADD_OP_SUPPORT( oper, name, _, _i )                     \
    template<typename T>                                        \
    binary_expr<ExprType,typename zr_boost_1_81::decay<T const>::type,  \
        op::name<ValType,typename zr_boost_1_81::decay<T const>::type> >\
    operator oper( T const& rhs ) const                         \
    {                                                           \
        typedef typename zr_boost_1_81::decay<T const>::type Rhs;       \
