...
#define BOOST_CHECK(exp)       \
  ( (exp)                      \
      ? static_cast<void>(0)   \
      : zr_boost_1_81::minimal_test::report_error(#exp,__FILE__,__LINE__, BOOST_CURRENT_FUNCTION) )

#define BOOST_REQUIRE(exp)     \
  ( (exp)                      \
      ? static_cast<void>(0)   \
      : zr_boost_1_81::minimal_test::report_critical_error(#exp,__FILE__,__LINE__,BOOST_CURRENT_FUNCTION))

#define BOOST_ERROR( msg_ )    \
        zr_boost_1_81::minimal_test::report_error( (msg_),__FILE__,__LINE__, BOOST_CURRENT_FUNCTION, true )
#define BOOST_FAIL( msg_ )     \
        zr_boost_1_81::minimal_test::report_critical_error( (msg_),__FILE__,__LINE__, BOOST_CURRENT_FUNCTION, true )
...
report_critical_error( const char* msg, const char* file, int line, const_string func_name, bool is_msg = false )
{
    ...
    throw zr_boost_1_81::execution_aborted();
}
...
{
    using namespace zr_boost_1_81::minimal_test;

    try {
        ::zr_boost_1_81::execution_monitor ex_mon;
        int run_result = ex_mon.execute( caller( argc, argv ) );

        BOOST_CHECK( run_result == 0 || run_result == zr_boost_1_81::exit_success );
    }
    catch( zr_boost_1_81::execution_exception const& exex ) {
        if( exex.code() != zr_boost_1_81::execution_exception::no_error )
            BOOST_ERROR( (std::string( "exception \"" ) + exex.what() + "\" caught").c_str() );
        std::cerr << "\n**** Testing aborted.";
    }

    if( zr_boost_1_81::minimal_test::errors_counter() != 0 ) {
        std::cerr << "\n**** " << errors_counter()
                  << " error" << (errors_counter() > 1 ? "s" : "" ) << " detected\n";

        return zr_boost_1_81::exit_test_failure;
    }
