...
namespace impl_fixture {
    ...
    struct has_setup {
    private:
        template<typename U> static auto Test(U*) -> typename fixture_detect<decltype(zr_boost_1_81::declval<U>().setup())>::type;
        template<typename U> static int Test(...);
    public:
        static const bool value = sizeof(Test<T>(0)) == sizeof(char);
    };
    ...
    struct has_teardown {
    private:
        template<typename U> static auto Test(U*) -> typename fixture_detect<decltype(zr_boost_1_81::declval<U>().teardown())>::type;
        template<typename U> static int Test(...);
    public:
        static const bool value = sizeof(Test<T>(0)) == sizeof(char);
    };
...
public:
    // Constructor
    function_based_fixture( zr_boost_1_81::function<void ()> const& setup_, zr_boost_1_81::function<void ()> const& teardown_ )
    : m_setup( setup_ )
...
private:
    ...
    // Data members
    zr_boost_1_81::function<void ()>    m_setup;
    zr_boost_1_81::function<void ()>    m_teardown;
};
