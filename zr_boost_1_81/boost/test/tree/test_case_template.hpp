...
public:
    void    operator()()    { TestCaseTemplate::run( (zr_boost_1_81::type<TestType>*)0 ); }
};
...
struct generate_test_case_4_type {
    ...
    void        operator()( mpl::identity<TestType> )
    {
        ...
        full_name += '<';
#if !defined(BOOST_NO_TYPEID) && !defined(BOOST_NO_RTTI)
        full_name += zr_boost_1_81::core::demangle(typeid(TestType).name()); // same as execution_monitor.ipp
#else
        ...
        typedef typename zr_boost_1_81::remove_reference<TestType>::type TestTypewoRef;
        if( zr_boost_1_81::is_const<TestTypewoRef>::value )
            full_name += "_const";
        if( zr_boost_1_81::is_volatile<TestTypewoRef>::value )
            full_name += "_volatile";
        if( zr_boost_1_81::is_rvalue_reference<TestType>::value )
            full_name += "_refref";
        else if( zr_boost_1_81::is_lvalue_reference<TestType>::value )
            full_name += "_ref";
...
class template_test_case_gen<
    TestCaseTemplate,
    C<parameter_pack...>,
    typename std::enable_if<!zr_boost_1_81::mpl::is_sequence<C<parameter_pack...>>::value>::type >
