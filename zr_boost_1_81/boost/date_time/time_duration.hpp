...
namespace date_time {
  ...
  */
  ...
  class BOOST_SYMBOL_VISIBLE time_duration : private
      zr_boost_1_81::less_than_comparable<T
    , zr_boost_1_81::equality_comparable<T
    > >
   ...
   */
  template<class base_duration, zr_boost_1_81::int64_t frac_of_second>
  ...
  private:
    ...
    BOOST_STATIC_CONSTANT(zr_boost_1_81::int64_t, adjustment_ratio = (traits_type::ticks_per_second >= frac_of_second ? traits_type::ticks_per_second / frac_of_second : frac_of_second / traits_type::ticks_per_second));

  public:
    // The argument (ss) must be an integral type
    template <typename T>
    BOOST_CXX14_CONSTEXPR explicit subsecond_duration(T const& ss,
                                                      typename zr_boost_1_81::enable_if<zr_boost_1_81::is_integral<T>,
