...
namespace local_time {
   ...
   * required to be in the form of a zr_boost_1_81::shared_ptr<time_zone_base>.
   */
  ...
  class BOOST_SYMBOL_VISIBLE local_date_time_base :  public date_time::base_time<utc_time_,
                                                            zr_boost_1_81::posix_time::posix_time_system> {
  public:
    ...
    local_date_time_base(utc_time_type t,
                         zr_boost_1_81::shared_ptr<tz_type> tz) :
    ...
    local_date_time_base(date_type d,
      ...
      zone_(tz)
    {
      if(tz != zr_boost_1_81::shared_ptr<tz_type>() && tz->has_dst()){
        ...
        if(result == invalid_time_label){
          ...
          ss << "time given: " << d << ' ' << td;
          zr_boost_1_81::throw_exception(time_label_invalid(ss.str()));
        }
          ...
          zr_boost_1_81::throw_exception(dst_not_valid(ss.str()));
        }
      ...
      zone_(tz)
    {
      ...
      if(result == ambiguous) {
        if(calc_option == EXCEPTION_ON_ERROR){
          std::ostringstream ss;
          ss << "time given: " << d << ' ' << td;
          zr_boost_1_81::throw_exception(ambiguous_result(ss.str()));
        }
        else{ // NADT on error
    ...
    explicit local_date_time_base(const zr_boost_1_81::date_time::special_values sv,
                                  zr_boost_1_81::shared_ptr<tz_type> tz = zr_boost_1_81::shared_ptr<tz_type>()) :
      date_time::base_time<utc_time_type, time_system_type>(utc_time_type(sv)),
      zone_(tz)
    {}

    //! returns time zone associated with calling instance
    zr_boost_1_81::shared_ptr<tz_type> zone() const
    {
      return zone_;
    }
    ...
    bool is_dst() const
    {
      if(zone_ != zr_boost_1_81::shared_ptr<tz_type>() && zone_->has_dst() && !this->is_special()) {
    ...
    {
      if(zone_ != zr_boost_1_81::shared_ptr<tz_type>()){
    ...
    local_date_time_base local_time_in(zr_boost_1_81::shared_ptr<tz_type> new_tz,
    ...
    /*bool operator==(const date_time::base_time<zr_boost_1_81::posix_time::ptime,zr_boost_1_81::posix_time::posix_time_system>& rhs) const
    { // fails due to rhs.time_ being protected
      return date_time::base_time<zr_boost_1_81::posix_time::ptime,zr_boost_1_81::posix_time::posix_time_system>::operator==(rhs);
  ...
  private:
    zr_boost_1_81::shared_ptr<tz_type> zone_;
    //bool is_dst_;

    /*! Adjust the passed in time to UTC?
     */
    utc_time_type construction_adjustment(utc_time_type t,
                                          zr_boost_1_81::shared_ptr<tz_type> z,
                                          bool dst_flag)
    {
      if(z != zr_boost_1_81::shared_ptr<tz_type>()) {
