...
auto tie_as_flat_tuple(T& lvalue) noexcept {
    ...
    return zr_boost_1_81::pfr::detail::make_flat_tuple_of_references(lvalue, getter, size_t_<0>{}, size_t_<tuple_type::size_v>{});
}
...
auto tie_as_tuple(T& val) noexcept {
    ...
    static_assert(
        zr_boost_1_81::pfr::detail::is_flat_refelectable<T>( detail::make_index_sequence<zr_boost_1_81::pfr::detail::fields_count<T>()>{} ),
        "====================> Boost.PFR: Not possible in C++14 to represent that type without loosing information. Change type definition or enable C++17"
    );
    return zr_boost_1_81::pfr::detail::tie_as_flat_tuple(val);
}
...
struct next_step {
    ...
    operator Field() const {
         zr_boost_1_81::pfr::detail::for_each_field_in_depth(
             t,
             std::forward<F>(f),
             IndexSeq{},
             identity<Fields>{}...,
...
void for_each_field_in_depth(T& lvalue, F&& f, std::index_sequence<>, identity<Fields>...) {
    ...
    std::forward<F>(f)(
        zr_boost_1_81::pfr::detail::make_flat_tuple_of_references(lvalue, getter, size_t_<0>{}, size_t_<sizeof...(Fields)>{})
    );
}
...
void for_each_field_dispatcher_1(T& t, F&& f, std::index_sequence<I...>, std::true_type /*is_flat_refelectable*/) {
    std::forward<F>(f)(
        zr_boost_1_81::pfr::detail::tie_as_flat_tuple(t)
    );
}
