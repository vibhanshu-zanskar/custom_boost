...
namespace zr_boost_1_81 { namespace pfr {
...
///     my_struct s {10, 11};
///     assert(zr_boost_1_81::pfr::get<0>(s) == 10);
///     zr_boost_1_81::pfr::get<1>(s) = 0;
/// \endcode
...
constexpr auto get(T&, std::enable_if_t<!std::is_assignable<T, T>::value>* = nullptr) noexcept {
    static_assert(sizeof(T) && false, "====================> Boost.PFR: Calling zr_boost_1_81::pfr::get on non const non assignable type is allowed only in C++17");
    return 0;
}
#endif
...
/// \b Example:
/// \code
///     std::vector< zr_boost_1_81::pfr::tuple_element<0, my_structure>::type > v;
/// \endcode
template <std::size_t I, class T>
using tuple_element = detail::sequence_tuple::tuple_element<I, decltype( ::zr_boost_1_81::pfr::detail::tie_as_tuple(std::declval<T&>()) ) >;
...
/// \b Example:
/// \code
///     std::vector< zr_boost_1_81::pfr::tuple_element_t<0, my_structure> > v;
/// \endcode
template <std::size_t I, class T>
...
constexpr auto structure_tie(T&, std::enable_if_t<!std::is_assignable<T, T>::value>* = nullptr) noexcept {
    static_assert(sizeof(T) && false, "====================> Boost.PFR: Calling zr_boost_1_81::pfr::structure_tie on non const non assignable type is allowed only in C++17");
    return 0;
}
#endif
...
constexpr auto structure_tie(T&&, std::enable_if_t< std::is_rvalue_reference<T&&>::value>* = 0) noexcept {
    static_assert(sizeof(T) && false, "====================> Boost.PFR: Calling zr_boost_1_81::pfr::structure_tie on rvalue references is forbidden");
    return 0;
}
...
void for_each_field(T&& value, F&& func) {
    constexpr std::size_t fields_count_val = zr_boost_1_81::pfr::detail::fields_count<std::remove_reference_t<T>>();

    ::zr_boost_1_81::pfr::detail::for_each_field_dispatcher(
        value,
        [f = std::forward<F>(func)](auto&& t) mutable {
            // MSVC related workaround. Its lambdas do not capture constexprs.
            constexpr std::size_t fields_count_val_in_lambda
                = zr_boost_1_81::pfr::detail::fields_count<std::remove_reference_t<T>>();

            ::zr_boost_1_81::pfr::detail::for_each_field_impl(
                t,
                std::forward<F>(f),
