...
namespace zr_boost_1_81 { namespace parallel {
...
void PBGL_DISTRIB_PMAP::prune_ghost_cells() const
{
  ...
  while (data->ghost_cells->size() > data->max_ghost_cells) {
    ...
    if (data->model & cm_flush) {
      ...
      zr_boost_1_81::parallel::detail::untracked_pair<key_type, value_type> const& victim
...
handle_put(int /*source*/, int /*tag*/, 
           const zr_boost_1_81::parallel::detail::untracked_pair<key_type, value_type>& req, trigger_receive_context)
{
                ...
                trigger_receive_context)
{
  ...
  typedef zr_boost_1_81::parallel::detail::untracked_pair<key_type, value_type> key_value;
...
handle_multiget_reply
  (int source, int tag, 
   const std::vector<zr_boost_1_81::parallel::detail::untracked_pair<key_type, value_type> >& msg,
   trigger_receive_context)
{
  shared_ptr<data_t> data(data_ptr);
  BOOST_ASSERT(data);
...
PBGL_DISTRIB_PMAP::set_max_ghost_cells(std::size_t max_ghost_cells)
{
  if ((data->model & cm_backward) && max_ghost_cells > 0)
      zr_boost_1_81::throw_exception(std::runtime_error("distributed_property_map::set_max_ghost_cells: "
...
} } // end namespace zr_boost_1_81::parallel
