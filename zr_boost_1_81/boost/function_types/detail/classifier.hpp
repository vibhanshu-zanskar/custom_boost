...
template<bits_t Flags, bits_t CCID, std::size_t Arity> struct encode_charr
{
  typedef typename char_array<
    ::zr_boost_1_81::function_types::detail::encode_charr_impl<Flags,CCID,Arity>::value 
  >::type type;
};
...
{
  static typename zr_boost_1_81::add_reference<T>::type tester;

  BOOST_STATIC_CONSTANT(bits_t,value = (bits_t)sizeof(
    zr_boost_1_81::function_types::detail::classifier_impl(& tester) 
  )-1);
};
...
template<typename T> struct classifier
{
  typedef detail::constant<
    ::zr_boost_1_81::function_types::detail::decode_bits<
      ::zr_boost_1_81::function_types::detail::classifier_bits<T>::value
    >::tag_bits > 
  bits;
