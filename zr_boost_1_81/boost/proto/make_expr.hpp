...
#define BOOST_PROTO_AS_CHILD_TYPE(Z, N, DATA)                                                   \
    typename zr_boost_1_81::proto::detail::protoify<                                                    \
...
#define BOOST_PROTO_AS_CHILD(Z, N, DATA)                                                        \
    zr_boost_1_81::proto::detail::protoify<                                                             \
        BOOST_PP_CAT(BOOST_PP_TUPLE_ELEM(3, 0, DATA), N)                                        \
      , BOOST_PP_TUPLE_ELEM(3, 2, DATA)                                                         \
    ...
    namespace detail
    {
        ...
        template<typename T, typename Domain>
        struct protoify<zr_boost_1_81::reference_wrapper<T>, Domain>
          : Domain::template as_child<T>
        {};

        template<typename T, typename Domain>
        struct protoify<zr_boost_1_81::reference_wrapper<T> const, Domain>
          : Domain::template as_child<T>
        {};
    ...
    namespace result_of
    {
        ...
        >
        {
            ...
            /// typedef for <tt>zr_boost_1_81::result_of\<Domain(expr\<tag::terminal,
            /// term\<A0\> \>)\>::type</tt>.
            ///
            /// Otherwise, \c type is a typedef for <tt>zr_boost_1_81::result_of\<Domain(expr\<Tag,
            ...
            /// child type is \c A.
            /// \li If \c A is <tt>B &</tt> or <tt>cv zr_boost_1_81::reference_wrapper\<B\></tt>,
            /// and <tt>is_expr\<B\>::value</tt> is \c true, then the
            /// child type is <tt>B &</tt>.
            /// \li If <tt>is_expr\<A\>::value</tt> is \c false, then the
            /// child type is <tt>zr_boost_1_81::result_of\<Domain(expr\<tag::terminal, term\<A\> \>
            /// )\>::type</tt>.
            /// \li If \c A is <tt>B &</tt> or <tt>cv zr_boost_1_81::reference_wrapper\<B\></tt>,
            /// and <tt>is_expr\<B\>::value</tt> is \c false, then the
            /// child type is <tt>zr_boost_1_81::result_of\<Domain(expr\<tag::terminal, term\<B &\> \>
            /// )\>::type</tt>.
            typedef
                typename detail::make_expr_<
                    Tag
    ...
    /// \li If \c x is a <tt>zr_boost_1_81::reference_wrapper\<\></tt>,
