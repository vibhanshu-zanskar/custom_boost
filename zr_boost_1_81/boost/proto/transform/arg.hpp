...
namespace zr_boost_1_81 { namespace proto
{
    ...
    /// assert( zr_boost_1_81::addressof(i) == zr_boost_1_81::addressof(j) );
    /// \endcode
    ...
    /// in a \c zr_boost_1_81::reference_wrapper\<\>.
    ///
    /// Example:
    ///
    /// \code
    /// proto::terminal<int>::type i = {42};
    /// zr_boost_1_81::reference_wrapper<proto::terminal<int>::type> j
    ///     = proto::when<_, proto::_byref(_)>()(i);
    /// assert( zr_boost_1_81::addressof(i) == zr_boost_1_81::addressof(j.get()) );
    /// \endcode
    struct _byref : callable
    {
        ...
        {
            typedef zr_boost_1_81::reference_wrapper<T const> const type;
        };

        template<typename This, typename T>
        struct result<This(T &)>
        {
            typedef zr_boost_1_81::reference_wrapper<T> const type;
        };

        /// Wrap the parameter \c t in a \c zr_boost_1_81::reference_wrapper\<\>
        /// \param t The object to wrap
        /// \return <tt>zr_boost_1_81::ref(t)</tt>
        /// \throw nothrow
        template<typename T>
        zr_boost_1_81::reference_wrapper<T> const operator ()(T &t) const
        {
            return zr_boost_1_81::reference_wrapper<T>(t);
        }

        /// \overload
        ///
        template<typename T>
        zr_boost_1_81::reference_wrapper<T const> const operator ()(T const &t) const
        {
            return zr_boost_1_81::reference_wrapper<T const>(t);
        }
    };

    /// \brief A unary CallableTransform that strips references
    /// and \c zr_boost_1_81::reference_wrapper\<\> from its argument.
    ///
    /// Example:
    ///
    /// \code
    /// proto::terminal<int>::type i = {42};
    /// int j = 67;
    /// int k = proto::when<_, proto::_byval(proto::_state)>()(i, zr_boost_1_81::ref(j));
    /// assert( 67 == k );
    /// \endcode
    struct _byval : callable
    {
        ...
        template<typename This, typename T>
        struct result<This(zr_boost_1_81::reference_wrapper<T>)>
          : result<This(T)>
        {};
        ...
        ///
        template<typename T>
        T operator ()(zr_boost_1_81::reference_wrapper<T> const &t) const
        {
            return t;
        }
    };
