...
{ // Error checks:
  ...
  {
     if((sigma <= 0) || (!(zr_boost_1_81::math::isfinite)(sigma)))
     {
  ...
  {
     if((x < 0) || (zr_boost_1_81::math::isnan)(x))
     {
        *presult = policies::raise_domain_error<RealType>(
           function,
...
public:
   ...
   {
      RealType err;
      detail::verify_sigma("zr_boost_1_81::math::rayleigh_distribution<%1%>::rayleigh_distribution", l_sigma, &err, Policy());
...
rayleigh_distribution(RealType)->rayleigh_distribution<typename zr_boost_1_81::math::tools::promote_args<RealType>::type>;
#endif
...
inline RealType pdf(const rayleigh_distribution<RealType, Policy>& dist, const RealType& x)
{
   ...
   RealType result = 0;
   static const char* function = "zr_boost_1_81::math::pdf(const rayleigh_distribution<%1%>&, %1%)";
...
inline RealType logpdf(const rayleigh_distribution<RealType, Policy>& dist, const RealType& x)
{
   ...
   static const char* function = "zr_boost_1_81::math::logpdf(const rayleigh_distribution<%1%>&, %1%)";
...
inline RealType cdf(const rayleigh_distribution<RealType, Policy>& dist, const RealType& x)
{
   ...
   RealType result = 0;
   RealType sigma = dist.sigma();
   static const char* function = "zr_boost_1_81::math::cdf(const rayleigh_distribution<%1%>&, %1%)";
   if(false == detail::verify_sigma(function, sigma, &result, Policy()))
   {
      return result;
   }
   ...
   result = -zr_boost_1_81::math::expm1(-x * x / ( 2 * sigma * sigma), Policy());
   return result;
} // cdf
...
inline RealType quantile(const rayleigh_distribution<RealType, Policy>& dist, const RealType& p)
{
   ...
   RealType result = 0;
   RealType sigma = dist.sigma();
   static const char* function = "zr_boost_1_81::math::quantile(const rayleigh_distribution<%1%>&, %1%)";
   if(false == detail::verify_sigma(function, sigma, &result, Policy()))
      return result;
   ...
   result = sqrt(-2 * sigma * sigma * zr_boost_1_81::math::log1p(-p, Policy()));
   return result;
} // quantile
...
inline RealType mean(const rayleigh_distribution<RealType, Policy>& dist)
{
   RealType result = 0;
   RealType sigma = dist.sigma();
   static const char* function = "zr_boost_1_81::math::mean(const rayleigh_distribution<%1%>&, %1%)";
   if(false == detail::verify_sigma(function, sigma, &result, Policy()))
   {
      return result;
   }
   using zr_boost_1_81::math::constants::root_half_pi;
...
inline RealType variance(const rayleigh_distribution<RealType, Policy>& dist)
{
   RealType result = 0;
   RealType sigma = dist.sigma();
   static const char* function = "zr_boost_1_81::math::variance(const rayleigh_distribution<%1%>&, %1%)";
   if(false == detail::verify_sigma(function, sigma, &result, Policy()))
   {
      return result;
   }
   using zr_boost_1_81::math::constants::four_minus_pi;
...
{
   using zr_boost_1_81::math::constants::root_ln_four;
