...
{
   if((prob < 0) || (prob > 1) || !(zr_boost_1_81::math::isfinite)(prob))
   {
...
{ //  df > 0 but NOT +infinity allowed.
   if((df <= 0) || !(zr_boost_1_81::math::isfinite)(df))
   {
      *result = policies::raise_domain_error<RealType>(
         function,
...
{  // df > 0 or +infinity are allowed.
   if( (df <= 0) || (zr_boost_1_81::math::isnan)(df) )
...
inline bool check_scale(
      ...
      const Policy& pol)
{
   if((scale <= 0) || !(zr_boost_1_81::math::isfinite)(scale))
...
inline bool check_positive_x(
      ...
      const Policy& pol)
{
   if(!(zr_boost_1_81::math::isfinite)(x) || (x < 0))
   {
      *result = policies::raise_domain_error<RealType>(
         function,
...
inline bool check_non_centrality(
      ...
      const Policy& pol)
{
   if((ncp < 0) || !(zr_boost_1_81::math::isfinite)(ncp))
   { // Assume scale == 0 is NOT valid for any distribution.
      *result = policies::raise_domain_error<RealType>(
         function,
