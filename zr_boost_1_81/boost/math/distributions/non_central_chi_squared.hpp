   ...
   namespace math
   {
      ...
      namespace detail{
         ...
         T non_central_chi_square_q(T x, T f, T theta, const Policy& pol, T init_sum = 0)
         {
            ...
            T errtol = zr_boost_1_81::math::policies::get_epsilon<T, Policy>();
            T sum = init_sum;
            //
            ...
            T poisf = zr_boost_1_81::math::gamma_p_derivative(static_cast<T>(1 + k), lambda, pol);
            T poisb = poisf * k / lambda;
            // Initial forwards central chi squared term:
            T gamf = zr_boost_1_81::math::gamma_q(del + k, y, pol);
            // Forwards and backwards recursion terms on the central chi squared:
            T xtermf = zr_boost_1_81::math::gamma_p_derivative(del + 1 + k, y, pol);
         ...
         T non_central_chi_square_p_ding(T x, T f, T theta, const Policy& pol, T init_sum = 0)
         {
            ...
            if(x == 0)
               return 0;
            T tk = zr_boost_1_81::math::gamma_p_derivative(f/2 + 1, x/2, pol);
            T lambda = theta / 2;
            T vk = exp(-lambda);
            T uk = vk;
         ...
         T non_central_chi_square_p(T y, T n, T lambda, const Policy& pol, T init_sum)
         {
            ...
            T gamkf = zr_boost_1_81::math::gamma_p(a, x, pol);
            ...
            T xtermf = zr_boost_1_81::math::gamma_p_derivative(a, x, pol);
         ...
         inline RealType non_central_chi_squared_cdf(RealType x, RealType k, RealType l, bool invert, const Policy&)
         {
            ...
            value_type result;
            if(l == 0)
              return invert == false ? cdf(zr_boost_1_81::math::chi_squared_distribution<RealType, Policy>(k), x) : cdf(complement(zr_boost_1_81::math::chi_squared_distribution<RealType, Policy>(k), x));
            else if(x > k + l)
            {
            ...
            return policies::checked_narrowing_cast<RealType, forwarding_policy>(
               result,
               "zr_boost_1_81::math::non_central_chi_squared_cdf<%1%>(%1%, %1%, %1%)");
         }
         ...
         RealType nccs_quantile(const non_central_chi_squared_distribution<RealType, Policy>& dist, const RealType& p, bool comp)
         {
            ...
            if(guess < 0.005)
            {
               ...
               guess = pow(pow(value_type(2), (k / 2 - 1)) * exp(l / 2) * pp * k * zr_boost_1_81::math::tgamma(k / 2, forwarding_policy()), (2 / k));
               if(guess == 0)
         ...
         if(l == 0)
            return pdf(zr_boost_1_81::math::chi_squared_distribution<RealType, forwarding_policy>(dist.degrees_of_freedom()), x);
         ...
         else
         {
            ...
            else
            {
               r = exp(r);
               r = 0.5f * r
                  * zr_boost_1_81::math::cyl_bessel_i(k/2 - 1, sqrt(l * x), forwarding_policy());
            }
         }
         return policies::checked_narrowing_cast<RealType, forwarding_policy>(
               r,
      ...
      public:
         ...
         {
            const char* function = "zr_boost_1_81::math::non_central_chi_squared_distribution<%1%>::non_central_chi_squared_distribution(%1%,%1%)";
            RealType r;
            detail::check_df(
               function,
               df, &r, Policy());
      ...
      non_central_chi_squared_distribution(RealType,RealType)->non_central_chi_squared_distribution<typename zr_boost_1_81::math::tools::promote_args<RealType>::type>;
      #endif
      ...
      { // Mean of poisson distribution = lambda.
         const char* function = "zr_boost_1_81::math::non_central_chi_squared_distribution<%1%>::mean()";
      ...
      { // variance.
         const char* function = "zr_boost_1_81::math::non_central_chi_squared_distribution<%1%>::variance()";
         RealType k = dist.degrees_of_freedom();
         RealType l = dist.non_centrality();
         RealType r;
         if(!detail::check_df(
      ...
      { // skewness = sqrt(l).
         const char* function = "zr_boost_1_81::math::non_central_chi_squared_distribution<%1%>::skewness()";
         RealType k = dist.degrees_of_freedom();
         RealType l = dist.non_centrality();
         RealType r;
         if(!detail::check_df(
      ...
      {
         const char* function = "zr_boost_1_81::math::non_central_chi_squared_distribution<%1%>::kurtosis_excess()";
         RealType k = dist.degrees_of_freedom();
         RealType l = dist.non_centrality();
         RealType r;
         if(!detail::check_df(
      ...
      {
         const char* function = "zr_boost_1_81::math::non_central_chi_squared_distribution<%1%>::cdf(%1%)";
         RealType k = dist.degrees_of_freedom();
         RealType l = dist.non_centrality();
         RealType r;
         if(!detail::check_df(
