...
RealType cdf_imp(const cauchy_distribution<RealType, Policy>& dist, const RealType& x, bool complement)
{
   ...
   static const char* function = "zr_boost_1_81::math::cdf(cauchy<%1%>&, %1%)";
   RealType result = 0;
      ...
      bool complement)
{
   ...
   //
   static const char* function = "zr_boost_1_81::math::quantile(cauchy<%1%>&, %1%)";
   BOOST_MATH_STD_USING // for ADL of std functions
...
public:
   ...
   {
    static const char* function = "zr_boost_1_81::math::cauchy_distribution<%1%>::cauchy_distribution";
     RealType result;
...
cauchy_distribution(RealType)->cauchy_distribution<typename zr_boost_1_81::math::tools::promote_args<RealType>::type>;
template <class RealType>
cauchy_distribution(RealType,RealType)->cauchy_distribution<typename zr_boost_1_81::math::tools::promote_args<RealType>::type>;
#endif
...
{  
   ...
   static const char* function = "zr_boost_1_81::math::pdf(cauchy<%1%>&, %1%)";
   RealType result = 0;
   RealType location = dist.location();
   RealType scale = dist.scale();
   if(false == detail::check_scale("zr_boost_1_81::math::pdf(cauchy<%1%>&, %1%)", scale, &result, Policy()))
   {
      return result;
   }
   if(false == detail::check_location("zr_boost_1_81::math::pdf(cauchy<%1%>&, %1%)", location, &result, Policy()))
   {
      return result;
   }
...
{  // There is no mean:
   ...
   return policies::raise_domain_error<RealType>(
      "zr_boost_1_81::math::mean(cauchy<%1%>&)",
...
inline RealType variance(const cauchy_distribution<RealType, Policy>& /*dist*/)
{
   ...
   return policies::raise_domain_error<RealType>(
      "zr_boost_1_81::math::variance(cauchy<%1%>&)",
...
inline RealType skewness(const cauchy_distribution<RealType, Policy>& /*dist*/)
{
   ...
   return policies::raise_domain_error<RealType>(
      "zr_boost_1_81::math::skewness(cauchy<%1%>&)",
...
inline RealType kurtosis(const cauchy_distribution<RealType, Policy>& /*dist*/)
{
   ...
   return policies::raise_domain_error<RealType>(
      "zr_boost_1_81::math::kurtosis(cauchy<%1%>&)",
...
inline RealType kurtosis_excess(const cauchy_distribution<RealType, Policy>& /*dist*/)
{
   ...
   return policies::raise_domain_error<RealType>(
      "zr_boost_1_81::math::kurtosis_excess(cauchy<%1%>&)",
      "The Cauchy distribution does not have a kurtosis: "
      "the only possible return value is %1%.",
      std::numeric_limits<RealType>::quiet_NaN(), Policy());
}
