    ...
    class kolmogorov_smirnov_distribution
{
    ...
    kolmogorov_smirnov_distribution( RealType n ) : n_obs_(n)
    {
        RealType result;
        detail::check_df(
                "zr_boost_1_81::math::kolmogorov_smirnov_distribution<%1%>::kolmogorov_smirnov_distribution", n_obs_, &result, Policy());
    }
...
template <class RealType>
kolmogorov_smirnov_distribution(RealType)->kolmogorov_smirnov_distribution<typename zr_boost_1_81::math::tools::promote_args<RealType>::type>;
#endif
...
{
  kolmogorov_smirnov_quantile_functor(const zr_boost_1_81::math::kolmogorov_smirnov_distribution<RealType, Policy> dist, RealType const& p)
    : distribution(dist), prob(p)
  {
  }

  zr_boost_1_81::math::tuple<RealType, RealType> operator()(RealType const& x)
  {
    ...
    return zr_boost_1_81::math::make_tuple(fx, dx);
  }
private:
  const zr_boost_1_81::math::kolmogorov_smirnov_distribution<RealType, Policy> distribution;
  RealType prob;
};
...
{
  kolmogorov_smirnov_complementary_quantile_functor(const zr_boost_1_81::math::kolmogorov_smirnov_distribution<RealType, Policy> dist, RealType const& p)
    : distribution(dist), prob(p)
  {
  }
...
inline RealType pdf(const kolmogorov_smirnov_distribution<RealType, Policy>& dist, const RealType& x)
{
   ...
   static const char* function = "zr_boost_1_81::math::pdf(const kolmogorov_smirnov_distribution<%1%>&, %1%)";
...
inline RealType cdf(const kolmogorov_smirnov_distribution<RealType, Policy>& dist, const RealType& x)
{
   ...
   static const char* function = "zr_boost_1_81::math::cdf(const kolmogorov_smirnov_distribution<%1%>&, %1%)";
   RealType error_result;
   RealType n = dist.number_of_observations();
   if(false == detail::check_x_not_NaN(function, x, &error_result, Policy()))
      return error_result;
...
inline RealType cdf(const complemented2_type<kolmogorov_smirnov_distribution<RealType, Policy>, RealType>& c) {
    BOOST_MATH_STD_USING // for ADL of std function exp.
    RealType x = c.param;
   static const char* function = "zr_boost_1_81::math::cdf(const complemented2_type<const kolmogorov_smirnov_distribution<%1%>&, %1%>)";
   RealType error_result;
...
{
    BOOST_MATH_STD_USING
   static const char* function = "zr_boost_1_81::math::quantile(const kolmogorov_smirnov_distribution<%1%>&, %1%)";
   // Error check:
   RealType error_result;
   RealType n = dist.number_of_observations();
   ...
   return tools::newton_raphson_iterate(detail::kolmogorov_smirnov_quantile_functor<RealType, Policy>(dist, p),
           k, RealType(0), zr_boost_1_81::math::tools::max_value<RealType>(), get_digits, m);
} // quantile
...
{
    BOOST_MATH_STD_USING
   static const char* function = "zr_boost_1_81::math::mode(const kolmogorov_smirnov_distribution<%1%>&)";
   RealType n = dist.number_of_observations();
   RealType error_result;
   if(false == detail::check_df(function, n, &error_result, Policy()))
      return error_result;

    std::pair<RealType, RealType> r = zr_boost_1_81::math::tools::brent_find_minima(
...
{
    BOOST_MATH_STD_USING
   static const char* function = "zr_boost_1_81::math::mean(const kolmogorov_smirnov_distribution<%1%>&)";
    RealType n = dist.number_of_observations();
    RealType error_result;
    if(false == detail::check_df(function, n, &error_result, Policy()))
        return error_result;
...
{
   static const char* function = "zr_boost_1_81::math::variance(const kolmogorov_smirnov_distribution<%1%>&)";
    RealType n = dist.number_of_observations();
    RealType error_result;
    if(false == detail::check_df(function, n, &error_result, Policy()))
        return error_result;
...
{
    BOOST_MATH_STD_USING
   static const char* function = "zr_boost_1_81::math::skewness(const kolmogorov_smirnov_distribution<%1%>&)";
    RealType n = dist.number_of_observations();
    RealType error_result;
    if(false == detail::check_df(function, n, &error_result, Policy()))
        return error_result;
    RealType ex3 = RealType(0.5625) * constants::root_half_pi<RealType>() * constants::zeta_three<RealType>() / n / sqrt(n);
    RealType mean = zr_boost_1_81::math::mean(dist);
    RealType var = zr_boost_1_81::math::variance(dist);
...
{
    BOOST_MATH_STD_USING
   static const char* function = "zr_boost_1_81::math::kurtosis(const kolmogorov_smirnov_distribution<%1%>&)";
    RealType n = dist.number_of_observations();
    RealType error_result;
    if(false == detail::check_df(function, n, &error_result, Policy()))
        return error_result;
    ...
    RealType mean = zr_boost_1_81::math::mean(dist);
    RealType var = zr_boost_1_81::math::variance(dist);
    RealType skew = zr_boost_1_81::math::skewness(dist);
...
{
   static const char* function = "zr_boost_1_81::math::kurtosis_excess(const kolmogorov_smirnov_distribution<%1%>&)";
    RealType n = dist.number_of_observations();
    RealType error_result;
    if(false == detail::check_df(function, n, &error_result, Policy()))
        return error_result;
