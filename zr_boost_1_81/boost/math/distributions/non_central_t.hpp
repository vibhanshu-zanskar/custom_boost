...
namespace zr_boost_1_81
{
   namespace math
   {
      ...
      namespace detail{
         ...
         T non_central_t2_q(T v, T delta, T x, T y, const Policy& pol, T init_val)
         {
            ...
            if((k < static_cast<int>(max_factorial<T>::value)) && (d2 < tools::log_max_value<T>()) && (log(d2) * k < tools::log_max_value<T>()))
            {
               ...
               pois /= zr_boost_1_81::math::tgamma(T(k + 1 + 0.5), pol);
         ...
         T non_central_t_cdf(T v, T delta, T t, bool invert, const Policy& pol)
         {
            ...
            if(x < cross)
            {
               ...
               if(x != 0)
               {
                  ...
                  result = 0;
               result += cdf(zr_boost_1_81::math::normal_distribution<T, Policy>(), -delta);
            }
            else
            {
               //
               ...
               if(x != 0)
               {
                  ...
                  result = cdf(complement(zr_boost_1_81::math::normal_distribution<T, Policy>(), -delta));
            }
            if(invert)
               result = 1 - result;
            return result;
     ...
     // now passed as function
            ...
            value_type guess = 0;
            if ( ((zr_boost_1_81::math::isinf)(v)) || (v > 1 / zr_boost_1_81::math::tools::epsilon<T>()) )
            ...
            int s;
            if(p < q)
               s = zr_boost_1_81::math::sign(p - pzero);
            else
               s = zr_boost_1_81::math::sign(pzero - q);
            if(s != zr_boost_1_81::math::sign(guess))
            {
         ...
         T mean(T v, T delta, const Policy& pol)
         {
            ...
            BOOST_MATH_STD_USING
            if (v > 1 / zr_boost_1_81::math::tools::epsilon<T>() )
            {
              //normal_distribution<T, Policy> n(delta, 1);
              //return zr_boost_1_81::math::mean(n);
              return delta;
            }
            else
            {
         ...
         T skewness(T v, T delta, const Policy& pol)
         {
            ...
            T mean = zr_boost_1_81::math::detail::mean(v, delta, pol);
            T l2 = delta * delta;
      ...
      public:
         ...
         {
            const char* function = "zr_boost_1_81::math::non_central_t_distribution<%1%>::non_central_t_distribution(%1%,%1%)";
            RealType r;
      ...
      non_central_t_distribution(RealType,RealType)->non_central_t_distribution<typename zr_boost_1_81::math::tools::promote_args<RealType>::type>;
      #endif
      ...
      {
         const char* function = "zr_boost_1_81::math::cdf(non_central_t_distribution<%1%>&, %1%)";
//   was const char* function = "zr_boost_1_81::math::non_central_t_distribution<%1%>::cdf(%1%)";
         typedef typename policies::evaluation<RealType, Policy>::type value_type;
         typedef typename policies::normalise<
            Policy,
            policies::promote_float<false>,
  ...
  // was       const char* function = "zr_boost_1_81::math::non_central_t_distribution<%1%>::cdf(%1%)";
         const char* function = "zr_boost_1_81::math::cdf(const complement(non_central_t_distribution<%1%>&), %1%)";
         typedef typename policies::evaluation<RealType, Policy>::type value_type;
         typedef typename policies::normalise<
            Policy,
            policies::promote_float<false>,
