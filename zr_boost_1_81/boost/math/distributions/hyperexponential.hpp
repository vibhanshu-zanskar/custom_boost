...
bool check_probabilities(char const* function, std::vector<RealT> const& probabilities, RealT* presult, PolicyT const& pol)
{
    ...
    for (std::size_t i = 0; i < n; ++i)
    {
        if (probabilities[i] < 0
            || probabilities[i] > 1
            || !(zr_boost_1_81::math::isfinite)(probabilities[i]))
        {
...
bool check_rates(char const* function, std::vector<RealT> const& rates, RealT* presult, PolicyT const& pol)
{
    ...
    {
        if (rates[i] <= 0
            || !(zr_boost_1_81::math::isfinite)(rates[i]))
        {
            *presult = policies::raise_domain_error<RealT>(function,
...
{
    if (p < 0 || p > 1 || (zr_boost_1_81::math::isnan)(p))
    {
...
RealT quantile_impl(hyperexponential_distribution<RealT, PolicyT> const& dist, RealT const& p, bool comp)
{
    ...
    static const char* function = comp ? "zr_boost_1_81::math::quantile(const zr_boost_1_81::math::complemented2_type<zr_boost_1_81::math::hyperexponential_distribution<%1%>, %1%>&)"
                                       : "zr_boost_1_81::math::quantile(const zr_boost_1_81::math::hyperexponential_distribution<%1%>&, %1%)";
...
class hyperexponential_distribution
{
    ...
    {
        RealT err;
        hyperexp_detail::check_dist("zr_boost_1_81::math::hyperexponential_distribution<%1%>::hyperexponential_distribution",
                                    probs_,
                                    rates_,
                                    &err,
                                    PolicyT());
    ...
    private: template <typename T, typename = void>
             ...
             template <typename T>
             struct is_iterator<T, zr_boost_1_81::math::tools::void_t<typename std::iterator_traits<T>::difference_type>>
             {
...
RealT pdf(hyperexponential_distribution<RealT, PolicyT> const& dist, RealT const& x)
{
    ...
    if (!hyperexp_detail::check_x("zr_boost_1_81::math::pdf(const zr_boost_1_81::math::hyperexponential_distribution<%1%>&, %1%)", x, &result, PolicyT()))
    {
        return result;
    }
...
RealT cdf(hyperexponential_distribution<RealT, PolicyT> const& dist, RealT const& x)
{
    ...
    if (!hyperexp_detail::check_x("zr_boost_1_81::math::cdf(const zr_boost_1_81::math::hyperexponential_distribution<%1%>&, %1%)", x, &result, PolicyT()))
    {
        return result;
    }
...
RealT cdf(complemented2_type<hyperexponential_distribution<RealT,PolicyT>, RealT> const& c)
{
    ...
    if (!hyperexp_detail::check_x("zr_boost_1_81::math::cdf(zr_boost_1_81::math::complemented2_type<const zr_boost_1_81::math::hyperexponential_distribution<%1%>&, %1%>)", x, &result, PolicyT()))
    {
        return result;
    }
...
RealT variance(hyperexponential_distribution<RealT, PolicyT> const& dist)
{
    ...
    const RealT mean = zr_boost_1_81::math::mean(dist);
