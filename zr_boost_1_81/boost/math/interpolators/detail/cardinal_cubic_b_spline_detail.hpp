...
namespace zr_boost_1_81{ namespace math{ namespace interpolators{ namespace detail{
...
Real b3_spline(Real x)
{
    ...
    if (absx < 1)
    {
        Real y = 2 - absx;
        Real z = 1 - absx;
        return zr_boost_1_81::math::constants::sixth<Real>()*(y*y*y - 4*z*z*z);
    }
    if (absx < 2)
    {
        Real y = 2 - absx;
        return zr_boost_1_81::math::constants::sixth<Real>()*y*y*y;
    }
...
Real b3_spline_prime(Real x)
{
    ...
    if (x < 1)
    {
        return x*(3*zr_boost_1_81::math::constants::half<Real>()*x - 2);
    }
    if (x < 2)
    {
        return -zr_boost_1_81::math::constants::half<Real>()*(2 - x)*(2 - x);
    }
    return static_cast<Real>(0);
}
...
{
    using zr_boost_1_81::math::constants::third;

    std::size_t length = end_p - f;

    if (length < 5)
    {
        if (zr_boost_1_81::math::isnan(left_endpoint_derivative) || zr_boost_1_81::math::isnan(right_endpoint_derivative))
        {
    ...
    if (zr_boost_1_81::math::isnan(left_endpoint))
    {
    ...
    if (zr_boost_1_81::math::isnan(a1))
    {
    ...
    if (zr_boost_1_81::math::isnan(b1))
    {
        size_t n = length - 1;
    ...
    {
        if (zr_boost_1_81::math::isnan(f[i]))
        {
...
Real cardinal_cubic_b_spline_imp<Real>::operator()(Real x) const
{
    ...
    size_t k_min = static_cast<size_t>((max)(static_cast<long>(0), zr_boost_1_81::math::ltrunc(ceil(t - 2))));
    size_t k_max = static_cast<size_t>((max)((min)(static_cast<long>(m_beta.size() - 1), zr_boost_1_81::math::ltrunc(floor(t + 2))), 0l));
...
Real cardinal_cubic_b_spline_imp<Real>::prime(Real x) const
{
    ...
    size_t k_min = static_cast<size_t>((max)(static_cast<long>(0), zr_boost_1_81::math::ltrunc(ceil(t - 2))));
    size_t k_max = static_cast<size_t>((min)(static_cast<long>(m_beta.size() - 1), zr_boost_1_81::math::ltrunc(floor(t + 2))));
