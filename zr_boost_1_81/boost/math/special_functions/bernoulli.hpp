...
OutputIterator bernoulli_number_imp(OutputIterator out, std::size_t start, std::size_t n, const Policy& pol, const std::integral_constant<int, N>& tag)
{
   ...
   {
      // We must overflow:
      *out = (i & 1 ? 1 : -1) * policies::raise_overflow_error<T>("zr_boost_1_81::math::bernoulli_b2n<%1%>(n)", nullptr, T(i), pol);
      ++out;
   }
   return out;
}
...
inline T bernoulli_b2n(const int i, const Policy &pol)
{
   ...
   if(i < 0)
   {
      return policies::raise_domain_error<T>("zr_boost_1_81::math::bernoulli_b2n<%1%>", "Index should be >= 0 but got %1%", T(i), pol);
   }

   T result {};
   zr_boost_1_81::math::detail::bernoulli_number_imp<T>(&result, static_cast<std::size_t>(i), 1u, pol, tag_type());
   return result;
}
...
{
   return zr_boost_1_81::math::bernoulli_b2n<T>(i, policies::policy<>());
}
                                    ...
                                    const Policy& pol)
{
   ...
   if(start_index < 0)
   {
      *out_it = policies::raise_domain_error<T>("zr_boost_1_81::math::bernoulli_b2n<%1%>", "Index should be >= 0 but got %1%", T(start_index), pol);
      return ++out_it;
   }

   return zr_boost_1_81::math::detail::bernoulli_number_imp<T>(out_it, start_index, number_of_bernoullis_b2n, pol, tag_type());
}
...
inline OutputIterator bernoulli_b2n(const int start_index,
                                    ...
                                    OutputIterator out_it)
{
   return zr_boost_1_81::math::bernoulli_b2n<T, OutputIterator>(start_index, number_of_bernoullis_b2n, out_it, policies::policy<>());
}
...
inline T tangent_t2n(const int i, const Policy &pol)
{
   if(i < 0)
   {
      return policies::raise_domain_error<T>("zr_boost_1_81::math::tangent_t2n<%1%>", "Index should be >= 0 but got %1%", T(i), pol);
   }

   T result {};
   zr_boost_1_81::math::detail::get_bernoulli_numbers_cache<T, Policy>().copy_tangent_numbers(&result, i, 1, pol);
   return result;
}
...
{
   return zr_boost_1_81::math::tangent_t2n<T>(i, policies::policy<>());
}
                                    ...
                                    const Policy& pol)
{
   if(start_index < 0)
   {
      *out_it = policies::raise_domain_error<T>("zr_boost_1_81::math::tangent_t2n<%1%>", "Index should be >= 0 but got %1%", T(start_index), pol);
      return ++out_it;
   }

   return zr_boost_1_81::math::detail::get_bernoulli_numbers_cache<T, Policy>().copy_tangent_numbers(out_it, start_index, number_of_tangent_t2n, pol);
}
...
inline OutputIterator tangent_t2n(const int start_index,
                                    ...
                                    OutputIterator out_it)
{
   return zr_boost_1_81::math::tangent_t2n<T, OutputIterator>(start_index, number_of_tangent_t2n, out_it, policies::policy<>());
}
