...
public:
   ...
   T operator()()
   {
      T result = term * zr_boost_1_81::math::bernoulli_b2n<T>(k) / (2 * k);
      term /= xx;
      ++k;
      return result;
   }
...
inline T digamma_imp_large(T x, const Policy& pol, const std::integral_constant<int, 0>*)
{
   ...
   result = -result;
   policies::check_series_iterations<T>("zr_boost_1_81::math::digamma<%1%>(%1%)", max_iter, pol);
   return result;
}
//
...
T digamma_imp(T x, const Tag* t, const Policy& pol)
{
   ...
   if(x <= -1)
   {
      ...
      if(remainder == 0)
      {
         return policies::raise_pole_error<T>("zr_boost_1_81::math::digamma<%1%>(%1%)", nullptr, (1-x), pol);
      }
      result = constants::pi<T>() / tan(constants::pi<T>() * remainder);
   }
   if(x == 0)
      return policies::raise_pole_error<T>("zr_boost_1_81::math::digamma<%1%>(%1%)", nullptr, x, pol);
   //
...
struct digamma_initializer
{
   struct init
   {
      ...
      {
         zr_boost_1_81::math::digamma(T(1.5), Policy());
         zr_boost_1_81::math::digamma(T(500), Policy());
      }
   ...
   digamma(T x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::digamma_imp(
      ...
      static_cast<const tag_type*>(nullptr), forwarding_policy()), "zr_boost_1_81::math::digamma<%1%>(%1%)");
}
