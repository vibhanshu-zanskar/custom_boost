...
inline T bessel_y_small_z_series(T v, T x, T* pscale, const Policy& pol)
{
   ...
   *pscale = scale;
   ...
   policies::check_series_iterations<T>("zr_boost_1_81::math::bessel_y_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
   result *= prefix;

   if(!need_logs)
   {
      prefix = zr_boost_1_81::math::tgamma(-v, pol) * zr_boost_1_81::math::cos_pi(v, pol) * p / constants::pi<T>();
   }
   else
   {
      int sgn;
      prefix = zr_boost_1_81::math::lgamma(-v, &sgn, pol) + p;
...
T bessel_yn_small_z(int n, T z, T* scale, const Policy& pol)
{
   ...
   {
      T p = pow(z / 2, n);
      T result = -((zr_boost_1_81::math::factorial<T>(n - 1, pol) / constants::pi<T>()));
