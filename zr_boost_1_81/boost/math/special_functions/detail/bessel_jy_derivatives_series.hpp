...
inline T bessel_j_derivative_small_z_series(T v, T x, const Policy& pol)
{
   ...
   T prefix;
   if (v < zr_boost_1_81::math::max_factorial<T>::value)
   {
      prefix = pow(x / 2, v - 1) / 2 / zr_boost_1_81::math::tgamma(v + 1, pol);
   }
   else
   {
      prefix = (v - 1) * log(x / 2) - constants::ln_two<T>() - zr_boost_1_81::math::lgamma(v + 1, pol);
      prefix = exp(prefix);
   }
   if (0 == prefix)
      return prefix;
   ...
   zr_boost_1_81::math::policies::check_series_iterations<T>("zr_boost_1_81::math::bessel_j_derivative_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
...
inline T bessel_y_derivative_small_z_series(T v, T x, const Policy& pol)
{
   ...
   T p = log(x / 2);
   T scale = 1;
   bool need_logs = (v >= zr_boost_1_81::math::max_factorial<T>::value) || (zr_boost_1_81::math::tools::log_max_value<T>() / v < fabs(p));
   if (!need_logs)
   {
      gam = zr_boost_1_81::math::tgamma(v, pol);
      p = pow(x / 2, v + 1) * 2;
      if (zr_boost_1_81::math::tools::max_value<T>() * p < gam)
      {
         scale /= gam;
         gam = 1;
         if (zr_boost_1_81::math::tools::max_value<T>() * p < gam)
      ...
      prefix = -gam / (zr_boost_1_81::math::constants::pi<T>() * p);
   }
   else
   {
      gam = zr_boost_1_81::math::lgamma(v, pol);
      p = (v + 1) * p + constants::ln_two<T>();
      prefix = gam - log(zr_boost_1_81::math::constants::pi<T>()) - p;
      if (zr_boost_1_81::math::tools::log_max_value<T>() < prefix)
      {
         prefix -= log(zr_boost_1_81::math::tools::max_value<T>() / 4);
         scale /= (zr_boost_1_81::math::tools::max_value<T>() / 4);
         if (zr_boost_1_81::math::tools::log_max_value<T>() < prefix)
         {
            return zr_boost_1_81::math::policies::raise_overflow_error<T>(function, nullptr, pol);
         }
   ...
   zr_boost_1_81::math::policies::check_series_iterations<T>("zr_boost_1_81::math::bessel_y_derivative_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
   result *= prefix;

   p = pow(x / 2, v - 1) / 2;
   if (!need_logs)
   {
      prefix = zr_boost_1_81::math::tgamma(-v, pol) * zr_boost_1_81::math::cos_pi(v, pol) * p / zr_boost_1_81::math::constants::pi<T>();
   }
   else
   {
      int sgn;
      prefix = zr_boost_1_81::math::lgamma(-v, &sgn, pol) + (v - 1) * log(x / 2) - constants::ln_two<T>();
      prefix = exp(prefix) * sgn / zr_boost_1_81::math::constants::pi<T>();
   }
   bessel_y_derivative_small_z_series_term_b<T, Policy> s2(v, x);
   max_iter = zr_boost_1_81::math::policies::get_max_series_iterations<Policy>();

   T b = zr_boost_1_81::math::tools::sum_series(s2, zr_boost_1_81::math::policies::get_epsilon<T, Policy>(), max_iter);
