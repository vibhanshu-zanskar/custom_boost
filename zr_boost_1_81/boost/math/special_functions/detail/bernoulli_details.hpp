...
T b2n_asymptotic(int n)
{
   ...
   const T approximate_log_of_bernoulli_bn =
        ((zr_boost_1_81::math::constants::half<T>() + nx) * log(nx))
        + ((zr_boost_1_81::math::constants::half<T>() - nx) * log(zr_boost_1_81::math::constants::pi<T>()))
        + (((T(3) / 2) - nx) * zr_boost_1_81::math::constants::ln_two<T>())
        + ((nx * (T(2) - (nx2 * 7) * (1 + ((nx2 * 30) * ((nx2 * 12) - 1))))) / (((nx2 * nx2) * nx2) * 2520));
   return ((n / 2) & 1 ? 1 : -1) * (approximate_log_of_bernoulli_bn > tools::log_max_value<T>()
      ? policies::raise_overflow_error<T>("zr_boost_1_81::math::bernoulli_b2n<%1%>(std::size_t)", nullptr, nx, Policy())
...
T t2n_asymptotic(int n)
{
   ...
   {
      return policies::raise_overflow_error<T>("zr_boost_1_81::math::tangent_t2n<%1%>(std::size_t)", nullptr, T(n), Policy());
   }
   t2n *= p2;
   p2 -= 1;
   if(tools::max_value<T>() / p2 < t2n)
   {
      return policies::raise_overflow_error<T>("zr_boost_1_81::math::tangent_t2n<%1%>(std::size_t)", nullptr, Policy());
   }
   t2n *= p2;
   return t2n;
}
...
struct max_bernoulli_root_functor
{
   ...
   double operator()(double n) const
   {
      ...
      const double approximate_log_of_bernoulli_bn
         =   ((zr_boost_1_81::math::constants::half<double>() + n) * log(n))
           + ((zr_boost_1_81::math::constants::half<double>() - n) * log(zr_boost_1_81::math::constants::pi<double>()))
           + (((static_cast<double>(3) / 2) - n) * zr_boost_1_81::math::constants::ln_two<double>())
...
inline std::size_t find_bernoulli_overflow_limit(const std::false_type&)
{
   ...
   unsigned long long t = lltrunc(zr_boost_1_81::math::tools::log_max_value<T>());
   max_bernoulli_root_functor fun(t);
   zr_boost_1_81::math::tools::equal_floor tol;
   std::uintmax_t max_iter = zr_boost_1_81::math::policies::get_max_root_iterations<Policy>();
   double result = zr_boost_1_81::math::tools::toms748_solve(fun, sqrt(static_cast<double>(t)), static_cast<double>(t), tol, max_iter).first / 2;
...
// zr_boost_1_81::container::static_vector here, but that allocates on the stack, which may well
...
public:
   bernoulli_numbers_cache() : m_overflow_limit((std::numeric_limits<std::size_t>::max)())
      , m_counter(0)
      , m_current_precision(zr_boost_1_81::math::tools::digits<T>())
   {}
   ...
   bool tangent(std::size_t m)
   {
      ...
      for(std::size_t i = std::max<size_t>(2, prev_size); i < m; i++)
      {
         ...
         if(i >= min_overflow_index && (zr_boost_1_81::math::tools::max_value<T>() / (i-1) < m_intermediates[1]) )
         {
            std::fill(tn.begin() + i, tn.end(), zr_boost_1_81::math::tools::max_value<T>());
            break;
         }
         ...
         for(std::size_t j = 2; j <= i; j++)
         {
            overflow_check =
                  ...
                  (zr_boost_1_81::math::tools::max_value<T>() / (i - j) < m_intermediates[j])
                  || (zr_boost_1_81::math::tools::max_value<T>() / (i - j + 2) < m_intermediates[j-1])
                  || (zr_boost_1_81::math::tools::max_value<T>() - m_intermediates[j] * (i - j) < m_intermediates[j-1] * (i - j + 2))
                  || ((zr_boost_1_81::math::isinf)(m_intermediates[j]))
                );
   ...
   OutputIterator copy_bernoulli_numbers(OutputIterator out, std::size_t start, std::size_t n, const Policy& pol)
   {
      ...
      if(start + n > bn.capacity())
      {
         ...
         for(; n; ++start, --n)
         {
            *out = policies::raise_overflow_error<T>("zr_boost_1_81::math::bernoulli_b2n<%1%>(std::size_t)", nullptr, T(start), pol);
            ++out;
         }
         return out;
      }
      ...
      //
      if(m_current_precision < zr_boost_1_81::math::tools::digits<T>())
      {
         bn.clear();
         tn.clear();
      ...
      if(start + n >= bn.size())
      {
         ...
         {
            return std::fill_n(out, n, policies::raise_evaluation_error<T>("zr_boost_1_81::math::bernoulli_b2n<%1%>(std::size_t)", "Unable to allocate Bernoulli numbers cache for %1% values", T(start + n), pol));
         }
      }

      for(std::size_t i = (std::max)(std::size_t(max_bernoulli_b2n<T>::value + 1), start); i < start + n; ++i)
      {
         *out = (i >= m_overflow_limit) ? policies::raise_overflow_error<T>("zr_boost_1_81::math::bernoulli_b2n<%1%>(std::size_t)", nullptr, T(i), pol) : bn[i];
         ++out;
      }
      #else
      //
      ...
      if((static_cast<std::size_t>(m_counter.load(std::memory_order_consume)) < start + n)
         || (static_cast<int>(m_current_precision.load(std::memory_order_consume)) < zr_boost_1_81::math::tools::digits<T>()))
      {
         ...
         if((static_cast<std::size_t>(m_counter.load(std::memory_order_consume)) < start + n)
            || (static_cast<int>(m_current_precision.load(std::memory_order_consume)) < zr_boost_1_81::math::tools::digits<T>()))
         {
            if(static_cast<int>(m_current_precision.load(std::memory_order_consume)) < zr_boost_1_81::math::tools::digits<T>())
            {
               bn.clear();
               tn.clear();
               m_intermediates.clear();
            ...
            if(start + n >= bn.size())
            {
               std::size_t new_size = (std::min)((std::max)((std::max)(std::size_t(start + n), std::size_t(bn.size() + 20)), std::size_t(50)), std::size_t(bn.capacity()));
               if (!tangent_numbers_series(new_size))
                  return std::fill_n(out, n, policies::raise_evaluation_error<T>("zr_boost_1_81::math::bernoulli_b2n<%1%>(std::size_t)", "Unable to allocate Bernoulli numbers cache for %1% values", T(new_size), pol));
            }
      ...
      {
         *out = (i >= m_overflow_limit) ? policies::raise_overflow_error<T>("zr_boost_1_81::math::bernoulli_b2n<%1%>(std::size_t)", nullptr, T(i), pol) : bn[static_cast<typename container_type::size_type>(i)];
         ++out;
      }
   ...
   OutputIterator copy_tangent_numbers(OutputIterator out, std::size_t start, std::size_t n, const Policy& pol)
   {
      ...
      if(start + n > bn.capacity())
      {
         ...
         for(; n; ++start, --n)
         {
            *out = policies::raise_overflow_error<T>("zr_boost_1_81::math::bernoulli_b2n<%1%>(std::size_t)", 0, T(start), pol);
            ++out;
         }
         return out;
      }
      ...
      for(std::size_t i = start; i < start + n; ++i)
      {
         if(i >= m_overflow_limit)
            *out = policies::raise_overflow_error<T>("zr_boost_1_81::math::bernoulli_b2n<%1%>(std::size_t)", nullptr, T(i), pol);
         else
         {
