...
namespace zr_boost_1_81 { namespace math {
   ...
   namespace detail {
      ...
      int temme_jy(T v, T x, T* Y, T* Y1, const Policy& pol)
      {
         ...
         T spv = zr_boost_1_81::math::sin_pi(v, pol);
         T spv2 = zr_boost_1_81::math::sin_pi(v/2, pol);
         T xp = pow(x/2, v);
         ...
         policies::check_series_iterations<T>("zr_boost_1_81::math::bessel_jy<%1%>(%1%,%1%) in temme_jy", k, pol);
         *Y = -sum;
         *Y1 = -2 * sum1 / x;
      ...
      int CF1_jy(T v, T x, T* fv, int* sign, const Policy& pol)
      {
         ...
         policies::check_series_iterations<T>("zr_boost_1_81::math::bessel_jy<%1%>(%1%,%1%) in CF1_jy", k / 100, pol);
         *fv = -f;
      ...
      int CF2_jy(T v, T x, T* p, T* q, const Policy& pol)
      {
         ...
         policies::check_series_iterations<T>("zr_boost_1_81::math::bessel_jy<%1%>(%1%,%1%) in CF2_jy", k, pol);
         *p = fr;
         *q = fi;
      ...
      int bessel_jy(T v, T x, T* J, T* Y, int kind, const Policy& pol)
      {
         ...
         static const char* function = "zr_boost_1_81::math::bessel_jy<%1%>(%1%,%1%)";
         ...
         {
            T z = (u + n % 2);
            cp = zr_boost_1_81::math::cos_pi(z, pol);
            sp = zr_boost_1_81::math::sin_pi(z, pol);
            if(u != 0)
         ...
         else if((x > 8) && hankel_PQ(v, x, &p, &q, pol))
         {
            ...
            //
            // chi = x - fmod(T(v / 2 + 0.25f), T(2)) * zr_boost_1_81::math::constants::pi<T>();
            //
            ...
            T sx = sin(x);
            T cx = cos(x);
            T sv = zr_boost_1_81::math::sin_pi(mod_v, pol);
            T cv = zr_boost_1_81::math::cos_pi(mod_v, pol);

            T sc = sx * cv - sv * cx; // == sin(chi);
            T cc = cx * cv + sx * sv; // == cos(chi);
            T chi = zr_boost_1_81::math::constants::root_two<T>() / (zr_boost_1_81::math::constants::root_pi<T>() * sqrt(x)); //sqrt(2 / (zr_boost_1_81::math::constants::pi<T>() * x));
            Yv = chi * (p * sc + q * cc);
            Jv = chi * (p * cc - q * sc);
         }
