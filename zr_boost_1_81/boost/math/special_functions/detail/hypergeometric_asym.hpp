...
#pragma warning(disable:4127)
  ...
  namespace detail {
     ...
     inline T hypergeometric_1F1_asym_large_z_series(T a, const T& b, T z, const Policy& pol, long long& log_scaling)
     {
        ...
        static const char* function = "zr_boost_1_81::math::hypergeometric_1F1_asym_large_z_series<%1%>(%1%, %1%, %1%)";
        T prefix;
        long long e;
        int s;
        if (z < 0)
        ...
        {
           prefix *= pow(z, a) * pow(z, -b) * zr_boost_1_81::math::tgamma(b, pol) / zr_boost_1_81::math::tgamma(a, pol);
        }
        else
        {
           T t = log(z) * (a - b);
           ...
           t = zr_boost_1_81::math::lgamma(b, &s, pol);
           e = lltrunc(t, pol);
           log_scaling += e;
           prefix *= s * exp(t - e);

           t = zr_boost_1_81::math::lgamma(a, &s, pol);
           e = lltrunc(t, pol);
           log_scaling -= e;
           prefix /= s * exp(t - e);
        }
        ...
        do
        {
           ...
           a1_poch += 1;
           a2_poch += 1;
           if (fabs(sum) * zr_boost_1_81::math::policies::get_epsilon<T, Policy>() > fabs(term))
              break;
           if(fabs(sum) / abs_sum < zr_boost_1_81::math::policies::get_epsilon<T, Policy>())
              return zr_boost_1_81::math::policies::raise_evaluation_error<T>(function, "Large-z asymptotic approximation to 1F1 has destroyed all the digits in the result due to cancellation.  Current best guess is %1%", 
                 prefix * sum, Policy());
           if(k > zr_boost_1_81::math::policies::get_max_series_iterations<Policy>())
              return zr_boost_1_81::math::policies::raise_evaluation_error<T>(function, "1F1: Unable to locate solution in a reasonable time:"
                 " large-z asymptotic approximation.  Current best guess is %1%", prefix * sum, Policy());
           if((k > 10) && (fabs(term) > fabs(last_term)))
              return zr_boost_1_81::math::policies::raise_evaluation_error<T>(function, "Large-z asymptotic approximation to 1F1 is divergent.  Current best guess is %1%", prefix * sum, Policy());
        } while (true);
