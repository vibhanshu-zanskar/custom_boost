...
inline T bessel_i_small_z_series(T v, T x, const Policy& pol)
{
   ...
   {
      prefix = pow(x / 2, v) / zr_boost_1_81::math::tgamma(v + 1, pol);
   }
   else
   {
      prefix = v * log(x / 2) - zr_boost_1_81::math::lgamma(v + 1, pol);
      prefix = exp(prefix);
   }
   if(prefix == 0)
      return prefix;
   ...
   policies::check_series_iterations<T>("zr_boost_1_81::math::bessel_j_small_z_series<%1%>(%1%,%1%)", max_iter, pol);
...
int temme_ik(T v, T x, T* K, T* K1, const Policy& pol)
{
    ...
    T gp = zr_boost_1_81::math::tgamma1pm1(v, pol);
    T gm = zr_boost_1_81::math::tgamma1pm1(-v, pol);

    a = log(x / 2);
    b = exp(v * a);
    sigma = -a * v;
    c = abs(v) < tools::epsilon<T>() ?
       T(1) : T(zr_boost_1_81::math::sin_pi(v, pol) / (v * pi<T>()));
    ...
    policies::check_series_iterations<T>("zr_boost_1_81::math::bessel_ik<%1%>(%1%,%1%) in temme_ik", k, pol);
...
int CF1_ik(T v, T x, T* fv, const Policy& pol)
{
    ...
    policies::check_series_iterations<T>("zr_boost_1_81::math::bessel_ik<%1%>(%1%,%1%) in CF1_ik", k, pol);
...
int CF2_ik(T v, T x, T* Kv, T* Kv1, const Policy& pol)
{
    ...
    policies::check_series_iterations<T>("zr_boost_1_81::math::bessel_ik<%1%>(%1%,%1%) in CF2_ik", k, pol);
...
int bessel_ik(T v, T x, T* I, T* K, int kind, const Policy& pol)
{
    ...
    static const char* function = "zr_boost_1_81::math::bessel_ik<%1%>(%1%,%1%)";
    ...
    if (x == 0)
    {
       ...
       {
           T z = (u + n % 2);
           Iv = zr_boost_1_81::math::sin_pi(z, pol) == 0 ?
               Iv :
    ...
    for (k = 1; k <= n; k++)                   // forward recurrence for K
    {
        ...
        if(!will_overflow && ((tools::max_value<T>() - fabs(prev)) / fact < fabs(current)))
        {
           prev /= current;
           scale /= current;
           scale_sign *= zr_boost_1_81::math::sign(current);
           current = 1;
        }
    ...
    {
        T z = (u + n % 2);
        T fact = (2 / pi<T>()) * (zr_boost_1_81::math::sin_pi(z, pol) * Kv);
        if(fact == 0)
           *I = Iv;
