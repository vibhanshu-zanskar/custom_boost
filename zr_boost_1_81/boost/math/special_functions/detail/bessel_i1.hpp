...
T bessel_i1_imp(const T& x, const std::integral_constant<int, 24>&)
{
   BOOST_MATH_STD_USING
      if(x < 7.75)
      {
         ...
         T a = x * x / 4;
         T Q[3] = { 1, 0.5f, zr_boost_1_81::math::tools::evaluate_polynomial(P, a) };
         return x * zr_boost_1_81::math::tools::evaluate_polynomial(Q, a) / 2;
      }
      else
      {
         ...
         T ex = exp(x / 2);
         T result = ex * zr_boost_1_81::math::tools::evaluate_polynomial(P, T(1 / x)) / sqrt(x);
         result *= ex;
         return result;
      }
}
...
T bessel_i1_imp(const T& x, const std::integral_constant<int, 53>&)
{
   ...
   else if(x < 500)
   {
      ...
      return exp(x) * zr_boost_1_81::math::tools::evaluate_polynomial(P, T(1 / x)) / sqrt(x);
   }
   else
   {
...
{
   if(zr_boost_1_81::math::tools::digits<T>() <= 24)
      return bessel_i1_imp(x, std::integral_constant<int, 24>());
   else if(zr_boost_1_81::math::tools::digits<T>() <= 53)
      return bessel_i1_imp(x, std::integral_constant<int, 53>());
   else if(zr_boost_1_81::math::tools::digits<T>() <= 64)
      return bessel_i1_imp(x, std::integral_constant<int, 64>());
   else if(zr_boost_1_81::math::tools::digits<T>() <= 113)
