...
struct erf_roots
{
   zr_boost_1_81::math::tuple<T,T,T> operator()(const T& guess)
   {
      BOOST_MATH_STD_USING
      T derivative = sign * (2 / sqrt(constants::pi<T>())) * exp(-(guess * guess));
      T derivative2 = -2 * guess * derivative;
      return zr_boost_1_81::math::make_tuple(((sign > 0) ? static_cast<T>(zr_boost_1_81::math::erf(guess, Policy()) - target) : static_cast<T>(zr_boost_1_81::math::erfc(guess, Policy())) - target), derivative, derivative2);
   }
...
T erf_inv_imp(const T& p, const T& q, const Policy& pol, const std::integral_constant<int, 0>*)
{
   ...
   if(policies::digits<T, Policy>() > 64)
   {
      ...
      policies::check_root_iterations<T>("zr_boost_1_81::math::erf_inv<%1%>", max_iter, pol);
   }
   else
   {
      result = guess;
...
struct erf_inv_initializer
{
   struct init
   {
      ...
      static void do_init()
      {
         ...
         {
            zr_boost_1_81::math::erf_inv(static_cast<T>(0.25), Policy());
            zr_boost_1_81::math::erf_inv(static_cast<T>(0.55), Policy());
            zr_boost_1_81::math::erf_inv(static_cast<T>(0.95), Policy());
            zr_boost_1_81::math::erfc_inv(static_cast<T>(1e-15), Policy());
            ...
            if(is_value_non_zero(static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 1e-130))))
               zr_boost_1_81::math::erfc_inv(static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 1e-130)), Policy());
            ...
            if(is_value_non_zero(static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 1e-800))))
               zr_boost_1_81::math::erfc_inv(static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 1e-800)), Policy());
            if(is_value_non_zero(static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 1e-900))))
               zr_boost_1_81::math::erfc_inv(static_cast<T>(BOOST_MATH_BIG_CONSTANT(T, 64, 1e-900)), Policy());
#else
            if(is_value_non_zero(static_cast<T>(BOOST_MATH_HUGE_CONSTANT(T, 64, 1e-800))))
               zr_boost_1_81::math::erfc_inv(static_cast<T>(BOOST_MATH_HUGE_CONSTANT(T, 64, 1e-800)), Policy());
            if(is_value_non_zero(static_cast<T>(BOOST_MATH_HUGE_CONSTANT(T, 64, 1e-900))))
               zr_boost_1_81::math::erfc_inv(static_cast<T>(BOOST_MATH_HUGE_CONSTANT(T, 64, 1e-900)), Policy());
#endif
         }
      }
...
typename tools::promote_args<T>::type erfc_inv(T z, const Policy& pol)
{
   ...
   //
   static const char* function = "zr_boost_1_81::math::erfc_inv<%1%>(%1%, %1%)";
   if((z < 0) || (z > 2))
...
typename tools::promote_args<T>::type erf_inv(T z, const Policy& pol)
{
   ...
   // Begin by testing for domain errors, and other special cases:
   //
   static const char* function = "zr_boost_1_81::math::erf_inv<%1%>(%1%, %1%)";
   if((z < -1) || (z > 1))
