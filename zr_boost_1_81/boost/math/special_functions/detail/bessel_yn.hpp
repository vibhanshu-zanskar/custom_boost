...
T bessel_yn(int n, T x, const Policy& pol)
{
    ...
    static const char* function = "zr_boost_1_81::math::bessel_yn<%1%>(%1%,%1%)";
    ...
    if(x < policies::get_epsilon<T, Policy>())
    {
       ...
       if(tools::max_value<T>() * fabs(scale) < fabs(value))
          return zr_boost_1_81::math::sign(scale) * zr_boost_1_81::math::sign(value) * policies::raise_overflow_error<T>(function, nullptr, pol);
       value /= scale;
    }
    ...
    else
    {
       ...
       policies::check_series_iterations<T>("zr_boost_1_81::math::bessel_y_n<%1%>(%1%,%1%)", n, pol);
       T mult = 2 * k / x;
