...
inline T asymptotic_bessel_y_large_x_2(T v, T x, const Policy& pol)
{
   ...
   T cx = cos(x);
   T sx = sin(x);
   T ci = zr_boost_1_81::math::cos_pi(v / 2 + 0.25f, pol);
   T si = zr_boost_1_81::math::sin_pi(v / 2 + 0.25f, pol);
...
{
   T c = 1;
   T p = (v / zr_boost_1_81::math::sin_pi(v, pol)) * pow(x / 2, -v) / zr_boost_1_81::math::tgamma(1 - v, pol);
   T q = (v / zr_boost_1_81::math::sin_pi(v, pol)) * pow(x / 2, v) / zr_boost_1_81::math::tgamma(1 + v, pol);
   T f = (p - q) / v;
   T g_prefix = zr_boost_1_81::math::sin_pi(v / 2, pol);
...
T asymptotic_bessel_i_large_x(T v, T x, const Policy& pol)
{
   ...
   return (zr_boost_1_81::math::isfinite)(s) ?
      s : policies::raise_overflow_error<T>("zr_boost_1_81::math::asymptotic_bessel_i_large_x<%1%>(%1%,%1%)", nullptr, pol);
}
