...
struct zeta_series_cache_size
{
   ...
   // to zeta for example.
   //
   typedef typename zr_boost_1_81::math::policies::precision<T,Policy>::type precision_type;
...
T zeta_series_imp(T s, T sc, const Policy&)
{
   ...
   return sum * 1 / -zr_boost_1_81::math::powm1(T(2), sc);
}
...
inline T zeta_series2_imp(T s, const Policy& pol)
{
   ...
   T result = tools::sum_series(
      ...
      max_iter);
   policies::check_series_iterations<T>("zr_boost_1_81::math::zeta_series2<%1%>(%1%)", max_iter, pol);
   return result;
}
#endif
...
T zeta_polynomial_series(T s, T sc, Policy const &)
{
   ...
   //
   BOOST_MATH_STD_USING
   int n = itrunc(T(log(zr_boost_1_81::math::tools::epsilon<T>()) / -2));
   T sum = 0;
   T two_n = ldexp(T(1), n);
   int ej_sign = 1;
   for(int j = 0; j < n; ++j)
...
{
   BOOST_MATH_STD_USING
   static const char* function = "zr_boost_1_81::math::zeta<%1%>";
   if(sc == 0)
   ...
   if(floor(s) == s)
   {
         ...
         if(v == s)
         {
            if(v < 0)
            {
               if(((-v) & 1) == 0)
                  return 0;
               int n = (-v + 1) / 2;
               if(n <= (int)zr_boost_1_81::math::max_bernoulli_b2n<T>::value)
                  return T((-v & 1) ? -1 : 1) * zr_boost_1_81::math::unchecked_bernoulli_b2n<T>(n) / (1 - v);
            }
            else if((v & 1) == 0)
            {
               if(((v / 2) <= (int)zr_boost_1_81::math::max_bernoulli_b2n<T>::value) && (v <= (int)zr_boost_1_81::math::max_factorial<T>::value))
                  return T(((v / 2 - 1) & 1) ? -1 : 1) * ldexp(T(1), v - 1) * pow(constants::pi<T, Policy>(), v) *
                     zr_boost_1_81::math::unchecked_bernoulli_b2n<T>(v / 2) / zr_boost_1_81::math::unchecked_factorial<T>(v);
               return T(((v / 2 - 1) & 1) ? -1 : 1) * ldexp(T(1), v - 1) * pow(constants::pi<T, Policy>(), v) *
                  zr_boost_1_81::math::bernoulli_b2n<T>(v / 2) / zr_boost_1_81::math::factorial<T>(v, pol);
            }
            else
      ...
      catch(const zr_boost_1_81::math::rounding_error&){} // Just fall through, s is too large to round
   ...
   else if(s < 0)
   {
      ...
      else
      {
         ...
         {
            T mult = zr_boost_1_81::math::sin_pi(0.5f * sc, pol) * 2 * zeta_imp(s, sc, pol, tag);
            result = zr_boost_1_81::math::lgamma(s, pol);
            ...
            if(tools::max_value<T>() / fabs(mult) < result)
               return zr_boost_1_81::math::sign(mult) * policies::raise_overflow_error<T>(function, nullptr, pol);
            result *= mult;
         }
         else
         {
            result = zr_boost_1_81::math::sin_pi(0.5f * sc, pol)
               * 2 * pow(2 * constants::pi<T>(), -s)
               * zr_boost_1_81::math::tgamma(s, pol)
...
struct zeta_initializer
{
   struct init
   {
      ...
      static void do_init(const std::integral_constant<int, 0>&){ zr_boost_1_81::math::zeta(static_cast<T>(5), Policy()); }
      static void do_init(const std::integral_constant<int, 53>&){ zr_boost_1_81::math::zeta(static_cast<T>(5), Policy()); }
      static void do_init(const std::integral_constant<int, 64>&)
      {
         zr_boost_1_81::math::zeta(static_cast<T>(0.5), Policy());
         zr_boost_1_81::math::zeta(static_cast<T>(1.5), Policy());
         zr_boost_1_81::math::zeta(static_cast<T>(3.5), Policy());
         zr_boost_1_81::math::zeta(static_cast<T>(6.5), Policy());
         zr_boost_1_81::math::zeta(static_cast<T>(14.5), Policy());
         zr_boost_1_81::math::zeta(static_cast<T>(40.5), Policy());

         zr_boost_1_81::math::zeta(static_cast<T>(5), Policy());
      }
      static void do_init(const std::integral_constant<int, 113>&)
      {
         ...
         zr_boost_1_81::math::zeta(static_cast<T>(1.5), Policy());
         zr_boost_1_81::math::zeta(static_cast<T>(3.5), Policy());
         zr_boost_1_81::math::zeta(static_cast<T>(5.5), Policy());
         zr_boost_1_81::math::zeta(static_cast<T>(9.5), Policy());
         zr_boost_1_81::math::zeta(static_cast<T>(16.5), Policy());
         zr_boost_1_81::math::zeta(static_cast<T>(25.5), Policy());
         zr_boost_1_81::math::zeta(static_cast<T>(70.5), Policy());
...
inline typename tools::promote_args<T>::type zeta(T s, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::zeta_imp(
      ...
      forwarding_policy(),
      tag_type()), "zr_boost_1_81::math::zeta<%1%>(%1%)");
}
