...
{ // The function returns the natural logarithm of 1 + x.
   ...
   static const char* function = "zr_boost_1_81::math::log1p<%1%>(%1%)";

   if((x < -1) || (zr_boost_1_81::math::isnan)(x))
...
struct log1p_initializer
{
   struct init
   {
      ...
      {
         zr_boost_1_81::math::log1p(static_cast<T>(0.25), Policy());
      }
...
inline typename tools::promote_args<T>::type log1p(T x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      detail::log1p_imp(static_cast<value_type>(x), forwarding_policy(), tag_type()), "zr_boost_1_81::math::log1p<%1%>(%1%)");
}
...
inline float log1p(float x, const Policy& pol)
{
   return static_cast<float>(zr_boost_1_81::math::log1p(static_cast<double>(x), pol));
}
#ifndef _WIN32_WCE
//
...
{
   return zr_boost_1_81::math::log1p(x, policies::policy<>());
}
//
// Compute log(1+x)-x:
//
   ...
   log1pmx(T x, const Policy& pol)
{
   typedef typename tools::promote_args<T>::type result_type;
   BOOST_MATH_STD_USING
   static const char* function = "zr_boost_1_81::math::log1pmx<%1%>(%1%)";
   ...
   if(a < tools::epsilon<result_type>())
      return -x * x / 2;
   zr_boost_1_81::math::detail::log1p_series<T> s(x);
   s();
   std::uintmax_t max_iter = policies::get_max_series_iterations<Policy>();

   T result = zr_boost_1_81::math::tools::sum_series(s, policies::get_epsilon<T, Policy>(), max_iter);
