...
T expm1_imp(T x, const std::integral_constant<int, 0>&, const Policy& pol)
{
   ...
   T a = fabs(x);
   if((zr_boost_1_81::math::isnan)(a))
   {
      return policies::raise_domain_error<T>("zr_boost_1_81::math::expm1<%1%>(%1%)", "expm1 requires a finite argument, but got %1%", a, pol);
   }
   if(a > T(0.5f))
   {
      ...
      {
         if(x > 0)
            return policies::raise_overflow_error<T>("zr_boost_1_81::math::expm1<%1%>(%1%)", nullptr, pol);
         return -1;
      }
      return exp(x) - T(1);
   }
   ...
   policies::check_series_iterations<T>("zr_boost_1_81::math::expm1<%1%>(%1%)", max_iter, pol);
   return result;
}
...
inline typename tools::promote_args<T>::type expm1(T x, const Policy& /* pol */)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::expm1_imp(
      ...
      tag_type(), forwarding_policy()), "zr_boost_1_81::math::expm1<%1%>(%1%)");
}
