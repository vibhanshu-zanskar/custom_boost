...
#include <boost/math/tools/precision.hpp> // zr_boost_1_81::math::tools::max_value().
...
{
  BOOST_MATH_STD_USING
  static const T max_diff = zr_boost_1_81::math::tools::root_epsilon<T>() * fabs(w_est);
...
#ifndef BOOST_MATH_STANDALONE
   return zr_boost_1_81::lexical_cast<double>(z);
#else
...
inline T schroeder_update(const T w, const T y)
{
  ...
  using zr_boost_1_81::math::float_distance;
...
// (This is NOT used for 128-bit quad zr_boost_1_81::multiprecision::float128 type which required a suffix Q
...
inline T lambert_w0_small_z(T z, const Policy& pol, std::integral_constant<int, 5> const&)
{
  ...
  using zr_boost_1_81::math::policies::get_epsilon; // for type T.
  using zr_boost_1_81::math::tools::sum_series;
  using zr_boost_1_81::math::tools::evaluate_polynomial;
  ...
  policies::check_series_iterations<T>("zr_boost_1_81::math::lambert_w0_small_z<%1%>(%1%)", max_iter, pol);
...
{
   BOOST_MATH_STD_USING
   const T p2 = 2 * (zr_boost_1_81::math::constants::e<T>() * z + 1);
   const T p = sqrt(p2);
   return p;
}
template <typename T, typename Policy>
...
T lambert_w_positive_rational_float(T z)
{
   ...
   if (z < 2)
   {
      ...
      { // 0.05 < z < 0.5
        ...
        // Maximum Relative Change in Control Points : 7.555e-04 Y offset : -8.196592331e-01
         ...
         return z * (Y + zr_boost_1_81::math::tools::evaluate_polynomial(P, z) / zr_boost_1_81::math::tools::evaluate_polynomial(Q, z));
      }
      else
      { // 0.5 < z < 2
        // Max error in interpolated form: 1.018e-08
         ...
         return z * (Y + zr_boost_1_81::math::tools::evaluate_rational(P, Q, z));
      }
   }
   else if (z < 6)
   {
      ...
      return Y + zr_boost_1_81::math::tools::evaluate_rational(P, Q, z);
   }
   else if (z < 18)
   {
      // 6 < z < 18
   ...
   else if (z < 9897.12905874)  // 2.8 < log(z) < 9.2
   {
      ...
      T log_w = log(z);
      return log_w + Y + zr_boost_1_81::math::tools::evaluate_polynomial(P, log_w) / zr_boost_1_81::math::tools::evaluate_polynomial(Q, log_w);
   }
...
T lambert_w_negative_rational_float(T z, const Policy& pol)
{
   ...
   { // Very close to branch singularity.
     // Max error in interpolated form: 5.269e-08
      ...
      return -d / (Y + zr_boost_1_81::math::tools::evaluate_polynomial(P, d) / zr_boost_1_81::math::tools::evaluate_polynomial(Q, d));
   }
   else
   {
...
{
  static const char* function = "zr_boost_1_81::math::lambert_w0<%1%>"; // For error messages.
  BOOST_MATH_STD_USING // Aid ADL of std functions.

  if ((zr_boost_1_81::math::isnan)(z))
  {
    return zr_boost_1_81::math::policies::raise_domain_error<T>(function, "Expected a value > -e^-1 (-0.367879...) but got %1%.", z, pol);
  }
  if ((zr_boost_1_81::math::isinf)(z))
  {
    return zr_boost_1_81::math::policies::raise_overflow_error<T>(function, "Expected a finite value but got %1%.", z, pol);
  }
   ...
   else if (z <= -0.3678794411714423215955237701614608674458111310f)
   {
      if (z < -0.3678794411714423215955237701614608674458111310f)
         return zr_boost_1_81::math::policies::raise_domain_error<T>(function, "Expected z >= -e^-1 (-0.367879...) but got %1%.", z, pol);
      return -1;
   }
   else // z < 0.05
   {
...
T lambert_w_positive_rational_double(T z)
{
   ...
   if (z < 2)
   {
      if (z < 0.5)
      {
         ...
         return z * (offset + zr_boost_1_81::math::tools::evaluate_polynomial(P, z) / zr_boost_1_81::math::tools::evaluate_polynomial(Q, z));
      }
      else
      {
         ...
         return z * (offset + zr_boost_1_81::math::tools::evaluate_rational(P, Q, z));
      }
   }
   else if (z < 6)
   {
   ...
   else if (z < 18)
   {
      ...
      return offset + zr_boost_1_81::math::tools::evaluate_rational(P, Q, z);
   }
   else if (z < 9897.12905874)  // 2.8 < log(z) < 9.2
   {
      ...
      T log_w = log(z);
      return log_w + Y + zr_boost_1_81::math::tools::evaluate_rational(P, Q, log_w);
   }
   else if (z < 7.896296e+13)  // 9.2 < log(z) <= 32
   {
...
{
   static const char* function = "zr_boost_1_81::math::lambert_w0<%1%>";
   BOOST_MATH_STD_USING // Aid ADL of std functions.
...
inline T lambert_w0_imp(T z, const Policy& pol, const std::integral_constant<int, 0>&)
{
   ...
   { // Very close to branch point so rational polynomials are not usable.
      if (z <= -zr_boost_1_81::math::constants::exp_minus_one<T>())
      {
         if (z == -zr_boost_1_81::math::constants::exp_minus_one<T>())
...
T lambert_wm1_imp(const T z, const Policy&  pol)
{
  ...
  const char* function = "zr_boost_1_81::math::lambert_wm1<RealType>(<RealType>)"; // Used for error messages.
  ...
  { // All real types except arbitrary precision.
    if (!(zr_boost_1_81::math::isnormal)(z))
  ...
  if (z > -zr_boost_1_81::math::tools::min_value<T>())
  ...
  if (z == -zr_boost_1_81::math::constants::exp_minus_one<T>()) // == singularity/branch point z = -exp(-1) = -3.6787944.
  ...
  { // Close to singularity/branch point z = -0.3678794411714423215955237701614608727 but on W-1 branch.
    ...
    if (p2 > 0)
    {
      ...
      if (zr_boost_1_81::math::tools::digits<T>() > 53)
  ...
  { // T is more precise than 64-bit double (or long double, or ?),
    ...
    using zr_boost_1_81::math::policies::precision;
    using zr_boost_1_81::math::policies::digits10;
    using zr_boost_1_81::math::policies::digits2;
    using zr_boost_1_81::math::policies::policy;
  ...
  { // Use a lookup table to find the nearest integer part of Lambert W as starting point for Bisection.
    using namespace zr_boost_1_81::math::lambert_w_detail::lambert_w_lookup;
...
//! Lambert W0 using User-defined policy.
  template <typename T, typename Policy>
  inline
    typename zr_boost_1_81::math::tools::promote_args<T>::type
  ...
  lambert_w0_prime(T z, const Policy& pol)
  {
    ...
    if (z == - zr_boost_1_81::math::constants::exp_minus_one<result_type>())
    {
        return numeric_limits<result_type>::has_infinity ? numeric_limits<result_type>::infinity() : zr_boost_1_81::math::tools::max_value<result_type>();
    }
  ...
  lambert_wm1_prime(T z, const Policy& pol)
  {
    ...
    //}
    //if (z == - zr_boost_1_81::math::constants::exp_minus_one<result_type>())
    if (z == 0 || z == - zr_boost_1_81::math::constants::exp_minus_one<result_type>())
    {
        return numeric_limits<result_type>::has_infinity ? -numeric_limits<result_type>::infinity() : -zr_boost_1_81::math::tools::max_value<result_type>();
    }
