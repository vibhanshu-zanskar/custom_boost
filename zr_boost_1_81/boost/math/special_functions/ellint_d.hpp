...
T ellint_d_imp(T phi, T k, const Policy& pol)
{
    ...
    if(phi >= tools::max_value<T>())
    {
       ...
       result = policies::raise_overflow_error<T>("zr_boost_1_81::math::ellint_d<%1%>(%1%,%1%)", nullptr, pol);
    }
    ...
    else
    {
       ...
       if(k2 * sinp * sinp > 1)
       {
          return policies::raise_domain_error<T>("zr_boost_1_81::math::ellint_d<%1%>(%1%, %1%)", "The parameter k is out of range, got k = %1%", k, pol);
       }
       else if(rphi == 0)
       {
          result = 0;
...
T ellint_d_imp(T k, const Policy& pol)
{
    ...
    if (abs(k) >= 1)
    {
       return policies::raise_domain_error<T>("zr_boost_1_81::math::ellint_d<%1%>(%1%)",
...
inline typename tools::promote_args<T>::type ellint_d(T k, const Policy& pol, const std::true_type&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(detail::ellint_d_imp(static_cast<value_type>(k), pol), "zr_boost_1_81::math::ellint_d<%1%>(%1%)");
}
...
{
   return zr_boost_1_81::math::ellint_d(k, phi, policies::policy<>());
}
...
inline typename tools::promote_args<T1, T2>::type ellint_d(T1 k, T2 phi, const Policy& pol)
{
   ...
   typedef typename policies::evaluation<result_type, Policy>::type value_type;
   return policies::checked_narrowing_cast<result_type, Policy>(detail::ellint_d_imp(static_cast<value_type>(phi), static_cast<value_type>(k), pol), "zr_boost_1_81::math::ellint_2<%1%>(%1%,%1%)");
}
