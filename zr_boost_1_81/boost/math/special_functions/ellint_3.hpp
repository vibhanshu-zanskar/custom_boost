...
T ellint_pi_imp(T v, T phi, T k, T vc, const Policy& pol)
{
   ...
   static const char* function = "zr_boost_1_81::math::ellint_3<%1%>(%1%,%1%,%1%)";
   ...
   if((phi > constants::half_pi<T>()) || (phi < 0))
   {
      ...
      {
         T rphi = zr_boost_1_81::math::tools::fmod_workaround(T(fabs(phi)), T(constants::half_pi<T>()));
         T m = zr_boost_1_81::math::round((fabs(phi) - rphi) / constants::half_pi<T>());
         int sign = 1;
         if((m != 0) && (k >= 1))
         {
   ...
   if(k == 0)
   {
      ...
      else
      {
         ...
         T vcr = sqrt(-vc);
         T arg = vcr * tan(phi);
         return (zr_boost_1_81::math::log1p(arg, pol) - zr_boost_1_81::math::log1p(-arg, pol)) / (2 * vcr);
      }
   }
   if((v < 0) && fabs(k) <= 1)
   {
...
T ellint_pi_imp(T v, T k, T vc, const Policy& pol)
{
    ...
    static const char* function = "zr_boost_1_81::math::ellint_pi<%1%>(%1%,%1%)";
    ...
    if(v == 0)
    {
       return (k == 0) ? zr_boost_1_81::math::constants::pi<T>() / 2 : zr_boost_1_81::math::ellint_1(k, pol);
    }
    ...
    if(v < 0)
    {
       ...
       T Nm1 = (1 - k2) / (1 - v);
       T result = 0;
       result = zr_boost_1_81::math::detail::ellint_pi_imp(N, k, Nm1, pol);
       // This next part is split in two to avoid spurious over/underflow:
       result *= -v / (1 - v);
       result *= (1 - k2) / (k2 - v);
       result += zr_boost_1_81::math::ellint_1(k, pol) * k2 / (k2 - v);
       return result;
    }
...
{
   return zr_boost_1_81::math::ellint_3(k, v, phi, policies::policy<>());
}
...
inline typename tools::promote_args<T1, T2>::type ellint_3(T1 k, T2 v, const Policy& pol, const std::true_type&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(
      detail::ellint_pi_imp(
         ...
         pol), "zr_boost_1_81::math::ellint_3<%1%>(%1%,%1%)");
}
...
inline typename tools::promote_args<T1, T2, T3>::type ellint_3(T1 k, T2 v, T3 phi, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, Policy>(
      detail::ellint_pi_imp(
         ...
         static_cast<value_type>(k),
         static_cast<value_type>(1-v),
         forwarding_policy()), "zr_boost_1_81::math::ellint_3<%1%>(%1%,%1%,%1%)");
}
