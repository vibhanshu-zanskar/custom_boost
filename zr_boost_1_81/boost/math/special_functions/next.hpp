...
#endif
template <>
struct has_hidden_guard_digits<zr_boost_1_81::math::concepts::real_concept> : public std::false_type {};
template <>
struct has_hidden_guard_digits<zr_boost_1_81::math::concepts::std_real_concept> : public std::false_type {};
...
template <class T, bool b = zr_boost_1_81::math::tools::detail::has_backend_type<T>::value>
...
T float_next_imp(const T& val, const std::true_type&, const Policy& pol)
{
   ...
   int fpclass = (zr_boost_1_81::math::fpclassify)(val);
...
inline double float_next(const double& val, const Policy& pol)
{
   ...
   if(!(zr_boost_1_81::math::isfinite)(val) && (val > 0))
...
inline typename tools::promote_args<T, U>::type nextafter(const T& val, const U& direction, const Policy& pol)
{
   ...
   return val < direction ? zr_boost_1_81::math::float_next<result_type>(val, pol) : val == direction ? val : zr_boost_1_81::math::float_prior<result_type>(val, pol);
}
...
T float_distance_imp(const T& a, const T& b, const std::true_type&, const Policy& pol)
{
   ...
   if(!(zr_boost_1_81::math::isfinite)(a))
      return policies::raise_domain_error<T>(
         function,
         "Argument a must be finite, but got %1%", a, pol);
   if(!(zr_boost_1_81::math::isfinite)(b))
      return policies::raise_domain_error<T>(
         function,
   ...
   if(zr_boost_1_81::math::sign(a) != zr_boost_1_81::math::sign(b))
   ...
   //
   (void)frexp(((zr_boost_1_81::math::fpclassify)(a) == (int)FP_SUBNORMAL) ? tools::min_value<T>() : a, &expon);
   ...
   T mb, x, y, z;
   if(((zr_boost_1_81::math::fpclassify)(a) == (int)FP_SUBNORMAL) || (b - a < tools::min_value<T>()))
   {
      //
...
T float_distance_imp(const T& a, const T& b, const std::false_type&, const Policy& pol)
{
   ...
   // significant bits in the representation:
   //
   expon = 1 + ilogb(((zr_boost_1_81::math::fpclassify)(a) == (int)FP_SUBNORMAL) ? tools::min_value<T>() : a);
...
{
   return zr_boost_1_81::math::float_distance(a, b, policies::policy<>());
}
...
{
   return zr_boost_1_81::math::float_advance(val, distance, policies::policy<>());
}
