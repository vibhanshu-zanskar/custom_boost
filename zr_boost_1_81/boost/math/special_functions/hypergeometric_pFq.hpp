   ...
   namespace math {
      ...
      namespace detail {
         ...
         struct timed_iteration_terminator
         {
            ...
            {
               if (iter > max_iter)
                  BOOST_MATH_THROW_EXCEPTION(zr_boost_1_81::math::detail::pFq_termination_exception("pFq exceeded maximum permitted iterations."));
               if (std::chrono::duration<double>(std::chrono::system_clock::now() - start_time).count() > max_time)
                  BOOST_MATH_THROW_EXCEPTION(zr_boost_1_81::math::detail::pFq_termination_exception("pFq exceeded maximum permitted evaluation time."));
               return false;
            }
      ...
      inline typename tools::promote_args<Real, typename Seq::value_type>::type hypergeometric_pFq(const Seq& aj, const Seq& bj, const Real& z, Real* p_abs_error, const Policy& pol)
      {
         ...
         long long scale = 0;
         std::pair<value_type, value_type> r = zr_boost_1_81::math::detail::hypergeometric_pFq_checked_series_impl(aj, bj, value_type(z), pol, zr_boost_1_81::math::detail::iteration_terminator(zr_boost_1_81::math::policies::get_max_series_iterations<forwarding_policy>()), scale);
         r.first *= exp(Real(scale));
         r.second *= exp(Real(scale));
         if (p_abs_error)
            *p_abs_error = static_cast<Real>(r.second) * zr_boost_1_81::math::tools::epsilon<Real>();
         return policies::checked_narrowing_cast<result_type, Policy>(r.first, "zr_boost_1_81::math::hypergeometric_pFq<%1%>(%1%,%1%,%1%)");
      }
      ...
      {
         return hypergeometric_pFq(aj, bj, z, p_abs_error, zr_boost_1_81::math::policies::policy<>());
      }
      ...
      Real hypergeometric_pFq_precision(const Seq& aj, const Seq& bj, Real z, unsigned digits10, double timeout, const Policy& pol)
      {
         ...
         do
         {
            ...
            {
               long long scale = 0;
               std::pair<Real, Real> rp = zr_boost_1_81::math::detail::hypergeometric_pFq_checked_series_impl(aa, bb, z, pol, zr_boost_1_81::math::detail::timed_iteration_terminator(zr_boost_1_81::math::policies::get_max_series_iterations<Policy>(), timeout), scale);
               ...
               catch (const zr_boost_1_81::math::rounding_error&)
               {
            ...
            catch (const detail::pFq_termination_exception& e)
            {
               ...
               throw zr_boost_1_81::math::evaluation_error(e.what());
            }
         } while (true);
      ...
      {
         return hypergeometric_pFq_precision(aj, bj, z, digits10, timeout, zr_boost_1_81::math::policies::policy<>());
      }
      ...
      {
         return hypergeometric_pFq_precision< std::initializer_list<Real>, Real>(aj, bj, z, digits10, timeout, zr_boost_1_81::math::policies::policy<>());
      }
