...
T sinpx(T z)
{
   ...
   if(dist > 0.5)
      dist = 1 - dist;
   T result = sin(dist*zr_boost_1_81::math::constants::pi<T>());
...
T gamma_imp(T z, const Policy& pol, const Lanczos& l)
{
...
#endif
   static const char* function = "zr_boost_1_81::math::tgamma<%1%>(%1%)";
   ...
   if(z <= 0)
   {
      ...
      if(z <= -20)
      {
         ...
         if((fabs(result) < 1) && (tools::max_value<T>() * fabs(result) < zr_boost_1_81::math::constants::pi<T>()))
            return -zr_boost_1_81::math::sign(result) * policies::raise_overflow_error<T>(function, "Result of tgamma is too large to represent.", pol);
         result = -zr_boost_1_81::math::constants::pi<T>() / result;
         if(result == 0)
            return policies::raise_underflow_error<T>(function, "Result of tgamma is too small to represent.", pol);
         if((zr_boost_1_81::math::fpclassify)(result) == (int)FP_SUBNORMAL)
   ...
   else
   {
      ...
      T zgh = (z + static_cast<T>(Lanczos::g()) - zr_boost_1_81::math::constants::half<T>());
      T lzgh = log(zgh);
      BOOST_MATH_INSTRUMENT_VARIABLE(result);
      ...
      {
         BOOST_MATH_INSTRUMENT_VARIABLE(zgh);
         BOOST_MATH_INSTRUMENT_VARIABLE(pow(zgh, z - zr_boost_1_81::math::constants::half<T>()));
         BOOST_MATH_INSTRUMENT_VARIABLE(exp(zgh));
         result *= pow(zgh, z - zr_boost_1_81::math::constants::half<T>()) / exp(zgh);
         BOOST_MATH_INSTRUMENT_VARIABLE(result);
      }
   }
   return result;
...
T lgamma_imp(T z, const Policy& pol, const Lanczos& l, int* sign = nullptr)
{
   ...
   static const char* function = "zr_boost_1_81::math::lgamma<%1%>(%1%)";
   ...
   if(z <= -tools::root_epsilon<T>())
   {
      ...
      result = log(zr_boost_1_81::math::constants::pi<T>()) - lgamma_imp(z, pol, l) - log(t);
   }
   else if (z < tools::root_epsilon<T>())
   {
      if (0 == z)
   ...
   {
      // regular evaluation:
      T zgh = static_cast<T>(z + Lanczos::g() - zr_boost_1_81::math::constants::half<T>());
      result = log(zgh) - 1;
      result *= z - 0.5f;
      //
...
inline T upper_gamma_fraction(T a, T z, T eps)
{
   ...
   return 1 / (z - a + 1 + zr_boost_1_81::math::tools::continued_fraction_a(f, eps));
}
...
inline T lower_gamma_series(T a, T z, const Policy& pol, T init_value = 0)
{
   ...
   T result = zr_boost_1_81::math::tools::sum_series(s, factor, max_iter, init_value);
   policies::check_series_iterations<T>("zr_boost_1_81::math::detail::lower_gamma_series<%1%>(%1%)", max_iter, pol);
   return result;
}
...
std::size_t highest_bernoulli_index()
{
   const float digits10_of_type = (std::numeric_limits<T>::is_specialized
                                      ...
                                      : static_cast<float>(zr_boost_1_81::math::tools::digits<T>() * 0.301F));
...
int minimum_argument_for_bernoulli_recursion()
{
   ...
   const float digits10_of_type = (std::numeric_limits<T>::is_specialized
                                    ...
                                    : (float) (zr_boost_1_81::math::tools::digits<T>() * 0.301F));
   ...
   if(digits10_of_type < 50.0F)
   {
      ...
      // const float limit = std::ceil(std::pow(1.0f / std::ldexp(1.0f, 1-zr_boost_1_81::math::tools::digits<T>()), 1.0f / 20.0f));
...
T scaled_tgamma_no_lanczos(const T& z, const Policy& pol, bool islog = false)
{
   ...
   T sum = (zr_boost_1_81::math::bernoulli_b2n<T>(1) / 2) * one_over_x_pow_two_n_minus_one;
   const T target_epsilon_to_break_loop = sum * zr_boost_1_81::math::tools::epsilon<T>();
   const T half_ln_two_pi_over_z = sqrt(zr_boost_1_81::math::constants::two_pi<T>() / z);
   T last_term = 2 * sum;
   ...
   for (std::size_t n = 2U;; ++n)
   {
      ...
      const T term = (zr_boost_1_81::math::bernoulli_b2n<T>(static_cast<int>(n)) * one_over_x_pow_two_n_minus_one) / (n2 * (n2 - 1U));
...
T gamma_imp(T z, const Policy& pol, const lanczos::undefined_lanczos&)
{
   ...
   if((zr_boost_1_81::math::isnan)(z) || (is_at_zero) || ((zr_boost_1_81::math::isinf)(z) && (z < 0)))
   ...
   if((!b_neg) && floor_of_z_is_equal_to_z && (z < zr_boost_1_81::math::max_factorial<T>::value))
   {
      return zr_boost_1_81::math::unchecked_factorial<T>(itrunc(z) - 1);
   }
   ...
   {
      const T a0(1);
      const T a1(zr_boost_1_81::math::constants::euler<T>());
      const T six_euler_squared((zr_boost_1_81::math::constants::euler<T>() * zr_boost_1_81::math::constants::euler<T>()) * 6);
      const T a2((six_euler_squared -  zr_boost_1_81::math::constants::pi_sqr<T>()) / 12);
   ...
   {
      n_recur = zr_boost_1_81::math::itrunc(min_arg_for_recursion - zz) + 1;
   ...
   if(b_neg)
   {
      ...
      const bool result_is_too_large_to_represent = (   (abs(gamma_value) < 1)
                                                     && ((tools::max_value<T>() * abs(gamma_value)) < zr_boost_1_81::math::constants::pi<T>()));

      if(result_is_too_large_to_represent)
         return policies::raise_overflow_error<T>(function, "Result of tgamma is too large to represent.", pol);

      gamma_value = -zr_boost_1_81::math::constants::pi<T>() / gamma_value;
      BOOST_MATH_INSTRUMENT_VARIABLE(gamma_value);

      if(gamma_value == 0)
         return policies::raise_underflow_error<T>(function, "Result of tgamma is too small to represent.", pol);

      if((zr_boost_1_81::math::fpclassify)(gamma_value) == static_cast<int>(FP_SUBNORMAL))
...
inline T log_gamma_near_1(const T& z, Policy const& pol)
{
   ...
   do
   {
      term = power_term * zr_boost_1_81::math::polygamma(n - 1, T(1), pol);
      result += term;
      ++n;
      power_term *= z / n;
...
T lgamma_imp(T z, const Policy& pol, const lanczos::undefined_lanczos&, int* sign)
{
   ...
   if((!b_neg) && floor_of_z_is_equal_to_z && (z < zr_boost_1_81::math::max_factorial<T>::value))
   {
      if (sign)
         *sign = 1;
      return log(zr_boost_1_81::math::unchecked_factorial<T>(itrunc(z) - 1));
   }
   ...
   if(b_neg)
   {
      ...
      log_gamma_value = - log_gamma_value
                        + log(zr_boost_1_81::math::constants::pi<T>())
                        - log(t);
   }
...
T tgammap1m1_imp(T dz, Policy const& pol, const Lanczos& l)
{
   ...
   if(dz < 0)
   {
      if(dz < -0.5)
      {
         ...
         result = zr_boost_1_81::math::tgamma(1+dz, pol) - 1;
         BOOST_MATH_INSTRUMENT_CODE(result);
      }
      else
      {
         // Use expm1 on lgamma:
         result = zr_boost_1_81::math::expm1(-zr_boost_1_81::math::log1p(dz, pol)
   ...
   else
   {
      ...
      {
         // Use expm1 on lgamma:
         result = zr_boost_1_81::math::expm1(lgamma_small_imp<T>(dz+1, dz, dz-1, tag_type(), pol, l), pol);
         BOOST_MATH_INSTRUMENT_CODE(result);
      }
      else
      {
...
inline T tgammap1m1_imp(T z, Policy const& pol,
                 const ::zr_boost_1_81::math::lanczos::undefined_lanczos&)
{
   BOOST_MATH_STD_USING // ADL of std names

   if(fabs(z) < 0.55)
   {
      return zr_boost_1_81::math::expm1(log_gamma_near_1(z, pol));
   }
   return zr_boost_1_81::math::expm1(zr_boost_1_81::math::lgamma(1 + z, pol));
}
...
T full_igamma_prefix(T a, T z, const Policy& pol)
{
   ...
   //
   if((zr_boost_1_81::math::fpclassify)(prefix) == (int)FP_INFINITE)
      return policies::raise_overflow_error<T>("zr_boost_1_81::math::detail::full_igamma_prefix<%1%>(%1%, %1%)", "Result of incomplete gamma function is too large to represent.", pol);
...
T regularised_gamma_prefix(T a, T z, const Policy& pol, const Lanczos& l)
{
   ...
   else if((fabs(d*d*a) <= 100) && (a > 150))
   {
      ...
      prefix = a * zr_boost_1_81::math::log1pmx(d, pol) + z * static_cast<T>(0.5 - Lanczos::g()) / agh;
      prefix = exp(prefix);
   }
   else
   {
   ...
   prefix *= sqrt(agh / zr_boost_1_81::math::constants::e<T>()) / Lanczos::lanczos_sum_expG_scaled(a);
   return prefix;
}
//
...
T regularised_gamma_prefix(T a, T z, const Policy& pol, const lanczos::undefined_lanczos& l)
{
   ...
   if((a < 1) && (z < 1))
   {
      ...
      return pow(z, a) * exp(-z) / zr_boost_1_81::math::tgamma(a, pol);
   }
   ...
   else
   {
      ...
      else
      {
         ...
         if ((0 == power_term_1) || (0 == power_term_2) || (0 == power_term_3) || (fabs(a + shift - z) > tools::log_max_value<T>()))
         {
            ...
            return exp(a * log(z) - z - zr_boost_1_81::math::lgamma(a, pol));
         }
...
inline T tgamma_small_upper_part(T a, T x, const Policy& pol, T* pgam = 0, bool invert = false, T* pderivative = 0)
{
   ...
   //
   T result;
   result = zr_boost_1_81::math::tgamma1pm1(a, pol);
   if(pgam)
      *pgam = (result + 1) / a;
   T p = zr_boost_1_81::math::powm1(x, a, pol);
   result -= p;
   result /= a;
   ...
   result = -p * tools::sum_series(s, zr_boost_1_81::math::policies::get_epsilon<T, Policy>(), max_iter, (init_value - result) / p);
   policies::check_series_iterations<T>("zr_boost_1_81::math::tgamma_small_upper_part<%1%>(%1%, %1%)", max_iter, pol);
   if(invert)
      result = -result;
   return result;
}
...
inline T finite_gamma_q(T a, T x, Policy const& pol, T* pderivative = 0)
{
   ...
   if(pderivative)
   {
      *pderivative = e * pow(x, a) / zr_boost_1_81::math::unchecked_factorial<T>(itrunc(T(a - 1), pol));
   }
   return sum;
}
//
...
T finite_half_gamma_q(T a, T x, T* p_derivative, const Policy& pol)
{
   ...
   //
   BOOST_MATH_STD_USING
   T e = zr_boost_1_81::math::erfc(sqrt(x), pol);
   if((e != 0) && (a > 1))
   {
...
T incomplete_tgamma_large_x(const T& a, const T& x, const Policy& pol)
{
   ...
   std::uintmax_t max_iter = zr_boost_1_81::math::policies::get_max_series_iterations<Policy>();
   T result = zr_boost_1_81::math::tools::sum_series(s, zr_boost_1_81::math::policies::get_epsilon<T, Policy>(), max_iter);
   zr_boost_1_81::math::policies::check_series_iterations<T>("zr_boost_1_81::math::tgamma<%1%>(%1%,%1%)", max_iter, pol);
   return result;
}
...
{
   static const char* function = "zr_boost_1_81::math::gamma_p<%1%>(%1%, %1%)";
   if(a <= 0)
      ...
      else
      {
         ...
         if(result == 0)
         {
            if(invert)
            {
               ...
               result = log(result) - a + (a - 0.5f) * log(a) + log(zr_boost_1_81::math::constants::root_two_pi<T>());
               if(p_derivative)
         ...
         {
            result = log(result) + zr_boost_1_81::math::lgamma(a, pol);
         }
      }
   ...
   case 0:
      {
         ...
         if(!normalised)
            result *= zr_boost_1_81::math::tgamma(a, pol);
         break;
      }
   case 1:
      {
   ...
   case 2:
      {
         ...
         if(result != 0)
         {
            ...
            if(invert)
            {
               init_value = (normalised ? 1 : zr_boost_1_81::math::tgamma(a, pol));
   ...
   case 6:
      {
         ...
         else
         {
            ...
            {
#endif
               result = pow(x, a) / zr_boost_1_81::math::tgamma(a + 1, pol);
#ifndef BOOST_NO_EXCEPTIONS
            }
   ...
   if(invert)
   {
      T gam = normalised ? 1 : zr_boost_1_81::math::tgamma(a, pol);
      result = gam - result;
   }
   if(p_derivative)
   {
...
T tgamma_delta_ratio_imp_lanczos(T z, T delta, const Policy& pol, const Lanczos& l)
{
   ...
   if(z < tools::epsilon<T>())
   {
      ...
      //
      if(zr_boost_1_81::math::max_factorial<T>::value < delta)
      {
         T ratio = tgamma_delta_ratio_imp_lanczos(delta, T(zr_boost_1_81::math::max_factorial<T>::value - delta), pol, l);
         ratio *= z;
         ratio *= zr_boost_1_81::math::unchecked_factorial<T>(zr_boost_1_81::math::max_factorial<T>::value - 1);
         return 1 / ratio;
      }
      else
      {
         return 1 / (z * zr_boost_1_81::math::tgamma(z + delta, pol));
      }
   }
   ...
   if(z + delta == z)
   {
      if (fabs(delta / zgh) < zr_boost_1_81::math::tools::epsilon<T>())
      {
         // We have:
         // result = exp((constants::half<T>() - z) * zr_boost_1_81::math::log1p(delta / zgh, pol));
         // 0.5 - z == -z
...
T tgamma_delta_ratio_imp_lanczos(T z, T delta, const Policy& pol, const lanczos::undefined_lanczos& l)
{
   ...
   if (numerator_shift == 0 && denominator_shift == 0)
   {
      ...
      result *= exp(z * zr_boost_1_81::math::log1p(-delta / (z + delta), pol)) * pow((delta + z) / constants::e<T>(), -delta);
      return result;
   }
   //
...
T tgamma_delta_ratio_imp(T z, T delta, const Policy& pol)
{
   ...
   if((z <= 0) || (z + delta <= 0))
   {
      ...
      return zr_boost_1_81::math::tgamma(z, pol) / zr_boost_1_81::math::tgamma(z + delta, pol);
   }
...
T tgamma_ratio_imp(T x, T y, const Policy& pol)
{
   ...
   if((x <= 0) || (zr_boost_1_81::math::isinf)(x))
      return policies::raise_domain_error<T>("zr_boost_1_81::math::tgamma_ratio<%1%>(%1%, %1%)", "Gamma function ratios only implemented for positive arguments (got a=%1%).", x, pol);
   if((y <= 0) || (zr_boost_1_81::math::isinf)(y))
      return policies::raise_domain_error<T>("zr_boost_1_81::math::tgamma_ratio<%1%>(%1%, %1%)", "Gamma function ratios only implemented for positive arguments (got b=%1%).", y, pol);
   ...
   if((x < max_factorial<T>::value) && (y < max_factorial<T>::value))
   {
      ...
      return zr_boost_1_81::math::tgamma(x, pol) / zr_boost_1_81::math::tgamma(y, pol);
   }
   T prefix = 1;
   if(x < 1)
   {
      if(y < 2 * max_factorial<T>::value)
      {
         ...
         return prefix * zr_boost_1_81::math::tgamma(x, pol) / zr_boost_1_81::math::tgamma(y, pol);
      }
      //
      // result is almost certainly going to underflow to zero, try logs just in case:
      //
      return exp(zr_boost_1_81::math::lgamma(x, pol) - zr_boost_1_81::math::lgamma(y, pol));
   }
   if(y < 1)
   {
   ...
   //
   return zr_boost_1_81::math::tgamma_delta_ratio(x, y - x, pol);
}
...
T gamma_p_derivative_imp(T a, T x, const Policy& pol)
{
   ...
   //
   if(a <= 0)
      return policies::raise_domain_error<T>("zr_boost_1_81::math::gamma_p_derivative<%1%>(%1%, %1%)", "Argument a to the incomplete gamma function must be greater than zero (got a=%1%).", a, pol);
   if(x < 0)
      return policies::raise_domain_error<T>("zr_boost_1_81::math::gamma_p_derivative<%1%>(%1%, %1%)", "Argument x to the incomplete gamma function must be >= 0 (got x=%1%).", x, pol);
   //
   // Now special cases:
   //
   if(x == 0)
   {
      return (a > 1) ? 0 :
         (a == 1) ? 1 : policies::raise_overflow_error<T>("zr_boost_1_81::math::gamma_p_derivative<%1%>(%1%, %1%)", nullptr, pol);
   }
   //
   // Normal case:
   //
   ...
   {
      // overflow:
      return policies::raise_overflow_error<T>("zr_boost_1_81::math::gamma_p_derivative<%1%>(%1%, %1%)", nullptr, pol);
   }
   if(f1 == 0)
   {
   ...
   tgamma(T z, const Policy& /* pol */, const std::true_type)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::gamma_imp(static_cast<value_type>(z), forwarding_policy(), evaluation_type()), "zr_boost_1_81::math::tgamma<%1%>(%1%)");
}
...
struct igamma_initializer
{
   struct init
   {
      ...
      static void do_init(const std::integral_constant<int, N>&)
      {
         ...
         {
            zr_boost_1_81::math::gamma_p(static_cast<T>(400), static_cast<T>(400), Policy());
         }
      }
...
struct lgamma_initializer
{
   struct init
   {
      ...
      {
         zr_boost_1_81::math::lgamma(static_cast<T>(2.5), Policy());
         zr_boost_1_81::math::lgamma(static_cast<T>(1.25), Policy());
         zr_boost_1_81::math::lgamma(static_cast<T>(1.75), Policy());
      }
      static void do_init(const std::integral_constant<int, 113>&)
      {
         zr_boost_1_81::math::lgamma(static_cast<T>(2.5), Policy());
         zr_boost_1_81::math::lgamma(static_cast<T>(1.25), Policy());
         zr_boost_1_81::math::lgamma(static_cast<T>(1.5), Policy());
         zr_boost_1_81::math::lgamma(static_cast<T>(1.75), Policy());
      }
   ...
   tgamma(T1 a, T2 z, const Policy&, const std::false_type)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(
      ...
      forwarding_policy(), static_cast<value_type*>(nullptr)), "zr_boost_1_81::math::tgamma<%1%>(%1%, %1%)");
}
   ...
   lgamma(T z, int* sign, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::lgamma_imp(static_cast<value_type>(z), forwarding_policy(), evaluation_type(), sign), "zr_boost_1_81::math::lgamma<%1%>(%1%)");
}
...
{
   return ::zr_boost_1_81::math::lgamma(x, nullptr, pol);
}
...
inline typename tools::promote_args<T>::type
   lgamma(T x)
{
   return ::zr_boost_1_81::math::lgamma(x, nullptr, policies::policy<>());
}
   ...
   tgamma1pm1(T z, const Policy& /* pol */)
{
   ...
   return policies::checked_narrowing_cast<typename std::remove_cv<result_type>::type, forwarding_policy>(detail::tgammap1m1_imp(static_cast<value_type>(z), forwarding_policy(), evaluation_type()), "zr_boost_1_81::math::tgamma1pm1<%!%>(%1%)");
}
   ...
   tgamma_delta_ratio(T1 z, T2 delta, const Policy& /* pol */)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::tgamma_delta_ratio_imp(static_cast<value_type>(z), static_cast<value_type>(delta), forwarding_policy()), "zr_boost_1_81::math::tgamma_delta_ratio<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type
   ...
   tgamma_ratio(T1 a, T2 b, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::tgamma_ratio_imp(static_cast<value_type>(a), static_cast<value_type>(b), forwarding_policy()), "zr_boost_1_81::math::tgamma_delta_ratio<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type
   ...
   gamma_p_derivative(T1 a, T2 x, const Policy&)
{
   ...
   return policies::checked_narrowing_cast<result_type, forwarding_policy>(detail::gamma_p_derivative_imp(static_cast<value_type>(a), static_cast<value_type>(x), forwarding_policy()), "zr_boost_1_81::math::gamma_p_derivative<%1%>(%1%, %1%)");
}
template <class T1, class T2>
inline typename tools::promote_args<T1, T2>::type
