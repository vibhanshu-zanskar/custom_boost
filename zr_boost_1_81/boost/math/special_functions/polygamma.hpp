...
namespace zr_boost_1_81 { namespace math {
  ...
  inline typename tools::promote_args<T>::type polygamma(const int n, T x, const Policy& pol)
  {
     ...
     //
     if(n == 0)
        return zr_boost_1_81::math::digamma(x, pol);
     if(n == 1)
        return zr_boost_1_81::math::trigamma(x, pol);
     //
     ...
     return policies::checked_narrowing_cast<result_type, forwarding_policy>(
        ...
        "zr_boost_1_81::math::polygamma<%1%>(int, %1%)");
  }

  template<class T>
  inline typename tools::promote_args<T>::type polygamma(const int n, T x)
  {
      return zr_boost_1_81::math::polygamma(n, x, policies::policy<>());
  }
