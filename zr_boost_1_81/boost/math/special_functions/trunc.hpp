...
inline tools::promote_args_t<T> trunc(const T& v, const Policy& pol, const std::false_type&)
{
   ...
   {
      return policies::raise_rounding_error("zr_boost_1_81::math::trunc<%1%>(%1%)", nullptr, static_cast<result_type>(v), static_cast<result_type>(v), pol);
   }
...
inline int itrunc(const T& v, const Policy& pol)
{
   BOOST_MATH_STD_USING
   using result_type = tools::promote_args_t<T>;
   result_type r = zr_boost_1_81::math::trunc(v, pol);
   if(r > static_cast<result_type>((std::numeric_limits<int>::max)()) || r < static_cast<result_type>((std::numeric_limits<int>::min)()))
   {
      return static_cast<int>(policies::raise_rounding_error("zr_boost_1_81::math::itrunc<%1%>(%1%)", nullptr, static_cast<result_type>(v), 0, pol));
   }
...
inline long ltrunc(const T& v, const Policy& pol)
{
   ...
   {
      return static_cast<long>(policies::raise_rounding_error("zr_boost_1_81::math::ltrunc<%1%>(%1%)", nullptr, static_cast<result_type>(v), 0L, pol));
   }
...
inline long long lltrunc(const T& v, const Policy& pol)
{
   ...
   {
      return static_cast<long long>(policies::raise_rounding_error("zr_boost_1_81::math::lltrunc<%1%>(%1%)", nullptr, v, static_cast<long long>(0), pol));
   }
...
{
   using zr_boost_1_81::math::itrunc;
   return itrunc(v, pol);
}
...
{
   using zr_boost_1_81::math::ltrunc;
   return ltrunc(v, pol);
}
...
{
   using zr_boost_1_81::math::lltrunc;
