...
std::complex<T> acos(const std::complex<T>& z)
{
   ...
   static const T s_pi = zr_boost_1_81::math::constants::pi<T>();
...
#pragma warning(disable:4127)
   ...
   else if((zr_boost_1_81::math::isnan)(x))
   {
      if((zr_boost_1_81::math::isinf)(y))
         return std::complex<T>(x, ((zr_boost_1_81::math::signbit)(z.imag())) ? std::numeric_limits<T>::infinity() :  -std::numeric_limits<T>::infinity());
   ...
   else
   {
      ...
      //
      if((y == 0) && (x <= one))
         return std::complex<T>((x == 0) ? half_pi : std::acos(z.real()), (zr_boost_1_81::math::changesign)(z.imag()));
      //
      ...
      if((x < safe_max) && (x > safe_min) && (y < safe_max) && (y > safe_min))
      {
         ...
         if(a <= a_crossover)
         {
            ...
            imag = zr_boost_1_81::math::log1p(am1 + std::sqrt(am1 * (a + one)));
         }
         else
         {
      ...
      else
      {
         ...
         if(y <= (std::numeric_limits<T>::epsilon() * std::fabs(xm1)))
         {
            ...
            else
            {
               ...
               if(((std::numeric_limits<T>::max)() / xp1) > xm1)
               {
                  ...
                  imag = zr_boost_1_81::math::log1p(xm1 + std::sqrt(xp1*xm1));
               }
               else
               {
                  real = y / x;
         ...
         else if(x > one)
         {
            real = std::atan(y/x);
            T xoy = x/y;
            imag = log_two + std::log(y) + half * zr_boost_1_81::math::log1p(xoy*xoy);
         }
         else
         {
            real = half_pi;
            T a = std::sqrt(one + y*y);
            imag = half * zr_boost_1_81::math::log1p(static_cast<T>(2)*y*(y+a));
         }
      }
   }
