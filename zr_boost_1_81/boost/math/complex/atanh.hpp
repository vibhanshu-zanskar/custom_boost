...
std::complex<T> atanh(const std::complex<T>& z)
{
   ...
   static const T pi = zr_boost_1_81::math::constants::pi<T>();
   ...
   static const T log_two = zr_boost_1_81::math::constants::ln_two<T>();
...
#pragma warning(disable:4127)
   ...
   //
   if((zr_boost_1_81::math::isnan)(x))
   {
      if((zr_boost_1_81::math::isnan)(y))
         return std::complex<T>(x, x);
      else if((zr_boost_1_81::math::isinf)(y))
         return std::complex<T>(0, ((zr_boost_1_81::math::signbit)(z.imag()) ? -half_pi : half_pi));
      else
         return std::complex<T>(x, x);
   }
   else if((zr_boost_1_81::math::isnan)(y))
   {
      if(x == 0)
   ...
   else if((x > safe_lower) && (x < safe_upper) && (y > safe_lower) && (y < safe_upper))
   {
      ...
      // 
      real = zr_boost_1_81::math::log1p(four * x / (mxm1*mxm1 + yy));
      real /= four;
      if((zr_boost_1_81::math::signbit)(z.real()))
         real = (zr_boost_1_81::math::changesign)(real);

      imag = std::atan2((y * two), (mxm1*(one+x) - yy));
      imag /= two;
      if(z.imag() < 0)
         imag = (zr_boost_1_81::math::changesign)(imag);
   }
   else
   {
      //
      ...
      //    real = zr_boost_1_81::math::log1p(4x / ((x-1)^2 + y^2))
      ...
      if(x >= safe_upper)
      {
         ...
         if((zr_boost_1_81::math::isinf)(x) || (zr_boost_1_81::math::isinf)(y))
         {
            real = 0;
         }
         else if(y >= safe_upper)
         {
            ...
            real = zr_boost_1_81::math::log1p((four/y) / (x/y + y/x));
         }
         else if(y > one)
         {
            // Big x: divide through by x:
            real = zr_boost_1_81::math::log1p(four / (x + y*y/x));
         }
         else
         {
            // Big x small y, as above but neglect y^2/x:
            real = zr_boost_1_81::math::log1p(four/x);
         }
      }
      else if(y >= safe_upper)
      {
         if(x > one)
         {
            ...
            real = zr_boost_1_81::math::log1p((four*x/y) / (y + mxm1*mxm1/y));
         }
         else
         {
      ...
      else if (x != one)
      {
         ...
         if(y > safe_lower)
            div += y*y;
         real = zr_boost_1_81::math::log1p(four*x/div);
      }
      else
         real = zr_boost_1_81::math::changesign(two * (std::log(y) - log_two));
      ...
      imag /= two;
      if((zr_boost_1_81::math::signbit)(z.imag()))
         imag = (zr_boost_1_81::math::changesign)(imag);
   }
