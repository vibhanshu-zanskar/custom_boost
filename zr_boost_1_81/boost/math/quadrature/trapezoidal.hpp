...
namespace zr_boost_1_81{ namespace math{ namespace quadrature {
...
{
    static const char* function = "zr_boost_1_81::math::quadrature::trapezoidal<%1%>(F, %1%, %1%, %1%)";
    using std::abs;
    ...
    {
       return static_cast<K>(zr_boost_1_81::math::policies::raise_domain_error(function, "Left endpoint of integration must be finite for adaptive trapezoidal integration but got a = %1%.\n", a, pol));
    }
    if (!(zr_boost_1_81::math::isfinite)(b))
    {
       return static_cast<K>(zr_boost_1_81::math::policies::raise_domain_error(function, "Right endpoint of integration must be finite for adaptive trapezoidal integration but got b = %1%.\n", b, pol));
    }
...
auto trapezoidal(F f, Real a, Real b, Real tol = zr_boost_1_81::math::tools::root_epsilon<Real>(), std::size_t max_refinements = 12, Real* error_estimate = nullptr, Real* L1 = nullptr)->decltype(std::declval<F>()(std::declval<Real>()))
{
   return trapezoidal(f, a, b, tol, max_refinements, error_estimate, L1, zr_boost_1_81::math::policies::policy<>());
}
