...
namespace zr_boost_1_81 { namespace math { namespace quadrature { namespace detail {
...
std::pair<Real, Real> ooura_sin_node_and_weight(long n, Real h, Real alpha)
{
    ...
    if (n == 0) {
        ...
        Real weight = pi<Real>()*zr_boost_1_81::math::sin_pi(1/(eta_prime_0*h));
    ...
    if(eta > 1) {
        arg = n/( 1/exp_meta - 1 );
        s *= zr_boost_1_81::math::sin_pi(arg);
        if (n&1) {
            s *= -1;
        }
    }
...
std::pair<Real, Real> ooura_cos_node_and_weight(long n, Real h, Real alpha)
{
    ...
    if (eta < -1) {
        ...
        s *= zr_boost_1_81::math::cos_pi(arg);
    }
    else {
...
private:
    ...
    Real estimate_integral(F const & f, Real omega, size_t i) {
        ...
        //auto cond = zr_boost_1_81::math::tools::summation_condition_number<Real, true>(0);
        Real I0 = 0;
