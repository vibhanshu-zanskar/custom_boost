...
T chebyshev_coefficient(unsigned n, unsigned m)
{
   ...
   if(r & 1)
      result = -result;
   result /= n - r;
   result *= zr_boost_1_81::math::binomial_coefficient<T>(n - r, r);
   result *= ldexp(1.0f, m);
   return result;
}
...
public:
   ...
   polynomial(const polynomial<U>& p)
   {
      ...
      {
         m_data[i] = zr_boost_1_81::math::tools::real_cast<T>(p[i]);
      }
   }
#ifdef BOOST_MATH_HAS_IS_CONST_ITERABLE
    template <class Range, typename std::enable_if<zr_boost_1_81::math::tools::detail::is_const_iterable<Range>::value, bool>::type = true>
   ...
   {
      return m_data.size() > 0 ? zr_boost_1_81::math::tools::evaluate_polynomial((m_data).data(), z, m_data.size()) : T(0);
   }
...
polynomial<T> pow(polynomial<T> base, int exp)
{
    if (exp < 0)
        return policies::raise_domain_error(
                "zr_boost_1_81::math::tools::pow<%1%>",
