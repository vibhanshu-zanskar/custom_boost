...
namespace zr_boost_1_81{ namespace math{ namespace tools{
...
void bracket(F f, T& a, T& b, T c, T& fa, T& fb, T& d, T& fd)
{
   ...
   //
   if(zr_boost_1_81::math::sign(fa) * zr_boost_1_81::math::sign(fc) < 0)
   {
      d = b;
      fd = fb;
      b = c;
                           ...
                           unsigned count)
{
   ...
   //
   T c;
   if(zr_boost_1_81::math::sign(A) * zr_boost_1_81::math::sign(fa) > 0)
   {
      c = a;
   }
   else
...
std::pair<T, T> toms748_solve(F f, const T& ax, const T& bx, const T& fax, const T& fbx, Tol tol, std::uintmax_t& max_iter, const Policy& pol)
{
   ...
   static const char* function = "zr_boost_1_81::math::tools::toms748_solve<%1%>";
   ...
   b = bx;
   if(a >= b)
      return zr_boost_1_81::math::detail::pair_from_single(policies::raise_domain_error(
         function, 
   ...
   if(zr_boost_1_81::math::sign(fa) * zr_boost_1_81::math::sign(fb) > 0)
      return zr_boost_1_81::math::detail::pair_from_single(policies::raise_domain_error(
         function, 
...
{
   BOOST_MATH_STD_USING
   static const char* function = "zr_boost_1_81::math::tools::bracket_and_solve_root<%1%>";
   //
   ...
   if((fa < 0) == (guess < 0 ? !rising : rising))
   {
      ...
      {
         if(count == 0)
            return zr_boost_1_81::math::detail::pair_from_single(policies::raise_evaluation_error(function, "Unable to bracket root, last nearest value was %1%", b, pol));
         //
   ...
   else
   {
      ...
      while((zr_boost_1_81::math::sign)(fb) == (zr_boost_1_81::math::sign)(fa))
      {
         ...
         if(count == 0)
            return zr_boost_1_81::math::detail::pair_from_single(policies::raise_evaluation_error(function, "Unable to bracket root, last nearest value was %1%", a, pol));
         //
         // Heuristic: normally it's best not to increase the step sizes as we'll just end up
         // with a really wide range to search for the root.  However, if the initial guess was *really*
         // bad then we need to speed up the search otherwise we'll take forever if we're orders of
