...
std::pair<T, T> bisect(F f, T min, T max, Tol tol, std::uintmax_t& max_iter, const Policy& pol) noexcept(policies::is_noexcept_error_policy<Policy>::value&& BOOST_MATH_IS_FLOAT(T) && noexcept(std::declval<F>()(std::declval<T>())))
{
   ...
   // Error checking:
   //
   static const char* function = "zr_boost_1_81::math::tools::bisect<%1%>";
   if (min >= max)
   {
      return zr_boost_1_81::math::detail::pair_from_single(policies::raise_evaluation_error(function,
         "Arguments in wrong order in zr_boost_1_81::math::tools::bisect (first arg=%1%)", min, pol));
   }
   if (fmin * fmax >= 0)
   {
      return zr_boost_1_81::math::detail::pair_from_single(policies::raise_evaluation_error(function,
         "No change of sign in zr_boost_1_81::math::tools::bisect, either there is no root to find, or there are multiple roots in the interval (f(min) = %1%).", fmin, pol));
   }
...
T newton_raphson_iterate(F f, T guess, T min, T max, int digits, std::uintmax_t& max_iter) noexcept(policies::is_noexcept_error_policy<policies::policy<> >::value&& BOOST_MATH_IS_FLOAT(T) && noexcept(std::declval<F>()(std::declval<T>())))
{
   ...
   static const char* function = "zr_boost_1_81::math::tools::newton_raphson_iterate<%1%>";
   if (min > max)
   {
      return policies::raise_evaluation_error(function, "Range arguments in wrong order in zr_boost_1_81::math::tools::newton_raphson_iterate(first arg=%1%)", min, zr_boost_1_81::math::policies::policy<>());
   }
   ...
   do {
      ...
      {
         return policies::raise_evaluation_error(function, "There appears to be no root to be found in zr_boost_1_81::math::tools::newton_raphson_iterate, perhaps we have a local minima near current best guess of %1%", guess, zr_boost_1_81::math::policies::policy<>());
      }
...
namespace detail {
   ...
   T second_order_root_finder(F f, T guess, T min, T max, int digits, std::uintmax_t& max_iter) noexcept(policies::is_noexcept_error_policy<policies::policy<> >::value&& BOOST_MATH_IS_FLOAT(T) && noexcept(std::declval<F>()(std::declval<T>())))
   {
      BOOST_MATH_STD_USING
        ...
        << ", digits = " << digits << ", max_iter = " << max_iter << "\n";
#endif
      static const char* function = "zr_boost_1_81::math::tools::halley_iterate<%1%>";
      if (min >= max)
      {
         return policies::raise_evaluation_error(function, "Range arguments in wrong order in zr_boost_1_81::math::tools::halley_iterate(first arg=%1%)", min, zr_boost_1_81::math::policies::policy<>());
      }
...
std::pair<T, T> quadratic_roots_imp(T const& a, T const& b, T const& c)
{
   ...
   else if constexpr (zr_boost_1_81::math::tools::is_complex_type<T>::value)
   {
