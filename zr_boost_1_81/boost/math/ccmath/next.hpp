...
constexpr T normalize_value(const T& val, const std::true_type&) 
{
    ...
    std::intmax_t shift = static_cast<std::intmax_t>(std::numeric_limits<T>::digits) - static_cast<std::intmax_t>(zr_boost_1_81::math::ccmath::ilogb(val)) - 1;
    T result = zr_boost_1_81::math::ccmath::scalbn(val, shift);
    result = zr_boost_1_81::math::ccmath::round(result);
    return zr_boost_1_81::math::ccmath::scalbn(result, -shift); 
}
...
{
   return zr_boost_1_81::math::ccmath::ldexp(tools::min_value<T>(), tools::digits<T>() + 1);
}
...
constexpr T calc_min_shifted(const std::false_type&)
{
   ...
   return zr_boost_1_81::math::ccmath::scalbn(tools::min_value<T>(), std::numeric_limits<T>::digits + 1);
}
...
template <typename T, bool b = zr_boost_1_81::math::tools::detail::has_backend_type_v<T>>
...
constexpr T float_next_imp(const T& val, const std::true_type&)
{
    ...
    int fpclass = zr_boost_1_81::math::ccmath::fpclassify(val);
    ...
    if ((fpclass != FP_SUBNORMAL) && (fpclass != FP_ZERO) 
        && (zr_boost_1_81::math::ccmath::fabs(val) < detail::get_min_shift_value<T>()) 
        && (val != -tools::min_value<T>()))
    {
        ...
        //
        return zr_boost_1_81::math::ccmath::ldexp(zr_boost_1_81::math::ccmath::detail::float_next(static_cast<T>(zr_boost_1_81::math::ccmath::ldexp(val, 2 * tools::digits<T>()))), -2 * tools::digits<T>());
    }

    if (-0.5f == zr_boost_1_81::math::ccmath::frexp(val, &expon))
    {
        --expon; // reduce exponent when val is a power of two, and negative.
    }
    T diff = zr_boost_1_81::math::ccmath::ldexp(static_cast<T>(1), expon - tools::digits<T>());
    if(diff == 0)
    {
...
constexpr T float_next_imp(const T& val, const std::false_type&)
{
        ...
        && (val != -tools::min_value<T>()))
    {
        ...
        // This avoids issues with the Intel SSE2 registers when the FTZ or DAZ flags are set.
        //
        return zr_boost_1_81::math::ccmath::scalbn(zr_boost_1_81::math::ccmath::detail::float_next(static_cast<T>(zr_boost_1_81::math::ccmath::scalbn(val, 2 * std::numeric_limits<T>::digits))), -2 * std::numeric_limits<T>::digits);
    }

    expon = 1 + zr_boost_1_81::math::ccmath::ilogb(val);
    if(-1 == zr_boost_1_81::math::ccmath::scalbn(val, -expon) * std::numeric_limits<T>::radix)
    {
        --expon; // reduce exponent when val is a power of base, and negative.
    }

    T diff = zr_boost_1_81::math::ccmath::scalbn(static_cast<T>(1), expon - std::numeric_limits<T>::digits);
    if(diff == 0)
    {
        diff = detail::get_smallest_value<T>();
    }
...
constexpr T float_prior_imp(const T& val, const std::true_type&)
{
        ...
        && (val != tools::min_value<T>()))
    {
        ...
        // This avoids issues with the Intel SSE2 registers when the FTZ or DAZ flags are set.
        //
        return zr_boost_1_81::math::ccmath::ldexp(zr_boost_1_81::math::ccmath::detail::float_prior(static_cast<T>(zr_boost_1_81::math::ccmath::ldexp(val, 2 * tools::digits<T>()))), -2 * tools::digits<T>());
    }

    if(T remain = zr_boost_1_81::math::ccmath::frexp(val, &expon); remain == 0.5f)
    {
...
constexpr T float_prior_imp(const T& val, const std::false_type&)
{
        ...
        && (val != tools::min_value<T>()))
    {
        ...
        // This avoids issues with the Intel SSE2 registers when the FTZ or DAZ flags are set.
        //
        return zr_boost_1_81::math::ccmath::scalbn(zr_boost_1_81::math::ccmath::detail::float_prior(static_cast<T>(zr_boost_1_81::math::ccmath::scalbn(val, 2 * std::numeric_limits<T>::digits))), -2 * std::numeric_limits<T>::digits);
    }

    expon = 1 + zr_boost_1_81::math::ccmath::ilogb(val);
    
    if (T remain = zr_boost_1_81::math::ccmath::scalbn(val, -expon); remain * std::numeric_limits<T>::radix == 1)
    {
        --expon; // when val is a power of two we must reduce the exponent
    }
...
constexpr result_type nextafter(const T& val, const U& direction)
{
    ...
    {
        if (zr_boost_1_81::math::ccmath::isnan(val))
        {
            return val;
        }
        else if (zr_boost_1_81::math::ccmath::isnan(direction))
        {
            return direction;
        }
        else if (val < direction)
        {
            return zr_boost_1_81::math::ccmath::detail::float_next(val);
        }
        else if (val == direction)
        {
        ...
        return zr_boost_1_81::math::ccmath::detail::float_prior(val);
    }
    else
    {
        using std::nextafter;
...
{
    return zr_boost_1_81::math::ccmath::nextafter(val, direction);
}
...
constexpr return_type nexttoward(T val, long double direction)
{
    if (BOOST_MATH_IS_CONSTANT_EVALUATED(val))
    {
        return static_cast<return_type>(zr_boost_1_81::math::ccmath::nextafter(static_cast<result_type>(val), direction));
    }
    else
    {
...
{
    return zr_boost_1_81::math::ccmath::nexttoward(val, direction);
}
