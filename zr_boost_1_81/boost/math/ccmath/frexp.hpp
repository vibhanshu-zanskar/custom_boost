...
inline constexpr Real frexp(Real arg, int* exp)
{
    if(BOOST_MATH_IS_CONSTANT_EVALUATED(arg))
    {
        return arg == Real(0)  ? detail::frexp_zero_impl(arg, exp) : 
               ...
               zr_boost_1_81::math::ccmath::isinf(arg) ? detail::frexp_zero_impl(arg, exp) : 
               zr_boost_1_81::math::ccmath::isnan(arg) ? detail::frexp_zero_impl(arg, exp) :
               zr_boost_1_81::math::ccmath::detail::frexp_impl(arg, exp);
    }
    else
    {
        using std::frexp;
...
{
    return zr_boost_1_81::math::ccmath::frexp(static_cast<double>(arg), exp);
}

inline constexpr float frexpf(float arg, int* exp)
{
    return zr_boost_1_81::math::ccmath::frexp(arg, exp);
}
