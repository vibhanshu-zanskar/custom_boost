...
inline constexpr Real fmax(Real x, Real y) noexcept
{
    ...
    {
        return zr_boost_1_81::math::ccmath::isnan(x) && zr_boost_1_81::math::ccmath::isnan(y) ? std::numeric_limits<Real>::quiet_NaN() :
               zr_boost_1_81::math::ccmath::isnan(x) ? y :
               zr_boost_1_81::math::ccmath::isnan(y) ? x :
               zr_boost_1_81::math::ccmath::detail::fmax_impl(x, y);
    }
    else
    {
        using std::fmax;
...
inline constexpr auto fmax(T1 x, T2 y) noexcept
{
    if (BOOST_MATH_IS_CONSTANT_EVALUATED(x))
    {
        ...
        return zr_boost_1_81::math::ccmath::fmax(promoted_type(x), promoted_type(y));
    }
    else
    {
        using std::fmax;
...
{
    return zr_boost_1_81::math::ccmath::fmax(x, y);
}
