...
constexpr Real fma(Real x, Real y, Real z) noexcept
{
    ...
    {
        if (x == 0 && zr_boost_1_81::math::ccmath::isinf(y))
        {
            return std::numeric_limits<Real>::quiet_NaN();
        }
        else if (y == 0 && zr_boost_1_81::math::ccmath::isinf(x))
        {
            return std::numeric_limits<Real>::quiet_NaN();
        }
        ...
        else if (zr_boost_1_81::math::ccmath::isnan(z))
        {
            return std::numeric_limits<Real>::quiet_NaN();
        }

        return zr_boost_1_81::math::ccmath::detail::fma_imp(x, y, z);
    }
    else
    {
        using std::fma;
...
constexpr auto fma(T1 x, T2 y, T3 z) noexcept
{
    if (BOOST_MATH_IS_CONSTANT_EVALUATED(x))
    {
        ...
        return zr_boost_1_81::math::ccmath::fma(promoted_type(x), promoted_type(y), promoted_type(z));
    }
    else
    {
        using std::fma;
...
{
    return zr_boost_1_81::math::ccmath::fma(x, y, z);
}
