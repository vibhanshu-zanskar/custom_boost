...
inline constexpr Real ldexp(Real arg, int exp) noexcept
{
    if(BOOST_MATH_IS_CONSTANT_EVALUATED(arg))
    {
        return zr_boost_1_81::math::ccmath::abs(arg) == Real(0) ? arg :
               ...
               zr_boost_1_81::math::ccmath::detail::ldexp_impl(arg, exp);
    }
    else
    {
        using std::ldexp;
...
{
    return zr_boost_1_81::math::ccmath::ldexp(static_cast<double>(arg), exp);
}

inline constexpr float ldexpf(float arg, int exp) noexcept
{
    return zr_boost_1_81::math::ccmath::ldexp(arg, exp);
}
