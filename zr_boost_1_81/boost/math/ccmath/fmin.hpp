...
inline constexpr Real fmin(Real x, Real y) noexcept
{
    if (BOOST_MATH_IS_CONSTANT_EVALUATED(x))
    {
        return zr_boost_1_81::math::ccmath::isnan(x) && zr_boost_1_81::math::ccmath::isnan(y) ? std::numeric_limits<Real>::quiet_NaN() :
               ...
               zr_boost_1_81::math::ccmath::detail::fmin_impl(x, y);
    }
    else
    {
        using std::fmin;
...
inline constexpr auto fmin(T1 x, T2 y) noexcept
{
    if (BOOST_MATH_IS_CONSTANT_EVALUATED(x))
    {
        ...
        return zr_boost_1_81::math::ccmath::fmin(promoted_type(x), promoted_type(y));
    }
    else
    {
        using std::fmin;
...
{
    return zr_boost_1_81::math::ccmath::fmin(x, y);
}
