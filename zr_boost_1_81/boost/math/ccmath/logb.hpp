...
{
    int exp = 0;
    zr_boost_1_81::math::ccmath::frexp(arg, &exp);
...
inline constexpr Real logb(Real arg) noexcept
{
    ...
    {
        return zr_boost_1_81::math::ccmath::abs(arg) == Real(0) ? -std::numeric_limits<Real>::infinity() :
               zr_boost_1_81::math::ccmath::isinf(arg) ? std::numeric_limits<Real>::infinity() :
               zr_boost_1_81::math::ccmath::isnan(arg) ? std::numeric_limits<Real>::quiet_NaN() :
               zr_boost_1_81::math::ccmath::detail::logb_impl(arg);
    }
    else
    {
        using std::logb;
...
{
    return zr_boost_1_81::math::ccmath::logb(static_cast<double>(arg));
}

inline constexpr float logbf(float arg) noexcept
{
    return zr_boost_1_81::math::ccmath::logb(arg);
}
