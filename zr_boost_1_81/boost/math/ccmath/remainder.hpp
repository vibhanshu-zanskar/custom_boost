...
constexpr T remainder_impl(const T x, const T y)
{
    ...
    if (T fractional_part = zr_boost_1_81::math::ccmath::modf((x / y), &n); fractional_part > static_cast<T>(1.0/2))
    {
        ++n;
    }
...
constexpr Real remainder(Real x, Real y)
{
    ...
    {
        if (zr_boost_1_81::math::ccmath::isinf(x) && !zr_boost_1_81::math::ccmath::isnan(y))
        {
        ...
        return zr_boost_1_81::math::ccmath::detail::remainder_impl(x, y);
    }
    else
    {
        using std::remainder;
...
constexpr auto remainder(T1 x, T2 y)
{
    if (BOOST_MATH_IS_CONSTANT_EVALUATED(x))
    {
        ...
        return zr_boost_1_81::math::ccmath::remainder(promoted_type(x), promoted_type(y));
    }
    else
    {
        using std::remainder;
...
{
    return zr_boost_1_81::math::ccmath::remainder(x, y);
}
