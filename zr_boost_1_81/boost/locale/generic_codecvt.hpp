...
namespace zr_boost_1_81 { namespace locale {
...
#    define BOOST_LOCALE_DO_LENGTH_MBSTATE_CONST
    ...
    /// class latin1_codecvt :zr_boost_1_81::locale::generic_codecvt<CharType,latin1_codecvt<CharType> >
    /// {
    /// public:
    ///
    ///     /* Standard codecvt constructor */
    ///     latin1_codecvt(size_t refs = 0): zr_boost_1_81::locale::generic_codecvt<CharType,latin1_codecvt<CharType> >(refs)
    ///     {
    ///     }
    ///
    ...
    ///     }
    ///
    ///     zr_boost_1_81::locale::utf::code_point to_unicode(state_type &,char const *&begin,char const *end) const
    ///     {
    ///        if(begin == end)
    ///           return zr_boost_1_81::locale::utf::incomplete;
    ///        return *begin++;
    ///     }
    ///
    ///     zr_boost_1_81::locale::utf::code_point from_unicode(state_type &,zr_boost_1_81::locale::utf::code_point u,char *begin,char
    ///     const *end) const
    ///     {
    ///        if(u >= 256)
    ///           return zr_boost_1_81::locale::utf::illegal;
    ///        if(begin == end)
    ///           return zr_boost_1_81::locale::utf::incomplete;
    ///        *begin = u;
    ///        return 1;
    ...
    /// \code
    /// template<typename CharType>
    /// class icu_codecvt :zr_boost_1_81::locale::generic_codecvt<CharType,icu_codecvt<CharType> >
    /// {
    /// public:
    ///
    ///     /* Standard codecvt constructor */
    ///     icu_codecvt(std::string const &name,refs = 0):
    ///         zr_boost_1_81::locale::generic_codecvt<CharType,latin1_codecvt<CharType> >(refs)
    ///     { ... }
    ///
    ///     /* State is unused but required by generic_codecvt */
    ...
    ///     }
    ///
    ///     zr_boost_1_81::locale::utf::code_point to_unicode(state_type &ptr,char const *&begin,char const *end) const
    ///     {
    ///         UErrorCode err = U_ZERO_ERROR;
    ///         zr_boost_1_81::locale::utf::code_point cp = ucnv_getNextUChar(ptr.get(),&begin,end,&err);
    ///         ...
    ///     }
    ///     ...
    /// };
    ...
    protected:
        ...
        {
            zr_boost_1_81::uint16_t& state = *reinterpret_cast<zr_boost_1_81::uint16_t*>(&s);
#ifdef DEBUG_CODECVT
          ...
          size_t max) const override
        {
            ...
            zr_boost_1_81::uint16_t& state = *reinterpret_cast<zr_boost_1_81::uint16_t*>(&std_state);
#else
            size_t save_max = max;
            zr_boost_1_81::uint16_t state = *reinterpret_cast<const zr_boost_1_81::uint16_t*>(&std_state);
#endif
            ...
            while(max > 0 && from < from_end) {
                ...
                zr_boost_1_81::uint32_t ch = implementation().to_unicode(cvt_state, from, from_end);
                if(ch == zr_boost_1_81::locale::utf::incomplete || ch == zr_boost_1_81::locale::utf::illegal) {
                    from = prev_from;
                    break;
                }
                max--;
                                        ...
                                        uchar*& to_next) const override
        {
            ...
            while(to < to_end && from < from_end) {
                ...
                if(ch == zr_boost_1_81::locale::utf::illegal) {
                    from = from_saved;
                    r = std::codecvt_base::error;
                    break;
                }
                if(ch == zr_boost_1_81::locale::utf::incomplete) {
                    from = from_saved;
                ...
                if(ch <= 0xFFFF) {
                    *to++ = static_cast<uchar>(ch);
                    ...
                    ch -= 0x10000;
                    zr_boost_1_81::uint16_t w1 = static_cast<zr_boost_1_81::uint16_t>(0xD800 | (ch >> 10));
                    zr_boost_1_81::uint16_t w2 = static_cast<zr_boost_1_81::uint16_t>(0xDC00 | (ch & 0x3FF));
                    if(state == 0) {
                        from = from_saved;
                        *to++ = w1;
                        state = 1;
                                         ...
                                         char*& to_next) const override
        {
            ...
            while(to < to_end && from < from_end) {
                ...
                if(!zr_boost_1_81::locale::utf::is_valid_codepoint(ch)) {
                    r = std::codecvt_base::error;
                    break;
                }
                zr_boost_1_81::uint32_t len = implementation().from_unicode(cvt_state, ch, to, to_end);
                if(len == zr_boost_1_81::locale::utf::incomplete) {
                    r = std::codecvt_base::partial;
                    break;
                } else if(len == zr_boost_1_81::locale::utf::illegal) {
                    r = std::codecvt_base::error;
                    break;
                } else
                    to += len;
...
}} // namespace zr_boost_1_81::locale
