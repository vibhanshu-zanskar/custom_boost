...
class handler
{
    zr_boost_1_81::optional<error_code> ec_;
    bool pass_ = false;
    zr_boost_1_81::source_location loc_{BOOST_CURRENT_LOCATION};
public:
    handler(zr_boost_1_81::source_location loc = BOOST_CURRENT_LOCATION) : loc_(loc) {}

    explicit
    handler(error_code ec, zr_boost_1_81::source_location loc = BOOST_CURRENT_LOCATION)
        : ec_(ec), loc_(loc)
    {
    }

    explicit
    handler(zr_boost_1_81::none_t, zr_boost_1_81::source_location loc = BOOST_CURRENT_LOCATION) : loc_(loc)
    {
    }

    handler(handler&& other,zr_boost_1_81::source_location loc = BOOST_CURRENT_LOCATION)
        : ec_(other.ec_)
        , pass_(zr_boost_1_81::exchange(other.pass_, true))
        , loc_(loc)
    ...
    ~handler()
    {
        ::zr_boost_1_81::beast::unit_test::suite::this_suite()->expect(pass_, loc_.file_name(), loc_.line());
    }
    ...
    operator()(error_code ec, Args&&...)
    {
        ::zr_boost_1_81::beast::unit_test::suite::this_suite()->expect(!pass_, loc_.file_name(), loc_.line());
        if (ec_ && ec != *ec_)
            ::zr_boost_1_81::beast::unit_test::suite::this_suite()->fail(ec.message(), loc_.file_name(), loc_.line());
        else
            ::zr_boost_1_81::beast::unit_test::suite::this_suite()->pass();
        pass_ = true;
    }
    ...
    operator()()
    {
        ::zr_boost_1_81::beast::unit_test::suite::this_suite()->expect(!pass_, loc_.file_name(), loc_.line());
        if (ec_ && ec_->failed())
            ::zr_boost_1_81::beast::unit_test::suite::this_suite()->fail(ec_->message(), loc_.file_name(), loc_.line());
        else
            ::zr_boost_1_81::beast::unit_test::suite::this_suite()->pass();
...
inline
handler
success_handler(zr_boost_1_81::source_location loc = BOOST_CURRENT_LOCATION) noexcept
{
...
inline
handler
any_handler(zr_boost_1_81::source_location loc = BOOST_CURRENT_LOCATION) noexcept
{
    return handler(zr_boost_1_81::none, loc);
}
...
inline
handler
fail_handler(error_code ec,zr_boost_1_81::source_location loc = BOOST_CURRENT_LOCATION) noexcept
{
