...
teardown(
    role_type role,
    zr_boost_1_81::asio::ssl::stream<AsyncStream>& stream,
    error_code& ec)
{
    stream.shutdown(ec);
...
struct ssl_shutdown_op
    : zr_boost_1_81::asio::coroutine
{
    ssl_shutdown_op(
        zr_boost_1_81::asio::ssl::stream<AsyncStream>& s,
        role_type role)
        : s_(s)
        , role_(role)
    {
...
private:
    zr_boost_1_81::asio::ssl::stream<AsyncStream>& s_;
    role_type role_;
    error_code ec_;
};
...
{
    return zr_boost_1_81::asio::async_compose<TeardownHandler, void(error_code)>(
