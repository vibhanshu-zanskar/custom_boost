...
struct stream<NextLayer, deflateSupported>::impl_type
    : zr_boost_1_81::empty_value<NextLayer>
    , detail::service::impl_type
    , detail::impl_base<deflateSupported>
{
    NextLayer& stream() noexcept
    {
        return this->zr_boost_1_81::empty_value<
            NextLayer>::get();
    }

    zr_boost_1_81::weak_ptr<impl_type>
    weak_from_this()
    {
        return zr_boost_1_81::static_pointer_cast<
            impl_type>(this->detail::service::
                impl_type::shared_from_this());
    }

    zr_boost_1_81::shared_ptr<impl_type>
    shared_this()
    {
        return zr_boost_1_81::static_pointer_cast<
            impl_type>(this->detail::service::
    ...
    impl_type(Args&&... args)
        : zr_boost_1_81::empty_value<NextLayer>(
            zr_boost_1_81::empty_init_t{},
            std::forward<Args>(args)...)
        , detail::service::impl_type(
            this->get_context(
                this->zr_boost_1_81::empty_value<NextLayer>::get().get_executor()))
        , timer(this->zr_boost_1_81::empty_value<NextLayer>::get().get_executor())
    {
    ...
    begin_msg(std::size_t n_bytes)
    {
            ...
            role == role_type::client)
        {
            ...
            {
                wr_buf_size = wr_buf_opt;
                wr_buf = zr_boost_1_81::make_unique_noinit<
...
private:
    ...
    class timeout_handler
        : zr_boost_1_81::empty_value<Executor>
    {
        zr_boost_1_81::weak_ptr<impl_type> wp_;

    public:
        timeout_handler(
            Executor const& ex,
            zr_boost_1_81::weak_ptr<impl_type>&& wp)
            : zr_boost_1_81::empty_value<Executor>(
                zr_boost_1_81::empty_init_t{}, ex)
            , wp_(std::move(wp))
        {
        }
...
write_close(DynamicBuffer& db, close_reason const& cr)
{
    using namespace zr_boost_1_81::endian;
    detail::frame_header fh;
