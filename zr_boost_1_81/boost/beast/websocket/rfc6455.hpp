...
/** Returns `true` if the specified HTTP request is a WebSocket Upgrade.
    ...
    {
        zr_boost_1_81::beast::flat_buffer buffer;
        zr_boost_1_81::beast::http::request<zr_boost_1_81::beast::http::string_body> req;
        zr_boost_1_81::beast::http::read(sock, buffer, req);
        if(zr_boost_1_81::beast::websocket::is_upgrade(req))
        {
            zr_boost_1_81::beast::websocket::stream<decltype(sock)> ws{std::move(sock)};
            ws.accept(req);
        }
    }
    @endcode
