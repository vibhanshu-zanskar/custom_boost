...
/// \brief Provides methods for getting acceptable by zr_boost_1_81::dll::shared_library location of symbol, source line or program.
...
namespace detail {
    inline zr_boost_1_81::dll::fs::path program_location_impl(zr_boost_1_81::dll::fs::error_code& ec) {
        return zr_boost_1_81::dll::detail::path_from_handle(NULL, ec);
    }
} // namespace detail
#endif
    ...
    */
    template <class T>
    inline zr_boost_1_81::dll::fs::path symbol_location_ptr(T ptr_to_symbol, zr_boost_1_81::dll::fs::error_code& ec) {
        BOOST_STATIC_ASSERT_MSG(zr_boost_1_81::is_pointer<T>::value, "zr_boost_1_81::dll::symbol_location_ptr works only with pointers! `ptr_to_symbol` must be a pointer");
        zr_boost_1_81::dll::fs::path ret;
        if (!ptr_to_symbol) {
            ec = zr_boost_1_81::dll::fs::make_error_code(
                zr_boost_1_81::dll::fs::errc::bad_address
            );
        ...
        const void* ptr = zr_boost_1_81::dll::detail::aggressive_ptr_cast<const void*>(ptr_to_symbol);

#if BOOST_OS_WINDOWS
        zr_boost_1_81::winapi::MEMORY_BASIC_INFORMATION_ mbi;
        if (!zr_boost_1_81::winapi::VirtualQuery(ptr, &mbi, sizeof(mbi))) {
            ec = zr_boost_1_81::dll::detail::last_error_code();
            return ret;
        }

        return zr_boost_1_81::dll::detail::path_from_handle(reinterpret_cast<zr_boost_1_81::winapi::HMODULE_>(mbi.AllocationBase), ec);
#else
        Dl_info info;
    ...
    //! \overload symbol_location_ptr(const void* ptr_to_symbol, zr_boost_1_81::dll::fs::error_code& ec)
    template <class T>
    inline zr_boost_1_81::dll::fs::path symbol_location_ptr(T ptr_to_symbol) {
        zr_boost_1_81::dll::fs::path ret;
        zr_boost_1_81::dll::fs::error_code ec;
        ret = zr_boost_1_81::dll::symbol_location_ptr(ptr_to_symbol, ec);

        if (ec) {
            zr_boost_1_81::dll::detail::report_error(ec, "zr_boost_1_81::dll::symbol_location_ptr(T ptr_to_symbol) failed");
        }
    ...
    */
    template <class T>
    inline zr_boost_1_81::dll::fs::path symbol_location(const T& symbol, zr_boost_1_81::dll::fs::error_code& ec) {
        ec.clear();
        return zr_boost_1_81::dll::symbol_location_ptr(
            zr_boost_1_81::dll::detail::aggressive_ptr_cast<const void*>(zr_boost_1_81::addressof(symbol)),
            ec
        );
    }
    ...
    template <class T>
    inline zr_boost_1_81::dll::fs::path symbol_location(const T& symbol, const char* /*workaround*/ = 0)
#else
    //! \overload symbol_location(const T& symbol, zr_boost_1_81::dll::fs::error_code& ec)
    template <class T>
    inline zr_boost_1_81::dll::fs::path symbol_location(const T& symbol)
#endif
    {
        zr_boost_1_81::dll::fs::path ret;
        zr_boost_1_81::dll::fs::error_code ec;
        ret = zr_boost_1_81::dll::symbol_location_ptr(
            zr_boost_1_81::dll::detail::aggressive_ptr_cast<const void*>(zr_boost_1_81::addressof(symbol)),
            ec
        );

        if (ec) {
            zr_boost_1_81::dll::detail::report_error(ec, "zr_boost_1_81::dll::symbol_location(const T& symbol) failed");
        }
    ...
    * \throws std::bad_alloc in case of insufficient memory. Overload that does not accept \forcedlinkfs{error_code} also throws \forcedlinkfs{system_error}.
    */
    static inline zr_boost_1_81::dll::fs::path this_line_location(zr_boost_1_81::dll::fs::error_code& ec) {
        typedef zr_boost_1_81::dll::fs::path(func_t)(zr_boost_1_81::dll::fs::error_code& );
        func_t& f = this_line_location;
        return zr_boost_1_81::dll::symbol_location(f, ec);
    }

    //! \overload this_line_location(zr_boost_1_81::dll::fs::error_code& ec)
    static inline zr_boost_1_81::dll::fs::path this_line_location() {
        zr_boost_1_81::dll::fs::path ret;
        zr_boost_1_81::dll::fs::error_code ec;
        ret = this_line_location(ec);

        if (ec) {
            zr_boost_1_81::dll::detail::report_error(ec, "zr_boost_1_81::dll::this_line_location() failed");
        }
    ...
    * \throws std::bad_alloc in case of insufficient memory. Overload that does not accept \forcedlinkfs{error_code} also throws \forcedlinkfs{system_error}.
    */
    inline zr_boost_1_81::dll::fs::path program_location(zr_boost_1_81::dll::fs::error_code& ec) {
        ec.clear();
        return zr_boost_1_81::dll::detail::program_location_impl(ec);
    }

    //! \overload program_location(zr_boost_1_81::dll::fs::error_code& ec) {
    inline zr_boost_1_81::dll::fs::path program_location() {
        zr_boost_1_81::dll::fs::path ret;
        zr_boost_1_81::dll::fs::error_code ec;
        ret = zr_boost_1_81::dll::detail::program_location_impl(ec);

        if (ec) {
            zr_boost_1_81::dll::detail::report_error(ec, "zr_boost_1_81::dll::program_location() failed");
        }
