...
destructor<Class> load_dtor(Lib & lib, const mangled_storage_impl::dtor_sym & dt) {
    ...
    //standard_t dtor = &lib.template get< typename zr_boost_1_81::remove_pointer<standard_t>::type >(dt);
...
destructor<Class> load_dtor(Lib & lib, const mangled_storage_impl::dtor_sym & dt) {
    ...
    //see here for the abi http://mentorembedded.github.io/cxx-abi/abi.html#mangling-special-ctor-dtor
    if (!dt.D1.empty()) {
        s = &lib.template get< typename zr_boost_1_81::remove_pointer<stand>::type >(dt.D1);
    }

    if (!dt.D0.empty()) {
        d = &lib.template get< typename zr_boost_1_81::remove_pointer<delet>::type >(dt.D0);
    }
