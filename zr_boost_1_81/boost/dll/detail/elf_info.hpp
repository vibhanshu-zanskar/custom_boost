...
struct Elf_Ehdr_template {
  ...
  zr_boost_1_81::uint16_t   e_type;         /* Object file type */
  zr_boost_1_81::uint16_t   e_machine;      /* Architecture */
  zr_boost_1_81::uint32_t   e_version;      /* Object file version */
  ...
  zr_boost_1_81::uint32_t   e_flags;        /* Processor-specific flags */
  zr_boost_1_81::uint16_t   e_ehsize;       /* ELF header size in bytes */
  zr_boost_1_81::uint16_t   e_phentsize;    /* Program header table entry size */
  zr_boost_1_81::uint16_t   e_phnum;        /* Program header table entry count */
  zr_boost_1_81::uint16_t   e_shentsize;    /* Section header table entry size */
  zr_boost_1_81::uint16_t   e_shnum;        /* Section header table entry count */
  zr_boost_1_81::uint16_t   e_shstrndx;     /* Section header string table index */
};

typedef Elf_Ehdr_template<zr_boost_1_81::uint32_t> Elf32_Ehdr_;
typedef Elf_Ehdr_template<zr_boost_1_81::uint64_t> Elf64_Ehdr_;

template <class AddressOffsetT>
struct Elf_Shdr_template {
  zr_boost_1_81::uint32_t   sh_name;        /* Section name (string tbl index) */
  zr_boost_1_81::uint32_t   sh_type;        /* Section type */
  ...
  zr_boost_1_81::uint32_t   sh_link;        /* Link to another section */
  zr_boost_1_81::uint32_t   sh_info;        /* Additional section information */
  AddressOffsetT    sh_addralign;   /* Section alignment */
  AddressOffsetT    sh_entsize;     /* Entry size if section holds table */
};

typedef Elf_Shdr_template<zr_boost_1_81::uint32_t> Elf32_Shdr_;
typedef Elf_Shdr_template<zr_boost_1_81::uint64_t> Elf64_Shdr_;

template <class AddressOffsetT>
struct Elf_Sym_template;

template <>
struct Elf_Sym_template<zr_boost_1_81::uint32_t> {
  typedef zr_boost_1_81::uint32_t AddressOffsetT;

  zr_boost_1_81::uint32_t   st_name;    /* Symbol name (string tbl index) */
  AddressOffsetT    st_value;   /* Symbol value */
  AddressOffsetT    st_size;    /* Symbol size */
  unsigned char     st_info;    /* Symbol type and binding */
  unsigned char     st_other;   /* Symbol visibility */
  zr_boost_1_81::uint16_t   st_shndx;   /* Section index */
};

template <>
struct Elf_Sym_template<zr_boost_1_81::uint64_t> {
  typedef zr_boost_1_81::uint64_t AddressOffsetT;
...
typedef Elf_Sym_template<zr_boost_1_81::uint32_t> Elf32_Sym_;
typedef Elf_Sym_template<zr_boost_1_81::uint64_t> Elf64_Sym_;

template <class AddressOffsetT>
class elf_info {
    typedef zr_boost_1_81::dll::detail::Elf_Ehdr_template<AddressOffsetT>  header_t;
    typedef zr_boost_1_81::dll::detail::Elf_Shdr_template<AddressOffsetT>  section_t;
    typedef zr_boost_1_81::dll::detail::Elf_Sym_template<AddressOffsetT>   symbol_t;

    BOOST_STATIC_CONSTANT(zr_boost_1_81::uint32_t, SHT_SYMTAB_ = 2);
    BOOST_STATIC_CONSTANT(zr_boost_1_81::uint32_t, SHT_STRTAB_ = 3);
    BOOST_STATIC_CONSTANT(zr_boost_1_81::uint32_t, SHT_DYNSYM_ = 11);
...
public:
    static bool parsing_supported(std::ifstream& fs) {
        const unsigned char magic_bytes[5] = { 
            0x7f, 'E', 'L', 'F', sizeof(zr_boost_1_81::uint32_t) == sizeof(AddressOffsetT) ? 1 : 2
        };
...
private:
    ...
    static void checked_seekg(std::ifstream& fs, Integer pos) {
        ...
        if ((std::numeric_limits<std::streamoff>::max)() < pos) {
            zr_boost_1_81::throw_exception(std::runtime_error("Integral overflow while getting info from ELF file"));
        }
        if ((std::numeric_limits<std::streamoff>::min)() > pos){
            zr_boost_1_81::throw_exception(std::runtime_error("Integral underflow while getting info from ELF file"));
        }
        */
...
typedef elf_info<zr_boost_1_81::uint32_t> elf_info32;
typedef elf_info<zr_boost_1_81::uint64_t> elf_info64;
