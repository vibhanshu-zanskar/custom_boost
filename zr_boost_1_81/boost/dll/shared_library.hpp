...
/// \brief Contains the zr_boost_1_81::dll::shared_library class, core class for all the
/// DLL/DSO operations.
...
/// @cond
    : private zr_boost_1_81::dll::detail::shared_library_impl
/// @endcond
{
    typedef zr_boost_1_81::dll::detail::shared_library_impl base_t;
...
public:
    ...
    */
    shared_library(const shared_library& lib, zr_boost_1_81::dll::fs::error_code& ec)
        : base_t()
    {
        assign(lib, ec);
    }
    ...
    */
    shared_library(BOOST_RV_REF(shared_library) lib) BOOST_NOEXCEPT
        : base_t(zr_boost_1_81::move(static_cast<base_t&>(lib)))
    {}
    ...
    * \throw \forcedlinkfs{system_error}, std::bad_alloc in case of insufficient memory.
    */
    explicit shared_library(const zr_boost_1_81::dll::fs::path& lib_path, load_mode::type mode = load_mode::default_mode) {
    ...
    * \throw std::bad_alloc in case of insufficient memory.
    */
    shared_library(const zr_boost_1_81::dll::fs::path& lib_path, zr_boost_1_81::dll::fs::error_code& ec, load_mode::type mode = load_mode::default_mode) {
        shared_library::load(lib_path, mode, ec);
    }

    //! \overload shared_library(const zr_boost_1_81::dll::fs::path& lib_path, zr_boost_1_81::dll::fs::error_code& ec, load_mode::type mode = load_mode::default_mode)
    shared_library(const zr_boost_1_81::dll::fs::path& lib_path, load_mode::type mode, zr_boost_1_81::dll::fs::error_code& ec) {
        shared_library::load(lib_path, mode, ec);
    }
    ...
    */
    shared_library& operator=(BOOST_COPY_ASSIGN_REF(shared_library) lib) {
        zr_boost_1_81::dll::fs::error_code ec;
        assign(lib, ec);
        if (ec) {
            zr_boost_1_81::dll::detail::report_error(ec, "zr_boost_1_81::dll::shared_library::operator= failed");
        }
    ...
    * \throw std::bad_alloc in case of insufficient memory.
    */
    shared_library& assign(const shared_library& lib, zr_boost_1_81::dll::fs::error_code& ec) {
        ec.clear();
        ...
        zr_boost_1_81::dll::fs::path loc = lib.location(ec);
        if (ec) {
            return *this;
        }
    ...
    */
    shared_library& assign(const shared_library& lib) {
        ...
        assign(lib, ec);
        if (ec) {
            zr_boost_1_81::dll::detail::report_error(ec, "zr_boost_1_81::dll::shared_library::assign() failed");
        }
    ...
    *
    */
    void load(const zr_boost_1_81::dll::fs::path& lib_path, load_mode::type mode = load_mode::default_mode) {
        zr_boost_1_81::dll::fs::error_code ec;

        base_t::load(lib_path, mode, ec);

        if (ec) {
            zr_boost_1_81::dll::detail::report_error(ec, "zr_boost_1_81::dll::shared_library::load() failed");
        }
    }
    ...
    * \throw std::bad_alloc in case of insufficient memory.
    */
    void load(const zr_boost_1_81::dll::fs::path& lib_path, zr_boost_1_81::dll::fs::error_code& ec, load_mode::type mode = load_mode::default_mode) {
        ec.clear();
        base_t::load(lib_path, mode, ec);
    }

    //! \overload void load(const zr_boost_1_81::dll::fs::path& lib_path, zr_boost_1_81::dll::fs::error_code& ec, load_mode::type mode = load_mode::default_mode)
    void load(const zr_boost_1_81::dll::fs::path& lib_path, load_mode::type mode, zr_boost_1_81::dll::fs::error_code& ec) {
        ec.clear();
        base_t::load(lib_path, mode, ec);
    }
    ...
    */
    template <typename T>
    inline typename zr_boost_1_81::enable_if_c<zr_boost_1_81::is_member_pointer<T>::value || zr_boost_1_81::is_reference<T>::value, T>::type  get(const std::string& symbol_name) const {
        return get<T>(symbol_name.c_str());
    }

    //! \overload T& get(const std::string& symbol_name) const
    template <typename T>
    inline typename zr_boost_1_81::disable_if_c<zr_boost_1_81::is_member_pointer<T>::value || zr_boost_1_81::is_reference<T>::value, T&>::type get(const std::string& symbol_name) const {
        return get<T>(symbol_name.c_str());
    }

    //! \overload T& get(const std::string& symbol_name) const
    template <typename T>
    inline typename zr_boost_1_81::enable_if_c<zr_boost_1_81::is_member_pointer<T>::value || zr_boost_1_81::is_reference<T>::value, T>::type get(const char* symbol_name) const {
        return zr_boost_1_81::dll::detail::aggressive_ptr_cast<T>(
    ...
    //! \overload T& get(const std::string& symbol_name) const
    template <typename T>
    inline typename zr_boost_1_81::disable_if_c<zr_boost_1_81::is_member_pointer<T>::value || zr_boost_1_81::is_reference<T>::value, T&>::type get(const char* symbol_name) const {
        return *zr_boost_1_81::dll::detail::aggressive_ptr_cast<T*>(
            get_void(symbol_name)
        );
    }
...
private:
    ...
    void* get_void(const char* sb) const {
        ...
        if (!is_loaded()) {
            ...
            zr_boost_1_81::throw_exception(
                zr_boost_1_81::dll::fs::system_error(
                    ec, "zr_boost_1_81::dll::shared_library::get() failed: no library was loaded"
                )
            );
        }
        ...
        if (ec || !ret) {
            zr_boost_1_81::dll::detail::report_error(ec, "zr_boost_1_81::dll::shared_library::get() failed");
        }
...
public:
    ...
    */
    zr_boost_1_81::dll::fs::path location() const {
        zr_boost_1_81::dll::fs::error_code ec;
        if (!is_loaded()) {
            ec = zr_boost_1_81::dll::fs::make_error_code(
                zr_boost_1_81::dll::fs::errc::bad_file_descriptor
            ...
            zr_boost_1_81::throw_exception(
                zr_boost_1_81::dll::fs::system_error(
                    ec, "zr_boost_1_81::dll::shared_library::location() failed (no library was loaded)"
                )
            );
        }

        zr_boost_1_81::dll::fs::path full_path = base_t::full_module_path(ec);

        if (ec) {
            zr_boost_1_81::dll::detail::report_error(ec, "zr_boost_1_81::dll::shared_library::location() failed");
        }
    ...
    * \throw std::bad_alloc.
    */
    zr_boost_1_81::dll::fs::path location(zr_boost_1_81::dll::fs::error_code& ec) const {
        if (!is_loaded()) {
            ec = zr_boost_1_81::dll::fs::make_error_code(
                zr_boost_1_81::dll::fs::errc::bad_file_descriptor
            );
