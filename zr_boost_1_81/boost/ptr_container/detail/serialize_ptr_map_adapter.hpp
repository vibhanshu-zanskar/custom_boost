...
void save(Archive& ar, const ptr_container_detail::ptr_map_adapter_base<T, VoidPtrMap, CloneAllocator,Ordered>& c, unsigned int /*version*/)
{
    ...
    ar << zr_boost_1_81::serialization::make_nvp( ptr_container_detail::count(), 
                                          ptr_container_detail::serialize_as_const(c.size()) );

    const_iterator i = c.begin(), e = c.end();
    for(; i != e; ++i)
    {
        ar << zr_boost_1_81::serialization::make_nvp( ptr_container_detail::first(), i->first );
        ar << zr_boost_1_81::serialization::make_nvp( ptr_container_detail::second(),
...
void load(Archive& ar, ptr_map_adapter<T, VoidPtrMap, CloneAllocator,Ordered>& c, unsigned int /*version*/)
{
    ...
    c.clear();
    size_type n;
    ar >> zr_boost_1_81::serialization::make_nvp( ptr_container_detail::count(), n );

    for(size_type i = 0u; i != n; ++i)
    {
        key_type key;
        T* value;
        ar >> zr_boost_1_81::serialization::make_nvp( ptr_container_detail::first(), key );
        ar >> zr_boost_1_81::serialization::make_nvp( ptr_container_detail::second(), value );
