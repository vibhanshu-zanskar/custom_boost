...
template <std::size_t MinBits = 0, std::size_t MaxBits = 0, zr_boost_1_81::multiprecision::cpp_integer_type SignType = signed_magnitude, cpp_int_check_type Checked = unchecked, class Allocator = typename std::conditional<MinBits && (MinBits == MaxBits), void, std::allocator<limb_type> >::type>
...
template <std::size_t MinBits, std::size_t MaxBits, zr_boost_1_81::multiprecision::cpp_integer_type SignType, cpp_int_check_type Checked, class Allocator>
...
struct cpp_int_base<MinBits, MaxBits, signed_magnitude, Checked, Allocator, false>
    : private zr_boost_1_81::multiprecision::detail::empty_value<typename detail::rebind<limb_type, Allocator>::type>
{
   ...
   using base_type = zr_boost_1_81::multiprecision::detail::empty_value<allocator_type>;
...
private:
 ...
 private:
   union data_type
   {
      ...
      constexpr data_type(signed_limb_type i) noexcept : first(static_cast<limb_type>(zr_boost_1_81::multiprecision::detail::unsigned_abs(i))) {}
#if BOOST_MP_ENDIAN_LITTLE_BYTE
      constexpr data_type(double_limb_type i) noexcept : double_first(i)
      {}
      constexpr data_type(signed_double_limb_type i) noexcept : double_first(static_cast<double_limb_type>(zr_boost_1_81::multiprecision::detail::unsigned_abs(i))) {}
#endif
 ...
 public:
   ...
   BOOST_MP_FORCEINLINE constexpr cpp_int_base(signed_double_limb_type i) noexcept
       : m_data(i),
         m_limbs(i < 0 ? (static_cast<double_limb_type>(zr_boost_1_81::multiprecision::detail::unsigned_abs(i)) > static_cast<double_limb_type>(max_limb_value) ? 2 : 1) : (i > max_limb_value ? 2 : 1)),
         m_sign(i < 0),
         m_internal(true),
         m_alias(false) {}
#endif
   ...
   //
   struct scoped_shared_storage : private zr_boost_1_81::multiprecision::detail::empty_value<allocator_type>
   {
    private:
      limb_type*      data;
      std::size_t        capacity;
      ...
      bool            is_alias;
      allocator_type& allocator() noexcept { return zr_boost_1_81::multiprecision::detail::empty_value<allocator_type>::get(); }

    public:
      scoped_shared_storage(const allocator_type& a, std::size_t len)
          : zr_boost_1_81::multiprecision::detail::empty_value<allocator_type>(zr_boost_1_81::multiprecision::detail::empty_init_t(), a), capacity(len), allocated(0), is_alias(false)
      {
         data = allocator().allocate(len);
      }
      scoped_shared_storage(const cpp_int_base& i, std::size_t len)
          : zr_boost_1_81::multiprecision::detail::empty_value<allocator_type>(zr_boost_1_81::multiprecision::detail::empty_init_t(), i.allocator()), capacity(len), allocated(0), is_alias(false)
      {
         data = allocator().allocate(len);
      }
...
struct cpp_int_base<MinBits, MinBits, signed_magnitude, Checked, void, false>
{
 ...
 public:
   ...
   {}
   BOOST_MP_FORCEINLINE constexpr cpp_int_base(signed_double_limb_type i) noexcept
       : m_wrapper(double_limb_type(i < 0 ? static_cast<double_limb_type>(zr_boost_1_81::multiprecision::detail::unsigned_abs(i)) : i)),
         m_limbs(i < 0 ? (static_cast<double_limb_type>(zr_boost_1_81::multiprecision::detail::unsigned_abs(i)) > max_limb_value ? 2 : 1) : (i > max_limb_value ? 2 : 1)),
         m_sign(i < 0) {}
#endif
   template <limb_type... VALUES>
...
struct cpp_int_base<MinBits, MinBits, unsigned_magnitude, Checked, void, false>
{
 ...
 public:
   ...
   BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR cpp_int_base(signed_limb_type i) noexcept((Checked == unchecked))
       : m_wrapper(static_cast<limb_type>(zr_boost_1_81::multiprecision::detail::unsigned_abs(i))),
         m_limbs(1)
   {
      if (i < 0)
         negate();
...
//
// Traits classes to figure out a native type with N bits, these vary from zr_boost_1_81::uint_t<N> only
...
{
   using type = typename zr_boost_1_81::multiprecision::detail::uint_t<N>::least;
};
...
struct cpp_int_base<MinBits, MinBits, signed_magnitude, Checked, void, true>
{
 ...
 protected:
   template <class T>
   BOOST_MP_CXX14_CONSTEXPR typename std::enable_if<!(!zr_boost_1_81::multiprecision::detail::is_integral<T>::value || (std::numeric_limits<T>::is_specialized && (std::numeric_limits<T>::digits <= static_cast<int>(MinBits))))>::type
   check_in_range(T val, const std::integral_constant<int, checked>&)
   {
      using common_type = typename std::common_type<typename zr_boost_1_81::multiprecision::detail::make_unsigned<T>::type, local_limb_type>::type;

      if (static_cast<common_type>(zr_boost_1_81::multiprecision::detail::unsigned_abs(val)) > static_cast<common_type>(limb_mask))
 ...
 public:
   ...
   //
   template <class SI>
   BOOST_MP_FORCEINLINE constexpr cpp_int_base(SI i, typename std::enable_if<zr_boost_1_81::multiprecision::detail::is_signed<SI>::value && zr_boost_1_81::multiprecision::detail::is_integral<SI>::value && (Checked == unchecked)>::type const* = nullptr) noexcept(noexcept(std::declval<cpp_int_base>().check_in_range(std::declval<SI>())))
       : m_data(i < 0 ? static_cast<local_limb_type>(static_cast<typename zr_boost_1_81::multiprecision::detail::make_unsigned<SI>::type>(zr_boost_1_81::multiprecision::detail::unsigned_abs(i)) & limb_mask) : static_cast<local_limb_type>(i & limb_mask)), m_sign(i < 0) {}
   template <class SI>
   BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR cpp_int_base(SI i, typename std::enable_if<zr_boost_1_81::multiprecision::detail::is_signed<SI>::value && zr_boost_1_81::multiprecision::detail::is_integral<SI>::value && (Checked == checked)>::type const* = nullptr) noexcept(noexcept(std::declval<cpp_int_base>().check_in_range(std::declval<SI>())))
       : m_data(i < 0 ? (static_cast<local_limb_type>(static_cast<typename zr_boost_1_81::multiprecision::detail::make_unsigned<SI>::type>(zr_boost_1_81::multiprecision::detail::unsigned_abs(i)) & limb_mask)) : static_cast<local_limb_type>(i & limb_mask)), m_sign(i < 0)
   {
      check_in_range(i);
   }
   template <class UI>
   BOOST_MP_FORCEINLINE constexpr cpp_int_base(UI i, typename std::enable_if<zr_boost_1_81::multiprecision::detail::is_unsigned<UI>::value && (Checked == unchecked)>::type const* = nullptr) noexcept
       : m_data(static_cast<local_limb_type>(i) & limb_mask),
         m_sign(false) {}
   template <class UI>
   BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR cpp_int_base(UI i, typename std::enable_if<zr_boost_1_81::multiprecision::detail::is_unsigned<UI>::value && (Checked == checked)>::type const* = nullptr) noexcept(noexcept(std::declval<cpp_int_base>().check_in_range(std::declval<UI>())))
...
struct cpp_int_base<MinBits, MinBits, unsigned_magnitude, Checked, void, true>
{
 ...
 protected:
   ...
   template <class T>
   BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR void check_in_range(T val) noexcept(noexcept(std::declval<cpp_int_base>().check_in_range(std::declval<T>(), checked_type(), zr_boost_1_81::multiprecision::detail::is_signed<T>())))
   {
      check_in_range(val, checked_type(), zr_boost_1_81::multiprecision::detail::is_signed<T>());
   }
 ...
 public:
   ...
   template <class SI>
   BOOST_MP_FORCEINLINE constexpr cpp_int_base(SI i, typename std::enable_if<zr_boost_1_81::multiprecision::detail::is_signed<SI>::value && zr_boost_1_81::multiprecision::detail::is_integral<SI>::value && (Checked == unchecked)>::type const* = nullptr) noexcept
...
#endif
                                                    || literals::detail::is_value_pack<Arg>::value || (is_trivial_cpp_int<Base>::value && zr_boost_1_81::multiprecision::detail::is_arithmetic<Arg>::value),
          ...
          is_trivial_cpp_int<cpp_int_backend<MinBits, MaxBits, SignType, Checked, Allocator> >::value>
{
 ...
 private:
   ...
   BOOST_MP_CXX14_CONSTEXPR void do_assign(const cpp_int_backend<MinBits2, MaxBits2, SignType2, Checked2, Allocator2>& other, std::false_type const&, std::true_type const&)
   {
      ...
      *this = static_cast<
          typename zr_boost_1_81::multiprecision::detail::canonical<
 ...
 public:
   ...
   BOOST_MP_CXX14_CONSTEXPR typename std::enable_if<
         zr_boost_1_81::multiprecision::detail::is_unsigned<A>::value
   ...
   template <class A>
   BOOST_MP_CXX14_CONSTEXPR typename std::enable_if<
         !(zr_boost_1_81::multiprecision::detail::is_unsigned<A>::value || !zr_boost_1_81::multiprecision::detail::is_integral<A>::value)
         && trivial_tag::value, cpp_int_backend&>::type
      operator=(const A& val)
   ...
   {
      this->check_in_range(val);
      *this->limbs() = (val < 0) ? static_cast<typename self_type::local_limb_type>(zr_boost_1_81::multiprecision::detail::unsigned_abs(val)) : static_cast<typename self_type::local_limb_type>(val);
      this->sign(val < 0);
      this->normalize();
      return *this;
   }
   ...
   BOOST_MP_CXX14_CONSTEXPR typename std::enable_if<
         ...
         && !zr_boost_1_81::multiprecision::detail::is_integral<A>::value
         && trivial_tag::value, cpp_int_backend&>::type 
      operator=(const A& val)
   {
      this->check_in_range(val);
      *this->limbs() = (val < 0) ? static_cast<typename self_type::local_limb_type>(zr_boost_1_81::multiprecision::detail::abs(val)) : static_cast<typename self_type::local_limb_type>(val);
      this->sign(val < 0);
      this->normalize();
      return *this;
   }
   ...
   {
      this->resize(1, 1);
      *this->limbs() = static_cast<limb_type>(zr_boost_1_81::multiprecision::detail::unsigned_abs(i));
      this->sign(i < 0);
      return *this;
   }
   template <class A>
      ...
      operator=(A i) noexcept(noexcept(std::declval<cpp_int_backend>().sign(true)))
   {
      ...
      if (i < 0)
         s = true;
      double_limb_type                 ui = static_cast<double_limb_type>(zr_boost_1_81::multiprecision::detail::unsigned_abs(i));
      typename base_type::limb_pointer p  = this->limbs();
#ifdef __MSVC_RUNTIME_CHECKS
...
public:
 ...
 public:
   template <class Container>
   BOOST_MP_CXX14_CONSTEXPR cpp_int_backend(const Container& c, typename std::enable_if<zr_boost_1_81::multiprecision::detail::is_byte_container<Container>::value>::type const* = nullptr)
   {
      //
      // We assume that c is a sequence of (unsigned) bytes with the most significant byte first:
      //
   ...
   BOOST_MP_FORCEINLINE BOOST_MP_CXX14_CONSTEXPR typename std::enable_if<zr_boost_1_81::multiprecision::detail::is_arithmetic<Arithmetic>::value, int>::type compare(Arithmetic i) const
   {
      // braindead version:
      cpp_int_backend t;
      t = i;
...
using zr_boost_1_81::multiprecision::backends::cpp_int_backend;
