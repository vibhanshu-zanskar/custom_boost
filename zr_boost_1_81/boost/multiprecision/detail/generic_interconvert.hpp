...
template <class To, class B, ::zr_boost_1_81::multiprecision::expression_template_option et>
...
//#pragma warning(disable : 4127)
   ...
   else
   {
      ...
      while (!eval_is_zero(f))
      {
         ...
         typename zr_boost_1_81::multiprecision::detail::canonical<std::intmax_t, To>::type ll;
...
generic_convert_rational_to_float_imp(To& result, Integer& num, Integer& denom, const std::integral_constant<bool, false>&)
{
   ...
   if (shift > 0)
      ...
      denom <<= zr_boost_1_81::multiprecision::detail::unsigned_abs(shift);
   Integer q, r;
...
void generic_interconvert_float2rational(To& to, const From& from, const std::integral_constant<int, 2>& /*radix*/)
{
   ...
   while (val)
   {
      val = ldexp(val, shift);
      e -= shift;
      long long ll = zr_boost_1_81::multiprecision::detail::lltrunc(val);
      val -= ll;
      num <<= shift;
      num += ll;
   }
