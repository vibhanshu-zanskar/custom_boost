...
//
template <class T>
constexpr typename std::enable_if<(zr_boost_1_81::multiprecision::detail::is_signed<T>::value || std::is_floating_point<T>::value), T>::type abs(T t) noexcept
{
...
template <class T>
constexpr typename std::enable_if<zr_boost_1_81::multiprecision::detail::is_unsigned<T>::value, T>::type abs(T t) noexcept
{
   return t;
}

#define BOOST_MP_USING_ABS using zr_boost_1_81::multiprecision::detail::abs;

template <class T>
constexpr typename std::enable_if<(zr_boost_1_81::multiprecision::detail::is_signed<T>::value || std::is_floating_point<T>::value), typename zr_boost_1_81::multiprecision::detail::make_unsigned<T>::type>::type unsigned_abs(T t) noexcept
{
   // This strange expression avoids a hardware trap in the corner case
   // that val is the most negative value permitted in long long.
   // See https://svn.boost.org/trac/boost/ticket/9740.
   return t < 0 ? static_cast<typename zr_boost_1_81::multiprecision::detail::make_unsigned<T>::type>(1u) + static_cast<typename zr_boost_1_81::multiprecision::detail::make_unsigned<T>::type>(-(t + 1)) : static_cast<typename zr_boost_1_81::multiprecision::detail::make_unsigned<T>::type>(t);
}
template <class T>
constexpr typename std::enable_if<zr_boost_1_81::multiprecision::detail::is_unsigned<T>::value, T>::type unsigned_abs(T t) noexcept
{
   return t;
}
...
struct bits_of
{
   static_assert(zr_boost_1_81::multiprecision::detail::is_integral<T>::value || std::is_enum<T>::value || std::numeric_limits<T>::is_specialized, "Failed integer size check");
   static constexpr unsigned value =
       std::numeric_limits<T>::is_specialized ? std::numeric_limits<T>::digits
                                              : sizeof(T) * CHAR_BIT - (zr_boost_1_81::multiprecision::detail::is_signed<T>::value ? 1 : 0);
};
...
struct canonical_imp<Val, Backend, std::integral_constant<int, 4> >
{
   ...
   using tag = typename std::conditional<zr_boost_1_81::multiprecision::detail::is_signed<Val>::value, std::integral_constant<int, 0>, std::integral_constant<int, 1>>::type;
...
struct canonical
{
   using tag_type = typename std::conditional<
       zr_boost_1_81::multiprecision::detail::is_signed<Val>::value && zr_boost_1_81::multiprecision::detail::is_integral<Val>::value,
       std::integral_constant<int, 0>,
       typename std::conditional<
           zr_boost_1_81::multiprecision::detail::is_unsigned<Val>::value,
...
template <class T>
struct expression_storage : public expression_storage_base<T, zr_boost_1_81::multiprecision::detail::is_arithmetic<T>::value>
{};
...
void format_float_string(S& str, std::intmax_t my_exp, std::intmax_t digits, std::ios_base::fmtflags f, bool iszero)
{
   ...
   else
   {
      ...
      #ifndef BOOST_MP_STANDALONE
      e = zr_boost_1_81::lexical_cast<S>(abs(my_exp));
      #else
...
{};
template <class Num>
struct number_category : public number_category_base<Num, std::is_class<Num>::value || zr_boost_1_81::multiprecision::detail::is_arithmetic<Num>::value, std::is_abstract<Num>::value>
{};
template <class Backend, expression_template_option ExpressionTemplates>
...
template <>
struct number_category<zr_boost_1_81::multiprecision::int128_type> : public std::integral_constant<int, number_kind_integer>
{};
template <>
struct number_category<zr_boost_1_81::multiprecision::uint128_type> : public std::integral_constant<int, number_kind_integer>
{};
#endif
#ifdef BOOST_HAS_FLOAT128
template <>
struct number_category<zr_boost_1_81::multiprecision::float128_type> : public std::integral_constant<int, number_kind_floating_point>
{};
#endif
...
namespace zr_boost_1_81 { namespace math {
   namespace tools {
      ...
      template <class tag, class A1, class A2, class A3, class A4>
      struct promote_arg<zr_boost_1_81::multiprecision::detail::expression<tag, A1, A2, A3, A4> >
      {
         using type = typename zr_boost_1_81::multiprecision::detail::expression<tag, A1, A2, A3, A4>::result_type;
      };

      template <class R, class B, zr_boost_1_81::multiprecision::expression_template_option ET>
      inline R real_cast(const zr_boost_1_81::multiprecision::number<B, ET>& val)
      {
         return val.template convert_to<R>();
      }

      template <class R, class tag, class A1, class A2, class A3, class A4>
      inline R real_cast(const zr_boost_1_81::multiprecision::detail::expression<tag, A1, A2, A3, A4>& val)
      {
         using val_type = typename zr_boost_1_81::multiprecision::detail::expression<tag, A1, A2, A3, A4>::result_type;
         return val_type(val).template convert_to<R>();
      }

      template <class B, zr_boost_1_81::multiprecision::expression_template_option ET>
      struct is_complex_type<zr_boost_1_81::multiprecision::number<B, ET> > : public std::integral_constant<bool, zr_boost_1_81::multiprecision::number_category<B>::value == zr_boost_1_81::multiprecision::number_kind_complex> {};
...
template <class B, zr_boost_1_81::multiprecision::expression_template_option ET>
struct is_explicitly_convertible_from_string<zr_boost_1_81::multiprecision::number<B, ET> >
{
