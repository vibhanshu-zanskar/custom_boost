...
{
   using si_type = typename zr_boost_1_81::multiprecision::detail::canonical<std::int32_t, T>::type ;
   using ui_type = typename zr_boost_1_81::multiprecision::detail::canonical<std::uint32_t, T>::type;
   ...
   T tol;
   tol = ui_type(1);
   eval_ldexp(tol, tol, 1 - zr_boost_1_81::multiprecision::detail::digits2<number<T, et_on> >::value());
...
template <class T, unsigned N, bool b = zr_boost_1_81::multiprecision::detail::is_variable_precision<zr_boost_1_81::multiprecision::number<T> >::value>
...
void reduce_n_half_pi(T& arg, const T& n, bool go_down)
{
   ...
   //
   using reduction_type = typename zr_boost_1_81::multiprecision::detail::transcendental_reduction_type<T>::type;
   //
...
void eval_asin(T& result, const T& x)
{
   ...
   using guess_type = typename zr_boost_1_81::multiprecision::detail::canonical<long double, T>::type;
#else
   ...
   std::intmax_t target_precision  = std::numeric_limits<number<T> >::is_specialized ? 
      ...
      : current_precision - 1 - (zr_boost_1_81::multiprecision::detail::digits2<number<T> >::value() * 2) / 3;
...
inline typename std::enable_if<zr_boost_1_81::multiprecision::detail::is_arithmetic<A>::value, void>::type eval_atan2(T& result, const T& x, const A& a)
{
...
template <class T, class A>
inline typename std::enable_if<zr_boost_1_81::multiprecision::detail::is_arithmetic<A>::value, void>::type eval_atan2(T& result, const A& x, const T& a)
{
   using canonical_type = typename zr_boost_1_81::multiprecision::detail::canonical<A, T>::type         ;
   using cast_type = typename std::conditional<std::is_same<A, canonical_type>::value, T, canonical_type>::type;
   cast_type                                                                      c;
