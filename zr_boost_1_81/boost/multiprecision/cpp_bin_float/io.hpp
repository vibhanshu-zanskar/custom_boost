...
cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>& cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::operator=(const char* s)
{
   ...
   if (decimal_exp >= 0)
   {
      ...
      do
      {
         ...
         if (decimal_exp)
         {
            calc_exp = zr_boost_1_81::multiprecision::cpp_bf_io_detail::restricted_pow(t, cpp_int(5), decimal_exp, max_bits, error);
            calc_exp += zr_boost_1_81::multiprecision::cpp_bf_io_detail::restricted_multiply(t, t, n, max_bits, error);
         }
         else
            t = n;
         ...
         if (rshift > 0)
         {
            ...
            int roundup = zr_boost_1_81::multiprecision::cpp_bf_io_detail::get_round_mode(t, rshift - 1, error);
            t >>= rshift;
   ...
   else
   {
      ...
      {
         cpp_int d;
         calc_exp                   = zr_boost_1_81::multiprecision::cpp_bf_io_detail::restricted_pow(d, cpp_int(5), -decimal_exp, max_bits, error);
         ...
         if (gb == cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::bit_count - 1)
         {
            ...
            roundup = zr_boost_1_81::multiprecision::cpp_bf_io_detail::get_round_mode(r, d, error, q);
         }
...
std::string cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::str(std::streamsize dig, std::ios_base::fmtflags f) const
{
   ...
   if (exponent() <= cpp_bin_float<Digits, DigitBase, Allocator, Exponent, MinE, MaxE>::max_exponent)
   {
      ...
      if (digits_wanted < -1)
      {
         ...
         zr_boost_1_81::multiprecision::detail::format_float_string(s, base10_exp, dig, f, true);
         return s;
      }
      //
      ...
      if (power10)
      {
         const std::uintmax_t abs_power10 = static_cast<std::uintmax_t>(zr_boost_1_81::multiprecision::detail::abs(power10));
      ...
      do
      {
         ...
         if (shift < 0)
         {
            ...
            {
               cpp_int d;
               calc_exp = zr_boost_1_81::multiprecision::cpp_bf_io_detail::restricted_pow(d, cpp_int(5), -power10, max_bits, error);
               shift += calc_exp;
               BOOST_MP_ASSERT(shift < 0); // Must still be true!
               i <<= -shift;
               cpp_int r;
               divide_qr(i, d, i, r);
               roundup = zr_boost_1_81::multiprecision::cpp_bf_io_detail::get_round_mode(r, d, error, i);
               if (roundup < 0)
               {
#ifdef BOOST_MP_STRESS_IO
                  max_bits += 32;
         ...
         else
         {
            ...
            if (power10 > 0)
            {
               ...
               {
                  cpp_int t;
                  calc_exp = zr_boost_1_81::multiprecision::cpp_bf_io_detail::restricted_pow(t, cpp_int(5), power10, max_bits, error);
                  calc_exp += zr_boost_1_81::multiprecision::cpp_bf_io_detail::restricted_multiply(i, i, t, max_bits, error);
                  shift -= calc_exp;
               }
               ...
               if (shift)
               {
                  roundup = zr_boost_1_81::multiprecision::cpp_bf_io_detail::get_round_mode(i, shift - 1, error);
                  if (roundup < 0)
                  {
#ifdef BOOST_MP_STRESS_IO
                     max_bits += 32;
      ...
      {
         zr_boost_1_81::multiprecision::detail::round_string_up_at(s, static_cast<int>(s.size() - 1), base10_exp);
      }

      if (sign())
         s.insert(static_cast<std::string::size_type>(0), 1, '-');

      zr_boost_1_81::multiprecision::detail::format_float_string(s, base10_exp, dig, f, false);
   }
   else
   {
      switch (exponent())
      ...
      case exponent_zero:
         ...
         zr_boost_1_81::multiprecision::detail::format_float_string(s, 0, dig, f, true);
         break;
      case exponent_nan:
         s = "nan";
         break;
