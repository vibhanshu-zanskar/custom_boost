...
template <int Ndim, unsigned int SPECIALMUL, zr_boost_1_81::int64_t SPECIAL> // MIXMAX TEMPLATE PARAMETERS
class mixmax_engine{
public:
    // Interfaces required by C++11 std::random and zr_boost_1_81::random
    typedef zr_boost_1_81::uint64_t result_type ;
    BOOST_STATIC_CONSTANT(zr_boost_1_81::uint64_t,mixmax_min=0);
    BOOST_STATIC_CONSTANT(zr_boost_1_81::uint64_t,mixmax_max=((1ULL<<61)-1));
    ...
    explicit mixmax_engine(zr_boost_1_81::uint64_t);          ///< Constructor, one 64-bit seed
    ...
    void seed(zr_boost_1_81::uint64_t seedval=default_seed){seed_uniquestream( &S, 0, 0, (uint32_t)(seedval>>32), (uint32_t)seedval );} ///< seed with one 64-bit seed
    
private: // DATATYPES
    struct rng_state_st{
        zr_boost_1_81::array<zr_boost_1_81::uint64_t, Ndim> V;
        zr_boost_1_81::uint64_t sumtot;
        int counter;
    };
...
public: // SEEDING FUNCTIONS
    ...
    zr_boost_1_81::uint64_t operator()(){
        if (S.counter<=(Ndim-1) ){
    ...
    void discard(zr_boost_1_81::uint64_t nsteps) { for(zr_boost_1_81::uint64_t j = 0; j < nsteps; ++j)  (*this)(); } ///< discard n steps, required in zr_boost_1_81::random
    ...
    operator<< (std::basic_ostream<CharT,Traits>& ost, const mixmax_engine& me){
        ...
        ost <<  (zr_boost_1_81::uint64_t)me.S.V[j] << " ";
        }
        ost << "\n";
        ost.flush();
        return ost;
    ...
    operator>> (std::basic_istream<CharT,Traits> &in, mixmax_engine& me){
        ...
        zr_boost_1_81::array<zr_boost_1_81::uint64_t, Ndim> vec;
        zr_boost_1_81::uint64_t sum=0, savedsum=0, counter=0;
        in >> counter >> std::ws;
...
BOOST_STATIC_CONSTANT(zr_boost_1_81::uint64_t, M61=2305843009213693951ULL);
BOOST_STATIC_CONSTANT(zr_boost_1_81::uint64_t, default_seed=1);
inline zr_boost_1_81::uint64_t MOD_MERSENNE(zr_boost_1_81::uint64_t k) {return ((((k)) & M61) + (((k)) >> BITS) );}
inline zr_boost_1_81::uint64_t MULWU(zr_boost_1_81::uint64_t k);
...
inline zr_boost_1_81::uint64_t iterate_raw_vec(zr_boost_1_81::uint64_t* Y, zr_boost_1_81::uint64_t sumtotOld);
inline zr_boost_1_81::uint64_t apply_bigskip(zr_boost_1_81::uint64_t* Vout, zr_boost_1_81::uint64_t* Vin, uint32_t clusterID, uint32_t machineID, uint32_t runID, uint32_t  streamID );
inline zr_boost_1_81::uint64_t modadd(zr_boost_1_81::uint64_t foo, zr_boost_1_81::uint64_t bar);
inline zr_boost_1_81::uint64_t fmodmulM61(zr_boost_1_81::uint64_t cum, zr_boost_1_81::uint64_t s, zr_boost_1_81::uint64_t a);
};

template <int Ndim, unsigned int SPECIALMUL, zr_boost_1_81::int64_t SPECIAL> mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::mixmax_engine()
...
template <int Ndim, unsigned int SPECIALMUL, zr_boost_1_81::int64_t SPECIAL> mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::mixmax_engine(zr_boost_1_81::uint64_t seedval){
...
template <int Ndim, unsigned int SPECIALMUL, zr_boost_1_81::int64_t SPECIAL> mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::mixmax_engine(uint32_t clusterID, uint32_t machineID, uint32_t runID, uint32_t  streamID){
    // constructor, four 32-bit seeds for 128-bit seeding flexibility
    seed_uniquestream( &S, clusterID,  machineID,  runID,  streamID );
}

template <int Ndim, unsigned int SPECIALMUL, zr_boost_1_81::int64_t SPECIAL> uint64_t mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::MULWU (uint64_t k){ return (( (k)<<(SPECIALMUL) & M61) ^ ( (k) >> (BITS-SPECIALMUL))  )  ;}

template <int Ndim, unsigned int SPECIALMUL, zr_boost_1_81::int64_t SPECIAL> zr_boost_1_81::uint64_t mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::iterate_raw_vec(zr_boost_1_81::uint64_t* Y, zr_boost_1_81::uint64_t sumtotOld){
    // operates with a raw vector, uses known sum of elements of Y
    zr_boost_1_81::uint64_t  tempP=0, tempV=sumtotOld;
    Y[0] = tempV;
    zr_boost_1_81::uint64_t sumtot = Y[0], ovflow = 0; // will keep a running sum of all new elements
    for (int i=1; i<Ndim; i++){
        zr_boost_1_81::uint64_t tempPO = MULWU(tempP);
        tempV = (tempV+tempPO);
...
template <int Ndim, unsigned int SPECIALMUL, zr_boost_1_81::int64_t SPECIAL> void mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::seed_vielbein(rng_state_t* X, unsigned int index){
...
template <int Ndim, unsigned int SPECIALMUL, zr_boost_1_81::int64_t SPECIAL> void mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::seed_uniquestream( rng_state_t* Xin, uint32_t clusterID, uint32_t machineID, uint32_t runID, uint32_t  streamID ){
    seed_vielbein(Xin,0);
...
template <int Ndim, unsigned int SPECIALMUL, zr_boost_1_81::int64_t SPECIAL> zr_boost_1_81::uint64_t mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::apply_bigskip( zr_boost_1_81::uint64_t* Vout, zr_boost_1_81::uint64_t* Vin, uint32_t clusterID, uint32_t machineID, uint32_t runID, uint32_t  streamID ){
    /*
...
#include "boost/random/detail/mixmax_skip_N17.ipp"
    ...
    for (int IDindex=0; IDindex<4; IDindex++) { // go from lower order to higher order ID
        ...
        while (id){
            if (id & 1) {
                zr_boost_1_81::uint64_t* rowPtr = (zr_boost_1_81::uint64_t*)skipMat[r + IDindex*8*sizeof(uint32_t)];
                for (int i=0; i<Ndim; i++){ cum[i] = 0; }
                for (int j=0; j<Ndim; j++){              // j is lag, enumerates terms of the poly
                    // for zero lag Y is already given
                    zr_boost_1_81::uint64_t coeff = rowPtr[j]; // same coeff for all i
                    for (int i =0; i<Ndim; i++){
...
template <int Ndim, unsigned int SPECIALMUL, zr_boost_1_81::int64_t SPECIAL> inline zr_boost_1_81::uint64_t mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::fmodmulM61(zr_boost_1_81::uint64_t cum, zr_boost_1_81::uint64_t s, zr_boost_1_81::uint64_t a){
    // works on all platforms, including 32-bit linux, PPC and PPC64, ARM and Windows
    const zr_boost_1_81::uint64_t MASK32=0xFFFFFFFFULL;
    zr_boost_1_81::uint64_t o,ph,pl,ah,al;
    o=(s)*a;
    ph = ((s)>>32);
    pl = (s) & MASK32;
    ah = a>>32;
...
template <int Ndim, unsigned int SPECIALMUL, zr_boost_1_81::int64_t SPECIAL> zr_boost_1_81::uint64_t mixmax_engine  <Ndim, SPECIALMUL, SPECIAL> ::modadd(zr_boost_1_81::uint64_t foo, zr_boost_1_81::uint64_t bar){
    return MOD_MERSENNE(foo+bar);
}

/* @copydoc zr_boost_1_81::random::detail::mixmax_engine_doc */
