...
public:
    ...
    RealType operator()(URNG& eng) 
    {
        using std::sqrt;
        if (_param.k() > 1) {
            zr_boost_1_81::random::normal_distribution<RealType> n_dist;
            zr_boost_1_81::random::chi_squared_distribution<RealType> c_dist(_param.k() - RealType(1));
            ...
            zr_boost_1_81::random::poisson_distribution<> p_dist(_param.lambda()/RealType(2));
            zr_boost_1_81::random::poisson_distribution<>::result_type _p = p_dist(eng);
            zr_boost_1_81::random::chi_squared_distribution<RealType> c_dist(_param.k() + RealType(2)*_p);
            return c_dist(eng);
        }
    }
