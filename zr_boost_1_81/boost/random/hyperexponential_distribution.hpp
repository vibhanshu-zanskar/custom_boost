 ...
 *
 * Much of the code here taken by zr_boost_1_81::math::hyperexponential_distribution.
...
namespace zr_boost_1_81 { namespace random {
...
bool check_probabilities(std::vector<RealT> const& probabilities)
{
    ...
    for (std::size_t i = 0; i < n; ++i)
    {
        if (probabilities[i] < 0
            || probabilities[i] > 1
            || !(zr_boost_1_81::core::isfinite)(probabilities[i]))
        {
            return false;
        }
        sum += probabilities[i];
...
bool check_rates(std::vector<RealT> const& rates)
{
    ...
    {
        if (rates[i] <= 0
            || !(zr_boost_1_81::core::isfinite)(rates[i]))
        {
            return false;
        }
    }
...
class hyperexponential_distribution
{
    ...
    public: class param_type
    {
        ...
        public: template <typename ProbRangeT, typename RateRangeT>
                param_type(ProbRangeT const& prob_range,
                           ...
                           typename zr_boost_1_81::disable_if_c<zr_boost_1_81::has_pre_increment<ProbRangeT>::value || zr_boost_1_81::has_pre_increment<RateRangeT>::value>::type* = 0)
        : probs_(zr_boost_1_81::begin(prob_range), zr_boost_1_81::end(prob_range)),
          rates_(zr_boost_1_81::begin(rate_range), zr_boost_1_81::end(rate_range))
        {
            hyperexp_detail::normalize(probs_);
        ...
        public: template <typename RateIterT>
                param_type(RateIterT rate_first, 
                           RateIterT rate_last,  
                           typename zr_boost_1_81::enable_if_c<zr_boost_1_81::has_pre_increment<RateIterT>::value>::type* = 0)
        ...
        : probs_(zr_boost_1_81::size(rate_range), 1), // Will be normalized below
          rates_(zr_boost_1_81::begin(rate_range), zr_boost_1_81::end(rate_range))
        {
            hyperexp_detail::normalize(probs_);
    ...
    : dd_(std::vector<RealT>(zr_boost_1_81::size(rate_range), 1)),
      rates_(zr_boost_1_81::begin(rate_range), zr_boost_1_81::end(rate_range))
    {
        assert( hyperexp_detail::check_params(dd_.probabilities(), rates_) );
    }
            ...
            RealT operator()(URNG& urng) const
    {
        ...
        return zr_boost_1_81::random::exponential_distribution<RealT>(rates_[i])(urng);
    }
    ...
    {
        dd_.param(typename zr_boost_1_81::random::discrete_distribution<int,RealT>::param_type(param.probabilities()));
        rates_ = param.rates();
    }
    ...
    private: zr_boost_1_81::random::discrete_distribution<int,RealT> dd_; ///< The \c discrete_distribution used to sample the phase probability and choose the rate
    private: std::vector<RealT> rates_; ///< The <em>rate vector</em> parameter of the distribution
}; // hyperexponential_distribution

}} // namespace zr_boost_1_81::random
