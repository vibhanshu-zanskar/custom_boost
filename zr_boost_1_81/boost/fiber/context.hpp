...
private:
    ...
    zr_boost_1_81::context::fiber                               c_{};
...
public:
    ...
    zr_boost_1_81::context::fiber suspend_with_cc() noexcept;
    zr_boost_1_81::context::fiber terminate() noexcept;
    ...
    friend void intrusive_ptr_release( context * ctx) noexcept {
        ...
        if ( 1 == ctx->use_count_.fetch_sub( 1, std::memory_order_release) ) {
            ...
            zr_boost_1_81::context::fiber c = std::move( ctx->c_);
            // destruct context
            ctx->~context();
            // deallocated stack
            std::move( c).resume();
    ...
    run_( zr_boost_1_81::context::fiber && c) {
        {
            ...
            std::move( c).resume();
#else
            zr_boost_1_81::ignore_unused(c);
#endif
#if defined(BOOST_NO_CXX17_STD_APPLY)
           zr_boost_1_81::context::detail::apply( std::move( fn), std::move( arg) );
#else
...
public:
    ...
    worker_context( launch policy,
                    ...
                    zr_boost_1_81::context::preallocated const& palloc, StackAlloc && salloc,
                    Fn && fn, Arg ... arg) :
        ...
        c_ = zr_boost_1_81::context::fiber{ std::allocator_arg, palloc, std::forward< StackAlloc >( salloc),
...
static intrusive_ptr< context > make_worker_context_with_properties( launch policy,
    ...
    return intrusive_ptr< context >{ 
            new ( storage) context_t{
                policy,
                properties,
                zr_boost_1_81::context::preallocated{ storage, size, sctx },
